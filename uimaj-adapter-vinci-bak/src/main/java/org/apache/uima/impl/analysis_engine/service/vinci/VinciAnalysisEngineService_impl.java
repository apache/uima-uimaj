...
import org.apache.uima.impl.analysis_engine.service.vinci.util.NetworkUtil;
...
import org.apache.uima.impl.util.UIMALogFormatter;
import org.apache.uima.impl.util.UIMAStreamHandler;
...
/**
 * Main class for a Vinci Analysis Engine service. This class can also be used
 * to deploy CAS Consumers as Vinci services.
 * 
...
public class VinciAnalysisEngineService_impl extends VinciServableAdapter
{
	...
	/**
	 * Instantiate Analysis Engine from a given descriptor, debug mode, and
	 * instance Id
	 * 
	 * @param aResourceSpecifierPath -
	 *            descriptor location
	 */
	public VinciAnalysisEngineService_impl(String serviceConfigPath,
	...
	/**
	 * Instantiate Analysis Engine service from a given descriptor - possibly in
	 * debug mode.
	 * 
	 * @param aResourceSpecifierPath -
	 *            descriptor location
			...
			boolean debug) throws Exception
	{
		...
		UIMAFramework.getLogger().log(Level.FINE,
				"VinciAnalysisEngineService_impl: constructor");
	...
	private Frame getMetaData() throws Exception
	{
		UIMAFramework.getLogger().log(Level.FINEST,
				"VinciAnalysisEngineService.getMetaData()");
		// get metadata
	...
	/**
	 * Analyzes a given document by a AnalysisEngine. When completed this method
	 * returns a VinciFrame containing XCAS translated into a set of Vinci
	 * subFrames. Each subframe containing one annotation with all its
	 * attributes.
	 * 
	 * @param aRequestFrame
	 *            request frame
	...
	private Transportable analyze(CASTransportable ct) throws Exception
	{
		...
		try
		{
			...
			ct.getExtraDataFrame().fset(Constants.ANNOTATION_TIME,
					annotationTime);
			// Extract CAS
		...
		try
		{
			...
			if (args.length < 1)
			{
				System.err.println("Usage: java "
						+ VinciAnalysisEngineService_impl.class.getName()
			...
			{
				System.err.println("Deployment descriptor \"" + args[0]
						+ "\" does not exist.");
				System.exit(1);
			}
			...
			{
				vinciService = new VinciAnalysisEngineService_impl(
						descriptorFile.toString(), debug, args[1]);
			}
			else
			{

				vinciService = new VinciAnalysisEngineService_impl(
						descriptorFile.toString(), debug);
			}
			vinciService.start();
		}
		catch (Exception ex)
	...
	{
		return new CASTransportable(mCasPool, new OutOfTypeSystemData(), null,
				false);
	}
