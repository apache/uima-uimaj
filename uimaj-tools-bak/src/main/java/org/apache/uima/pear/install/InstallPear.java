...
package org.apache.uima.pear.install;
...
import org.apache.uima.impl.util.BrowserUtil;
 ...
 * directory chosen by the user and then run the installed AE in Gladis. 
 * <br />The required field is : The pear file must be specified. 
 * The User may or may not specify the installation directory. If the installation
 * directory is not specified, the current working directory is used by default.
 * Set JVM arguments:
 * <br />-Duima.home=%UIMA_HOME% (local directory where UIMA is installed)
 * 
 */ 
public class InstallPear extends JFrame {
	/**
	 * The <code>RunInstallation</code> class implements a thread that is 
	 * used to run the installation.
	 */
	protected static class RunInstallation implements Runnable {
		...
		/**
		 * Constructor that sets a given input PEAR file and a given 
		 * installation directory.
		 *
		 * @param pearFile The given PEAR file.
		 * @param installationDir The given installation directory.
		 */
		...
		/**
		 * Runs the PEAR installation process. Notifies waiting threads upon 
		 * completion.
		 */
		public void run() {
	...
	/**
	 * The <code>PEARFilter</code> class allows to filter directories, 
	 * as well as '.tear' and '.pear' files.
	 */
	protected static class PEARFilter extends FileFilter {
		static final String TEAR_EXT = "tear";
		static final String PEAR_EXT = "pear";
		/**
		 * Returns <code>true</code>, if the given input file is directory or 
		 * has 'tear' or 'pear' extension, <code>false</code> otherwise.
		 * 
		 * @return <code>true</code>, if the given input file is directory or 
		 * has 'tear' or 'pear' extension, <code>false</code> otherwise.
		 */
	 ...
	 * 
	 * @return Selected PEAR file path, or <code>null</code>, if nothing was 
	 * selected.
	 */
	private String selectPear() {
		...
		String lastFileName = (pearFileTextField.getText().length() > 0) ? pearFileTextField.getText() : userPrefs.get(LAST_FILE_NAME_CHOOSEN_KEY, "./");
		File directory = (lastFileName.length() > 0) ?
			new File(lastFileName).getParentFile() : new File( "./" );
	 ...
	 * 
	 * @return Selected installation directory path, or current directory 
	...
	private String selectDir() {
		...
		File directory = (lastDirName.length() > 0) ? 
			new File(lastDirName).getParentFile() : new File( "./" );
		fileChooser.setCurrentDirectory(directory);
	...
	/**
	 * Method that installs the given PEAR file to the given installation 
	 * directory.
	 * 
	 * @param localPearFile The given PEAR file path.
	 * @param installationDir The given installation directory.
	 */
	private static void installPear(File localPearFile, File installationDir) {
		...
		try {
			...
			insdObject = installationDescriptorHandler
			...
			if( insdObject.getDelegateComponents().size() > 0 ) {
				throw new RuntimeException( 
						"separate delegate components not supported \n" );
			}
		} catch (Exception err) {
			errorFlag = true;
			message = " terminated \n" + err.toString();
		...
		if (insdObject == null) {
			...
			/* installation failed */
			errorFlag = true;
			message = " \nInstallation of " + mainComponentId
			+ " failed => \n "
			...
			try {
				...
				+ " completed \n";
				printInConsole(errorFlag, message);
				message = "The " + mainComponentRootPath + "/" + 
				...
				if (installationController.verifyComponent()) {
					...
					File xmlDescFile = 
						new File( insdObject.getMainComponentDesc() );
					try {
						String uimaCompCtg = 
							UIMAUtil.identifyUimaComponentCategory( xmlDescFile );
						if( UIMAUtil.ANALYSIS_ENGINE_CTG.equals( uimaCompCtg ) )
							runButton.setEnabled(gladisExists);
					} catch( Exception e ) {}
					errorFlag = false;
					message = "Verification of " + mainComponentId
						+ " completed \n";					
					printInConsole(errorFlag, message);
				} else {
					runButton.setEnabled(false);
				...
				errorFlag = true;
				message = "Error in InstallationController.main(): "
					+ exc.toString();
				printInConsole(errorFlag, message);
			} finally {
	...
	/**
	 * Method that loads specified properties file. The properties file 
	 * should be on the CLASSPATH.
	 * 
	 * @param propFileName The specified properties file name.
	...
	private void runGladis() throws IOException {
		try {
			...
			String gladisPath = 
				gladisProperties.getProperty("env.PATH");
			...
			if (gladisPath != null && gladisPath.length() > 0) {
				...
				gladisPath = gladisPath.replaceAll(
					"%UIMA_HOME%", 
					System.getProperty("uima.home").replace( '\\', '/' ));
			} else
				gladisPath = "";
			// get Gladis specific CLASSPATH
			String gladisClassPath = 
				gladisProperties.getProperty("env.CLASSPATH");
			// substitute '\', ';' and UIMA_HOME
			if (gladisClassPath != null && gladisClassPath.length() > 0) {
				...
				gladisClassPath = gladisClassPath.replaceAll(
					"%UIMA_HOME%",
					System.getProperty("uima.home").replace( '\\', '/' ));
			} else
				gladisClassPath = "";
			...
			String compClassPath = InstallationController.
				buildComponentClassPath(mainComponentRootPath, insdObject);
			// get the rest of component specific env. vars
			Properties compEnvVars = InstallationController.
				buildTableOfEnvVars(insdObject);
			...
			while (gladisPropKeys.hasMoreElements()) {
				...
				if (key.startsWith("jvm.arg.")) {
					...
					// substitute UIMA_HOME
					String value = 
						gladisProperties.getProperty(key).replaceAll(
							"%UIMA_HOME%",
							System.getProperty("uima.home").replace( '\\', '/' ));
			...
			// add main class
			String mainClass = 
				gladisProperties.getProperty("main.class").trim();
			...
			while (sysEnvKeys.hasMoreElements()) {
				...
				if (sysKey.equalsIgnoreCase("CLASSPATH")) {
					value = compClassPath + File.pathSeparator
							+ gladisClassPath + File.pathSeparator + sysValue;
					classPathAdded = true;
				} else if (sysKey.equalsIgnoreCase("PATH") || 
							sysKey.equalsIgnoreCase("LD_LIBRARY_PATH")) {
					value = compPath + File.pathSeparator + gladisPath
							+ File.pathSeparator + sysValue;
					pathAdded = true;
				}
			...
			if (!classPathAdded) {
				String classPath = compClassPath + File.pathSeparator
						+ gladisClassPath;
			...
			while (gladisPropKeys.hasMoreElements()) {
				String key = (String) gladisPropKeys.nextElement();
				if (key.startsWith("env.") && !key.equals("env.PATH")
						&& !key.equals("env.CLASSPATH")) {
			...
			ProcessUtil.Runner pRunner = new ProcessUtil.Runner(
			 gladisProcess, "GLADIS");
		} catch (Throwable e) {
						...
						public void actionPerformed(java.awt.event.ActionEvent e) {
							...
							if (e.getActionCommand().equals("Browse Dir...")) {
								 ...
								 if ( dir == null ){
									 ...
									 pearConsole.append( " If you choose to install pear file, installation directory is: " + installationDir.getAbsolutePath()+ "\n" );
								}
								else {
					...
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							if (e.getActionCommand().equals("Install")) {
								localTearFile = new File(pearFileTextField
										.getText());
								installationDir = new File(installDirTextField
										.getText());
								RunInstallation installPear = new RunInstallation(
										localTearFile, installationDir);
								Thread thread = new Thread(installPear);
								thread.start();
								installButton.setCursor(new Cursor(
										Cursor.WAIT_CURSOR));
	...
	/**
	 * Prints messages and set foreground color in the console according to 
	 * a given errorFlag.
	 * 
	 * @param errorFlag The given error flag.
			...
			helpButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (e.getActionCommand().equals("Help")) {
						if(piHelp_URL == null )
							try {
								throw new IOException( "PEAR Installer help file does not exist: %UIMA_HOME%/docs/sith/piHelp.html" );
	...
	public static void main(String[] args) {
		...
		try {
			...
			gladisPath = gladisPath.replaceAll(
					"%UIMA_HOME%", UIMA_HOME.replace( '\\', '/' ));
			String pearHelpPath = mainProperties.getProperty( "help.file" );
			pearHelpPath = pearHelpPath.replaceAll(
					"%UIMA_HOME%", UIMA_HOME.replace( '\\', '/' ));
