...
import org.apache.uima.analysis_engine.metadata.*;
...
/**
 * The <code>PMUimaAgent</code> class implements UIMA-based utility methods 
 * utilized by the <code>{@link PMController}</code> class. The class allows 
 * generating analysis engine descriptor for output aggregate AE based on the 
 * specified input descriptors. 
 */

public class PMUimaAgent {
	/**
	 * Creates UIMA aggregate component description object, based on given 
	 * aggregate component name (ID), aggregate root directory and array of 
	 * delegate installation descriptors. Returns the UIMA aggregate 
	 * component description object. If the TAE mode is enabled, the method 
	 * create TAE description object, otherwise it creates analysis engine 
	 * description object.
	 * 
	 * @param aggCompName The given aggregate component name (ID).
	 * @param aggRootDir The given aggregate root directory.
	 * @param dlgInstDescs The given array of delegate installation 
	 * descriptors.
	 * @param aeModeEnabled If <code>true</code>, the method creates AE 
	 * description object, otherwise it creates more specific TAE description 
	 * object.
	 * @return The UIMA aggregate component description object.
	 */
	static AnalysisEngineDescription createAggregateDescription( 
			String aggCompName, File aggRootDir, 
		...
		try {
			...
			aggDescription = aeModeEnabled ?
					rsFactory.createAnalysisEngineDescription() :
					rsFactory.createTaeDescription();
			aggDescription.setPrimitive( false );
			// get Map of delegate specifiers with imports
			Map delegatesMap = aggDescription.
			...
			for( int i = 0; i < dlgInstDescs.length; i++ ) {
				...
				String dlgDescRelPath = dlgDescPath.replaceAll( 
			...
			// get AE metadata
			AnalysisEngineMetaData aggMetadata = 
				aggDescription.getAnalysisEngineMetaData();
			// set AE name and textual description
			aggMetadata.setName( aggCompName );
			aggMetadata.setDescription( "Merged aggregate component" + 
					"(" + PMController.PEAR_MERGER + ")" );
			...
			for( int i = 0; i < dlgInstDescs.length; i++ ) {
				lastInputNo = i + 1;
				ResourceSpecifier dlgSpecifier = 
					retrieveDelegateSpecifier( aggRootDir, dlgInstDescs[i] );
				if( dlgSpecifier instanceof AnalysisEngineDescription ) {
					// get AE metadata
					AnalysisEngineMetaData dlgAeMetadata = 
						((AnalysisEngineDescription)dlgSpecifier).
								getAnalysisEngineMetaData();
					// collect AE capabilities
					Capability[] dlgCapabilities = 
						dlgAeMetadata.getCapabilities();
					...
					OperationalProperties dlgOperProps = 
						dlgAeMetadata.getOperationalProperties();
					if( dlgOperProps != null ) {
						...
						if( dlgOperProps.getModifiesCas() )
							modifiesCas = true;
					} else // by default, AE modifies CAS
						modifiesCas = true;
				} else if( dlgSpecifier instanceof CasConsumerDescription ) {
					// get CC metadata
					ProcessingResourceMetaData dlgCcMetadata = 
						((CasConsumerDescription)dlgSpecifier).
								getCasConsumerMetaData();
					// collect CC capabilities
					Capability[] dlgCapabilities = 
						dlgCcMetadata.getCapabilities();
					if( dlgCapabilities != null )
						for( int n = 0; n < dlgCapabilities.length; n++ )
							allCapabilities.add( dlgCapabilities[n] );
					// check operational properties
					OperationalProperties dlgOperProps = 
						dlgCcMetadata.getOperationalProperties();
					if( dlgOperProps != null ) {
						if( ! dlgOperProps.isMultipleDeploymentAllowed() )
							isMultipleDeploymentAllowed = false;
					}
				} else // other categories (CR, CI) are not allowed
					throw new IllegalArgumentException( 
							"unsupported input component" );
			}
			// merge capabilities, excluding duplicates
			Capability[] mergedCapabilities = 
				mergeCapabilities( allCapabilities, rsFactory );
			// set aggregate capabilities
			aggMetadata.setCapabilities( mergedCapabilities );
			// set aggregate operational properties
			OperationalProperties aggOperProps = 
				aggMetadata.getOperationalProperties();
			if( aggOperProps != null ) {
				aggOperProps.setMultipleDeploymentAllowed( 
						isMultipleDeploymentAllowed );
				aggOperProps.setModifiesCas( modifiesCas );
			}
		} catch( IllegalArgumentException exc ) {
			PMController.logErrorMessage( 
					"Invalid input component # " + lastInputNo );
			PMController.logErrorMessage( 
			...
			if( lastInputNo > 0 )
				PMController.logErrorMessage( 
						"Error in input component # " + lastInputNo );
	 ...
	 * object.
	 */
	private static Capability[] mergeCapabilities( 
			ArrayList allCapabilities, ResourceSpecifierFactory rsFactory ) { 
		...
		while( allList.hasNext() ) {
			...
			for( int i = 0; i < entryOuts.length; i++ ) {
				...
				String name = nextTof.getName();
				TypeOrFeature prevTof = 
					(TypeOrFeature)mergedOutputs.get( name );
				if( prevTof != null ) {
					// choose next or prev, if it's 'type' 
					if( prevTof.isType() ) {
						// leave more general one
		...
		while( inpsList.hasMoreElements() ) {
			...
			if( tof.isType() )
				mergedCapability.addInputType( 
						name, tof.isAllAnnotatorFeatures() );
			else
		...
		while( outsList.hasMoreElements() ) {
			...
			if( tof.isType() )
				mergedCapability.addOutputType( 
						name, tof.isAllAnnotatorFeatures() );
			else
	...
	/**
	 * Creates <code>ResourceSpecifier</code> object for a delegate component 
	 * descriptor, specified by a given <code>InstallationDescriptor</code> 
	 * object. Returns the delegate component <code>ResourceSpecifier</code> 
	 * object.
	 * 
	 * @param aggRootDir The given aggregate root directory.
	 * @param dlgInstDesc The given delegate 
	 * <code>InstallationDescriptor</code> object.
	 * @return The given delegate component <code>ResourceSpecifier</code> 
	 * object.
	 * @throws IOException If an I/O exception occurred while creating 
	 * XML input source.
	 * @throws InvalidXMLException If <code>ResourceSpecifier</code> object 
	 * cannot be created from the specified descriptor.
	 */
	private static ResourceSpecifier retrieveDelegateSpecifier( 
			File aggRootDir, InstallationDescriptor dlgInstDesc ) 
			throws IOException, InvalidXMLException { 
		// get delegate desciptor path
		String aggRootDirPath = 
			aggRootDir.getAbsolutePath().replace( '\\', '/' );
		String dlgDescPath = dlgInstDesc.getMainComponentDesc().
			replaceAll( PMControllerHelper.MAIN_ROOT_REGEX, 
		...
		try {
			...
			else // otherwise, XML descriptor is not valid
				throw xmlExc;
			...
			if( UIMA_IllegalStateException.COULD_NOT_INSTANTIATE_XMLIZABLE.
					equals( msgKey ) )
	...
	/**
	 * Saves a given UIMA aggregate component desciption in a specified XML 
	 * descriptor file.
	 * 
	 * @param aggDescription The given UIMA aggregate component desciption.
	 * @param aggDescFile The given XML descriptor file.
	 * @throws IOException If an I/O exception occurrs.
	 */
	static void saveAggregateDescription( 
			AnalysisEngineDescription aggDescription, File aggDescFile ) 
			throws IOException {
	...
	/**
	 * Converts a given <code>XMLizable</code> object to String. This method 
	 * is useful for debugging.
	 * 
	 * @param content The given <code>XMLizable</code> object
	 * @return A String that represents the given <code>XMLizable</code> 
	 * object.
	 */
