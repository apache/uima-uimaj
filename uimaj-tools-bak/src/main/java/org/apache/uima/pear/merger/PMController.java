...
package org.apache.uima.pear.merger;
...
/**
 * The <code>PMController</code> class allows to merge several input PEAR 
 * files in one PEAR file and generate an aggregate component from the 
 * components encapsulated in the input PEARs.
 * 
 * @see org.apache.uima.pear.merger.PMControllerHelper
 * @see org.apache.uima.pear.merger.PMUimaAgent
 */
...
public class PMController {
	...
	/**
	 * The <code>PMLogFormatter</code> class formats messages for the log 
	 * file.
	 */
	static class PMLogFormatter extends SimpleFormatter {
		...
		public String format( LogRecord record ) {
			if( _firstTime ) {
				...
				if( message.indexOf( '\n' ) >= 0 )
					buffer.append( ": \n" );
				else // for single line msg - start msg in the same line
					buffer.append( ": " );
	...
	/**
	 * The command line application entry point. This method expects the 
	 * following command line arguments:
	 * <ul>
	 * <li>pear_file_1 ... pear_file_n - input PEAR files (required)</li>
	 * <li>-n agg_name - a name of the output aggregate component (required)
	 * </li>
	 * <li>-f agg_pear_file - output aggregate PEAR file (optional).
	 * <br />If the output PEAR file is not specified, the default output 
	 * PEAR file is created in the current working directory.</li>
	 * <li>-ae analysis engine mode flag (optional). If this flag is 
	 * specified, the utility creates aggregate analysis engine descriptor, 
	 * otherwise (by default) it creates more specific text analysis engine 
	 * descriptor.</li>
	 * </ul>
	 * 
	 * @param args pear_file_1 ... pear_file_n -n agg_name [-f agg_pear_file] 
	 *        [-ae]
	 */
	public static void main(String[] args) {
		...
		if( ! parseCommandLine( args ) ) {
			logErrorMessage( 
					PEAR_MERGER + " terminated: command line error" );
			return;
		}
		...
		try {
			...
			controller = new PMController( 
					__pearFiles, __aggregateName, __aggregatePearFile );
	...
	/**
	 * Logs a given error message to the log file and prints it to the 
	 * standard error console stream.
	 * 
	...
	/**
	 * Logs a given info message to the log file and prints it to the 
	 * standard output console stream.
	 * 
	 * @param message The given info message.
	 */
	...
	/**
	 * Logs a given warning message to the log file and prints it to the 
	 * standard error console stream.
	 * 
	...
	/**
	 * Parses and validates the command line and initializes static 
	 * attributes. 
	 * 
	...
	private static boolean parseCommandLine( String[] args ) {
		...
		if( args.length < 4 ) {
			logErrorMessage( PEAR_MERGER + " args: " + 
				"pear_file_1 ... pear_file_n -n agg_name " + 
				"[-f agg_pear_file] [-ae]" );
			return false;
		}
			...
			if( args[i].equals( AGGREGATE_NAME_ARG ) ) {
				...
				if( inPear.isFile() ) {
					...
					for( int n = 0; n < listOfPears.size(); n++ ) {
						try {
							String fPath = ((File)listOfPears.get( n )).
									getCanonicalPath();
							String inPath = inPear.getCanonicalPath();
							if( fPath.equals( inPath ) ) { 
								logErrorMessage( PEAR_MERGER + " error: " + 
									"duplicated input file " + args[i] );
								return false;
							}
						} catch( IOException e ) {
							logErrorMessage( 
								PEAR_MERGER + " error: " + e.toString() );
							return false;
						}
					}
		...
		if( listOfPears.size() < 2 ) {
			logErrorMessage( PEAR_MERGER + " error: " + 
					"input PEAR files not specified" );
			return false;
		} else {
		...
		if( __aggregateName == null ) {
			logErrorMessage( PEAR_MERGER + " error: " + 
					"output aggregate name not specified" );
			return false;
		}
		return true;
	}
	 ...
	 * 
	 * @param enable if <code>true</code>, the log file is enabled, otherwise 
	 * it is disabled.
	 */
	...
	/**
	 * Constructor that takes a given array of input PEAR files, a given 
	 * output component name (ID) and a given output PEAR file.
	 *
	 * @param inpPearFiles The given array of input PEAR files.
	 * @param outCompName The given output component name (ID).
	 * @param outPearFile The given output PEAR file.
	 * @exception IOException If any I/O exception occurred during 
	 * initialization.
	 */
	public PMController( 
			File[] inpPearFiles, String outCompName, File outPearFile ) 
			throws IOException {
		...
		for( int i = 0; i < _inpPearFiles.length; i++ )
			logInfoMessage( 
					">> " + _inpPearFiles[i].getAbsolutePath() );
		logInfoMessage( "> Output PEAR =>" ); 
		logInfoMessage( ">> Name = " + _outAggCompName ); 
		logInfoMessage( ">> File = " + _outAggPearFile.getAbsolutePath() );
		logInfoMessage( 
				"> Output root dir: " + _outAggRootDir.getAbsolutePath() );
	}
	/**
	 * Deletes all temporary directories and files after the merging 
	...
	/**
	 * Extracts all specified input PEARs to their corresponding folders 
	 * inside the output root folder. Returns the total size (bytes) of 
	 * extracted files.
	 * 
	 * @return The total size of extracted files.
	 * @throws IOException If any I/O exception occurred during extraction.
	 */
	private long extractInputPears() throws IOException {
		...
		for( int i = 0; i < _inpPearFiles.length; i++ ) {
			...
			totalSize += 
				FileUtil.extractFilesFromJar( pearFile, _outDlgRootDirs[i] );
		}
		return totalSize;
	}
	/**
	 * Intializes the 'PEAR Merger' task attributes, based on the specified 
	 * input.
	 *
	 * @exception IOException If any I/O exception occurred during 
	 * initialization.
	...
	private void initializeTaskAttributes() throws IOException {
		...
		if( _outAggRootDir.isDirectory() ) {
			if( ! FileUtil.deleteDirectory( _outAggRootDir ) )
				throw new IOException( "cannot delete existing folder " + 
						_outAggRootDir.getAbsolutePath() );
	...
	/**
	 * Merges specified input PEARs into one PEAR, which encapsulates 
	 * aggregate AE that refers to input components, as delegates. 
	 * Returns <code>true</code>, if the merging operation completed 
	 * successfully, <code>false</code> otherwise.
	 * 
	 * @return <code>true</code>, if the merge operation completed 
	 * successfully, <code>false</code> otherwise.
	 * @throws IOException If an I/O exception occurred during the merging 
	 * operation.
	 */
	public boolean mergePears() throws IOException {
		...
		logInfoMessage( "[" + PEAR_MERGER + "]: " + 
				"extracting delegate PEARs ..." );
		long totalSize = extractInputPears();
		logInfoMessage( "[" + PEAR_MERGER + "]: " + 
				totalSize + " bytes extracted successfully" );
		// 2nd step: process files in delegate packages and create InsD objs
		for( int i = 0; i < _outDlgRootDirs.length; i++ ) {
			_dlgInstDescs[i] = PMControllerHelper.
								processDescriptors( _outDlgRootDirs[i] );
			if( _dlgInstDescs[i] == null ) {
				logErrorMessage( "[" + PEAR_MERGER + "]: " + 
						"failed to process input package in " + 
		...
		File pkgDescDir = 
			new File( _outAggRootDir, PackageBrowser.DESCRIPTORS_DIR );
		File pkgMetadataDir = 
			new File( _outAggRootDir, PackageBrowser.METADATA_DIR );
		if( pkgDescDir.mkdirs() && pkgMetadataDir.mkdirs() )
			logInfoMessage( "[" + PEAR_MERGER + "]: " + 
					"created merged package directory structure" );
		else
		...
		AnalysisEngineDescription aggDescription = 
			PMUimaAgent.createAggregateDescription( 
					_outAggCompName, _outAggRootDir, _dlgInstDescs, 
					_aeModeEnabled );
		if( aggDescription != null ) {
			PMUimaAgent.
				saveAggregateDescription( aggDescription, aggDescFile );
			logInfoMessage( "[" + PEAR_MERGER + "]: " + 
					"generated aggregate component descriptor" );
			if( System.getProperty( "DEBUG" ) != null )
				logInfoMessage( 
						PMUimaAgent.toXmlString( aggDescription ) );
		} else
			throw new IOException( 
					"cannot generate aggregate component descriptor" );
		// 5th step: generate merged installation descriptor
		_outAggInstDesc = PMControllerHelper.
							generateMergedInstallationDescriptor( 
									_outAggRootDir, 
									_outAggCompName, aggDescFile, 
									_dlgInstDescs, _outDlgRootDirs );
		if( _outAggInstDesc != null ) {
			...
			File outPearFile = 
				FileUtil.zipDirectory( _outAggRootDir, _outAggPearFile );
			logInfoMessage( "[" + PEAR_MERGER + "]: " + 
					"created output aggregate PEAR file - " + 
	...
	/**
	 * Enables or disables AE mode for the next merging operation. If the 
	 * AE mode is enabled, the merger creates AE output descriptor, instead 
	 * of a more specific TAE descriptor. 
	 * <br />Note: by default, the TAE mode is used.
	 * 
	 * @param aeModeFlag If <code>true</code>, the AE mode is enabled, 
	 * otherwise the default (TAE) mode is used.
	 */
