...
public class FSTreeModel implements TreeModel {
    ...
    private static final String defaultRootString = "<html><b>"
            + MainFrame.htmlGrayColor + "FS List - no selection</b></html>";
    ...
    public void update(String indexName, FSIndex index, CAS cas1) {
        ...
        this.rootString = "<html><font color=green>" + indexName
                + "</font> - <font color=blue>" + index.getType().getName()
                + "</font> [" + size + "]</html>";
        this.root = new FSNode(this, FSNode.DISPLAY_NODE, 0, null);
        ...
        for (it.moveToFirst(); it.isValid(); it.moveToNext()) {
            ...
            this.fss.add(new FSNode(this, getNodeType(fs.getType()), fs
                    .hashCode(), count));
            ++count;
        }
        ...
        for (int i = 0; i < this.treeModelListeners.size(); i++) {
            ((TreeModelListener) this.treeModelListeners.get(i))
    ...
    int getNodeType(int addr, Feature feat) {
        ...
        if (this.cas.isStringType(domType)) {
          ...
          return FSNode.DOUBLE_FS;
            ...
            final int featAddr = this.cas.getFeatureValue(addr,
                    ((FeatureImpl) feat).getCode());
            if (this.cas.isArrayType(this.cas.getTypeSystemImpl().getType(
                    this.cas.getHeapValue(featAddr)))) {
    ...
    static ArrayList createArrayChildren(int start, int end, ArrayList array,
            FSTreeModel model) {
        ...
        if (size <= ArrayNode.CUTOFF) {
            ...
            for (int i = 0; i < buckets; i++) {
                ...
                ArrayList grandkids = createArrayChildren(start_i, end_i,
                        array, model);
            ...
            if (rem > 0) {
                ...
                if (rem <= ArrayNode.CUTOFF) {
                    ...
                    node.setChildren(createArrayChildren(start_rem, end, array,
                            model));
                    kids.add(node);
                }
            }
