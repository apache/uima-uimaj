    ...
    public static AnnotationExtent[] createAnnotationMarkups(FSIterator it,
        ...
        for (it.moveToFirst(); it.isValid(); it.moveToNext()) {
            ...
            if (ext.start < fs.getBegin()) {
                ++pos;
                list.add(pos, new AnnotExtent(fs.getBegin(), ext.end,
                        ext.depth + 1, fs.getType().getName()));
                ext.end = fs.getBegin();
                ext = (AnnotExtent) list.get(pos);
            } else {
                // Start at same point.
            ...
            if (ext.end > fs.getEnd()) {
                ...
                // split
                // the extent.
                list.add(pos + 1, new AnnotExtent(fs.getEnd(), ext.end,
                        ext.depth - 1, ext.annotName));
                ext.end = fs.getEnd();
                // ++ext.depth;
                ...
                if (ext.start < fs.getEnd()) {
                    list.add(tmp + 1, new AnnotExtent(fs.getEnd(), ext.end,
                            ext.depth, ext.annotName));
                    ext.end = fs.getEnd();
                }
                ++ext.depth;
            }
        ...
        Style unmarkedStyle = StyleContext.getDefaultStyleContext().getStyle(
        ...
        for (int i = 0; i < size; i++) {
            ...
            switch (ext.depth) {
            case 0: {
                extentArray[i] = new AnnotationExtent(ext.start, ext.end,
                        unmarkedStyle);
                break;
            }
            case 1: {
    ...
    public static MarkupExtent[] createMarkupExtents(FSIterator it, int textLen) {
        ...
        for (it.moveToFirst(); it.isValid(); it.moveToNext()) {
            ...
            if (ext.start < fs.getBegin()) {
                ++pos;
                list
                        .add(pos, new Extent(fs.getBegin(), ext.end,
                                ext.depth + 1));
                ext.end = fs.getBegin();
                ext = (Extent) list.get(pos);
            } else {
            ...
            if (ext.end > fs.getEnd()) {
                ...
                // split
                // the extent.
                list.add(pos + 1, new Extent(fs.getEnd(), ext.end,
                        ext.depth - 1));
                ext.end = fs.getEnd();
                // ++ext.depth;
            } else if (ext.end < fs.getEnd()) {
                ...
                // its depth.
                if (ext.start < fs.getEnd()) {
                    list.add(tmp + 1, new Extent(fs.getEnd(), ext.end,
                            ext.depth));
                    ext.end = fs.getEnd();
                }
                ++ext.depth;
