...
class FSNode extends FSTreeNode {
    ...
    protected void initChildren() {
        ...
        if (this.fSTreeModel.getCas().isArrayType(type)) {
            ...
            if (nodeClass1 == INT_FS
                || nodeClass1 == FLOAT_FS
                || nodeClass1 == STRING_FS
                || nodeClass1 == STD_FS ) {
              for (int i = 0; i < arrayLength; i++) {
                  node = new FSNode(this.fSTreeModel, nodeClass1, cas
                          .getHeapValue(arrayPos), i);
                  arrayNodes.add(node);
                  ++arrayPos;
              }
            }
              ...
              for (int i = 0; i < arrayLength; i++) {
                node = new FSNode(this.fSTreeModel, nodeClass1, cas
                    .ll_getByteArrayValue((int)this.addr, i), i);
                arrayNodes.add(node);
                ++arrayPos;
              }
            }
              ...
              for (int i = 0; i < arrayLength; i++) {
                node = new FSNode(this.fSTreeModel, nodeClass1, cas
                    .ll_getShortArrayValue((int)this.addr, i), i);
                arrayNodes.add(node);
                ++arrayPos;
              }
            }
              ...
              for (int i = 0; i < arrayLength; i++) {
                ...
                node = new FSNode(this.fSTreeModel, nodeClass1, 
                    CASImpl.double2long(temp), i);
                arrayNodes.add(node);
                ++arrayPos;
              }
            }
            ...
            for (int i = 0; i < feats.size(); i++) {
                ...
                long featAddr = cas.getHeapValue((int)this.addr
                    + cas.getFeatureOffset(feat1.getCode()));
                if (cas.isDoubleType(feat1.getRange()) || cas.isLongType(feat1.getRange())) {
                  featAddr = cas.ll_getLongValue((int)featAddr);
                }
                FSNode childNode = new FSNode(this.fSTreeModel,
                        this.fSTreeModel.getNodeType((int)this.addr, feat1),
                        featAddr, feat1);
    ...
    int getStart() {
        ...
        if (isAnnotation()) {
            ...
            return cas.getHeapValue((int)this.addr
                    + cas.getFeatureOffset(feat1.getCode()));
        }
        return -1;
    }
