...
import org.apache.uima.impl.util.gui.ImageButton;
import org.apache.uima.impl.util.htmlview.AnnotationViewGenerator;

/**
 * A GUI for editing style maps for the annotation viewer.
 * <p>
 * A style map is an XML document that describes how each kind of output from a
 * Text Analysis Engine is to be displayed in the annotation viewer.  Each
 * output type that the user wants to be displayed will have an entry in the
 * style map.  Each entry in the style map contains:
 * <ul>
 * <li>The CSS style used to display annotations of that type (for example
 *     "background:blue; color:white;" to display white text on a blue
 *     background.</li>
 * <li>A label used to identify the annotations in the annotation viewer</li>
 * </ul>
 * <p>
 * To invoke the editor, call the
 * {@link launchEditor(TextAnalysisEngine,String)} method.
 *
 * 
 * 
 */
public class StyleMapEditor extends JDialog implements ActionListener
{
	...
	public StyleMapEditor(final JFrame aOwner, TCAS cas)
	{
		...
		JSplitPane splitPane =
			new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, annotationFeaturesPanel, tablePanel);
		...
		TableColumn labelTableColumn = annotationsTable.getColumnModel().getColumn(StyleConstants.LABEL_COLUMN);
		TableColumn typeNameTableColumn = annotationsTable.getColumnModel().getColumn(StyleConstants.TYPE_NAME_COLUMN);
...
//					e.setFeatureValue( "" + tableModel.getValueAt(row, StyleConstants.FEATURE_VALUE_COLUMN));
					vn.add(e.getLabel());
				}
...
//		return getToolkit().getScreenSize();
  ...
  public void setAnalysisEngine(AnalysisEngineDescription  ae)
  {
	 ...
	 * When the user has finished, the new style map is returned.
	 *
	 * @param aAnalysisEngineMetaData Metadata for the AnalysisEngine whose style map is
	 *     to be edited.  This contains the AE's capabilities and type system
	 *     definition, which are needed by the editor.
	 * @param aStyleMapXml An existing style map XML document that will be loaded
	 *    into the editor.  This is optional, if null is passed in, a default
	 *    style map will be automatically generated from the AE metadata.
	 *
	 * @return a new style map XML document.  If the user cancels, null is
	 *    returned.
	 */
	public String launchEditor(AnalysisEngineMetaData aAnalysisEngineMetaData, String aStyleMapXml, TCAS cas)
	{
	...
	/**
	 * Creates a List of StyleMapEntry objects from the given AnalysisEngineMetaData 
   * and style map XML.
	 *
	 * @param aAnalysisEngineMetaData Metadata for the AnalysisEngine whose style map 
   *     is being edited.
	 * @param aStyleMapXml An existing style map XML document.  This is optional,
	 *    if null is passed in, a default style map will be automatically
	 *    generated.
	 *
	 * @return an ArrayList containing one {@link StyleMapEntry} object for each
	 *    output type declared in <code>aTaeMetaData</code>.
	 */
...
//System.out.println(aStyleMapXml );
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		if (source == moveRowUpButton)
		{
			...
			if (selectedRow == -1)
			{
				JOptionPane.showMessageDialog(source, "Select table row", "Error", JOptionPane.ERROR_MESSAGE);
				return;
			}
		...
		else if (source == addTableEntryButton)
		{
			...
			if (typeName == null)
			{
				JOptionPane.showMessageDialog(
					source,
					"You must first select an annotation type or feature",
					"Error",
		...
		else if (source == removeTableRowButton)
		{
			...
			{
				String message =
					"Are you sure you want to remove " + tableModel.getValueAt(row, StyleConstants.LABEL_COLUMN);
				int rv =
			...
			else
			{
				JOptionPane.showMessageDialog(
					source,
					"You must first select a table row to remove",
					"Error",
					JOptionPane.ERROR_MESSAGE);
			}
  ...
  protected StyleMapTable getAnnotationsTable()
  {
	...
	/**
	 * Displays the Style Map Editor GUI and allows the user to interact with
	 * it.
	 *
	 * @param aStyleList an ArrayList containing the style map entries to
	 *   be edited.  When the user modifies a setting in the GUI, the elements
	 *   of this List will be updated.
	 *
	 * @return true if the user exits the dialog by clicking the OK button,
	 *    false if the user has clicked the Cancel button.
	 */
	private boolean launchGUI(ArrayList aStyleList, TCAS cas)
	{
		...
		//maxColumnWidths[StyleConstants.FEATURE_VALUE_COLUMN] = fm.stringWidth(StyleConstants.columnNames[StyleConstants.FEATURE_VALUE_COLUMN]);
		maxColumnWidths[StyleConstants.BG_COLUMN] = fm.stringWidth(StyleConstants.columnNames[StyleConstants.BG_COLUMN]);
		maxColumnWidths[StyleConstants.FG_COLUMN] = fm.stringWidth(StyleConstants.columnNames[StyleConstants.FG_COLUMN]);
...
//		colorChooser.setChooserPanels(preferredPanels);
		...
		final JDialog dialog =
			JColorChooser.createDialog(button, "Pick a Color", true, colorChooser, okListener, null);
	 ...
	 *   be written to XML.
	 *
	 * @return A style map XML document representing the information in
	 *    <code>aStyleList</code>.
	 */
	...
	public void addRow(String typeName) {
		...
		for (int i = 0; i < styleList.size(); i++)
		{
			...
			{
				JOptionPane.showMessageDialog(StyleMapEditor.this, "Duplicate Annotation Type",
