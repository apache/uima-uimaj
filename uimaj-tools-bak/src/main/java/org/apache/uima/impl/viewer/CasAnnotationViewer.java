...
package org.apache.uima.impl.viewer;
 ...
 * There is also a tree view for display details of annotations on which the user clicks.
 * This class extends {@link JPanel} and so can be reused within any Swing
 * application.
 * <p>
 ...
 * <ul>
 * <li>{@link #setConsistentColors(boolean)} - if set to true (default), the color assigned to
 *    any annotation type will be the same across all documents.  If set to false,
 *    colors may vary across documents.</li>
 * <li>{@link #setDisplayedTypes(String[])} - specifies a set of types that will 
 *    be highlighted in the viewer's text pane.</li>
 * <li>{@link #setHiddenTypes(String[])} - specifies a set of types that will NOT
 *    be highlighted in the viewer's text pane.</li>
 * <li>{@link #setHiddenFeatures(String[])} - specifies a set of features that will never
 *    shown in the viewer's annotation details tree.</li>
 * <li>{@link #setHighFrequencyTypes(String[])} - this can be used to specify a set of
 *    types that occur frequently.  These types will the be assigned the most
 *    distinguishable colors.</li>
 * <li>{@link #setInitiallySelectedTypes(String[])} - this can be used to specify a set of
 *    types that will initially be selected (i.e. have their checkboxes checked) in the viewer.
 *    The default is for all types to be initially selected.</li>
 * <li>{@link #setRightToLeftTextOrientation(boolean)} - switches the text pane from
 *    left-to-right (default) to right-to-left mode.  This is needed to support languages
 *    such as Arabic and Hebrew, which are read from right to left.</li>
 * </ul>
 */
    ...
    TreeWillExpandListener, TreeExpansionListener, ItemListener
{
  ...
  /**
   * @deprecated use the zero-argument constructor and call
   *   {@link #setEntityViewEnabled(boolean)}
   */
  ...
  /**
   * Configures whether the viewer will allow the user to switch to
   * "Entity" view, which highlight entities rather than annotations.
   * Entity view mode is only useful if the CAS contains
   * instances of org.apache.uima.klt.Entity.
   * <p>
   * NOTE: if this method is not called, the default is to display
   * entities if and only if the org.apache.uima.klt.Entity type is present
   * in the type system.
   * 
   * @param aDisplayEntities true to enable entity viewing mode, false to
   *   allow annotation viewing only
   */
  ...
  /**
   * Sets whether colors will be consistent in all documents viewed using this viewer.  If set to true,
   * assignments of color to annotation type will persist across documents; if false, colors will be
   * reassigned in each new document.  (Note that if high frequency types are set via
   * {@link #setHighFrequencyTypes(String[])}, the colors for those types will always be consistent,
   * regardless of the value passed to this method.
   * 
   * @param aConsistent true (the default) causes colors to be consistent across documents, false allows
   *   them to vary
   */
  ...
  /**
   * Sets the text orientation.  The default is left-to-right, but needs to be set to
   * right-to-left to properly display some languages, most notably Arabic and Hebrew.
   * 
   * @param aRightToLeft true to put the viewer in right-to-left mode, false for
   ...
   * This mode makes for a cleaner legend at the expense of making it more difficult to toggle
   * which types are selected.  There's also a button in the GUI that lets the user change this
   * setting.  
   * 
   * @param aConsistent true (the default) causes colors to be consistent across documents, false allows
   *   them to vary
   ...
   * @param aTCAS the TCAS to be viewed.
   * 
  ...
  public void setCAS(CAS aCAS)
  {
    ...
    {
      boolean entityTypeExists =
        mCAS.getTypeSystem().getType("org.apache.uima.klt.EntityAnnotation") != null;
    ...
    Feature sofaIdFeat = aCAS.getTypeSystem().getFeatureByFullName(
    ...
    while (sofas.hasNext())
    {
      ...
      if (sofa.getLocalStringData() != null)
      {
        ...
        if (viewOfSofa == aCAS)
        {
          sofaSelectionComboBox.setSelectedIndex(
              sofaSelectionComboBox.getItemCount() - 1);
        }
      }
    }
   ...
   * This is case-insensitive.  Call this method after {@link #setTCAS()}.  It wil apply only to
   * the current document, and will be reset on the next call to {@link #setTCAS()}.
   * 
  ...
  /**
   * Configures the viewer appropriately for displaying a hit against an XML fragments query.  This does
   * not use a sophisticated algorithm for determining the location of the document that matched the query.
   * Currently all it does is call {@link #setInitiallySelectedTypes(String[])} with the list of types
   * mentioned in the query and {@link #applyBoldfaceToKeyword(String[])} on any keywords mentioned in
   * the query.
   * 
   ...
   *    fully-qualified CAS type names.  This is optional; if not specified, type namespaces are
   *    ignored and any type whose local name matches the query will be selected.
   */
   ...
   * @param aNotChecked list of types not to be initially checked
   * JMP
   */
  ...
  /**
   * Creates/updates the display.  This is called when setCAS() is called and again
   * each time to user's mode or checkbox selections change.
   */
    ...
    while (iter.isValid())
    {
      ...
      if (checkbox == null)
      {
            ...
            && !typeNamesContains(mHiddenTypeNames, type.getName()))
        {
          ...
          ) || (mInitiallySelectedTypeNames != null && 
                typeNamesContains(mInitiallySelectedTypeNames, type.getName())));
   ...
   * 
   * @param aPosition the currently selected offset into the document.  All annotations overlapping
   *   this point will be rendered in the tree.
   */
...
//    }
   ...
   * Annotations in the tree are grouped by type.
   * 
  ...
  private void addAnnotationToTree(AnnotationFS aAnnotation)
  {
    ...
    while (typeNodes.hasMoreElements())
    {
      ...
      if (aAnnotation.getType().equals(
           ((TypeTreeNodeObject)node.getUserObject()).getType()))
      {
        typeNode = node;
        break;
      }
  ...
  public void treeExpanded(TreeExpansionEvent event)
  {
    ...
    if (lastPathComponent instanceof DefaultMutableTreeNode)
    {
      DefaultMutableTreeNode expandedNode = (DefaultMutableTreeNode) lastPathComponent;
      Object userObj = expandedNode.getUserObject();
      if (userObj instanceof TypeTreeNodeObject &&
          expandedNode.getChildCount() == 1)
      {
  ...
  private void doBoldface()
  {
    ...
    len -= len % 2; //to avoid ArrayIndexOutOfBoundsException if some numbskull passes in an odd-length array
    int i = 0;
    while (i < len)
    {
  ...
  /**
   * A panel that is to be placed in a JScrollPane that can only scroll vertically.  This panel should
   * have its width track the viewport's width, and increase its height as necessary to display all
   * components.
   * 
   * 
   */
