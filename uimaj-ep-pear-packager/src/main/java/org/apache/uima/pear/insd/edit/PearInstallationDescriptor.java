...
public class PearInstallationDescriptor {
	...
	public static void saveInstallationDescriptor(IContainer currentContainer, InstallationDescriptor insd) throws CoreException, IOException {
		...
		installFile.setContents(InstallationDescriptorHandler.getInstallationDescriptorAsStream(insd), false, true, null);
	}
	
	/**
	 * 
	 * Returns an InstallationDescriptor instance representing the installation descriptor of the given project.
	 * If not existent, it return and empty InstallationDescriptor instance.
	 * 
	 * @param currentContainer An IProject with the UIMa Nature
	 * @return InstallationDescriptor The Installation descriptor object 
	 * @throws IOException  if there is problem writing to the xml file of the installation descriptor
	 * @throws SAXException  if there is problem parsing the xml of the installation descriptor
	 * @throws CoreException if there is problem with resources
	 * @throws PearException if there is problem with PEAR related operations
	 * 
	 */
	public static InstallationDescriptor getInstallationDescriptor(IContainer currentContainer) throws IOException, SAXException, CoreException, PearException {
		...
		} else createInstallationDescriptor(currentContainer,insd,false);		
		return insd;		
	}
	 ...
	 * 
	 * @param newContainer An IProject with the UIMA nature
	 * @param insd an instance of Installationdescriptor
	 * @param overrideContentIfExist if true overrides any existing install.xml
	 * @throws PearException If a problem occurs.
	 */
	public static void createInstallationDescriptor(IContainer newContainer, InstallationDescriptor insd, boolean overrideContentIfExist) throws PearException {
		try {
			ProjectCustomizer.createFile(newContainer,INSTALLATION_DESCRIPTOR_PATH,InstallationDescriptorHandler.getInstallationDescriptorAsStream(insd), overrideContentIfExist);
		} catch (Throwable e) {
			PearException subEx = new PearException("The Installation Descriptor ("+INSTALLATION_DESCRIPTOR_PATH+") could not be created/saved properly.",e);
			throw subEx;
		}
	}
	...
	public static String addMacro(String s) {
		...
		if (s == null || s.length() == 0) return "";
			...
			if (s.startsWith("/") || s.startsWith("\\")) s = macro + s;
			else s = macro + File.separator + s;
			return s;
		}
	}
