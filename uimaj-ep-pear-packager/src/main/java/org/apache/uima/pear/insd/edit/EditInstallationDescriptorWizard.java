...
public class EditInstallationDescriptorWizard extends Wizard implements IWizard, InsdConstants {
	 ...
	 * Constructor.
	 * 
	 * @param project An IProject with the UIMA Nature
	 */
	public EditInstallationDescriptorWizard(IProject project) {		
		...
		try {
			...
			currentProject = project;
				...
				PearException subEx = new PearException("Operation failed because the wizard could not be initialized.\nPlease report this error.",e);
		...
		try {
			...
			PearException subEx = new PearException("Operation failed because the wizard could not customize your project as a UIMA project.",e);
			subEx.openErrorDialog(getShell());
			this.dispose();
		}
	}
	...
	private void handleComponentInformation() {		
		...
		insd.setMainComponentDesc(PearInstallationDescriptor.addMacro(componentPage.compDescriptorPath));					
	}
	private void addEnvOptions() {
		...
		if (os!=null && os.trim().length()>0) insd.addOSSpec(InstallationDescriptorHandler.NAME_TAG,os);	
		
		String jdkVersion = environmentPage.jdkVersionCombo.getText();
		if (jdkVersion!=null && jdkVersion.trim().length()>0) insd.addToolkitsSpec(InstallationDescriptorHandler.JDK_VERSION_TAG,jdkVersion);			
	...
	private void addEnvVars() {	
		...
		while (envVarsItr.hasNext()) {
			...
			if (envVarName!=null && envVarValue!=null && envVarName.trim().length()>0 && envVarValue.trim().length()>0) {	
