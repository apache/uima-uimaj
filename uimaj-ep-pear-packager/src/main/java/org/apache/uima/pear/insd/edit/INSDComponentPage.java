...
public class INSDComponentPage extends WizardPage implements InsdConstants {
	...
	void initializePage() {
		...
		if (temp==null || temp.trim().length()==0)	compID = currentContainer.getName();
		else compID = temp;
	...
	public void createControl(Composite parent) {
		...
		Label compIDLabel = new Label(gr, SWT.NULL);	compIDLabel.setText(" ");
		
		compDescriptorPathText = addTextField(gr, "&Component Descriptor*:         ", compDescriptorPath,true);												   
	...
	private void dialogChanged() {
		...
		compID = compIDText.getText(); System.out.println("compID: " + compID);
		compDescriptorPath = compDescriptorPathText.getText(); System.out.println("compDescriptorPath: " + compDescriptorPath);
		insd.setMainComponentDesc(compDescriptorPath);
				
		saveWizardData();
		if (compID == null || compID.trim().length() == 0  
						   || compDescriptorPath == null  || compDescriptorPath.trim().length() == 0) { 
	...
	private String validateFiles() {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (filename!=null && filename.trim().length()>0) {
				...
				if (!iFile.exists()) sb.append("\n  \""+filename+"\" was not found in the current project!" );
				else if (filename.trim().indexOf(".xml")==-1) sb.append("\n  \""+filename+"\" is not an xml file!" );
			}	
		}
		String s = sb.toString();
		if (s.length()> 0) message = s;		
		...
		if (defaultButton) {
			...
			SelectionListener listener = new SelectionAdapter() {
				public void widgetSelected(SelectionEvent e) {
					...
					ResourceSelectionDialog dialog = new ResourceSelectionDialog(getShell(), currentContainer,"Selection Dialog");
	...
	public void setVisible(boolean visible) {
		...
		if (visible) compIDText.setFocus();
	}
}