...
public class PearFileResourceExportPage extends WizardPage implements
	...
	protected void pageStateChanged() {
		...
		if (!isMetadataIncluded()) {
			setErrorMessage(PearExportMessages
					.getString("PearExport.metadataRequired")); //$NON-NLS-1$
		...
		if (new File(filename).isDirectory()) {
			setErrorMessage(PearExportMessages
					.getString("PearFileResourceExportPage.DirectorySelected")); //$NON-NLS-1$
			setPageComplete(false);
			return;
		}
		...
		if (filename == null || filename.trim().equals("")) { //$NON-NLS-1$
			...
			setMessage(PearExportMessages
					.getString("PearFileResourceExportPage.SelectFile"), //$NON-NLS-1$
	 ...
	 *         selected for export or the members cannot be determined,
	...
	/**
	 * Opens a file selection dialog to select a pear file as export location
	 * and sets the chosen value to the input field
	 */
	protected void handleDestinationBrowseButtonPressed() {
		final FileDialog dialog = new FileDialog(getContainer().getShell(),
				SWT.SAVE);
		dialog.setFilterExtensions(new String[] { "*.pear", "*.*" }); //$NON-NLS-1$ //$NON-NLS-2$
		dialog.setText(PearExportMessages
				.getString("PearExport.selectDestinationTitle")); //$NON-NLS-1$
	...
	/**
	 * @return The value of the Pear file export destination as chosen by the
	 *         user, or the last used one if the widget was not created yet
	 */
	...
	protected void createOptionsGroup(final Composite parent) {
		...
		group.setText(PearExportMessages
				.getString("PearFileResourceExportPage.Options")); //$NON-NLS-1$
	 ...
	 * and select/deselect all buttons
	 * 
	 * @param parent
	 *            the parent composite
	 */
	protected void createSourceControl(final Composite parent) {
		...
		selectAllButton.setText(PearExportMessages
				.getString("PearFileResourceExportPage.SelectAll")); //$NON-NLS-1$
	 ...
	 *            children
	 * @param checked
	 *            <code>true</code> to check all items, <code>false</code>
	...
	/**
	 * Creates the Pear file export destination controls, i.e. label, input
	...
	protected void createDestinationControl(final Composite parent) {
		...
		toPearFileLabel.setText(PearExportMessages
				.getString("PearExport.destinationLabel")); //$NON-NLS-1$

		fDestinationFileInput = new Text(container, SWT.BORDER);
		fDestinationFileInput.setText(fPreferenceStore.getString(PEAR_FILE));
		fDestinationFileInput.setLayoutData(new GridData(SWT.FILL, SWT.DEFAULT,
				true, false));
	...
	protected ContainerCheckedTreeViewer createTreeViewer(Composite parent) {
		final ContainerCheckedTreeViewer treeViewer = new ContainerCheckedTreeViewer(
				parent);
	...
	/**
	 * @return An {@link IRunnableWithProgress} that will export the files
	 *         chosen in the TreeViewer to the file chosen in the destination
	 *         input field when run
	 */
	public IRunnableWithProgress getExportRunnable() {
		...
		return new PearExportOperation(exports, fCurrentContainer,
				getDestinationValue(), fCompressCheckbox.getSelection());
	}
}
