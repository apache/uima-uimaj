...
/**
 * This class represents a simple pool of {@link CAS} instances.  This is
 * useful for multithreaded applications, where there is a need for multiple
 * CASes to be processed simultaneously.  Because CAS creation is expensive,
 * it is a good idea to create a pool of reusable CAS instances at 
 * initialization time, rather than creating a new CAS each time one is needed. 
 * <p>
 * Clients check-out CAS instances from the pool using the
 * {@link #getCas()} method and check-in CAS instances using the
 * {@link #releaseCas(CAS)} method.
 * 
 * 
 * 
 */
public class CasPool
{
   ...
   *
   * @param aNumInstances the number of CAS instances in the pool
   * @param aComponentDescriptionsOrMetaData a collection of {@link AnalysisEngineDescription}, 
   *    {@link CollectionReaderDescription}, {@link CasConsumerDescription}, 
   ...
   *
   * @param aNumInstances the number of CAS instances in the pool
   * @param aAnalysisEngine the analysis engine that will create the
   *    CAS instances and which will later be used to process them
   * 
   * @throws ResourceInitializationException if the CAS instances could not be
   *    created
   */
    ...
    throws ResourceInitializationException
  {
    ...
    fillPool(mdList, aAnalysisEngine.getPerformanceTuningSettings(), aAnalysisEngine.getResourceManager());
  }  
   ...
   * 
   * @param aNumInstances the number of CAS instances in the pool
   * @param aCasDefinition the Cas definition, which includes the type system,
   *   type priorities, and indexes for the CASes in the pool.
   * @param aPerformanceTuningSettings Properties object containing framework performance
   *    tuning settings using key names defined on {@link UIMAFramework} interface
   */
   ...
   * 
   * @return a CAS instance.  Returns <code>null</code>
  ...
  public synchronized void releaseCas(CAS aCas)
  {
    ...
    if (!mAllInstances.contains(cas) ||
        mFreeInstances.contains(cas))
    {
      UIMAFramework.getLogger(CLASS_NAME).logrb(Level.WARNING, CLASS_NAME.getName(),
      	 "releaseCas", LOG_RESOURCE_BUNDLE, "UIMA_return_cas_to_pool__WARNING");
    }
    else
    {
      //reset CAS
  ...
  /**
   * Checks out a CAS from the pool.  If none is currently available,
  ...
  /**
   * Gets the size of this pool (the total number of CAS instances that 
   * it can hold).
   * 
  ...
  public int getSize()
  {
    ...
    */
   private void fillPool(Collection mdList, Properties performanceTuningSettings, ResourceManager resourceManager)
   throws ResourceInitializationException
   {
