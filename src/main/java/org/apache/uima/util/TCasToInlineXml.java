...
/**
 * Generates an inline XML representation of a TCAS.  Annotation types are
 * represented as XML tags, features are represented as attributes.  Note that
...
public class TCasToInlineXml implements TCasFormatter
{
   ...
   * 
   * @param aTCAS TCAS to generate from
   */
  public String generateXML(TCAS aTCAS) 
    throws CASException
  {       
    return generateXML(aTCAS, null);
  }
   ...
   * 
   * @param aTCAS TCAS to generate from
   * @param aFilter constraint that determines which annotations are included
   *    in the output.  If null (or ommitted), all annotations are included.
   */
  public String generateXML(TCAS aTCAS, FSMatchConstraint aFilter)
    throws CASException
  {
    ...
    {    
      ...
      while (iterator.isValid())
      {
        ...
        //System.out.println("Type: " + fs.getType().getName() + ", Class:" + fs.getClass().getName());
        ...
        if (curAnnot == null || nextAnnot.getBegin() < curAnnot.getEnd())
        {
          ...
          {         
            ...
            {
            System.err.println("Invalid annotation range: " + 
               nextAnnot.getBegin() + "," + nextAnnot.getEnd() + 
               " in document of length " + docText.length());
            }                
          }
          iterator.moveToNext();
        }
        else
        {
          ...
          {
            System.err.println("Invalid annotation range: " + 
               curAnnot.getBegin() + "," + curAnnot.getEnd() + 
               " in document of length " + docText.length());
          }                
          handler.endElement("",curAnnot.getType().getName(),
            curAnnot.getType().getName());
  ...
  private final Attributes getFeatureAttributes(FeatureStructure aFS, TCAS aTCAS)
  {
    ...
    while (iter.hasNext())
    {
      ...
      if (aTCAS.getTypeSystem().subsumes(stringType, feat.getRange())) //must check for subtypes of string
      {
  ...
  private void replaceInvalidXmlChars(char[] aChars)
  {
    for (int i = 0; i < aChars.length; i++)
    {
      if ((aChars[i] < 0x20 && aChars[i] != 0x09 && aChars[i] != 0x0A && aChars[i] != 0x0D) ||
          (aChars[i]>0xD7FF && aChars[i] < 0xE000) ||
          aChars[i]==0xFFFE || aChars[i] == 0xFFFF)  
      {
        //System.out.println("Found invalid XML character: " + (int)aChars[i] + " at position " + i); //temp
        aChars[i] = ' ';    
      }
    }
  }
