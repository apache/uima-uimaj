  ...
  implements ResourceCreationSpecifier
{
  ...
  /**
   * Sets the MetaData for this <code>ResourceCreationSpecifier_impl</code>.
   * Users should not do this, so this method is not published through the
   * <code>ResourceCreationSpecifier</code> interface.
   * 
  ...
  public void setMetaData(ResourceMetaData aMetaData)
  {
	...
	{
		ExternalResourceDependency[] result = 
			new ExternalResourceDependency[mExternalResourceDependencies.size()];
   ...
   * 
   * @param aDesc the AnalysisEngineDescripion to validate
   *
  ...
  /**
   * Validates configuration parameters within this Resource,
   * and throws an exception if they are not valid.
   * <p>
   * This method checks to make sure that there are no duplicate configuration
   * group names or duplicate parameter names within groups.  For aggregates,
   * it also checks that parameter overrides are valid, and logs a warning
   * for parameters with no overrides.  (For primitives, there should be no overrides.)
   * 
   * @param aMetaData ResourceMetaData object containing parameters to validate
   * 
   * @throws ResourceInitializationException if the configuration parameters
   *   are invalid
   */
      ...
      throws ResourceInitializationException
  {
    ...
    else
    {
        ...
        for (int i = 0; i < commonParams.length; i++)
        {
          if (!commonParamNames.add(commonParams[i].getName()))
          {
            throw new ResourceInitializationException(
                ...
                new Object[] { commonParams[i].getName(), getMetaData().getName(), commonParams[i].getSourceUrlString() });
          }
        }
      }
      ...
      if (groups != null)
      {
        HashMap groupToParamSetMap = new HashMap(); //map from group name to HashSet of param names in that group
            ...
            if (paramsInGroup != null)
            {
              for (int k = 0; k < paramsInGroup.length; k++)
              {
                if (!paramNamesInGroup.add(paramsInGroup[k].getName()))
                {
                  throw new ResourceInitializationException(
                    ResourceInitializationException.DUPLICATE_CONFIGURATION_PARAMETER_NAME,
                    new Object[] { paramsInGroup[k].getName(), getMetaData().getName(), paramsInGroup[k].getSourceUrlString() });
                }             
              }
            }  
  ...
  /**
   * Checks for duplicate parameter names and throws an exception if any are
   * found.
   * 
   * @param aParams an array of ConfigurationParameters
   * 
   * @throws ResourceInitializationException if there is a duplicate parameter
   *   name in the arrays
   */
    ...
    throws ResourceInitializationException
  {
    ...
    for (int i = 0; i < aParams.length; i++)
    {
      if (!paramNames.add(aParams[i].getName()))
      {
        throw new ResourceInitializationException(
            ResourceInitializationException.DUPLICATE_CONFIGURATION_PARAMETER_NAME,
            new Object[] { aParams[i].getName(), getMetaData().getName(), aParams[i].getSourceUrlString() });
      }
    }
  }

  /**
   * Checks parameter overrides and throws a ResourceInitializationException
   * if they are invalid.  Note that since overrides are invalid in primitive
   * components, this default implementation throws an exception for ANY 
   * parameter override it finds.  AnalysisEngineDescription_impl overrides
   * this method to correctly handle parameter overrides in aggregates. 
   * 
   * @param aParams an array of ConfigurationParameters
   * @param aGroupName name of groups in which these parameters are contained.  Null if 
   *    no group
   * 
      ...
      if (overrides.length > 0)
      {
        throw new ResourceInitializationException(
            ResourceInitializationException.PARAM_OVERRIDE_IN_PRIMITIVE, new Object[] {
                aParams[i].getName(), getMetaData().getName(), aParams[i].getSourceUrlString() });
      }
    }
  }   
  ...
  static final protected XmlizationInfo XMLIZATION_INFO =
    new XmlizationInfo("resourceCreationSpecifier",
      new PropertyXmlInfo[]{
