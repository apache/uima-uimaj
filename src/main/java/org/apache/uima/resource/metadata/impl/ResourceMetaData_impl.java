...
public class ResourceMetaData_impl extends MetaDataObject_impl
  implements ResourceMetaData
{
   ...
   * types are compatible.  It does NOT check that all mandatory parameters have 
   * been assigned values - this should be done at resource initialization time
   * and not before.
   * <P>
   * NOTE: this method can cause a change to the ConfigurationParameterSettings
   *   object in the case where the value of a parameter is an empty Object[]
   *   and the parameter type is an array of a different type.  In this case
   *   the empty object array will be replaced by an empty array of the
   *   appropriate type.
   * 
  ...
  { 
    ConfigurationParameterDeclarations cfgParamDecls =
      getConfigurationParameterDeclarations();
    ConfigurationParameterSettings cfgParamSettings = 
   ...
   * 
   * @param aNVPs the parameter settings
   * @param aGroupName the group
   * @param aParamDecls Configuration Parameter Declarations
   * 
   * @throws ResourceConfigurationException if the configuration parameter
   *   settings are invalid
   */
  protected void validateConfigurationParameterSettings(
    NameValuePair[] aNVPs, String aGroupName,
   ...
   * <P>
   * NOTE: this method can cause a change to the NameValuePair
   *   object in the case where the value of a parameter is an empty Object[]
   *   and the parameter type is an array of a different type.  In this case
   *   the empty object array will be replaced by an empty array of the
   *   appropriate type.
   ...
   * @param aNVP name value pair containing candidate value
   * 
  ...
  protected void validateConfigurationParameterDataTypeMatch(
     ConfigurationParameter aParam, NameValuePair aNVP)
     throws ResourceConfigurationException
  {
    ...
    if (aParam.isMultiValued()) //value must be an array
    {
      ...
      {
        throw new ResourceConfigurationException(
          ResourceConfigurationException.ARRAY_REQUIRED,
          new Object[]{paramName, getName()});
      }
      valClass = valClass.getComponentType();
      //check for zero-length array special case
      if (Array.getLength(aNVP.getValue()) == 0 &&
          valClass.equals(Object.class))
      {        
        aNVP.setValue(
          Array.newInstance(getClassForParameterType(paramType), 0));  
        return;  
      }
    }
   ...
   *  
   * @param paramType parameter type name from ConfigurationParameterDeclarations
   * 
  ...
  static final protected XmlizationInfo XMLIZATION_INFO =
    new XmlizationInfo("resourceMetaData",
      new PropertyXmlInfo[]{
  ...
  /**
   * Overridden to validate configuration parameter data types immediately
