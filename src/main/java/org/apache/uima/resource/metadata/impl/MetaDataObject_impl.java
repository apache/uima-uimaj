 ...
 * Provides basic support for getting and setting property values given their
 * names, by storing all attribute values in a HashMap keyed on attribute name.
 * <p>
 * Also provides the ability to write objects to XML and build objects from
 * their DOM representation, as required to implement the
 * {@link XMLizable} interface, which is a superinterface of
 * {@link MetaDataObject}.  In future versions, this could be replaced by a 
 * non-proprietary XML binding solution such as JAXB or EMF.  
 * <p>

 * The implementation for getting and setting property values uses the JavaBeans
 * introspection API.  Therefore subclasses of this class must be valid
 * JavaBeans and either use the standard naming conventions for getters and
 * setters or else provide a BeanInfo class.
 * See <a href="http://java.sun.com/docs/books/tutorial/javabeans/">
...
public abstract class MetaDataObject_impl implements MetaDataObject
{
  ...
  /**
   * Returns a list of <code>NameClassPair</code> objects indicating the 
   * attributes of this object and the Classes of the attributes' values.
   * For primitive types, the wrapper classes will be returned (e.g. 
   * <code>java.lang.Integer</code> instead of int).
   * 
   ...
   * 
   * @return Class of value that may be assigned to the named attribute.
   *     Returns <code>null</code> if there is no attribute with the given name.
   */
  ...
  /**
   * Returns whether this object is modifiable.  MetaDataObjects are modifiable
   * by default.
  ...
  /**
   * Gets the relative path base used to resolve imports.  This is equal to the sourceUrl of
   * this object, if known (i.e. if the object was parsed from an XML file or if setSourceUrl was
  ...
  public String getSourceUrlString()
  {
     ...
     * This is used internally to update null relative base paths before doing import
     * resolution, without overriding user-specified settings.
	 *    
...
//      assert false : "All MetaDataObjects are Cloneable";
   ...
   * MetaDataObjects are equivalent if they share the same attributes
  ...
  /**
   * Gets the hash code for this object.  The hash codes of two
   * NameClassPairs <code>x</code> and </code>y</code> must be equal
   * if <code>x.equals(y)</code> returns true;
   * 
  ...
  /**
   * Called by the {@link toXML(Writer,String)} method to get the XML
   * attributes that will be written as part of the element's tag.
   * By default this method returns an empty Attributes object.  Subclasses may 
   * override it in order to write attributes to the XML.
   * 
  ...
  /**
   * To be implemented by subclasses to return information describing how
   * to represent this object in XML.
   * 
  ...
  /**
   * Looks in this class's XmlizationInfo for a property with the given XML
   * element name.
   * 
   * @param aXmlElementName the unqualified name of an XML element
   * 
   * @return information on the property that corresponds to the given element
   *    name, <code>null</code> if none.
   */ 
   ...
   * @param aContentHandler content handler to which this object will send
   *    events that describe its XML representation
   */
   ...
   * @param aArrayElementTagName name of tag to be assigned to each element
   *   of the array.  May be <code>null</code>, in which case each element
   *   will be assigned a value appropriate to its class.
   * @param aNamespace the XML namespace URI for this object
   * @param aContentHandler the ContentHandler to which this object will write
   *   events that describe its XML representation
   */
  protected void writeArrayPropertyAsElement(String aPropName,
     Class aPropClass, Object aValue, String aArrayElementTagName,
     String aNamespace, ContentHandler aContentHandler)
    throws SAXException
  {  
  ...
  /**
   * Utility method for writing to XML an property whose value is
   * a <code>Map</code> with <code>String</code> keys and
   * <code>XMLizable</code> values.
   * 
   * @param aPropName name of the property to write to XML
   * @param aXmlElementName name of the XML element for the property,
   *   <code>null</code> if none
   * @param aKeyXmlAttributeName name of the XML attribute for the key
   * @param aValueTagName XML element tag name to use for each
   *    entry in the Map
   * @param aOmitIfNull if true, null or empty map will not be written at all, 
   *    if false, null or empty map will be written as an empty element
   * @param aNamespace namespace for this object
   * @param aContentHandler ContentHandler to which this object will send
   *    events describing its XML representation
   */
  protected void writeMapPropertyToXml(String aPropName, 
    String aXmlElementName, String aKeyXmlAttribute,
    String aValueTagName, boolean aOmitIfNull,
    String aNamespace, ContentHandler aContentHandler)
    throws SAXException
  {          
    //get map
    ...
    if (theMap == null || theMap.isEmpty())
    {
      ...
      {
        aContentHandler.startElement(aNamespace,aXmlElementName,
            aXmlElementName, EMPTY_ATTRIBUTES);
        ...
        else
        {
          ...
          if (getXmlizationInfo().propertyInfo.length == 1)
          {
            readPropertyValueFromXMLElement(
              getXmlizationInfo().propertyInfo[0], aElement, aParser, aOptions);          
          }                 
          else
          {
   ...
   * @param aOptions option settings
   */
  protected void readPropertyValueFromXMLElement(
    PropertyXmlInfo aPropXmlInfo, Element aElement, XMLParser aParser,
    XMLParser.ParsingOptions aOptions)
    throws InvalidXMLException
  {      
    ...
    {
      readArrayPropertyValueFromXMLElement(aPropXmlInfo, propClass, aElement,
         aParser, aOptions);
    }   
      ...
      if (objElem != null)
      {
        ...
        if (objElem.getTagName().equals("array"))
        {
          readArrayPropertyValueFromXMLElement(
            aPropXmlInfo, Object.class, objElem, aParser, aOptions);   
        }
        else
        {
  ...
  /**
   * Utility method to read an array property's value from its DOM 
   * representation.
   * 
   * @param aPropXmlInfo information about the property to read
   ...
   * @param aOptions option settings
   */
  protected void readArrayPropertyValueFromXMLElement(
    PropertyXmlInfo aPropXmlInfo, Class aPropClass, Element aElement, XMLParser aParser,
    XMLParser.ParsingOptions aOptions)
    throws InvalidXMLException
  {
  	Constructor primitiveElementStringConstructor = null; //may be used to build objects from strings
        ...
        if (aPropXmlInfo.arrayElementTagName != null)
        {
          if (curElem.getTagName().equals(aPropXmlInfo.arrayElementTagName))
          {
            ...
            try
            {
              ...
              {
              	primitiveElementStringConstructor = 
                  aPropClass.getComponentType().getConstructor(new Class[]{String.class});
              }    
              //construct the object and add to list
              valueList.add(primitiveElementStringConstructor.newInstance(new Object[]{elemText}));
            }
            catch(Exception e)
            {
          ...
          else //element type does not match
            throw new InvalidXMLException(
              InvalidXMLException.INVALID_ELEMENT_TYPE,
    ...
    if (!(aPropClass == Object.class))
    {
      ...
      while (i.hasNext())
      {
        ...
        {
          throw new InvalidXMLException(
            InvalidXMLException.INVALID_CLASS,
  ...
  /**
   * Utility method that attempts to read a property value from an XML
   * element even though it is not known to which property the value
   * should be assigned.  If an object can be constructed from the XML element,
   * it will be assigned to any unasigned property that can accept it.
   * 
   * @param aElement DOM element to read from
   * @param aParser parser to use to construct complex values
   * @param aKnownPropertyNames List of propertiees that we've already values
   *   for (these values will not be overwritten)
   * 
  ...
  protected void readUnknownPropertyValueFromXMLElement(Element aElement, 
    XMLParser aParser, XMLParser.ParsingOptions aOptions, List aKnownPropertyNames)
    throws InvalidXMLException
  {      
  ...
  /**
   * Utility method for reading from XML an attribute whose value is
   * a <code>Map</code> with <code>String</code> keys and
   * <code>XMLizable</code> values.
   * 
   ...
   * @param aValueIsArray true if the value of the map entires is an array.
   *   This method only supports homogeneous arrays.
   */
  protected void readMapPropertyFromXml(String aPropName,
    Element aElement, String aKeyXmlAttribute, String aValueTagName, 
    XMLParser aParser, XMLParser.ParsingOptions aOptions, boolean aValueIsArray)
    throws InvalidXMLException
  {      
   ...
   * For example, <code>java.lang.Integer</code> is the wrapper class for
   * the primitive type <code>int</code>.
   *
   * @param aPrimitiveType <code>Class</code> object representing a primitive
   *    type
   * 
  ...
  * <p>
  * The JavaBeans introspector is used, with the IGNORE_ALL_BEANINFO flag.
  * This saves on initialization time by preventing the introspector from
  * searching for nonexistent BeanInfo classes for all the MetaDataObjects.
  * 
  * @return the <code>PropertyDescriptors</code> for all properties introduced
  *   by subclasses of <code>MetaDataObject_impl</code>.
  * 
    ...
    throws IntrospectionException
  {
    if (mPropertyDescriptors == null)
    {
      mPropertyDescriptors = 
        Introspector.getBeanInfo(this.getClass(),Introspector.IGNORE_ALL_BEANINFO).
