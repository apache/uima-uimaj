 ...
 * {@link org.apache.uima.resource.ConfigurationParameter}.
 * 
 * 
 */
...
*/  
  ...
  /**
   * Determines whether the given Java class is an appropriate value for a
   * parameter with the specified type.  For example, if the parameter's type
   * is "Integer", then java.lang.Integer is a match but java.lang.String
   * is not.
   * 
   * @param aClass class to check
   * @param aTypeName configuration parameter type, as defined by one of
   *    the TYPE constants on the {@link ConfigurationParameter} interface.
   * @param aMultiValued true if and only if the configuration parameter is
   *    multi-valued.  If true, <code>aClass</code> is expected to be an array.
   * 
   * @return true if and only if an object of class <code>aClass</code>
   *     can be legally assigned to a parameter described by 
   *     <code>aTypeName</code> and <code>aMultiValued</code>.
   */
  public static boolean typeMatch(Class aClass, String aTypeName,
   ...
   * Valid data type names are legal arguments to the
   * {@link setType(String)} method, and are defined by the TYPE constants on
   * the {@link ConfigurationParameter} interface.
   * 
   * @param aTypeName an Object to test
   * 
   * @return true if and only if <code>aTypeName</code> is a <code>String</code>
   *     that is a valid data type name.
   */
  protected static boolean isValidDataTypeName(Object aTypeName)
  {
    return TYPE_STRING.equals(aTypeName) ||
            TYPE_BOOLEAN.equals(aTypeName) ||
            TYPE_INTEGER.equals(aTypeName) ||
            TYPE_FLOAT.equals(aTypeName); 
  }

  /**
   * Overriden to allow both "param" and "parameter" as the
   * array element tags.  (For historical reasons.)
   * 
    ...
    throws InvalidXMLException
  {
      ...
      for (int i = 0; i < numChildren; i++)
      {
        ...
        if (curNode instanceof Element)
        {
          Element curElem = (Element)curNode;
	        ...
	        if ("parameter".equals(curElem.getTagName()) ||
	            "param".equals(curElem.getTagName()))
	        {
            //get text of element
    ...
    {
	    super.readArrayPropertyValueFromXMLElement(aPropXmlInfo,
	         aPropClass, aElement, aParser, aOptions);
    }    
  }
  ...
  static final private XmlizationInfo XMLIZATION_INFO =
    new XmlizationInfo("configurationParameter",
      new PropertyXmlInfo[]{
         new PropertyXmlInfo("name"),
         new PropertyXmlInfo("description"),
         new PropertyXmlInfo("type"),
         new PropertyXmlInfo("multiValued"),
  ...
  private boolean mMandatory;
	...
	/** The parameters that this Configuration Parameter overrides.
	 *  Elements of this array are of the form componentName/parameterName. */
