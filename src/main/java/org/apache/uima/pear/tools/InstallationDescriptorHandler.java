...
/**
 * The <code>InstallationDescriptorHandler</code> class allows parsing XML 
 * installation descriptor files, creating instances of the 
 * <code>InstallationDescriptor</code> class. The 
 * <code>InstallationDescriptorHandler</code> class also allows to save 
 * existing <code>InstallationDescriptor</code> objects as XML files.
 * 
 */
...
public class InstallationDescriptorHandler extends DefaultHandler {
	...
	/**
	 * Returns the content of the installation descriptor XML file for a given 
	 * intallation descriptor object as <code>InputStream</code> object (for 
	 * use in Eclipse plug-in).
	 * 
	 * @param insdObject The given intallation descriptor object.
	 * @return The <code>InputStream</code> object that contains the content 
	 * of the installation descriptor XML file.
	 * @throws IOException if any I/O exception occurred.
	 */
	public static InputStream getInstallationDescriptorAsStream( 
	...
	/**
	 * Prints a given <code>InstallationDescriptor</code> object in XML format
	 * to a given <code>PrintWriter</code>.
	 * 
	 * @param insdObject The given <code>InstallationDescriptor</code> object.
	 * @param oWriter The given <code>PrintWriter</code> object.
	 * @throws IOException if any I/O exception occurred.
	 */
	public static void printInstallationDescriptor( 
		InstallationDescriptor insdObject, PrintWriter oWriter ) 
		throws IOException {
		...
		if( insdObject.getOSSpecs().size() > 0 ) 
			XMLUtil.printAllXMLElements( insdObject.getOSSpecs(), 
					InstallationDescriptor.PROPERTY_DELIMITER, 
					tagOrder, oWriter, 2 );
		...
		if( insdObject.getMainComponentId() != null ) {
			...
			XMLUtil.printXMLElement( 
				ID_TAG, insdObject.getMainComponentId(), oWriter, 2 );
			oWriter.println();
			// NAME
			XMLUtil.printXMLElement( 
				NAME_TAG, insdObject.getMainComponentName(), oWriter, 2 );
			oWriter.println();
			// DESC
			XMLUtil.printXMLElement( 
				DESC_TAG, insdObject.getMainComponentDesc(), oWriter, 2 );
			oWriter.println();
			// DEPLOYMENT
			XMLUtil.printXMLElement( DEPLOYMENT_TAG, 
				insdObject.getMainComponentDeployment(), oWriter, 2 );
			oWriter.println();
			// SERVICE block
			InstallationDescriptor.ServiceInfo service = 
			    insdObject.getMainComponentService();
			if( service != null ) {
			    // SERVICE_COMMAND
				XMLUtil.printXMLElement( 
					SERVICE_COMMAND_TAG, service.command, oWriter, 2 );
				oWriter.println();
			    // SERVICE_WORKING_DIR
				XMLUtil.printXMLElement( SERVICE_WORKING_DIR_TAG, 
					service.workingDirPath, oWriter, 2 );
				oWriter.println();
			    // SERVICE_COMMAND_ARGS
			    XMLUtil.printXMLTag( 
					SERVICE_COMMAND_ARGS_TAG, oWriter, false, 2 );
				oWriter.println();
			    Iterator argList = service.getArgs().iterator();
			    while( argList.hasNext() ) {
			        // ARGUMENT - 3rd level
			        XMLUtil.printXMLTag( 
						ARGUMENT_TAG, oWriter, false, 3 );
				oWriter.println();
			        InstallationDescriptor.ArgInfo arg = 
			            (InstallationDescriptor.ArgInfo)argList.next();
			        // VALUE - 4th level elements
			        XMLUtil.printXMLElement( 
			        	VALUE_TAG, arg.value, oWriter, 4 );
				oWriter.println();
			        // COMMENTS
			        XMLUtil.printXMLElement( 
			        	COMMENTS_TAG, arg.comments, oWriter, 4 );
				oWriter.println();
			        // ARGUMENT end - 3rd level
			        XMLUtil.printXMLTag( 
						ARGUMENT_TAG, oWriter, true, 3 );
				oWriter.println();
			    }
			    // SERVICE_COMMAND_ARGS end
			    XMLUtil.printXMLTag( 
					SERVICE_COMMAND_ARGS_TAG, oWriter, true, 2 );
				oWriter.println();
			}
			// network component parameters block
			Set netParamNames = 
				insdObject.getMainComponentNetworkParamNames();
			if( netParamNames != null ) {
			    // NETWORK_PARAMETERS
			    XMLUtil.printXMLTag( 
			    		NETWORK_PARAMETERS_TAG, oWriter, false, 2 );
				oWriter.println();
			    Iterator nameList = netParamNames.iterator();
			    while( nameList.hasNext() ) {
			    	String name = (String)nameList.next();
			    	Properties attributes = 
			    		insdObject.getMainComponentNetworkParam( name );
			    	XMLUtil.printXMLElement( 
			    		name, attributes, null, oWriter, 3 );
				oWriter.println();
			    }
			    // NETWORK_PARAMETERS end
			    XMLUtil.printXMLTag( 
			    		NETWORK_PARAMETERS_TAG, oWriter, true, 2 );
				oWriter.println();
			}
			if( insdObject.getMainComponentProps().size() > 0 ) 
				XMLUtil.printAllXMLElements( 
						insdObject.getMainComponentProps(), oWriter, 2 );
			...
			if( collIterDesc != null || casInitDesc != null ) {
				...
				if( collIterDesc != null ) {
					XMLUtil.printXMLElement( COLLECTION_ITERATOR_DESC_TAG, 
							collIterDesc, oWriter, 3 );
					oWriter.println();
				}
				if( casInitDesc != null ) {
					XMLUtil.printXMLElement( 
						CAS_INITIALIZER_DESC_TAG, casInitDesc, oWriter, 3 );
					oWriter.println();
				}
		...
		while( dlgList.hasNext() ) {
			...
			InstallationDescriptor.ComponentInfo dlgInfo = 
				(InstallationDescriptor.ComponentInfo)dlgTable.get( dlgId );
		...
		while( actList.hasNext() ) {
			InstallationDescriptor.ActionInfo actInfo = 
				(InstallationDescriptor.ActionInfo)actList.next();
			// PROCESS specs - 2nd level
			XMLUtil.printXMLTag( PROCESS_TAG, oWriter, false, 2 );
			oWriter.println();
			// 3rd level elements
			XMLUtil.printXMLElement( 
				ACTION_TAG, actInfo.getName(), oWriter, 3 );
			oWriter.println();
	...
	/**
	 * Saves a given <code>InstallationDescriptor</code> object in a given
	 * XML file.
	 * 
	 * @param insdObject The given <code>InstallationDescriptor</code> object.
	 ...
	 * @throws IOException if any I/O exception occurred.
	 */
	public static void saveInstallationDescriptor( 
		InstallationDescriptor insdObject, File xmlFile ) throws IOException {
		PrintWriter oWriter = null;
		try {
			oWriter = new PrintWriter( new OutputStreamWriter( 
	...
	public void endElement( String uri, String localName, String qName )
		...
		if( OS_TAG.equals( _activeSection ) ) {
			...
			if( localName.equalsIgnoreCase( SUBMITTED_COMPONENT_TAG ) ) {
				...
				if( SERVICE_COMMAND_ARGS_TAG.equals( _activeSubSection ) && 
					_activeService != null && _activeArg != null && 
					_activeArg.value != null && _activeArg.value.length() > 0 )
				...
				if( SERVICE_COMMAND_ARGS_TAG.equals( _activeSubSection ) ) {
					...
					_insdObject.setMainComponentProperty( 
						COMMENTS_TAG, elemValue );
			...
			if( localName.equalsIgnoreCase( INSTALLATION_TAG ) )
				...
				if( DELEGATE_COMPONENT_TAG.equals( _activeSubSection ) ) {
					if( _activeComponentId != null ) {
						_insdObject.addDelegateComponent( 
							_activeComponentId, elemValue );
						_activeComponentId = null;
					}
				}
				...
				if( PROCESS_TAG.equals( _activeSubSection ) )
					_activeAction = 
						new InstallationDescriptor.ActionInfo( elemValue );
			} else {
				if( _activeAction != null )
	...
	/**
	 * @return <code>InstallationDescriptor</code> object after the 
	 * installation descriptor file has been loaded, or <code>null</code>, if 
	...
	/**
	 * Starts parsing a given XML file. After parsing is completed,
	 * the application may access parsing results using convenient methods.
	 * 
	 * @param xmlFile The given XML file.
	 ...
	 * wrapping another exception.
	 */
	public synchronized void parse( File xmlFile ) 
	 ...
	 * 
	 * @param xmlStream The given XML input stream.
	 * @throws IOException if any I/O exception occurred.
	 * @throws SAXException Any SAX exception, possibly wrapping another 
	 * exception.
	 */
	public synchronized void parse( InputStream xmlStream ) 
		throws IOException, SAXException {
		if( _parser == null )
			_parser = XMLUtil.createSAXParser();
	...
	/**
	 * Parses XML installation descriptor automatically extracting it from a 
	 * given PEAR (JAR) file.
	 * 
	 * @param pearFile The given PEAR (JAR) file.
	 * @throws IOException if any I/O exception occurred.
	 * @throws SAXException Any SAX exception, possibly wrapping another 
	 * exception.
	 */
	...
	/**
	 * Saves created <code>InstallationDescriptor</code> object to a given 
	 * XML file.
	 * 
	 * @param xmlFile The given XML file.
	 * @throws IOException if any I/O exception occurred.
	 */
	public synchronized void saveInstallationDescriptor( File xmlFile ) 
		throws IOException {
		if( _insdLoaded )
	...
	public void startElement(
		...
		if( _mainTag.equalsIgnoreCase( OS_TAG ) ) {
			...
			if( paramSpecs.size() > 0 )
				_insdObject.setMainComponentNetworkParam( 
					paramName, paramSpecs );
		}
	}
	/**
