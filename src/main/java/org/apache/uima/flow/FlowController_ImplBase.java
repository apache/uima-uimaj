...
/**
 * Implementation base class for FlowControllers.  Normally developers do not extend
 * this class directly.  Instead use {@link JCasFlowController_ImplBase} or
 * {@link JCasFlowController_ImplBase}, depending on which CAS interface you would like
 * to use.
 * <p>
 * This class implements the {@link #initialize(FlowControllerContext)} method and
 * stores the <code>FlowControllerContext</code> in a private field where it can be
 ...
 * {@link #destroy()} followed by {@link #initialize(FlowControllerContext)}.
 * Developers of FlowControllers with expensive initialization logic should
...
public abstract class FlowController_ImplBase implements FlowController
{
  ...
  /** 
   * Gets the FlowControllerContext for this FlowController.  This provides access to
   * configuration parameters, resources, and also to metadata for each
   * AnalysisEngine that is available for this FlowController to route CASes to.
   * 
