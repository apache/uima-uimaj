...
public class CharArraySpanMap {
    ...
    private final int isInList(final char[] inputArray, final int start,
            final int strLen, ArrayList entryList) {
        final int listLen = entryList.size();
        Entry entry;
        boolean found = false;
        int i = 0, k;
     ...
     */
    public final boolean containsKey(char[] characterArray, int start,
            int length) {
        final int hashCode = CharArrayString.hashCode(characterArray, start,
                (start + length));
