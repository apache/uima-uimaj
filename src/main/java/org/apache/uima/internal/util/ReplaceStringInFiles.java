...
public class ReplaceStringInFiles {
     ...
     */
    public static int indexOfCaseSensitive(String mainStr, String argStr,
            int start, boolean caseSensitive) {
        if (caseSensitive) {
     ...
     * @param res
     *            The result object, containing the result string and the number
     *            of times replacement happened.
     */
    public static final void replaceStringInString(String text,
            String toReplace, String replacement, ReplacementResult res,
            boolean caseSensitive) {
        StringBuffer buf = new StringBuffer();
        int current = 0;
        int next;
        int count = 0;
        while ((next = indexOfCaseSensitive(text, toReplace, current,
                caseSensitive)) >= 0) {
    ...
    public static final void replaceStringInFiles(File dir,
            FileFilter fileFilter, String toReplace, String replacement,
        ...
        for (int i = 0; i < fileList.length; i++) {
            ...
            if (!file.canWrite()) {
                System.err.println("Warning, can't write file: "
                        + file.getAbsolutePath());
                continue;
            }
            ...
            replaceStringInString(fileContents, toReplace, replacement, result,
                    caseSensitive);
            ...
            if (result.numReplaced > 0) {
                ...
                System.out
                        .println("File modified, number of instances replaced: "
                                + result.numReplaced);
            }
        }
        ...
        for (int i = 0; i < fileList.length; i++) {
            replaceStringInFiles(fileList[i], fileFilter, toReplace,
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (dir.isDirectory()) {
                replaceStringInFiles(dir, new ExtFileFilter(extension),
                        toReplace, replacement, caseSensitive);
            }
