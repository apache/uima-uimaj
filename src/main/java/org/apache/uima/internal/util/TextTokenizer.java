...
/**
 * An implementation of a text tokenizer for whitespace separated natural
 * lanuage text.
 * 
 * <p>
 * The tokenizer knows about four different character classes: regular word
 * characters, whitespace characters, sentence delimiters and separator
 * characters. Tokens can consist of
 * <ul>
 * 
 * <li>sequences of word characters and sentence delimiters where the last
 ...
 * 
 * <p>
 * The character classes are completely user definable. By default, whitespace
 * characters are the Unicode whitespace characters. All other characters are
 * word characters. The two separator classes are empty by default. The
 * different classes may have non-empty intersections. When determining the
 * class of a character, the user defined classes are considered in the
 * following order: end-of-sentence delimiter before other separators before
 * whitespace before word characters. That is, if a character is defined to be
 * both a separator and a whitespace character, it will be considered to be a
 * separator.
 * 
 * <p>
 * By default, the tokenizer will return all tokens, including whitespace. That
 * is, appending the sequence of tokens will recover the original input text.
 * This behavior can be changed so that whitespace and/or separator tokens are
 * skipped.
 * 
 * <p>
 * A tokenizer provides a standard iterator interface similar to
 * {@link java.util.StringTokenizer StringTokenizer}. The validity of the
 * iterator can be queried with <code>hasNext()</code>, and the next token
 * can be queried with <code>nextToken()</code>. In addition,
 * <code>getNextTokenType()</code> returns the type of the token as an
 * integer. NB that you need to call <code>getNextTokenType()</code> before
 * calling <code>nextToken()</code>, since calling <code>nextToken()</code>
...
public class TextTokenizer {
    ...
    /**
     * Set the set of whitespace characters (in addition to the Unicode
     * whitespace chars).
     */
    ...
    public String nextToken() {
        ...
        return new String(this.text, this.nextTokenStart, this.nextTokenEnd
                - this.nextTokenStart);
    }
    ...
    private boolean computeNextToken() {
        ...
        case WSP: {
            ...
            while (this.pos <= this.end
                    && getCharType(this.text[this.pos]) == WSP) {
                ++this.pos;
            }
    ...
    /**
     * Add the characters in <code>s</code> to the sorted array of characters
     * in <code>list</code>, returning a new, sorted array.
     */
