...
/**
 * Integer Red-Black Tree node. Not for public use. Use the interface in
 * IntRedBlackTree instead. This should probably be an internal class to
 * IntRedBlackTree, but it's easier to read in a seperate file. See comments in
 * IntRedBlackTree.
 * 
 * 
 */
class IntRBTNode {
     ...
     */
    private IntRBTNode(int key, boolean color, IntRBTNode parent,
            IntRBTNode left, IntRBTNode right, int element) {
        this.key = key;
        this.color = color;
        this.parent = parent;
        this.left = left;
    ...
    /**
     * Delete a given node from the tree. The node must be contained in the
     * tree! Our code is more complicated than CLR because we don't use a NIL
     * sentinel.
     */
    ...
    /**
     * Like deleteFixup(), only that the node we should be working on is null,
     * and we actually hand in the node's mother. Special case because we don't
     * use sentinels.
     */
    ...
    /**
     * Fill an array with the keys contained in the tree. The array must at
     * least have the size of the tree! Returns the size of the tree, for
     * internal reasons.
     */
     ...
     * {@link org.apache.uima.internal.util.rb_trees.IntRBTArray IntRBTArray} for a
     * definition of the resulting array format.
     * 
     * @param offset
     *            See
