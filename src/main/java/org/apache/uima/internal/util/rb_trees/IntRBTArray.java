...
/**
 * Helper class to read array-based binary search trees with integers as keys
 * and values. No write access to the tree is provided. See
 * {@link org.apache.uima.internal.util.rb_trees.IntRedBlackTree IntRedBlackTree} on how to
 * generate such an array representation. The name is a bit of a misnomer, since
 * nothing in this class is specific to red-black trees.
 * 
 * <p>
 * Suppose <code>i</code> is the position of the first cell encoding a tree
 * node in array <code>a</code>. Then the expected memory layout of
 * <code>a</code> is:
 * <ul>
 ...
 * <li><code>IntRBTArray.LEFTDTR</code>: this node only has a left daughter,
 * so <code>a[i+3]</code> is the first cell of the left daughter node</li>
 * <li><code>IntRBTArray.RIGHTDTR</code>: this node only has a right
 * daughter, so <code>a[i+3]</code> is the first cell of the right daughter
 * node</li>
 * <li><code>IntRBTArray.TWODTRS</code>: this node has two daughters.
 * <code>a[i+3]</code> contains the address of the right daughter, and
 * <code>a[i+4]</code> is the start of the left daughter node</li>
 * </ul>
 * </li>
 * </ul>
 * 
 * <p>
 * Note that the array from which an IntRBTArray object is constructed can
 * contain other data as well. However, we assume that the addressing (the right
 * daughter addresses, to be precise), must be absolute (i.e., not relative to
 * some starting point within the array).
 */
public class IntRBTArray {
     ...
     * @param i
     *            The key.
     * @return The address of the value for <code>i</code>, if it's found;
     *         <code>-1</code>, else. This routine may also return
     *         <code>-1</code> when the tree is corrupted. Of course, with a
     *         corrupted tree, results will in general be unpredictable.
     *         However, this routine will not throw an
