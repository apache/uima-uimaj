...
public interface AnnotationFS extends AnnotationBaseFS {
   ...
   * The smallest possible start position is <code>0</code>, the offset of the
   ...
   * The end position points at the first character after the annotation, such
   * that <code>(getEnd()-getBegin()) == getCoveredText().length()</code>.
   * @return The end position.
   */
  int getEnd();
  
  /**
   * Get the text covered by an annotation as a string.  If <code>docText</code>
   * is your document text and <code>annot</code> an annotation, then <code>
