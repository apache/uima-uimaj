...
/**
 * TCAS: Text Common Analysis System Implementation. These APIs are internal.
 * Note: even though the TCAS interface is no longer needed in v2.0, it is maintained
 * for backwards compatibility with v1.x.  To support v1.x components, we must make
 * sure that all CAS views (not just text views) are instances of TCASImpl.  
 */
public class TCASImpl extends CASImpl implements TCAS, TCASMgr {
    ...
    public TCASImpl(CAS cas, SofaFS aSofa) {
        ...
        this.indexRepository = (this.mySofaRef == -1) ?
        	(FSIndexRepositoryImpl) ((CASImpl)cas).getSofaIndexRepository(1) :
        	(FSIndexRepositoryImpl)	((CASImpl)cas).getSofaIndexRepository(aSofa);
        ...
        this.langFeat = this.docType
                .getFeatureByBaseName(TCAS.FEATURE_BASE_NAME_LANGUAGE);
        
        this.startFeatCode = ((FeatureImpl)getBeginFeature()).getCode();
        this.endFeatCode = ((FeatureImpl)getEndFeature()).getCode();
        this.langFeatCode = getLowLevelCAS().ll_getTypeSystem()
                .ll_getCodeForFeature(this.langFeat);
    }
    ...
    public void updateDocumentAnnotation() {
        ...
        final Feature sofaString = SofaType
                .getFeatureByBaseName(FEATURE_BASE_NAME_SOFASTRING);
    ...
    public void initTypeSystem(boolean fromExTCAS) {
        if (!fromExTCAS) {
            /*
             * ee Document Annotation now created in base CAS via use of
             * BuiltInUIMATypes.xml Type top =
             * this.ts.getType(CAS.TYPE_NAME_TOP); Type intT =
             * this.ts.getType(CAS.TYPE_NAME_INTEGER); // Add annotations
             * this.annotType = this.ts.addType(TCAS.TYPE_NAME_ANNOTATION, top);
             * //assert(this.annotType != null); this.startFeat =
             * this.ts.addFeature(TCAS.FEATURE_BASE_NAME_BEGIN, annotType,
             * intT); this.endFeat =
             * this.ts.addFeature(TCAS.FEATURE_BASE_NAME_END, annotType, intT);
             * this.docType = this.ts.addType(TYPE_NAME_DOCUMENT_ANNOTATION,
             * annotType); this.langFeat = this.ts.addFeature(
             * FEATURE_BASE_NAME_LANGUAGE, docType,
    ...
    public void setDocumentText(String text) throws TCASRuntimeException {
        if (!this.canSetText) {
            throw new TCASRuntimeException(
                    TCASRuntimeException.SET_DOC_TEXT_DISABLED);
        }
    ...
    public FSIndex getAnnotationIndex() {
        return new AnnotationIndexImpl(this.indexRepository
                .getIndex(TCAS.STD_ANNOTATION_INDEX));
    }
