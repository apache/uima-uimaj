...
public class FSIndexRepositoryImpl implements FSIndexRepositoryMgr,
    ...
    /**
     * A pair of an index and an iterator cache. An iterator cache is the set of
     * all indexes necessary to create an iterator for the type of the index.
    ...
    private class IndexIteratorCachePair implements Comparable {
        ...
        public boolean equals(Object o) {
            ...
            return this.index.getComparator()
                    .equals(iicp.index.getComparator())
                    && (this.index.getIndexingStrategy() == iicp.index
                            .getIndexingStrategy());
        }
        ...
        private void createIndexIteratorCache() {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (indexPos >= 0) {
                    this.iteratorCache.add(((IndexIteratorCachePair) indexList
                            .get(indexPos)).index);
                }
            }
        }
        ...
        public int compareTo(Object o) {
            ...
            if (typeCode1 < typeCode2) {
                ...
                return this.index.getComparator().compareTo(
                        cp.index.getComparator());
            }
        }
    ...
    /**
     * Comparator wrapper; will used wrapped comparator, and on equality use
     * address of FS for further distinction.
     */
    ...
    /**
     * The iterator implementation for indexes. Tricky because the iterator
     * needs to be able to move backwards as well as forwards.
     */
    private class PointerIterator implements IntPointerIterator,
        ...
        private void initPointerIterator(IndexIteratorCachePair iicp0) {
            ...
            this.iteratorComparator = new IteratorComparator(
                    (FSLeafIndexImpl) iteratorCache.get(0));
            ComparableIntPointerIterator it;
            for (int i = 0; i < this.indexesSize; i++) {
                final FSLeafIndexImpl leafIndex = ((FSLeafIndexImpl) iteratorCache
                        .get(i));
    ...
    private class IndexImpl implements FSIndex, FSIndexImpl {
        ...
        public FSIterator iterator() {
            return new FSIteratorWrapper(new PointerIterator(this.iicp),
        ...
        public FSIterator iterator(FeatureStructure fs) {
            return new FSIteratorWrapper(new PointerIterator(this.iicp,
                    ((FeatureStructureImpl) fs).getAddress()),
                    FSIndexRepositoryImpl.this.cas);
        }
        ...
        public LowLevelIterator ll_iterator(boolean ambiguous) {
            ...
            return new LLUnambiguousIteratorImpl(this.ll_iterator(),
                    this.iicp.index.lowLevelCAS);
        }
    ...
    FSIndexRepositoryImpl(CASImpl cas, FSIndexRepositoryImpl baseIndexRepo) {
        ...
        if (!keys.isEmpty()) {
            ...
            while (keysIter.hasNext()) {
                ...
                IndexIteratorCachePair iicp = (IndexIteratorCachePair) baseIndexRepo.name2indexMap
                        .get(key);
    ...
    private IndexIteratorCachePair addNewIndex(FSIndexComparator comparator,
            int indexType) {
    ...
    private IndexIteratorCachePair addNewIndex(FSIndexComparator comparator,
        ...
        default: {
            ...
            ind = new FSIntArrayIndex(this.cas, type, initialSize,
    ...
    /*
     * private IndexIteratorCachePair addIndex( FSIndexComparator comparator,
     * int initialSize) { final Type type = comparator.getType(); final int
     * typeCode = ((TypeImpl) type).getCode(); final Vector indexVector =
     * this.indexArray[typeCode]; final int vecLen = indexVector.size();
     * FSLeafIndexImpl ind;
     * 
     * for (int i = 0; i < vecLen; i++) { ind = ((IndexIteratorCachePair)
     * indexVector.get(i)).index; if (comparator.equals(ind.getComparator())) {
     * return null; } }
     * 
     * ind = new FSRBTIndex(this.cas, type); // ind = new
     * FSVectorIndex(this.cas, initialSize); ind.init(comparator);
     * IndexIteratorCachePair iicp = new IndexIteratorCachePair(); iicp.index =
    ...
    // }
    private IndexIteratorCachePair addNewIndexRecursive(
            FSIndexComparator comparator, int indexType) {
        final FSIndexComparatorImpl compCopy = ((FSIndexComparatorImpl) comparator)
                .copy();
    ...
    private static final int findIndex(ArrayList indexes, FSIndexComparator comp) {
        ...
        for (int i = 0; i < max; i++) {
            indexComp = ((IndexIteratorCachePair) indexes.get(i)).index
                    .getComparator();
    ...
    /*
     * // Will modify comparator, so call with copy. private
     * IndexIteratorCachePair addIndexRec(FSIndexComparator comp) {
     * FSIndexComparator compCopy; IndexIteratorCachePair cp =
     * this.addIndex(comp); if (cp == null) { return null; // The index already
     * exists. } final Type superType = comp.getType(); final Vector types =
     * this.typeSystem.getDirectlySubsumedTypes(superType); final int max =
     * types.size(); for (int i = 0; i < max; i++) { compCopy =
     * ((FSIndexComparatorImpl)comp).copy(); compCopy.setType((Type)
     * types.get(i)); addIndexRec(compCopy); } return cp; }
     */
    // Will modify comparator, so call with copy.
    private IndexIteratorCachePair addNewIndexRec(FSIndexComparator comparator,
            int indexType) {
        IndexIteratorCachePair iicp = this.addNewIndex(comparator, indexType);
        final Type superType = comparator.getType();
        final Vector types = this.typeSystem
                .getDirectlySubsumedTypes(superType);
    ...
    private static final void addAllSubsumedTypes(Type t, TypeSystem ts,
            ArrayList v) {
        v.add(t);
    ...
    public LinearTypeOrder getDefaultTypeOrder() {
        if (this.defaultTypeOrder == null) {
            if (this.defaultOrderBuilder == null) {
                this.defaultOrderBuilder = new LinearTypeOrderBuilderImpl(
                        this.typeSystem);
            }
            try {
    ...
    /**
     * @see org.apache.uima.cas.admin.FSIndexRepositoryMgr#createIndex(FSIndexComparator,
     *      String)
     */
    public boolean createIndex(FSIndexComparator comp, String label,
            int indexType) throws CASAdminException {
        if (this.locked) {
    ...
    /**
     * This is public only until the xml specifier format supports specifying
     * index kinds (set, bag etc.).
     * 
     * @param comp
     * @param label
     * @param indexType
    ...
    public boolean createIndexNoQuestionsAsked(FSIndexComparator comp,
        ...
        IndexIteratorCachePair cp = (IndexIteratorCachePair) this.name2indexMap
                .get(label);
    ...
    public Iterator getLabels(FSIndexComparator comp) {
        ...
        while (it.hasNext()) {
            label = (String) it.next();
            if (((IndexIteratorCachePair) this.name2indexMap.get(label)).index
                    .getComparator().equals(comp)) {
                labels.add(label);
            }
        }
    ...
    public FSIndex getIndex(String label, Type type) {
        IndexIteratorCachePair iicp = (IndexIteratorCachePair) this.name2indexMap
                .get(label);
        if (iicp == null) {
            return null;
        }
    ...
    public int getIndexSize(Type type) {
        ...
        final Vector typeVector = this.typeSystem
                .getDirectlySubsumedTypes(type);
    ...
    /**
     * Return an array containing all FSs in any index. This is intended to be
     * used for serialization. Note that duplicate entries in indexes will
     * appear in the array as many times as they occur in an index. The order in
     * which FSs occur in the array does not reflect the order in which they
     * were added to the repository. This means that set indexes deserialized
     * from this list may contain different but equal elements than the original
     * index.
     */
        ...
        for (int i = 0; i < this.indexArray.length; i++) {
            ...
            if (cv.size() > 0) {
                ...
                for (int k = 0; k < cv.size(); k++) {
                    it = ((IndexIteratorCachePair) cv.get(k)).index
                            .refIterator();
                    while (it.isValid()) {
                        set.add(it.get());
                        it.inc();
    ...
    public LinearTypeOrderBuilder createTypeSortOrder() {
        LinearTypeOrderBuilder orderBuilder = new LinearTypeOrderBuilderImpl(
                this.typeSystem);
        if (this.defaultOrderBuilder == null) {
    ...
    public LowLevelIndex ll_getIndex(String indexName, int typeCode) {
        if (!this.typeSystem.isType(typeCode)
                || !this.cas.isFSRefType(typeCode)) {
            LowLevelException e = new LowLevelException(
                    LowLevelException.INVALID_INDEX_TYPE);
            e.addArgument(Integer.toString(typeCode));
            throw e;
        }
        return (LowLevelIndex) getIndex(indexName, this.typeSystem
    ...
    /**
     * An iterator that is always invalid. TODO: remove this once we implement
     * the default bag index.
     */
