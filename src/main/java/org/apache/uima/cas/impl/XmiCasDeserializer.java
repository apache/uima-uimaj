...
/**
 * XMI CAS deserializer.  Used to read in a CAS from XML Metadata Interchange (XMI)
 * format.  
 */
public class XmiCasDeserializer
{
  ...
  private class XmiCasDeserializerHandler extends DefaultHandler
  {
    ...
    // These are special because their "head" feature needs remapping but their "tail" feature doesn't.
    ...
    private XmiCasDeserializerHandler(CASImpl aCAS, boolean lenient, XmiSerializationSharedData sharedData)
    {
      super();
    ...
    /**
     * Handles the processing of a cas:View element in the XMI.
     * The cas:View element encodes indexed FSs.
     * 
     * @param sofa xmi:id of the sofa for this view, null indicates base CAS "view"
     * @param membersString whitespace-separated string of FS addresses.
     *    Each FS is to be added to the specified sofa's index repository
     */
    private void processView(String sofa, String membersString)
    {
      ...
      if (membersString != null)
      {
        ...
        FSIndexRepositoryImpl indexRep = 
          (FSIndexRepositoryImpl) indexRepositories.get(sofaNum);
    ...
    private void readFS(final int addr, Attributes attrs) throws SAXParseException
    {
      ...
      for (int i = 0; i < attrs.getLength(); i++)
      {
        ...
        {
          if (sofaTypeCode == typeCode && 
              attrName.equals(CAS.FEATURE_BASE_NAME_SOFAID))
          {
          ...
          else if (sofaTypeCode == typeCode && 
              attrName.equals(CAS.FEATURE_BASE_NAME_SOFANUM))
          {
    ...
    /**
     * Parse an XMI multi-valued attribute into a String array,
     * by splitting on whitespace.
     * 
     * @param val XMI attribute value
    ...
    /**
     * Set a CAS feature from an array of Strings.  This supports the XMI syntax
     * where each value is listed as a separate subelement.
     * 
     * @param addr address of FS containing the feature
     * @param featCode code of feature to set
     ...
     * @throws SAXParseException
     */
    private void handleFeature(int addr, int featCode, List featVals)
        throws SAXParseException
    {    
      switch (featureType[featCode])
      {
     ...
     * @param arrayType CAS type code for the array
    ...
    private int createArray(int arrayType, List values, int xmiId)
    {
	  ...
	  if (cas.isBooleanArrayType(arrayType)) {
		 fs = (FeatureStructureImplC) cas.createBooleanArrayFS(values
						.size());
	  } else if (cas.isByteArrayType(arrayType)) {
		 fs = (FeatureStructureImplC) cas.createByteArrayFS(values
						.size());
	  } else if (cas.isShortArrayType(arrayType)) {
		 fs = (FeatureStructureImplC) cas.createShortArrayFS(values
						.size());
	  } else if (cas.isLongArrayType(arrayType)) {
		 fs = (FeatureStructureImplC) cas.createLongArrayFS(values
						.size());
	  } else if (cas.isDoubleArrayType(arrayType)) {
		 fs = (FeatureStructureImplC) cas.createDoubleArrayFS(values
						.size());
	  } else {
     ...
     * @param arrayType CAS type code for the array
     * @param hexString value of the byte array as a hex string
     * @return
     */ 
        ...
        throws SAXException
    {
      switch (this.state)
      {
        ...
        case FEAT_STATE:
        {
          ...
          if (currentType != null)
          {
            if (cas.isArrayType(currentType) &&
                !cas.isByteArrayType(currentType))
            {
    ...
    /**
     * Adds this FS to the appropriate index, and applies ID remappings.
     * For each nonprimitive, non-multivalued-property feature, we need to update
     * the feature value to point to the correct heap address of the target FS.
     * @param fsInfo
     */
    private void finalizeFS(int addr)
    {
      ...
      for (int i = 0; i < feats.length; i++)
      {
        ...
        if (cas.ll_isRefType(typeCode)
            && (featureType[feats[i]] == LowLevelCAS.TYPE_CLASS_FS || 
    ...
    /** 
     * Rempas ID for the "head" feature of NonEmptyFSList, but not the "tail" feature.
     * Used for FSList nodes deserialized from multi-valued properties, which already
     * have their tail set correctly.
     * @param i
     */
    ...
    /**
     * Walk an array, remapping IDs.  If called on a primitive array,this
     * method does nothing.
     * 
     * @param type CAS type code for the array
  ...
  /**
   * Create a new deserializer from a type system.  Note: all CAS arguments
   * later supplied to <code>getXCASHandler()</code> must have this type system
   * as their type system.
   * @param ts The type system of the CASes to be deserialized.  
   */
   ...
   * 
   * @param cas This CAS will be used to hold the data deserialized from the XMI
   * 
  ...
  /**
   * Create a default handler for deserializing a CAS from XMI.  By default this is 
   * not lenient, meaning that if the XMI references Types that are not in the 
   * Type System, an Exception will be thrown.  Use 
   * {@link XmiCasDeserializer#getXmiCasHandler(CAS,boolean)} to turn on lenient
   * mode and ignore any unknown types.
   * 
   * @param cas This CAS will be used to hold the data deserialized from the XMI
   * @param lenient if true, unknown Types will be ignored.  If false, unknown Types will
   ...
   * 
   * @param aStream input stream from which to read the XMI document
   * @param aCAS CAS into which to deserialize.  This CAS must be set up with a type system
   *   that is compatible with that in the XMI
   * 
   ...
   * Deserializes a CAS from XMI.  
   * 
   * @param aStream input stream from which to read the XCMI document
   * @param aCAS CAS into which to deserialize.  This CAS must be set up with a type system
   *   that is compatible with that in the XMI
   ...
   * 
   * @param nsUri the namespace URI of the XMI element
   * @param localName the local name of the XMI element
   * 
   * @return the UIMA type name corresponding to the XMI element name
   */
  private String xmiElementName2uimaTypeName(String nsUri, String localName)
    throws SAXException
  {
