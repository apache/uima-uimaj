...
public class LinearTypeOrderBuilderImpl implements LinearTypeOrderBuilder {
	...
	private static class TotalTypeOrder implements LinearTypeOrder {
		...
		private TotalTypeOrder(String[] typeList, TypeSystem ts)
				throws CASException {
			this(encodeTypeList(typeList, ts), ts);
		}

		private static int[] encodeTypeList(String[] typeList, TypeSystem ts)
				throws CASException {
			...
			for (int i = 0; i < a.length; i++) {
				...
				if (t == LowLevelTypeSystem.UNKNOWN_TYPE_CODE) {
					CASException e = new CASException(
							CASException.TYPEORDER_UNKNOWN_TYPE);
					e.addArgument(typeList[i]);
					throw e;
				}
				a[i] = t;
			}
		...
		private TotalTypeOrder(int[] typeList, TypeSystem ts) {
			...
			this.order = typeList;
			this.typeCodeToOrder = new int[order.length + tsi.getSmallestType()];
			for (int i = 0; i < order.length; i++) {
				typeCodeToOrder[order[i]] = i;
			}
		}

		// Look-up.
		public boolean lessThan(Type t1, Type t2) {
			return lessThan(((TypeImpl) t1).getCode(), ((TypeImpl) t2)
					.getCode());
	...
	private class Node extends GraphNode {
		...
		private void addAllPredecessors(ArrayList predecessors) {
			for (Iterator it = predecessors.iterator(); it.hasNext();) {
				...
				//        		else {
				//        			System.out.println("Testing: add predec: found loop from " + this.getElement() + " to " + n.getElement());
				//        		}
			}
		}

		private void addAllSuccessors(ArrayList successors) {
			for (Iterator it = successors.iterator(); it.hasNext();) {
				...
				//        		else {
				//        			System.out.println("Testing: add succ: found loop from " + this.getElement() + " to " + n.getElement());
				//        		}
			}
		}
	...
	private class Graph {
		...
		private Graph copy(Node inRank0nodes) {
			...
			while (it.hasNext()) {
				...
				copy.nodeMap.put(key, copy.getNode(key)); // getNode makes a new node
			}
	...
	public void add(String[] types) throws CASException {
		...
		for (int i = 0; i < max; i++) {
			...
			if (!rc) {
				CASException e = new CASException(
						CASException.CYCLE_IN_TYPE_ORDER);
				e.addArgument(types[i]);
				e.addArgument(types[i + 1]);
				throw e;
			}
		}
	}
	...
	private void addInheritanceTypes() {
		...
		for (Iterator tsi = ts.getTypeIterator(); tsi.hasNext();) {
			...
			for (Iterator ni = typesToModify.iterator(); ni.hasNext();) {
				...
				if (doIn && nIn != null) {
					if (n.inRank() == 0) {
						...
						doIn = false; // when going up the tree, when you find one filled in, stop
					}
				}
				if (doOut && nOut != null) {
					if (n.outRank() == 0) {
