...
final class StringHeap {
    ...
    StringHeap() {
        this(DEFAULT_REF_HEAP_BASE_SIZE, DEFAULT_REF_HEAP_MULT_LIMIT,
                DEFAULT_STRING_HEAP_BASE_SIZE, DEFAULT_STRING_HEAP_MULT_LIMIT);
    }

    StringHeap(int refHeapBaseSize, int refHeapMultLimit, int strHeapBaseSize,
            int strHeapMultLimit) {
        super();
    ...
    int copyCharsToBuffer(int stringCode, char[] buffer, int start) {
        ...
        if (strCode == 0) {
            final int strLen = this.refHeap[strInfoRef
    ...
    private final void ensureRefHeapSize() {
        this.refHeap = IntArrayUtils.ensure_size(this.refHeap, this.refHeapPos,
                GROWTH_FACTOR, this.refHeapMultLimit);
    }
    ...
    private void ensureCharHeapSize() {
        this.stringHeap = IntArrayUtils.ensure_size(this.stringHeap,
                this.charHeapPos, GROWTH_FACTOR, this.strHeapMultLimit);
    }
