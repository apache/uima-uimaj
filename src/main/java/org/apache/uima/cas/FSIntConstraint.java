...
/**
 * Interface for an integer constraint. A integer constraint contains 0 or more
 * tests, the results of which are "anded" together. To set a test, use any of
 * the methods on this class (except "match").
 * <p>
 * To use the constraint, invoke its {@link #match(int)} method, passing the
 * value to test. You can also embed this test with a path specification, using
 * the
 * {@link org.apache.uima.cas.ConstraintFactory#embedConstraint(FeaturePath, FSConstraint)}
 * method, and use it to test feature structures, or combine it with other tests
 * using the
