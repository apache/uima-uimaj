...
public class CasDataUtils
{
		...
		while (it.hasNext())
		{
			...
			if (object instanceof FeatureStructure)
			{
				...
				UIMAFramework.getLogger(CLASS_NAME).logrb(
					...
					"dumpFeatures",
					LOG_RESOURCE_BUNDLE,
					"UIMA_cas_feature_structure_type__FINE", fs.getType());
				...
				for (int i = 0; names != null && i < names.length; i++)
				{
					...
					if (fValue != null)
					{
						UIMAFramework.getLogger(CLASS_NAME).logrb(
							...
							"dumpFeatures",
							LOG_RESOURCE_BUNDLE,
							"UIMA_cas_feature_name__FINE", new Object[]{names[i], fValue.toString()});
					}
				}
			}
		}
	...
	public static String getFeatureValueByType(CasData aCAS, String featureName)
	{
		...
		while (it.hasNext())
		{
			...
			{
				FeatureStructure fs = (FeatureStructure) object;
				if ( System.getProperty("DEBUG") != null)	System.out.println("FeatureName::::::::::::::::::::::::::::::::::::::::::>"+fs.getType()+" Searching For::"+featureName);
				if ( featureName.equals( fs.getType() ) )
				{
					String[] names = fs.getFeatureNames();
					for ( int i=0; names != null && i < names.length; i++ )
					{
						if ( System.getProperty("DEBUG") != null)	System.out.println("Feature Structure:::"+fs.getType()+" Has Value::"+names[i]);
					}
	...
	public static String getFeatureValueByType(CasData aCAS,String aFeatureStructure,  String featureName)
	{
		...
		{
			Object object = it.next();
			if (object instanceof FeatureStructure && ((FeatureStructure) object).getType().equals(aFeatureStructure))
			{
				FeatureStructure fs = (FeatureStructure) object;
				FeatureValue fValue = fs.getFeatureValue(featureName);
				if (fValue != null)
		...
		while (it.hasNext())
		{
			...
			if (object instanceof FeatureStructure && ((FeatureStructure)object).getType().equals(aFeatureStructureName) )
			{
				...
				for( int i=0; i < featureNames.length; i++ )
				{
					...
					//System.out.println("DATACasUtils.getCasDataFeatures()-Name::"+valuePairSet[i].getName()+" Value:::"+valuePairSet[i].getValue().toString());
				}
			}
		}
		return valuePairSet;    	
