...
public class Ecore2UimaTypeSystem
{
   ...
   * (the default), UIMA array types (e.g. FSArray) will be generated.  If set to Boolean.TRUE, UIMA list
   * types (e.g. FSList) will be generated.  Note that for primitive types that have no corresponding
   * list type (Byte, Short, Long, Double, and Boolean), array types will always be used.
   */
  public static final String OPTION_GENERATE_UIMA_LIST_TYPES = "OPTION_GENERATE_UIMA_LIST_TYPES";
  
  /**
   * Configures the assignment of supertypes to EClasses that have no declared supertype.  If set to Boolean.TRUE
   * (the default), if such an EClass has "begin" and "end" properties of type EInt, the superclass will be set
   * to uima.tcas.Annotation.  If set to Boolean.FALSE, all EClasses with no declared supertype will have their
   * supertype set to uima.cas.TOP.
   */
   ...
   * 
   * @param aEcoreFilePath file path to a .ecore model file
   ...
   * @throws URISyntaxException if there is a problem finding or reading the .ecore file
   */
   ...
   * Converts an Ecore model to a UIMA TypeSytemDescription.
   * 
   * @param aEcoreResource An EMF Resource containing the Ecore model
   * @param aOptions a Map defining options for the conversion.  Valid keys for
   *   this map are defined as constants on this class.
   *   
   * @return The UIMA TypeSystemDescription corresponding to the Ecore model
   * @throws URISyntaxException if there is a problem reading from the resource
   */  
  ...
  private static TypeDescription eclass2UimaType(EClass aEClass, String aUimaNamespace, Map aOptions) throws URISyntaxException
  {
    ...
    if (supertypes.isEmpty()) //supertype not defined in the Ecore model
    {
      ...
      {
        //if this class has "begin" and "end" attributes of type EInt, make it a subtype of annotation
  ...
  private static String getUimaTypeName(EClassifier aEcoreType, boolean aMultiValued, Map aOptions) throws URISyntaxException
  {
    ...
    else //primitive type
    {
      ...
      { 
	    ...
	    {
	      return aMultiValued ? 
	          (useUimaLists ? CAS.TYPE_NAME_FLOAT_LIST : CAS.TYPE_NAME_FLOAT_ARRAY) : 
	          CAS.TYPE_NAME_FLOAT;
	    }
  ...
  /** 
   * Main program.  Takes two arguments: the filename of an input .ecore file and
   * the filename of the UIMA TypeSystem file to generate.
   */
