...
public class UimaTypeSystem2Ecore
{
   ...
   * @param aOptions a Map defining options for the conversion.  Valid keys for
   *   this map are defined as constants on this class.
   *   
   * @throws InvalidXMLException if the TypeSystem descriptor, or one of its imports, is not
   *   valid or if there are duplicate, inconsistent definitions of the same type.
   * @throws IOException if an failure occur while reading the descriptor file
   */   
  public static void uimaTypeSystem2Ecore(String aUimaTypeSystemFilePath, Resource aOutputResource, Map aOptions)
   ...
   *   this map are defined as constants on this class.
   *   
   * @throws InvalidXMLException if the TypeSystem descriptor imports another descriptor
   *   that could not be successfully parsed, or if there are duplicate, inconsistent 
   *   definitions of the same type.
   */     
  public static void uimaTypeSystem2Ecore(TypeSystemDescription aTypeSystem, Resource aOutputResource, Map aOptions)
   ...
   * @param aOptions a Map defining options for the conversion.  Valid keys for
   *   this map are defined as constants on this class.
   * @param aSchemaLocationMap optional parameter - if non-null, this map will be
   *   populated with (Namespace URI, Schema Location) pairs, suitable for 
   *   inclusion in the "schemaLocation" attribute of XMI instance documents.
   */     
  public static void uimaTypeSystem2Ecore(TypeSystemDescription aTypeSystem, Resource aOutputResource, Map aOptions, Map aSchemaLocationMap)
    throws InvalidXMLException
  {
    ...
    //Do this in two passes.  First pass creates EPackages, EClasses, and EEnums (for string subtypes)
    ...
    for (int i = 0; i < types.length; i++)
    {
      ...
      {
        String schemaLoc = aOutputResource.getURI() + "#" + aOutputResource.getURIFragment(eclassifier.getEPackage());
    ...
    for (int i = 0; i < types.length; i++)
    {
      ...
      if (eclassifier instanceof EClass)
      {
        ...
        EClassifier superclass = lookupEClassifierForType(supertypeName); //creates EClass if not already existing
        ...
        for (int j = 0; j < features.length; j++)
        {
          eclass.getEStructuralFeatures().add(
              uimaFeature2EStructuralFeature(features[j], aOptions));      
        }
      }
    }
  ...
  private static Resource loadUimaBuiltinsEcore(ResourceSet resourceSet, Map aSchemaLocationMap)
  {
    ...
    Resource uimaEcoreResource = resourceSet.getResource(URI.createURI(uimaEcoreUrl.toString()), true);
    ...
    while (iter.hasNext())
    {
      ...
      if (current instanceof EPackage)
      {
        ...
        if (aSchemaLocationMap != null)
        {
          String schemaLoc = uimaEcoreResource.getURI() + "#" + uimaEcoreResource.getURIFragment(pkg);
  ...
  private static EStructuralFeature uimaFeature2EStructuralFeature(FeatureDescription aFeature, Map aOptions)
  {
    ...
    boolean multiRefAllowed = aFeature.getMultipleReferencesAllowed() == null ?
        false : aFeature.getMultipleReferencesAllowed().booleanValue();
    ...
    else if ((CAS.TYPE_NAME_STRING_ARRAY.equals(range) ||
              CAS.TYPE_NAME_STRING_LIST.equals(range)) && !multiRefAllowed)
    {
      efeat = EcoreFactory.eINSTANCE.createEAttribute();
      efeat.setEType(EcorePackage.eINSTANCE.getEString());
    ...
    else if ((CAS.TYPE_NAME_FLOAT_ARRAY.equals(range) ||
        CAS.TYPE_NAME_FLOAT_LIST.equals(range)) && !multiRefAllowed)
    {
      efeat = EcoreFactory.eINSTANCE.createEAttribute();
      efeat.setEType(EcorePackage.eINSTANCE.getEFloat());
      efeat.setUpperBound(-1);
    ...
    else if ((CAS.TYPE_NAME_FS_ARRAY.equals(range) ||
        CAS.TYPE_NAME_FS_LIST.equals(range)) && !multiRefAllowed)
    {
  ...
  /**
   * Gets or creates an EPackage for a UIMA namespace.  Actually will create
   * a whole chain of nested EPackages, one for each component of the UIMA namespace,
   * but only the leaf node of the chain will be returned.
   * 
   * @param uimaNamespace UIMA namespace
  ...
  /** 
   * Main program.  Takes two arguments: the filename of an input TypeSystem 
   * descriptor file and the filename of the Ecore/XMI file to generate.
   */
