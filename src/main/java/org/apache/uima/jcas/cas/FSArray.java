...
public final class FSArray extends TOP implements ArrayFS {
	...
	/** each cover class when loaded sets an index.
		* used in the JCas typeArray to go from 
		* the cover class or class instance to the corresponding
		* instance of the _Type class
		*/
	...
	public FSArray(JCas jcas, int length) {
		this(
			 /* addr */ jcas.getLowLevelCas().ll_createArray(
										 jcas.getType(typeIndexID).casTypeCode, length),
  ...
  /** return the size of the array.*/
    ...
    jcasType.casImpl.checkArrayBounds(addr, destOffset, length);
		for (int i = 0; i < length; i++) {
			jcasType.ll_cas.ll_setRefArrayValue(addr, i + destOffset, jcasType.ll_cas.ll_getFSRef(src[i + srcOffset]));
		}
	}
    ...
    jcasType.casImpl.checkArrayBounds(addr, srcOffset, length);
		for (int i = 0; i < length; i++) {
			dest[i + destOffset] = jcasType.ll_cas.ll_getFSForRef(jcasType.ll_cas.ll_getRefArrayValue(addr, i + srcOffset));
		}
	}
