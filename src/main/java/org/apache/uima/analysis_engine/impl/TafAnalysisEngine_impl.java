...
public class TafAnalysisEngine_impl extends AnalysisEngineImplBase implements
	...
	public boolean initialize(ResourceSpecifier aSpecifier,
		...
		ProcessingResourceMetaData md = (ProcessingResourceMetaData) mDescription
				.getMetaData();

		getLogger().logrb(Level.CONFIG, CLASS_NAME.getName(), "initialize",
		...
		mVerificationMode = aAdditionalParams
				.containsKey(PARAM_VERIFICATION_MODE);
	...
	public void destroy() {
		if (mAnnotator != null)
			mAnnotator.destroy();
		getLogger().logrb(Level.CONFIG, CLASS_NAME.getName(), "initialize",
				LOG_RESOURCE_BUNDLE, "UIMA_analysis_engine_destroyed__CONFIG",
				getMetaData().getName());
    super.destroy();    
	}
	 ...
	 */
	protected void callAnalysisComponentProcess(CAS aCAS)
			throws AnalysisEngineProcessException {
		// logging and instrumentation
		String resourceName = getMetaData().getName();
		getLogger().logrb(Level.FINE, CLASS_NAME.getName(), "process",
		...
		try {
			...
			try {
				...
				AbstractCas casToPass = getCasManager().getCasInterface(
						baseCas, requiredInterface);
	...
	/**
	 * A utility method that performs initialization logic for a primitive
	 * AnalysisEngine.
	 * 
	 * @param aDescription
	 *            the AnalysisEngine description for this AnalysisEngine
	 * @param aConfigParamSettings
	 *            the configuration parameter settings that are the result of
	 *            merging this primitive AE's parameter settings with those
	 *            specified in any enclosing aggregate AE.
	 * 
	 * @throws ResourceInitializationException
	 *             if an initialization failure occurs
	 */
	protected void initializeAnalysisComponent(
			AnalysisEngineDescription aDescription)
			throws ResourceInitializationException {
	...
	/**
	 * For an aggregate TAF AE, sets this aggregate AE's Type System, Type
	 * Priorities, and FS Index Descriptions equal to the result of merging the
	 * information from its delegate AEs.
	 * 
	 * @throws ResourceInitializationException
	 *             if an error occurs
	 */
	protected void mergeDelegateAnalysisEngineMetaData()
			throws ResourceInitializationException {
		// do the merge
		TypeSystemDescription aggTypeSystem = CasCreationUtils
				.mergeDelegateAnalysisEngineTypeSystems(mDescription,
						getResourceManager());
		TypePriorities aggTypePriorities = CasCreationUtils
				.mergeDelegateAnalysisEngineTypePriorities(mDescription,
						getResourceManager());
		...
		AnalysisEngineMetaData aggregateMD = mDescription
	 ...
	 */
	protected void enableProhibitedAnnotatorCasFunctions(CAS aCAS,
			boolean aEnable) {
	...
	protected AnalysisProcessData_impl createAnalysisProcessData() {
		try {
			return new AnalysisProcessData_impl(newCAS(),
	...
	/**
	 * For an aggregate AnalysisEngine only, gets a Map from each component's
	 * key to the specifier for that component.
	 * 
	...
	/**
	 * For testing purposes, returns whether this analysis engine has created
	 * its own AnalysisProcessData object. This should only be the case if the
	 * application has called deprecated methods.
	 */
	...
	/**
	 * The object that encapsulates all data that is modified during this
	 * AnalysisEngine's {@link process(ResultSpecification)} method. This
	 * includes the {@link CAS} and the {@link ProcessTrace}.
	 * 
	 * @deprecated
	 */
	...
	/**
	 * For a primitive AnalysisEngine only, the Annotator instance that contains
	 * the analysis logic.
	 */
	 ...
	 * the Annotator uses to access its environment.
	 */
	...
	/**
	 * For a primitive AE only, the last CAS TypeSystem that was sent to the
	 * Annotator's typeSystemInit() method.
	 */
	...
	/**
	 * If this is set it indicates that the AnalysisEngine is being constructed
	 * only to verify the validity of the descriptor. The Annotator classes
	 * should not be instantiated in this case.
	 */
	private boolean mVerificationMode = false;

	/**
	 * Key that must be inserted into the aAdditionalParams map to turn on
	 * verification mode. Also passed down to delegates.
	 */
	public static final String PARAM_VERIFICATION_MODE = "VERIFICATION_MODE";

	/**
	 * Implements the iterator that steps through all outputs from an
	 * AnalysisComponent.
	 */
			...
			try {
				...
				if (!analysisComponentHasNext) {
					throw new UIMA_IllegalStateException(
							UIMA_IllegalStateException.NO_NEXT_CAS,
							new Object[0]);
				}
