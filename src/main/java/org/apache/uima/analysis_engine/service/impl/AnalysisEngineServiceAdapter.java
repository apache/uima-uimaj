...
/**
 * Base class for analysis engine service adapters. Implements the 
 * {@link AnalysisEngine} interface by communicating with an Analysis Engine 
 * service.  This insulates the application from having to know whether it is 
 * calling a local AnalysisEngine or a remote service.
 * <p>
 * Subclasses must provide an implementation of the 
 * {@link #initialize(ResourceSpecifier,Map)} method, which must create an
 * {@link AnalysisEngineStub} object that can communicate with the remote
 * service.  The stub must be passed to the {@link #setStub(AnalysisEngineStub)}
 * method of this class.
 * 
 * 
 */
public abstract class AnalysisEngineServiceAdapter 
  extends AnalysisEngineImplBase implements TextAnalysisEngine, CasConsumer
{
  ...
  {
    return getMetaData().getConfigurationParameterSettings().
        getParameterValue(aGroupName, aParamName);
  }
   ...
   */
  public void setConfigParameterValue(
  ...
  /**
   * The resource metadata, cached so that service does not have to
   * be called each time metadata is needed.
   */
