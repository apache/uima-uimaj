...
public class FileCompare {
	 ...
	 * 
	 * @param filename1 filename of the first file
	 * @param filename2 filename of the second file
	 ...
	 * 
	 * @param filename1 filename of the first file
	 * @param in an input Sream
	 * 
	 * @return - true if the content is the same
	 * 
	 * @throws IOException
	 */
	public static boolean compare(String filename1, InputStream in)
			throws IOException {
		InputStream file1 = null;
	...
	public static boolean compare(InputStream in1, InputStream in2)
      ...
      in2 = new BufferedInputStream(in2, byteBufferSize);
		...
		while (in1byte != -1) {
			...
			if ((!(Character.isWhitespace((char) in1byte))) && (in1byte != ' ')
					&& (in1byte != '\n') && (in1byte != '\r')) {
				//read one byte form file2
				in2byte = in2.read();

				//read bytes until byte is no whitespace or blank
				while ((Character.isWhitespace((char) in2byte))
						|| (in2byte == ' ') || (in2byte == '\n')
						|| (in2byte == '\r')) {
	 ...
	 * Different notations for empty tags are considered equal.
	 * 
	 * @param filename1 Filename of the first XML file.
	 * @param filename2 Filename of the second XML file.
	 * @return
	 * @throws IOException
	 */
	public static boolean compareXML(String filename1, String filename2)
			throws IOException {
		File file1 = null;
		File file2 = null;
	 ...
	 * 
	 * @param xml The XML file where the empty tags are to be replaced as string.
   ...
   * uimaj-core package.  We can't have a compile dependency on uimaj-core since
   * that introduces a cycle.  Not sure what the best way of handling this is.
   */
