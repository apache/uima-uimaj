...
public class JUnitTestRunner
{
	...
	public boolean runTest(String[] args){
		...
		{
			System.out.println("Usage: JUnitTestRunner [-abortOnFail] [-testcase <FullTestCaseClassname>] " +
		...
		try
		{
			...
			//check if result output filename was specifed
			 if(resultFileName != null)
			 {
				...
				if(teeResult == true)
				{
					PrintStream tempResultWriter =  new PrintStream(new FileOutputStream(resultFile,false),true,"UTF-8");
			 ...
			 if(consoleFileName != null)
			 {
				...
				if(teeConsole == true)
				{
					PrintStream tempConsoleWriter =  new PrintStream(new FileOutputStream(consoleFile,false),true,"UTF-8");
