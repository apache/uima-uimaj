...
public class CASTransportable extends DefaultHandler implements Transportable
{
   ...
   * We don't check a CAS out of the pool until we get a request.
   * @param casPool
  ...
  /**
   * This constructor is used on the client side, where we have a dedicated CAS
  ...
  class XTalkSerializer extends DefaultHandler
  {
        ...
        throws SAXException
    {
      try
      {
        ...
        XTalkTransporter.writeInt(serializer.getNumChildren(), os); // HACK to find out # of children
      } catch (IOException e)
      {
        throw new SAXException("IOException: " + e);
      }
  ...
  /**
   * Serialize the CAS to the stream in XTalk format. After serialization is complete the cas is returned 
   * to the pool (if it was allocated from a pool.)
   */
