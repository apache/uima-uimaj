...
import org.apache.uima.adapter.vinci.util.UimaSaxVinciFrameBuilder;
...
/**
 * A Vinci CAS Serializer for the XCAS spec which does NOT include the full text of the document.
 * However, for each annotation it DOES include that annotation's spanned text as the content of its
 * Vinci FrameLeaf.
 * 
 * 
 */
public class VinciXCASSerializer_NoDocText implements VinciCASSerializer {
  ...
  public void serialize(CAS aCAS, AFrame aParentFrame, OutOfTypeSystemData aOutOfTypeSystemData)
    ...
    if (((CASImpl) aCAS).isBackwardCompatibleCas()) {
      ...
      vinciFrameBuilder = new UimaSaxVinciFrameBuilder(true, true, docText);
    } else {
      vinciFrameBuilder = new UimaSaxVinciFrameBuilder(true, true, null);
    }
