...
/**
 * Provides the same function as the FSIndexRepository
 * except that the variants that take a "type" argument
 * take type arguments obtainable easily from the JCas type.
 *
 */
...
public interface JFSIndexRepository {
	 ...
	 * @param label The name of the index
	 * @param type A subtype of the type of the index, written as Foo.type
	 * @return The specified index, or <code>null</code> if an index with that name
	 ...
	 * @param type The annotation type the index is restricted to, written as Foo.type
