...
public class TOP_Type {
  ...
  /** This generator can be set up to either
   * get the identical Java object (if it exists) or just
   * make a new one.  If making a new one - the java object made
   * cannot contain any other data (because updates won't be reflected).
   * Each class generated by JCasGen either does this 
   * (if it has additional java fields imbedded)
   * or not.
   */
  ...
  /** Internal - this constructor is called when new CAS creates corresponding
   * jcas instance.  During this process, all the types defined in the CAS
   * are used to see if there are any corresponding jcas type defs defined.
   * If so, they are loaded.  This constructor is called via its being the
  ...
  protected TOP_Type(JCas jcas, Type casType, boolean installGenerator) {
    ...
    if (installGenerator)
    {
      ((CASImpl)ll_cas).getFSClassRegistry().addGeneratorForType((TypeImpl)this.casType, getFSGenerator()); 
    }
  }
  ...
  // ************ No Object support ********************
	...
	protected void invalidTypeArg(int inst) {
		CASRuntimeException e =
			new CASRuntimeException(CASRuntimeException.INAPPROP_TYPE);
	...
	/** add the corresponding FeatureStructure to all Cas indexes*/
		public void addToIndexes (int inst) {jcas.getLowLevelIndexRepository().ll_addFS(inst);}
  	
		/** remove the corresponding FeatureStructure from all Cas indexes*/
	public void removeFromIndexes (int inst) {jcas.getLowLevelIndexRepository().ll_removeFS(inst);}
   
	public int noObjCreate() {return casImpl.createTempFS(casTypeCode);}
}
