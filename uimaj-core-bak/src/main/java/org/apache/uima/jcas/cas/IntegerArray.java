...
//	}
	...
	/**
	 * return the indexed value from the corresponding Cas IntegerArray_JCasImpl
	 * as an int.
	 */
	...
	/**
	 * update the Cas, setting the indexed value to the passed in Java int
	 * value.
	 */
   ...
   */
    ...
    jcasType.casImpl.checkArrayBounds(addr, srcOffset, length);
		for (int i = 0; i < length; i++) {
			dest[i + destOffset] = Integer.toString(jcasType.ll_cas.ll_getIntArrayValue(addr, i + srcOffset));
		}
	}
   ...
   */
    ...
    jcasType.casImpl.checkArrayBounds(addr, destOffset, length);
    for (int i = 0; i < length; i++) {
      jcasType.ll_cas.ll_setIntArrayValue(addr, i + destOffset, Integer.parseInt(src[i + srcOffset]));
    }
	}
