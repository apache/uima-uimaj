...
public abstract class CasFlowController_ImplBase extends FlowController_ImplBase
{
  ...
  /**
   * Overriden to check that <code>aCAS</code> is an instanceof {@link CAS}.
   * If it is, then {@link #computeFlow(CAS)} is called.  If not, an 
   * exception is thrown.
  ...
  /**
   * This method must be overriden by subclasses. It takes a {@link CAS} and 
   * returns a {@link Flow} object that is responsible for routing this particular CAS 
   * through the components of this Aggregate.  The <code>Flow</code> object should be 
   * given a handle to the CAS, so that it can use information in the CAS to make 
   * routing decisions.
   * <p>
   * FlowController implementations will typically define their own class that implements 
   * {@link Flow} by extending from the base class {@link CasFlow_ImplBase}.  This method 
   * would then just instantiate the flow object, call its <code>setCas</code> method to 
   * provide a handle to the CAS, and return the flow object.  
   * 
   * @param aCAS the CAS to be routed
   * 
  ...
  /**
   * This method may be overriden by subclasses. It is called whenever
   * th TypeSystem of the CAS changes.  This method will be called
   * immediately prior to the first call to {@link #computeFlow(CAS)},
   * and will be called again whenever the CAS TypeSystem changes.
   * <p>
   * In this method, the FlowController can use the {@link TypeSystem} to
   * resolve the names of Type and Features to the actual
   * {@link org.apache.uima.cas.Type} and {@link org.apache.uima.cas.Feature} 
   * objects, which can then be used to access information from the CAS
   * during routing.
   * 
   * @param aTypeSystem the typesystem
   *   
   * @throws CasProcessorComponentException if a problem occurs during processing
   */
