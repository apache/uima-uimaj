...
/**
 * Copy of functionality of TAF TokenProperties class with additional token
 * property for n-gram tokens.
 * 
 */
public class TokenProperties {
    ...
    /**
     * Check if string starts with uppercase letter and continues with lowercase
     * letters.
     * 
     * @return <code>true</code> if the string is non-empty, the first
     *         character is a uppercase letter, and does not contain specials,
     *         numerics or trailing uppers.
     */
    public boolean isInitialUppercaseWord() {
        return ((this.bits & TAF_TOKEN_PROP_LEADING_UPPER) != 0)
                && ((this.bits & (TAF_TOKEN_PROP_NUMERIC
                        | TAF_TOKEN_PROP_SPECIAL | TAF_TOKEN_PROP_TRAILING_UPPER)) == 0);
    }

    /**
     * true if hasNumeric() && !(hasLower() || hasUpper()) Note: this might have
     * decimal point and sign
     */
    public boolean isPlainNumber() {
        return ((this.bits & TAF_TOKEN_PROP_NUMERIC) != 0)
                && ((this.bits & (TAF_TOKEN_PROP_LEADING_UPPER
                        | TAF_TOKEN_PROP_LOWER | TAF_TOKEN_PROP_TRAILING_UPPER)) == 0);
    }
