...
public class LLUnambiguousIteratorImpl implements LowLevelIterator {
    ...
    public LLUnambiguousIteratorImpl(LowLevelIterator it, LowLevelCAS cas) {
        ...
        final int annotType = ts
                .ll_getCodeForTypeName(TCAS.TYPE_NAME_ANNOTATION);
        final int startFeat = ts
                .ll_getCodeForFeatureName(TCAS.FEATURE_FULL_NAME_BEGIN);
        final int endFeat = ts
                .ll_getCodeForFeatureName(TCAS.FEATURE_FULL_NAME_END);
        ...
        while (it.isValid()) {
            ...
            if (ts.ll_subsumes(annotType, curType)) {
                // Found an annotation.
                if (i == 0
                        || (cas.ll_getIntValue(curRef, startFeat) >= lastSeenEnd)) {
    ...
    public Object copy() {
        LLUnambiguousIteratorImpl copy = new LLUnambiguousIteratorImpl(
                this.theIterator, this.theCas);
        copy.pos = this.pos;
        return copy;
    }
