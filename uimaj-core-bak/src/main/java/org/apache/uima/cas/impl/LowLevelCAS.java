...
/**
 * Defines the low-level CAS APIs.  The low-level CAS APIs provide no access to
 * feature structure objects.  All access to CAS data is through integer IDs
 * that represent handles to the CAS data.
 * 
 * <p>The basic concepts of the low-level APIs is very simple.  The CAS does not
 * internally hold its data as Java objects, for performance reasons.  The
 * low-level APIs give you access to this data more or less directly.  Instead
 * of creating Java objects that you can manipulate in an OO fashion, you only
 * obtain int values that are used as references to internal data.  Thus, if you
 * would like to change values on a particular piece of data, you need to hand
 * the FS reference to the API, as opposed to calling methods on a FS object.
 * The tricky part about the low-level APIs is that it's all just ints, and it
 * is very easy to confuse an int that represents a type code with an int that
 * represents a feature code or a FS reference.  Particular care is therefore
 * necessary when using the low-level APIs.  Please follow the guidelines for
 * turning on a minimum of checks below.
 * 
 * <p>This API represents the supported access to low-level features of the
 * UIMA framework CAS implementation.  Other public APIs in the implementation are not
 * supported and are subject to change without notice.
 * 
 * <p>Please note that you really need to know what you're doing when using
 * these APIs.  Incorrect usage of these APIs can and will cause completely
 * unpredictable results; likely your application will crash, and it will do so
 * much later than where the incorrect usage occured.  The low-level APIs are very
 * hard to debug.  You should only use the low-level APIs if you have carefully
 * profiled your application and are sure that the high-level CAS APIs or the
 * JCAS represent a performance bottleneck.
 * 
 * <p>Note that most low-level APIs have versions that allow you to turn some
 * form of parameter checking on.  We strongly encourage you to use those
 * versions during development, they may save you a lot of time.  One way you
 * can use the type checking switch is by having a constant
 * 
 * <p>
 * <pre>static final boolean DO_TYPE_CHECK = true;</pre>
 * 
 * <p>which you can use during development.  For production level code, you can
 * later change the constant to <code>false</code>.  The performance difference
 * to the non-parametrized versions of the getters and setters is probably
 * negligible or may not even exist, depending on the Java runtime.
 * 
 * <p>Please note that even with the check switches turned on, it is still
 * possible to make mistakes that only show up much later in processing.  The
 * main problem is that it is simply impossible to determine for certain whether
 * a given FS reference is valid or not.  The implementation can only determine
 * that it looks like a reference that points at a valid piece of data, but this
 * could be accidental.
 * 
 */
public interface LowLevelCAS
{
  ...
  /** 
   * Integer type.  Type class constant returned by
   * {@link #ll_getTypeClass(int) ll_getTypeClass()}.
   */
  public static final int TYPE_CLASS_INT = 1;
  /** 
   * Float type.  Type class constant returned by
   * {@link #ll_getTypeClass(int) ll_getTypeClass()}.
   */
  public static final int TYPE_CLASS_FLOAT = 2;
  /** 
   * String type.  Type class constant returned by
   * {@link #ll_getTypeClass(int) ll_getTypeClass()}.
   */
  ...
  /**
   * Get the low-level version of the type system object.  It provides access
   * to the low-level type and feature codes you need to use the data creation and
   * access APIs.
  ...
  /**
   * Get the low-level version of the index repository.  Use it to gain access
   * to low-level indexes, and thus, low-level iterators.
   ...
   * @param typeCode The low-level code of the type of the FS that should be 
   * created.  If the <code>typeCode</code> is not a valid type code, the results
   ...
   * Create a new FS on the heap.
   * @param typeCode The low-level code of the type of the FS that should be 
   * created.  If the <code>typeCode</code> is not a valid type code and the
   * type check flag is not set, the results of this call are undefined.
   * @return The reference of the newly created FS.
   * @exception LowLevelException If the type checking switch is set and the
   * type code argument is not valid.
   */
  int ll_createFS(int typeCode, boolean doTypeCheck);

  /**
   * Create a new array.
   * @param typeCode The type code of the array type.  If this is not a valid
   * array type code, the behavior of this call is undefined.
   * Only works for arrays where a value is kept in the main heap 
   *   (use other ll_createXxxArray for boolean, byte, short, long, and double)
  ...
  /**
   * Create a new array.
   * @param typeCode The type code of the array to be created.
   * @param arrayLength The length of the array to be created.
   * @param doChecks Switch to turn on various sanity checks.
   * @return The address of the newly created array.
   */
   ...
   * @param arrayFsRef The array reference.
   * @return The size of the array.
   */
  int ll_getArraySize(int arrayFsRef);
  
  /**
   * Get the low-level reference from an existing FS object.  Use this API if you already
   * have a FS object from somewhere, and want to apply low-level APIs to it. 
   * @param fsImpl The FS object for which we want the reference.
   * @return The low-level reference of the FS object parameter.
   */
  int ll_getFSRef(FeatureStructure fsImpl);

  /**
   * Return a FS object that corresponds to a low-level reference.  Note that this
   * <b>must</b> be a valid reference that has been obtained from the low-level APIs.
   * If the input reference parameter does not represent a valid reference, the results of this
   * call are undefined.
   * @param fsRef The FS reference.
   ...
   * @param fsRef The reference to the FS from which to obtain the feature value.
   * @param featureCode The low-level code of the feature whose value is to be returned.
   ...
   * @param fsRef The reference to the FS from which to obtain the feature value.
   * @param featureCode The low-level code of the feature whose value is to be returned.
   * @param doTypeChecks Switch to turn on type checking.
   * @return The value of the feature.
   */
   ...
   * @param fsRef The reference of the FS on which the feature should be set.
   * @param featureCode The low-level feature code for the feature that should be set.
   ...
   * @param doTypeChecks Switch to turn on type checking.
   */
  void ll_setStringValue(
    int fsRef,
    int featureCode,
    String value,
    boolean doTypeChecks);

  void ll_setCharBufferValue(
    int fsRef,
    int featureCode,
    char[] buffer,
    int start,
   ...
   * @param fsRef The reference to the array FS.
   ...
   * @param fsRef The FS reference of the array.
   * @param position The position whose value will be changed.
   * @param doTypeChecks Switch to turn on type and bounds checking.
   * @param value The new value.
   */
  void ll_setIntArrayValue(
    int fsRef,
    int position,
    int value,
    boolean doTypeChecks);
  ...
  /**
   * Get the type code for a FS reference.  No bounds checks are performed.  If
   * <code>fsRef</code> is not a fs reference, the results are undefined.  There
   * is also a checked version of this call, which will give better error messages
   * in case of problems.
   * @param fsRef The FS reference.
   * @return The type code for the FS reference; a return value of <code>0</code>
   * means that the fsRef is invalid, i.e., <code>NULL_FS_REF</code> (but see
   * remarks on bounds checking for this method).
   */
   ...
   * @param fsRef The FS reference.
   * @param doChecks Check fsRef for out-of-range errors.  If this switch is not
   * set, and the input reference is not a valid reference, the results are
   * undefined.
   * @return The type code for the FS reference; a return value of <code>0</code>
   * means that the fsRef is invalid, i.e., <code>NULL_FS_REF</code> (but see
   * remarks on bounds checking for this method).
  ...
  /**
   * Determine the type class of a type.  This is useful for generic CAS 
   * exploiters to determine what kind of data they're looking at.  The type
   ...
   * <li><code>TYPE_CLASS_FS</code> -- FS type, i.e., all other types, including
   * all user-defined types.</li>
   * </ul>
   * This method is on the CAS, not the type system, since the specific 
   * properties of types are specific to the CAS.  The type system does not know,
   * for example, that the CAS treats arrays specially.
   * @param typeCode The type code.
   * @return A type class for the type code.  <code>TYPE_CLASS_INVALID</code> if
   * the type code argument does not represent a valid type code.
   */
  int ll_getTypeClass(int typeCode);
  
  /**
   * Checks if the type code is that of a reference type (anything that's not
   * a basic type, currently Integer, String and Float).
   * @param typeCode The type code to check.
   * @return <code>true</code> iff <code>typeCode</code> is the type code of a
   * reference type.
   */
