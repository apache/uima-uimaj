 ...
 * the type system, as well as the index repository. If more than one CAS that
 * use the same type system and index repository need to be serialized, this
 * information needs to be serialized only once.
 * 
 * 
 */
public class CASMgrSerializer implements Serializable {
     ...
     * <code>indexNames.length == nameToIndexMap.length</code> and for each
     * <code>i</code> in <code>nameToIndexMap</code>, <code>0 &lt;= i &lt;
     * indexTypes.length</code>.
     */
    public int[] nameToIndexMap;

    /**
     * For each index, the type of that index (encoded as a reference into the
     * type table).
     */
    ...
    /**
     * Encodings of the actual comparators. Each comparator occupies an odd
     * number of cells: one for the type, then feature/comparison pairs. The
     * feature is encoded with its type system code, and comparison operations
     * are encoded with
     * {@link org.apache.uima.cas.admin.FSIndexComparator#STANDARD_COMPARE STANDARD_COMPARE}
     * and
     ...
     * <code>1</code>, and we index the names according to their internal
     * code. That means that <code>typeNames[0] == null</code>.
     */
    public String[] typeNames = null;

    /**
     * A list of feature names (symbol table). Note: numbering of features
     * starts at <code>1</code>, , and we index the names according to their
     * internal code. That means that <code>featureNames[0] == null</code>.
     */
    public String[] featureNames = null;

    /**
     * Type inheritance information: for each type other than the top type, we
     * provide the parent in the inheritance scheme. We use the internal type
     * codes for indexing, which means that cells 0 (no type) and 1 (top type
     * doesn't inherit from anything) are not used.
     */
    public int[] typeInheritance;

    /**
     * Feature declarations. For each feature code <code>i</code>,
     * <code>featDecls[(i-1)*2]</code> is the domain type code, and
     * <code>featDecls[i*2-1]</code> is the range type code (since feature
     * codes start at <code>1</code>).
     */
    ...
    /**
     * The offsets for features. Optional, used for sanity checks. Since feature
     * numbering starts at 1, the length of the array is 1 + number of features.
     */
    ...
    /**
     * The string values for the string subtypes. Start and end postions for the
     * values for the individual types are in <code>stringSubtypeValuePos</code>.
     */
    public String[] stringSubtypeValues;

    /**
     * The start positions of the string value subarrays of
     * <code>stringSubtypeValues</code>.
     * <code>stringSubtypeValuePos.length == 
     * stringSubtypes.length</code>.
     * For each <code>i &lt; 
     * stringSubtypes.length</code>,
     * <code>stringSubtypeValuePos[i]</code> is the start of the string values
     * for <code>stringSubtypes[i]</code>.
     */
    ...
    public void addIndexRepository(FSIndexRepositoryImpl ir) {
        ...
        for (int i = 0; i < numIndexes; i++) {
            this.indexingStrategy[i] = ((FSIndex) indexVector.get(i))
                    .getIndexingStrategy();
        }
    ...
    private String[] getStringArray(int pos) {
        ...
        return getSubarray(this.stringSubtypeValues,
                this.stringSubtypeValuePos[pos], end);
    }
    ...
    public TypeSystemImpl getTypeSystem() {
        ...
        for (int i = 1; i < max; i++) {
            ...
            ts.addFeature(name, this.featDecls[i * 2],
                    this.featDecls[(i * 2) + 1]);
        }
        return ts;
    }
    ...
    public FSIndexRepositoryImpl getIndexRepository(CASImpl cas) {
        ...
        // Get the type order.
        ir.setDefaultTypeOrder(LinearTypeOrderBuilderImpl.createTypeOrder(
                this.typeOrder, cas.getTypeSystem()));
        FSIndexComparator comp;
        ...
        for (int i = 0; i < max; i++) {
            ...
            ir.createIndex(comp, this.indexNames[i],
                    this.indexingStrategy[this.nameToIndexMap[i]]);
        }
        ir.commit();
        return ir;
    }
