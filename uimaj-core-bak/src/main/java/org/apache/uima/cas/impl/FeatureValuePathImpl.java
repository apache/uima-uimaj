 ...
 * feature1/.../featureN. Each part that is enclosed within / is referred to as
 * "path snippet" below. Also contains the necessary evaluation logic to yield
 * the value of the feature path. For leaf snippets, the following "special
 * features" are defined:
 * <ul>
 * <li><code>coveredText()</code> can be accessed using
 * <code>evaluateAsString</code>
 * <li><code>typeName()</code> can be accessed using
 * <code>evaluateAsString</code>
 * <li><code>fsId()</code> can be accessed using <code>evaluateAsInt</code>.
 * Its result can be used to retrieve an FS from the current LowLevel-CAS.
 * <li><code>uniqueId()</code> can be accessed using
 * <code>evaluateAsInt</code>. Its result can be used to uniquely identify an
 * FS for a document (even if the document is split over several CAS chunks)
 * </ul>
 * 
 * <b>Handling of Arrays </b> <br/>
 * <ul>
 * <li>A feature path may contain 0 or more features of type
 * <code>FSArray</code>, but not as the last path snippet. The next path
 * snippet must contain the fully qualified type name, example:
 * <code>family/members[0]/somepackage.Person:name</code></li>
 * <li>A feature path may also contain 0 or 1 feature of type
 * <code>IntArray, StringArray, FloatArray</code>, but only as the last path
 * snippet.</li>
 * </ul>
 ...
 * <li><code>[]</code> returns an array of values. <code>[]</code> is only
 * allowed 0 or 1 time in a feature path. If it is used,
 * <code>getValueType</code> will return one of the following:
 * <code>TCAS.TYPE_NAME_STRING_ARRAY ,TCAS.TYPE_NAME_INTEGER_ARRAY,TCAS.TYPE_NAME_FLOAT_ARRAY</code>.
 * </ul>
 * If the feature path is defined directly for an <code>FSArray</code>, an
 * actual feature name can be omitted, and only the array access operator can be
 * used. Examples:
 * 
 * <pre>
 *               
 ...
 * <br/><b>Usage </b>
 * <ul>
 * <li>To create the feature path, use <code>FeaturePath.getFeaturePath</code>.
 * Note that the client code needs to keep track of the "start type" of the
 * feature path, that is, the type that contains the attribute used in the first
 * snippet of the path.
 * <li>At <code>typeSystemInit</code> of your component (CAS consumer or
 * TAE), call <code>typeSystemInit</code> of the feature path.
 * <li>Call <code>getValueType</code> to find out whether the feature path
 * evaluates to a String, and int, a float, or their array counterparts.
 * <li>Depending on the leaf type, call the appropriate <coce>evaluateAs
 * </code> methods
 * </ol>
 * 
 */
public class FeatureValuePathImpl implements FeatureValuePath {
    ...
    private static final String[] EMPTY_LIST_TYPE_NAMES = new String[] {
            CAS.TYPE_NAME_EMPTY_FS_LIST, CAS.TYPE_NAME_EMPTY_INTEGER_LIST,
            CAS.TYPE_NAME_EMPTY_FLOAT_LIST, CAS.TYPE_NAME_EMPTY_STRING_LIST };
    ...
    private static final String[] LIST_TYPE_NAMES = new String[] {
            CAS.TYPE_NAME_FS_LIST, CAS.TYPE_NAME_INTEGER_LIST,
            CAS.TYPE_NAME_FLOAT_LIST, CAS.TYPE_NAME_STRING_LIST };

    private static final String[] SIMPLE_VAL_TYPES = { CAS.TYPE_NAME_STRING,
            CAS.TYPE_NAME_STRING_ARRAY, CAS.TYPE_NAME_INTEGER,
            CAS.TYPE_NAME_INTEGER_ARRAY, CAS.TYPE_NAME_FLOAT,
            CAS.TYPE_NAME_FLOAT_ARRAY, CAS.TYPE_NAME_INTEGER_LIST,
            CAS.TYPE_NAME_FLOAT_LIST, CAS.TYPE_NAME_STRING_LIST };
    ...
    static {
        ...
        CONTAINER_TO_ELEMENTYPE_MAP.put(CAS.TYPE_NAME_FLOAT_ARRAY,
                CAS.TYPE_NAME_FLOAT);
        CONTAINER_TO_ELEMENTYPE_MAP.put(CAS.TYPE_NAME_INTEGER_LIST,
                CAS.TYPE_NAME_INTEGER);
    ...
    public static FeatureValuePathImpl getFeaturePath(String featurePath)
        ...
        // the leaf element
        FeatureValuePathImpl result = new FeatureValuePathImpl(
                snippets[snippets.length - 1], null);
    ...
    private FeatureValuePathImpl(String pathSnippet, FeatureValuePathImpl child)
        ...
        if (pathSnippet == null || pathSnippet.length() == 0) {
            ...
            exception.addArgument(pathSnippet);
            throw exception;
        }
        ...
        // path
        if ((this.isCoveredTextFeature || this.isFsIdFeature
                || this.isUniqueIdFeature || this.isTypeNameFeature)
                && child != null) {
            CASRuntimeException exception = new CASRuntimeException(
                    CASRuntimeException.INVALID_FEATURE_PATH);
            exception.addArgument(pathSnippet);
    ...
    public Float evaluateAsFloat(int currentFS, LowLevelCAS cas) {
        ...
        if (this.isArrayType) {
            ...
            int arrayFS = (this.isBracketsOnly ? currentFS : cas
                    .ll_getRefValue(currentFS, this.featureCode));
            ...
            case LowLevelCAS.TYPE_CLASS_FLOATARRAY:
                ...
                return new Float(cas.ll_getFloatArrayValue(arrayFS, position,
                        false));
            ...
            default:
                throw new IllegalStateException(
                        "feature path snippet is neither float nor fs array");
            }
        }
        ...
        if (this.isListType) {
            ...
            int listFS = (this.isBracketsOnly ? currentFS : cas.ll_getRefValue(
                    currentFS, this.featureCode));
            ...
            default:
                throw new IllegalStateException(
                        "feature path snippet is neither float nor fs list");
            }
        }

        if (this.childPath != null) {
            int childFS = cas.ll_getRefValue(currentFS, this.featureCode,
                    CAS_TYPE_CHECKS);
            return this.childPath.evaluateAsFloat(childFS, cas);
        } else if (this.isCoveredTextFeature || this.isUniqueIdFeature
                || this.isFsIdFeature || this.isTypeNameFeature) {
            throw new IllegalStateException(
                    "feature path does not denote a float");
        } else {
            int typeClass = cas.ll_getTypeClass(this.featureRangeType);
            switch (typeClass) {
            case LowLevelCAS.TYPE_CLASS_FLOAT:
                return new Float(cas.ll_getFloatValue(currentFS,
                        this.featureCode, CAS_TYPE_CHECKS));
            default:
                throw new IllegalStateException(
                        "feature path does not denote a float");
            }
    ...
    public Float[] evaluateAsFloatArray(int currentFS, LowLevelCAS cas) {
        if (!getValueType().equals(CAS.TYPE_NAME_FLOAT_ARRAY)) {
            throw new IllegalStateException(
                    "Feature path does not denote a float array");
        }
        if (currentFS == 0) {
            return null;
        }
        ...
        if (this.isArrayType) {
            ...
            if (this.arrayIndex == USE_ALL_ENTRIES) {
                ...
                if (this.childPath != null) { // this snippet denotes an
                    ...
                    for (int i = 0; i < arraySize; i++) {
                        int childFS = cas.ll_getRefArrayValue(arrayFS, i,
                                CAS_TYPE_CHECKS);
                        result[i] = this.childPath
                                .evaluateAsFloat(childFS, cas);
                    }
                } else {
                    // this snippet denotes a float array, just collect it
                    for (int i = 0; i < arraySize; i++) {
                        result[i] = new Float(cas.ll_getFloatArrayValue(
                                arrayFS, i));
                    }
                }
                return result;
            ...
            if (this.arrayIndex == USE_ALL_ENTRIES) {
                ArrayList resultList = (ArrayList) getValueAtListIndex(cas,
                        listFS);
    ...
    public Integer evaluateAsInt(int currentFS, LowLevelCAS cas) {
        ...
        if (this.isArrayType) {
            ...
            case LowLevelCAS.TYPE_CLASS_INTARRAY:
                int position = getArrayIndex(arraySize);
                return new Integer(cas.ll_getIntArrayValue(arrayFS, position,
                        false));
            case LowLevelCAS.TYPE_CLASS_FSARRAY:
                int childFS = getFsAtIndex(arrayFS, cas, arraySize);
            ...
            default:
                throw new IllegalStateException(
                        "feature path snippet is neither int nor fs array");
            }
        }
        ...
        if (this.isListType) {
            ...
            default:
                throw new IllegalStateException(
                        "feature path snippet is neither int nor fs list");
            }
        }
        ...
        if (this.childPath != null) {
            ...
            throw new IllegalStateException(
                    "feature path does not denote an int");
            ...
            case LowLevelCAS.TYPE_CLASS_INT:
                return new Integer(cas.ll_getIntValue(currentFS,
                        this.featureCode, CAS_TYPE_CHECKS));
            default:
                throw new IllegalStateException(
                        "feature path does not denote an int");
    ...
    public Integer[] evaluateAsIntArray(int currentFS, LowLevelCAS cas) {
        if (!getValueType().equals(CAS.TYPE_NAME_INTEGER_ARRAY)) {
            throw new IllegalStateException(
                    "Feature path does not denote an int array");
        }
        if (currentFS == 0) {
            return null;
        }
        ...
        if (this.isArrayType) {
            ...
            if (this.arrayIndex == USE_ALL_ENTRIES) {
                ...
                if (this.childPath != null) { // this snippet denotes an
                    ...
                    for (int i = 0; i < arraySize; i++) {
                        result[i] = new Integer(cas.ll_getIntArrayValue(
                                arrayFS, i));
                    }
                }
                return result;
    ...
    /**
     * Evaluates each snippet of the feature path. Returns a String
     * representation of the leaf value of the path. Returns <code>null</code>
     * if some feature within the path is not set. If the leaf snippet is
     * <code>COVERED_TEXT</code>, returns the covered text of
     * <code>currentFS</code>.
     * 
     * @param currentFS
     * @param cas
    ...
    public String evaluateAsString(int currentFS, LowLevelCAS cas) {
        ...
        if (this.isArrayType) {
            ...
            default:
                throw new IllegalStateException(
                        "feature path snippet is neither string nor fs array");
            }
        }
        ...
        if (this.childPath != null) {
            ...
            AnnotationFS annotation = (AnnotationFS) cas
                    .ll_getFSForRef(currentFS);
            return annotation.getCoveredText();
        } else if (this.isTypeNameFeature) {
            Type type = cas.ll_getTypeSystem().ll_getTypeForCode(
                    cas.ll_getFSRefType(currentFS));
            return type.getName();
        } else if (this.isFsIdFeature) {
            ...
            case LowLevelCAS.TYPE_CLASS_STRING:
                return cas.ll_getStringValue(currentFS, this.featureCode,
                        CAS_TYPE_CHECKS);
            case LowLevelCAS.TYPE_CLASS_FLOAT:
            ...
            default:
                throw new IllegalStateException(
                        "feature path does not denote a string");
            }
        }
    }
    ...
    public String[] evaluateAsStringArray(int currentFS, LowLevelCAS cas) {
        if (!getValueType().equals(CAS.TYPE_NAME_STRING_ARRAY)) {
            throw new IllegalStateException(
                    "Feature path does not denote a String array");
        }
        if (currentFS == 0 ) {
            return null;
        }
            ...
            if (this.arrayIndex == USE_ALL_ENTRIES) {
                ...
                if (this.childPath != null) { // this snippet denotes an
                    ...
                    for (int i = 0; i < arraySize; i++) {
                        int childFS = cas.ll_getRefArrayValue(arrayFS, i,
                                CAS_TYPE_CHECKS);
                        result[i] = this.childPath.evaluateAsString(childFS,
                                cas);
                    }
                } else {
    ...
    /**
     * Returns the type for which the last feature in the feature path is
     * defined. Assumes that <code>typeSystemInit</code> has been called prior
     * to this method.
     * <ul>
     * <li>For a feature path <code>feature1/.../featureN-1/featureN</code>,
     * returns the type of featureN.
     * <li>For a feature path
     * <code>feature1/.../featureN-1/typeN:featureN</code>, returns the type
     * code for typeN. (For example, if the range type of featureN-1 is FSList
     * or FSArray)
     * <li>For a feature path <code>feature1</code>, where feature1 is
     * simple-valued, returns the type that was used in
     * <code>typeSystemInit</code>
     * </ul>
     * 
     * @return int the type for which the last feature in the feature path is
     *         defined.
     */
    ...
    /**
     * Returns the type that this feature path will evaluate to. Can be used to
     * select the correct "evaluateAs" method.
     * 
     * @return String the type that this feature path will evaluate to. Will be
    ...
    public void typeSystemInit(int fsType, LowLevelTypeSystem ts)
        ...
        if (fsType == LowLevelTypeSystem.UNKNOWN_TYPE_CODE) {
            CASRuntimeException exception = new CASRuntimeException(
                    CASRuntimeException.INVALID_FEATURE_PATH);
            exception.addArgument(this.typeNameInSnippet);
            throw exception;
        }
        ...
        if (!(isBuiltInFeature() || this.isBracketsOnly)) {
            ...
            if (found) {
                ...
                exception.addArgument(this.featureName);
                exception.addArgument(type.getName());
                throw exception;
            }
        }
        ...
        if (isBuiltInFeature()) {
            ...
            this.isSimpleRangeType = Arrays.binarySearch(SIMPLE_VAL_TYPES, type
                    .getName()) >= 0;
        }

        if (this.isArrayOrList) {
            int arrayType = ts.ll_getCodeForTypeName(CAS.TYPE_NAME_ARRAY_BASE);
            this.isArrayType = ((TypeSystemImpl) ts).subsumes(arrayType,
                    rangeTypeCode);
            if (!this.isArrayType) {
                // check whether the feature points to a list
                for (int i = 0; i < LIST_TYPE_NAMES.length && !this.isListType; i++) {
                    int candidateType = ts
                            .ll_getCodeForTypeName(LIST_TYPE_NAMES[i]);
                    this.isListType = ((TypeSystemImpl) ts).subsumes(
                            candidateType, rangeTypeCode);
                    if (this.isListType) {
                ...
                case TYPE_CLASS_FSLIST:
                    this.headFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_FS_LIST_HEAD);
                    this.tailFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_FS_LIST_TAIL);
                    break;
                case TYPE_CLASS_STRINGLIST:
                    this.headFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_STRING_LIST_HEAD);
                    this.tailFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_STRING_LIST_TAIL);
                    break;
                case TYPE_CLASS_INTEGERLIST:
                    this.headFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_INTEGER_LIST_HEAD);
                    this.tailFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_INTEGER_LIST_TAIL);
                    break;
                case TYPE_CLASS_FLOATLIST:
                    this.headFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_FLOAT_LIST_HEAD);
                    this.tailFeature = ts
                            .ll_getCodeForFeatureName(CAS.FEATURE_FULL_NAME_FLOAT_LIST_TAIL);
                    break;
                default:
                    break;
                }
        ...
        if (this.childPath != null) {
            ...
            int annotationType = ts
                    .ll_getCodeForTypeName(TCAS.TYPE_NAME_ANNOTATION);
            ...
            this.valueTypeName = SIMPLE_VAL_TYPES[Arrays.binarySearch(
                    SIMPLE_VAL_TYPES, CAS.TYPE_NAME_STRING)];
        } else if (this.isFsIdFeature) {
            this.valueTypeName = SIMPLE_VAL_TYPES[Arrays.binarySearch(
                    SIMPLE_VAL_TYPES, CAS.TYPE_NAME_INTEGER)];
        } else if (this.isUniqueIdFeature) {
            this.valueTypeName = SIMPLE_VAL_TYPES[Arrays.binarySearch(
                    SIMPLE_VAL_TYPES, CAS.TYPE_NAME_INTEGER)];
        } else if (this.isTypeNameFeature) {
            ...
            if (this.isArrayOrList && (this.arrayIndex != USE_ALL_ENTRIES)) {
                ...
                this.valueTypeName = (String) CONTAINER_TO_ELEMENTYPE_MAP
                        .get(type.getName());
                ...
                // accordingly
                this.valueTypeName = (String) LIST_TO_ARRAYTYPE_MAP.get(type
                        .getName());
            } else {
                this.valueTypeName = SIMPLE_VAL_TYPES[Arrays.binarySearch(
                        SIMPLE_VAL_TYPES, type.getName())];
            }
        }
    }

    private boolean isBuiltInFeature() {
        return this.isFsIdFeature || this.isUniqueIdFeature
                || this.isCoveredTextFeature || this.isTypeNameFeature;
    }

    /**
     * Checks whether the feature snippet denotes array access (i.e., has [..]
     * attached to it). If so, determines the arrayIndex to use within
     * evaluation, which can be a number, the special element "last" or simple
     * [], which means "all elements"
     * 
     * @throws CASRuntimeException
    ...
    private final void determineArray() throws CASRuntimeException {
        ...
        if (endIndex == -1) { // we're missing the ending bracket
            CASRuntimeException exception = new CASRuntimeException(
                    CASRuntimeException.INVALID_FEATURE_PATH);
            exception.addArgument(this.toString());
            throw exception;
        }

        this.isArrayOrList = true;

        String arrayIndexString = this.featureName.substring(startIndex + 1,
                endIndex);
    ...
    private int getArrayIndex(int arraySize) {
        return (LAST_ARRAY_ENTRY != this.arrayIndex ? this.arrayIndex
                : arraySize - 1);
    }
    ...
    private int getFsAtIndex(int currentFS, LowLevelCAS cas, int arraySize) {
        ...
        // locate the right FS
        int childFS = cas.ll_getRefArrayValue(currentFS, arrayInd,
                CAS_TYPE_CHECKS);
        return childFS;
    }

    /**
     * Assumes that <code>arrayIndex</code>!=<code>USE_ALL_ENTRIES</code>,
     * and that the <code>listType</code> is <code>TYPE_CLASS_FSLIST</code>
     * 
     * @return int A reference to the fs given in <code>arrayIndex</code>, or
     *         0 if the list does not contain an entry for that index.
     * 
     */
    ...
    private Object getValueAtIndexRec(LowLevelCAS cas, int listFS,
            ArrayList list, int count) {
        if (listFS == 0) {
            return null;
        }
    ...
    /**
     * @param listFS
     * @return Object If arrayIndex = USE_ALL_ENTRIES, returns an
     *         <code>ArrayList</code> containing all entries. Otherwise,
     *         returns a String, Integer, or Float. Returns <code>null</code>
     *         if the list does not contain an entry for arrayIndex, or is
     *         empty.
     */
