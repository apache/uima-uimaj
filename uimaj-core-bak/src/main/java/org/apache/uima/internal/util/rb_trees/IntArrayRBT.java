...
/**
 * Red-black tree implementation based on integer arrays. Preliminary
 * performance measurements on j2se 1.4 indicate that the performance
 * improvement as opposed to an object-based implementation are miniscule. This
 * seems to indicate a much improved object creation handling in this vm.
 * 
 * <p>
 * This tree implementation knows two modes of insertion: keys that are already
 * in the tree can be rejected, or inserted as duplicates. Duplicate key
 * insertion is randomized so that the tree's performance degrades gracefully in
 * the presence of many identical keys.
 * 
 * 
 */
public class IntArrayRBT {
    ...
    private class ComparablePointerIterator extends PointerIterator implements
        ...
        public Object copy() {
            ComparablePointerIterator copy = new ComparablePointerIterator(
                    this.comp);
    ...
    private class ComparableIterator extends IntArrayRBTKeyIterator implements
        ...
        public int compareTo(Object o) {
            ...
            return this.comparator.compare(
                    IntArrayRBT.this.key[this.currentNode], it
        ...
        while (node != NIL) {
            ...
            if (k < this.key[node]) {
                ...
                while ((this.left[node] != NIL)
                        && (this.key[this.left[node]] == this.key[node])) {
                    node = this.left[node];
                }
                return node;
            } else {
    ...
    private boolean satisfiesRBProps(int node, final int blackDepth,
            int currentBlack) {
        if (node == NIL) {
            return (currentBlack == blackDepth);
        }
        if (this.color[node] == red) {
            if (this.color[this.left[node]] == red
                    || this.color[this.right[node]] == red) {
                return false;
            }
        } else {
            ++currentBlack;
