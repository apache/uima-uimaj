...
/**
 * Provides access to XInclude processing functionality, which is implemented
...
public final class XIncluder
{
  ...
  /**
   * Creates a new TransformerHandler (implements SAX Content Handler) that
   * processes XIncludes.  Use the
   * {@link TransformerHandler#setResult(javax.xml.transform.Result)} method
   * to specify where the output of the transform is sent.
   * 
   * @return a new XInclude Handler
   * 
   * @throws TransformerConfigurationException if the XInclude transformer
   *    could not be constructed
   */
  public static TransformerHandler newXIncludeHandler()
   ...
   * to specify where the output of the transform is sent.
   * <p>
   * If <code>aLeavePlaceholderElements</code> is set to true, this will
   * leave &lt;xincluded href=""&gt; elements at the point where each xinclude
   * occurred.  The included content will then be the children of the
   * &lt;xincluded&gt; element.  This option allows for retaining information
   * about the source of xincluded elements when necessary (for example, when
   * editing an XML file containing xincludes).
   * 
   * @return a new XInclude Handler
   * 
   * @throws TransformerConfigurationException if the XInclude transformer
  ...
  throws TransformerConfigurationException
  {
    ...
    TransformerHandler handler = 
        sTFactory.newTransformerHandler(getXIncludeTemplates()); 
  ...
  /**
   * Get the XSL Templates to be used for XInclude Processing.  The first 
   * time this method is called, the xinclude.xsl stylesheet will be parsed
    ...
    if (sXIncludeTemplates == null)
    {
      try
      {
        ...
        InputStream xslStream = 
          XIncluder.class.getResourceAsStream("/xinclude.xsl");
        if (xslStream == null)
        {
  ...
  /**
   * SAX Transformer Factory - used to create XSL Transformers for
   * XInclude processing.
   */
  ...
  static
  {
    ...
    {
      final String XALAN_FACTORY_CLASSNAME =  
      ...
      try
      {
        //see if proper Xalan class exists
      	if (null != debug) {
      	  URL url = XIncluder.class.getResource('/' + XALAN_FACTORY_CLASSNAME.replace('.', '/') + ".class");
      	  if (null == url) {
        ...
        Class transformerFactoryClass = Class.forName(XALAN_FACTORY_CLASSNAME, true, XIncluder.class.getClassLoader());
 ...
 /**
  * XSL Templates for XInclude processing - parsed from xinclude.xsl file
  * in first call to getXIncludeTemplates method.
  */
