 ...
 * <p>
 * Clients check-out Resources from the pool using the
 * {@link #getResource()} method and check-in Resources using the
 * {@link #releaseResource(Resource)} method.
 * <p>
 * This resource pool implementation does not deal with differently
 * configured resources in the same pool.  All resources are assumed to 
 * be equivalent and to share the same metadata.  Therefore, the resource
 * metadata can be retrieved via the {@link #getMetaDAta()} method without
 * checking out a Resource instance from the pool.  
 * 
 * 
 * 
 */
public class ResourcePool
{
   ...
   *
   * @param aNumInstances the number of Resource instances in the pool
  ...
  public ResourcePool(int aNumInstances, 
     ResourceSpecifier aResourceSpecifier, Class aResourceClass)
   ...
   *    Resource instances for the pool
   * @param aResourceClass class of resource to instantiate
   * @param aResourceInitParams additional parameters to be passed to
   *    {@link Resource#initialize(ResourceSpecifier,Map)} methods.
...
/*      UIMAFramework.getLogger().log(
            "Acquired resource " + r.getMetaData().getUUID() + " from pool.");
*/
      return r;
    }
    else
   ...
   *
   * @param aResource the resource to release
   */
  public synchronized void releaseResource(Resource aResource)
  {
    ...
    if (!mAllInstances.contains(aResource) ||
        mFreeInstances.contains(aResource))
    {
    ...
    {
/*      UIMAFramework.getLogger().log(
          "Returned resource " + aResource.getMetaData().getUUID() + " to the pool.");
*/
  ...
  /**
   * Gets the metadata for the Resources in this pool.  This pool implementation
   * currently assumes that all Resource instances in the pool are equivalent
    ...
    throws ResourceInitializationException
  {
    ...
    {
      Resource_ImplBase resource = (Resource_ImplBase)
