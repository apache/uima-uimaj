...
/**
 * The <code>PackageSelectorGUI</code> class provides simple Swing-based 
 * file dialog windows for selecting root directories of installed PEAR 
 * packages, as well as PEAR package files, which contain components that 
...
public class PackageSelectorGUI 
	...
	public static class PackageDirFilter extends FileFilter {
		/**
		 * Determines whether a given <code>File</code> object is accepted 
		 * by this filter.
		 * 
		 * @param file The given <code>File</code> object.
		 * @return <code>true</code>, if the given <code>File</code> object 
		 * is accepted, <code>fase</code> otherwise.
		 */
	...
	/**
	 * The <code>PackageFileFilter</code> class allows filtering PEAR package 
	 * files.
	 */
	public static class PackageFileFilter extends FileFilter {
		...
		public boolean accept( File file ) {
			...
			// show PEAR files
			String ext = FileUtil.
				getFileNameExtension( file.getName() ).toLowerCase();
	...
	/**
	 * Opens dialog window to select root directory of desired installed 
	 * component package. 
	 * 
	 * @param componentId The given component ID.
	 * @return Selected package root directory or <code>null</code>, if the 
	 * selection cancelled.
	 */
	public synchronized File selectPackageDirectory( String componentId ) {
		// get last package dir path
		Preferences userPrefs = Preferences.userNodeForPackage( getClass() );
		String lastDirPath = (userPrefs != null) ? 
				userPrefs.get( LAST_PACKAGE_DIR_KEY, "./" ) : "./";
		...
		fileChooser.setDialogTitle( 
			"Select " + componentId + " root directory" );
		...
		if( result == JFileChooser.APPROVE_OPTION ) {
			...
			if( selectedDir != null ) {
				...
				userPrefs.put( 
						LAST_PACKAGE_DIR_KEY, lastDir.getAbsolutePath() );
			}
		}
		return selectedDir;
	}
	/**
	 * Opens dialog window to select desired PEAR package file for a given 
	 * component.
	 * 
	 * @param componentId The given component ID.
	 * @return Selected PEAR package file for the given component, or 
	 * <code>null</code>, if the selection cancelled.
	 */
	public synchronized File selectPackageFile( String componentId ) {
		// get last package file path
		Preferences userPrefs = Preferences.userNodeForPackage( getClass() );
		String lastFilePath = (userPrefs != null) ? 
				userPrefs.get( LAST_PACKAGE_FILE_KEY, "" ) : "";
		File lastFile = lastFilePath.length() > 0 ? 
				new File( lastFilePath ): null;
		File lastDir = lastFile != null ? 
				lastFile.getParentFile() : new File( "." );
		// create JFileChooser
		JFileChooser fileChooser = new JFileChooser();
		...
		if( result == JFileChooser.APPROVE_OPTION ) {
			...
			if( selectedFile != null ) {
				...
				userPrefs.put( 
					LAST_PACKAGE_FILE_KEY, selectedFile.getAbsolutePath() );
			}
		}
		return selectedFile;
	}
	 ...
	 * 
	 * @param componentId The given component ID.
	 * @return The PEAR package URL for the given component or 
	 * <code>null</code>, if no URL is entered.
	 */
