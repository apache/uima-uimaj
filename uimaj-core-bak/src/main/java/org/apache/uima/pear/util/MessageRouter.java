...
/**
 * The <code>MessageRouter</code> class facilitates intra-process message 
 * routing. It provides application classes with convenient access to 
 * the message channels via the <code>PrintWriter</code> class. The 
 * <code>MessageRouter</code> class, by default, defines 2 standard message 
 * channels - for standard output and standard error messages. Applications 
 * can publish their standard output and standard error messages using the
 * <code>outWriter()</code> and <code>errWriter()</code> methods 
 * correspondingly. 
 * <br/>The <code>MessageRouter</code> class distributes the messages to a 
 * number of message channel listeners, added by applications. Standard 
 * message channel listeners should implement the
 * <code>MessageRouter.StdChannelListener</code> interface. The 
 * <code>MessageRouter</code> class collects all published messages. When 
 * a new message channel listener is added, it receives all collected messages 
 * from the message history.
 * <p>
 * The <code>MessageRouter</code> code runs in a separate thread that should 
 * be started and terminated by applications. Applications should use the 
 * <code>start()</code> and <code>terminate()</code> methods to start and
 * terminate the <code>MessageRouter</code> thread correspondingly.
 * <br/>For terminology see the 
 * <a href="http://www.eaipatterns.com/MessageRouter.html">
 * Enterprise Integration Patterns</a> book. 
 * </p>
 * 
 */
...
public class MessageRouter implements Runnable {
	/**
	 * The <code>StdChannelListener</code> interface declares methods that 
	 * should be implemented by each standard message channel listener.
	 * 
	 */
	...
	/**
	 * Default constructor for the <code>MessageRouter</code> class. This
	 * constructor allocates all resources, but does not start the main 
	 * service thread. Applications should start the 
	 * <code>MessageRouter</code> thread using the <code>start()</code> 
	 * method.
	 */
	...
	/**
	 * Adds a given object, implementing the <code>StdChannelListener</code> 
	 * interface, to the list of standard message channel listeners. Sends to 
	 * the new listener all previously collected messages for this channel.
	 * 
	 * @param listener The given new standard message channel listener.
	 */
	...
	/**
	 * @return <code>true</code>, if the router thread is running, 
	...
	/**
	 * Removes a given <code>StdChannelListener</code> object from the list 
	 * of standard channel listeners.
	 * 
	 * @param listener The <code>StdChannelListener</code> object to be 
		...
		while( ! terminated ) {
			...
			synchronized( this ) {
				...
				if( errMessage != null || outMessage != null ) {
					...
					while( list.hasNext() ) {
						StdChannelListener client = 
							(StdChannelListener)list.next();
						if( errMessage != null )
