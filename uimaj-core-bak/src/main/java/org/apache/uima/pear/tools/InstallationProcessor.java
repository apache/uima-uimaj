...
/**
 * The <code>InstallationProcessor</code> class implements operations specified 
 * in the installation descriptor. This class also allows generating the default 
 * Vinci Service descriptor for the specified component.
 * 
...
public class InstallationProcessor {
	...
	/**
	 * Builds $component_id$<suffix> reqular expression string for a given 
	 * component ID and a given 'suffix' string. Valid 'suffix' strings are 
	 * InstallationDescriptor.DELEGATE_ROOT_SUFFIX_REGEX for absolute path,
	 * InstallationDescriptor.DELEGATE_ROOT_REL_SUFFIX_REGEX for relative 
	 * path, InstallationDescriptor.DELEGATE_ROOT_URL_SUFFIX_REGEX for URL.
	 * 
	 * @param componentId The given component ID.
	 * @return The $component_id$root reqular expression string.
	 */
	protected static String componentIdRootRegExp( 
			String componentId, String suffix ) {
	...
	/**
	 * Generates default Vinci Service descriptor for a specified component, 
	 * and puts it to a specified location.
	 * 
	 * @param insdObject The given installation descriptor of the component.
	 * @param mainRootDir The given root directory of the component.
	 * @throws IOException if any I/O exception occurred.
	 */
	public static void generateVSDescriptor( 
		InstallationDescriptor insdObject, File mainRootDir ) 
		throws IOException {
	...
	/**
	 * Generates default Vinci Service descriptor for a specified component, 
	 * and returns the content of the descriptor as a stream (for Eclipse 
	 * plug-in).
	 * 
	 * @param insdObject The given installation descriptor of the component.
	 * @return The stream that contains the default Vinci Service descriptor 
	 * for the specified component.
	 * @throws IOException if any I/O exception occurred.
	 */
	public static InputStream generateVSDescriptorAsStream( 
	...
	/**
	 * Generates the default Vinci Service descriptor content for a specified 
	 * component. 
	 * 
	 * @param insdObject The given installation descriptor of the component.
	 * @return The content of the default Vinci Service descriptor for the 
	 * specified component.
	 */
	protected static String generateVSDescriptorContent( 
		...
		buffer.append( "org.apache.uima.impl.analysis_engine." );
		buffer.append( "service.vinci.VinciXCASSerializer_NoDocText\"/>\n" );
	 ...
	 * 
	 * @param action The given 'find_and_replace_path' installation action.
	 * @throws IOException if any I/O exception occurred.
	 */
	protected static void 
		...
		String filePath = action.params.getProperty( 
				InstallationDescriptorHandler.FILE_TAG );
		if( filePath == null )
			throw new RuntimeException( "no " + 
				InstallationDescriptorHandler.FILE_TAG + " defined" );
		String findString = action.params.getProperty( 
			InstallationDescriptorHandler.FIND_STRING_TAG );
		if( findString == null )
			throw new RuntimeException( "no " + 
				InstallationDescriptorHandler.FIND_STRING_TAG + " defined" );
		String replaceWith = action.params.getProperty( 
			InstallationDescriptorHandler.REPLACE_WITH_TAG );
		if( replaceWith == null )
			throw new RuntimeException( "no " + 
				InstallationDescriptorHandler.REPLACE_WITH_TAG + " defined" );
		// replace all specified path-strings in specified file
		File inputFile = new File( filePath );
		FileUtil.replaceStringInFile( inputFile, 
				StringUtil.toRegExpString( findString ), replaceWith );
	}
	/**
	 * Substitutes '$dlg_comp_id$root_url' and '$dlg_comp_id$root' macros 
	 * in a given string for a given delegate component.
	 * 
	 * @param source The given source string.
	 * @param compId The given component ID.
	 * @param compRootPath The given component root directory path.
	 * @return The result string with all the 'delegate' macros substituted 
	 * and all '\' characters replaced with '/' characters.
	 */
	protected static String substituteCompIdRootInString( 
			String source, String compId, String compRootPath ) {
		String result = source;
		// substitute '$dlg_comp_id$root_url'
		String regExp = 
			componentIdRootRegExp( compId, DELEGATE_ROOT_URL_SUFFIX_REGEX );
		...
		regExp = 
			componentIdRootRegExp( compId, DELEGATE_ROOT_SUFFIX_REGEX );
		replacement = StringUtil.toRegExpReplacement( compRootPath );
		result = result.replaceAll( regExp, replacement );
		return result.trim().replace( '\\', '/' );
	}
	/**
	 * Substitutes '$main_root_url' and '$main_root' macros in a given 
	 * string.
	 * 
	 * @param source The given source string.
	 * @param mainRootPath The given main component root directory path.
	 * @return The result string with all the 'main' macros substituted 
	 * and all '\' characters replaced with '/' characters.
	 */
	protected static String substituteMainRootInString( 
			String source, String mainRootPath ) {
		String result = source;
		// substitute '$main_root_url'
		String replacement = StringUtil.toRegExpReplacement( 
				FileUtil.localPathToFileUrl( mainRootPath ) );
	...
	/**
	 * Constructs an instance of the <code>InstallationProcessor</code> class, 
	 * using given main component root path and a table of installed delegate 
	 * components.
	 * 
	 * @param mainRootPath The given main component root path.
	 * @param installationTable The given table of installed delegate 
	 * components.
	 */
	public InstallationProcessor( 
			String mainRootPath, Hashtable installationTable ) {
		this( mainRootPath, installationTable, null );
	}
	/**
	 * Similar to previous constructor, but sets a given 
	 * <code>InstallationController</code> object as the requestor.
	 * 
	 * @param mainRootPath The given main component root path.
	 * @param installationTable The given table of installed delegate 
	 * components.
	 * @param controller The given <code>InstallationController</code> 
	 * requestor.
	 */
	public InstallationProcessor( 
			String mainRootPath, Hashtable installationTable, 
	...
	/**
	 * @return The modified <code>InstallationDescriptor</code> object, if 
	 * the processing completed, <code>null</code> otherwise.
	 */
	public synchronized InstallationDescriptor getInstallationDescriptor() {
		return _completed ? _insdObject : null;
	}
	/**
	 * Initializes two <code>Hashtable</code> objects that are used to 
	 * substitute $dlg_comp_id$root macros in URL and absolute path 
	 * expressions.
	 * 
	 * @param mainRootPath The given main component root directory path.
	 */
	protected void initSubstitutionTables( String mainRootPath ) {
		...
		while( idList.hasMoreElements() ) {
			...
			String compIdRootUrlRegEx = 
				componentIdRootRegExp( id, DELEGATE_ROOT_URL_SUFFIX_REGEX );
			String compIdRootRegEx = 
				componentIdRootRegExp( id, DELEGATE_ROOT_SUFFIX_REGEX );
	...
	/**
	 * Starts processing installation instructions from the specified 
	...
	public synchronized void process() throws IOException {
		...
		while( actionList.hasNext() ) {
			InstallationDescriptor.ActionInfo action = 
				(InstallationDescriptor.ActionInfo)actionList.next();
	...
	/**
	 * Substitutes two $main_root as well as two $comp_id$root macros 
	 * in a given 'action' <code>Properties</code> object values.
	 * 
	 * @param params The given <code>Properties</code> object.
	 */
		...
		while( paramNames.hasMoreElements() ) {
			...
			if( paramName.equals( InstallationDescriptorHandler.FILE_TAG ) ||
				...
				while( regexList.hasMoreElements() ) {
					String regex = (String)regexList.nextElement();
					String replacement = 
						(String)_pathSubstitutionTable.get( regex );
					paramValue = paramValue.replaceAll( regex, 
							StringUtil.toRegExpReplacement( replacement ) );
				}
	...
	/**
	 * Substitutes two $main_root as well as three $comp_id$root macros 
	 * in all files in a given directory, including its sub-directories. 
	 * 
	...
	protected void substituteStringVariablesInFiles( File dir ) 
		...
		while( fileList.hasNext() ) {
			...
			// substitute '$main_root_url'
			String replacement = 
				FileUtil.localPathToFileUrl( _mainRootPath );
			FileUtil.replaceStringInFile( file, 
					MAIN_ROOT_URL_REGEX, replacement );
			// substitute '$main_root'
			replacement = _mainRootPath;
			FileUtil.replaceStringInFile( file, 
					MAIN_ROOT_REGEX, replacement );
			...
			while( compList.hasMoreElements() ) {
				...
				String compRootPath = 
					(String)_installationTable.get( compId );
				String regex = componentIdRootRegExp( compId, 
						DELEGATE_ROOT_REL_SUFFIX_REGEX );
				try {
					replacement = FileUtil.computeRelativePath( 
						file.getParentFile(), new File( compRootPath ) );
					if( replacement != null )
						FileUtil.
							replaceStringInFile( file, regex, replacement );
				} catch( Exception e ) {}
			}
			// substitute '$dlg_comp_id$root_url'
			Enumeration regexList = _urlSubstitutionTable.keys();
