...
/**
 * The <code>LocalInstallationAgent</code> allows localizing files of UIMA 
 * compliant components within the single PEAR structure, and verifying 
 * localized component's files using UIMA framework API.
 * 
 ...
 * </code><br />
 * where the <code>main_component_root_directory</code> is the path to the 
 * root directory of the main component (root directory of the single PEAR 
 * structure);
 * </li>
 * <li>As a Java object -
 * <br />in this case the caller is expected to set the <code>UIMA_HOME</code>
 * variable, using the <code>setUimaHomePath()</code> method, immediately 
 * after creating a new instance of the <code>LocalInstallationAgent</code> 
 * class.
 * <br /><b>Note:</b> Some TAEs require large heap size, so the 
 * '-Xmx[heapSize]' JVM option may be needed.
 * <br />Localization of component files is performed by using the 
 * <code>localizeComponent()</code> method.
 * <br />Verification of localized files is performed by using the
 * </code>verifyLocalizedComponent()</code> method.
 * <br />The applications prints all messages to the standard output and error 
 * messages to the standard error output.
 * </li>
 * </ul>
 * In both modes the application uses the <code>metadata/PEAR.properties</code> 
 * file for the component localization information. 
 * <br /><b>Note:</b> during the localization phase the application creates 
 * backup copies of all files in both the <code>conf</code> and 
 * <code>desc</code> directories, adding the extension ".$" to each backup 
 * file. If the application fails, please make sure all original files in both 
 * the directories are restored from appropriate "*.$" backup copies.
 * 
...
public class LocalInstallationAgent {
	...
	/**
	 * Checks that a specified PEAR configuration corresponds to a given 
	 * installation descriptor.
	 * 
	 * @param packageConfig The specified PEAR configuration 
	 * (<code>Properties</code> object).
	 * @param insdObject The given installation descriptor object.
	 * @return <code>true</code> if the specified PEAR configuration 
	 * corresponds to the given installation descriptor, <code>false</code> 
	 * otherwise.
	 */
	...
	/**
	 * Performs localization of a given component file using information from a 
	 * given installation descriptor and a specified PEAR configuration.
	 * 
	 * @param file The given component file to be localized.
	 * @param insdObject The given installation descriptor object.
	 * @param packageConfig The specified PEAR configuration.
	 * @throws java.io.IOException if any I/O exception occurred.
	 */
	public static void localizeComponentFile( File file, 
		InstallationDescriptor insdObject, Properties packageConfig ) 
		throws IOException {
		...
		if( mainRootPath != null ) {
			...
			FileUtil.replaceStringInFile( file, 
					InstallationProcessor.MAIN_ROOT_URL_REGEX, replacement );
			// substitute '$main_root'
			replacement = mainRootPath;
			FileUtil.replaceStringInFile( file, 
					InstallationProcessor.MAIN_ROOT_REGEX, replacement );
		}
		// substitute $comp_id$root macros for all delegates
		Iterator idList = 
			insdObject.getDelegateComponents().keySet().iterator();
		while( idList.hasNext() ) {
			String id = (String)idList.next();
			String idRoot = COMP_ROOT_PREFIX + id + COMP_ROOT_SUFFIX;
			...
			String replacement = FileUtil.computeRelativePath( 
	...
	/**
	 * Performs localization of a given installation descriptor object using 
	 * information from a specified PEAR configuration.
	 * 
	 * @param insdObject
	 * @param packageConfig
	 */
	public static void localizeInstallationDescriptor( 
		InstallationDescriptor insdObject, Properties packageConfig ) {
		// set main root
		String mainRootPath = packageConfig.getProperty( MAIN_ROOT );
	 ...
	 * </ul>
	 * 
	 * @param args main_component_root_dir
	 */
	public static void main(String[] args) {
		// check args
		if( args.length < 1 ) {
			System.err.println( "usage: LocalInstallationAgent " + 
				"main_component_root_dir" );
			return;
		}
		LocalInstallationAgent installer = 
			new LocalInstallationAgent( args[0] );
		try {
			// localize PEAR
			if( installer.localizeComponent() ) {
				System.out.println( "[LocalInstallationAgent]: " + 
			...
			try {
				...
				System.err.println( 
					"Error trying to undo component localization: " + e );
				System.out.println( "> Please, make sure that all *.$ files " +
					"in conf and desc dirs\n" + 
					"> are renamed back to their original names" );
			}
		}
	}
	/**
	 * Constructs new instance of the <code>LocalInstallationAgent</code> 
	 * class, using a given main component root directory.
	 * 
	 * @param mainComponentRootPath The given main component root directory
	 * path.
	 */
	public LocalInstallationAgent( String mainComponentRootPath ) {
		_uimaHomePath = 
			System.getProperty( InstallationController.UIMA_HOME_ENV );
	...
	/**
	 * Performs localization of the component files in the 'conf' and 'desc' 
	 * subdirectories of the specified main root directory, as well as  
	 * localization of the installation descriptor.
	 * 
	 * @return <code>true</code> if the localization process completed 
	...
	public synchronized boolean localizeComponent() throws IOException {
		...
		File insdFile = 
			new File( _mainRootDir, InstallationProcessor.INSD_FILE_PATH );
		try {
		...
		if( _insdObject == null )
			throw new RuntimeException( 
				"failed to load installation descriptor" );
		System.out.println( "[LocalInstallationAgent]: " + 
				"loaded installation descriptor" );
		// load PEAR configuration
		File packageConfigFile = 
			new File( _mainRootDir, InstallationController.PACKAGE_CONFIG_FILE );
	...
	/**
	 * Performs localization of the component files in the 'conf' and 'desc' 
	 * directories by replacing $main_root and $component_id$root macros 
	 * with the actual path values from the PEAR configuration file. Back-up 
	 * copies of original files are stored with '.$' extension. 
	 * 
	...
	protected synchronized File[] localizeComponentFiles() throws IOException {
		...
		File confDir = 
			new File( _mainRootDir, InstallationController.PACKAGE_CONF_DIR );
		Collection confDirFiles = FileUtil.createFileList( confDir, false );
		File descDir = 
			new File( _mainRootDir, InstallationController.PACKAGE_DESC_DIR );
	...
	/**
	 * Restores original files in the 'conf' and 'desc' directories from the 
	 * back-up copies (with extension '.$').
	 * 
	...
	public synchronized boolean undoComponentLocalization() throws IOException {
		...
		for( int i = 0; i < _localizedFiles.length; i++ ) {
			...
			if( FileUtil.copyFile( bakFile, orgFile ) ) {
				...
				System.err.println( "[LocalInstallationAgent]: " + 
					"failed to undo changes for the file " + 
	 ...
	 * <code>InstallationTester</code> application.
	 * 
	...
	public synchronized boolean verifyLocalizedComponent() throws IOException {
		...
		if( _uimaHomePath == null )
			throw new RuntimeException( InstallationController.UIMA_HOME_ENV + 
				" variable not specified" );
		// build component classpath
		String compClassPath = InstallationController.
			buildComponentClassPath( mainRootDirPath, _insdObject );
		// build UIMA classpath
		String uimaClassPath = InstallationController.
			buildUIMAClassPath( _uimaHomePath );
		// set java.library.path
		String javaLibPath = InstallationController.
			buildComponentPath( mainRootDirPath, _insdObject );
		// set other required env vars
		Properties tableOfEnvVars = InstallationController.
			buildTableOfEnvVars( _insdObject );
		...
		InstallationController.TestStatus status = InstallationController.
			deployInstallationVerificationTest( 
				mainRootDirPath, _insdObject, mainDescPath, 
