...
/**
 * Base class for Collection Readers, which developers should extend with their
 * own Collection Reader implementation classes.
 * 
 * 
 */
  ...
  implements CollectionReader 
{
  ...
  /**
   * Default implementation of destroy, which calls {@link #close()}.  If close throws
   * an IOException, it will be logged.
  ...
  /**
   * Default implementation of typeSystemInit, which calls the CAS Initializer's
   * typeSystemInit method if a CAS Initializer is present.
  ...
  /**
   * Gets the metadata for this CollectionReader, which was extracted from the 
   * descriptor during initialization.
  ...
  /**
   * Gets the CAS initializer associated with this CollectionReader, if one
   * was supplied via {@link #setCasInitializer(CasInitializer)}.
  ...
  /**
   * Stores the CAS initializer in a private field and provides access to it
   * via {@link #getCasInitializer()}.
  ...
  /**
   * Notifies this Collection Reader that its configuration parameter settings have
   * been changed.  By default this method just calls {@link #destroy()} followed
   * by {@link #initialize()}.  Collection Readers that have expensive initialization
