...
/**
 * Convenience base class for Analysis Engine Services. Analysis Engine
 * services are not required to extends this class, but it it useful for
 * those services that communicate using binary data.
 * 
 * 
 */
public class AnalysisEngineService_impl extends ResourceService_impl
{
   ...
   * @see org.apache.uima.resource.service.impl.ResourceService_impl#initialize(org.apache.uima.resource.ResourceSpecifier, int, java.util.Map, int)
   */
    ...
    throws ResourceInitializationException
  {
    ...
    Integer numInstances = 
      (Integer)aResourceInitParams.get(AnalysisEngine.PARAM_NUM_SIMULTANEOUS_REQUESTS);
    ...
    Integer timeoutInteger =  
      (Integer)aResourceInitParams.get(AnalysisEngine.PARAM_TIMEOUT_PERIOD);
  ...
  /**
   * An alternative form of initialize that takes the number of simultaneous
   * requests and timeout period as explicit arguments.
   */
  public void initialize(ResourceSpecifier aResourceSpecifier,
    int aNumSimultaneousRequests, int aTimeout)
    throws ResourceInitializationException
  {
    Map initParams = new HashMap();
    initParams.put(AnalysisEngine.PARAM_NUM_SIMULTANEOUS_REQUESTS, 
        new Integer(aNumSimultaneousRequests));
    initParams.put(AnalysisEngine.PARAM_TIMEOUT_PERIOD,
        new Integer(aTimeout));
    ...
    try
    {
      ...
      if (cas == null)
      {
        throw new ResourceServiceException(
          ResourceServiceException.RESOURCE_UNAVAILABLE, new Object[0]);
      }
  ...
  /**
   * Gets a CAS from the CAS Pool.  Throws an exception if the
   * timeout period elapses.
   * 
   * @param aTimeout maximum time to wait in milliseconds
   * 
   * @return a CAS that has been checked-out of the pool
   * 
   * @throws ResourceServiceException if a CAS is not available within
   *    the timeout period.
   */  
    ...
    throws ResourceServiceException
  {
    ...
    {
      throw new ResourceServiceException(
        ResourceServiceException.RESOURCE_UNAVAILABLE, null);
    }    
    return cas;
  }
  ...
  /**
   * Timeout period, in milliseocnds, to wait when attempting to get CAS 
   * from the pool.
   */
