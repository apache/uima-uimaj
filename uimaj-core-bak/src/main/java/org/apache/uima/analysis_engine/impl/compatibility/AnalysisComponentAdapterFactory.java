...
/**
 * Factory that builds {@link CasProcessorComponent} instances from
 * AnalysisEngineDescription, CasConsumerDescription, or CollectionReaderDescription
 * objects.
 */
public class AnalysisComponentAdapterFactory
{
   ...
   * {@link AnalysisComponent} interface.  This is needed because UIMA
   * has distinct interfaces that are implemented by different types of 
   * components, e.g. Annotator, CAS Consumer, CollectionReader, but
   * the UIMA framework itself simplifies things by interacting with all
   * of them through a single, common AnalysisComponent interface.
   * 
   * @param aAdaptee Object to adapet to the AnalysisComponent interface
   * @param aMetaData metadata for the AnalysisEngine containing this component
  ...
  public static AnalysisComponent createAdapter(Object aAdaptee,
      AnalysisEngineMetaData aMetaData, Map aAdditionalParams)
  throws ResourceInitializationException
  {
  ...
  /**
   * Determines whether this factory is capable of producing an adapter that
   * adapts the given class to the AnalysisComponent interface.
   * @param cls the adaptee class
  ...
  {
    return BaseAnnotator.class.isAssignableFrom(cls) ||
            CasConsumer.class.isAssignableFrom(cls) ||
