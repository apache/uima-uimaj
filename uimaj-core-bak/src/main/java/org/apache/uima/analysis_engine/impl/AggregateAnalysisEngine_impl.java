...
public class AggregateAnalysisEngine_impl extends AnalysisEngineImplBase implements AnalysisEngine
{
  ...
  public void setResultSpecification(ResultSpecification aResultSpec)
  {
    ...
    {    
      ...
      ResultSpecification resultSpecForComponents =
        (ResultSpecification)aResultSpec.clone();
  ...
  /**
   * A utility method that performs initialization logic for a aggregate 
   * AnalysisEngine.
   * 
   * @param aDescription the AnalysisEngine description for this AnalysisEngine
   * @param aAdditionalParams additional parameters that were passed to this
   *    AnalysisEngine's initialize method.
   *
  ...
  /**
   * A utility method that creates and configures the ASB component.  The
   * ASB will create and initialize the delegate AnalysisEngines.  This method 
   * also retrieves delegate AnalysisEngine metadata from the ASB and provides 
   * access to that method via the {@link _getDelegateAnalysisEngineMetaData()} 
   * method.
   * 
   * @param aDescription the AnalysisEngine description for this AnalysisEngine
   * @param aAdditionalParams parameters that will be passed to the ASB's
   *   initialize method.
   *
   * @throws ResourceInitializationException if the ASB or a delegate 
   *   AnalysisEngine could not be created.
   */
      ...
      throws ResourceInitializationException
  {
    ...
    FlowControllerDeclaration flowControllerDecl =
      aAnalysisEngineDescription.getFlowControllerDeclaration();
    if (flowControllerDecl != null)
    {
      try
      {
        aAnalysisEngineDescription.getFlowControllerDeclaration().resolveImports(getResourceManager());
      }
      catch (InvalidXMLException e)
      {
        throw new ResourceInitializationException(e);
    ...
    mASB.setup(_getComponentCasProcessorSpecifierMap(), getUimaContextAdmin(), 
  ...
  /**
   * Does processing using the delegate AnalysisEngine metadata once it becomes
   * available.
   * <p>
   * Specifically, sets this aggregate AE's Type System, Type Priorities, and 
   * FS Index Descriptions equal to the result of merging the information from 
   * its delegate AEs.  
   * 
  ...
  protected void processDelegateAnalysisEngineMetaData() throws ResourceInitializationException
  {
    ...
    //now do merge
		TypeSystemDescription aggTypeDesc = 
		    CasCreationUtils.mergeTypeSystems(typeSystems, getResourceManager());
		TypePriorities aggTypePriorities = 
		    CasCreationUtils.mergeTypePriorities(typePriorities, getResourceManager());
		FsIndexCollection aggIndexColl = 
		    CasCreationUtils.mergeFsIndexes(fsIndexCollections, getResourceManager());		
  ...
  /**
   * Checks operational properties in an aggregate to ensure they are
   * not inconsistent with operational properties of the components.
   * For example, an aggregate cannot have multipleDeploymentAlloiwed == true
   * if it contains a component with multipleDeploymentAllowed == false.
   * 
   * @param aDesc the aggregate analysis engine description to validate
   * 
   * @throws ResourceInitializationException if there is an invalid parameter
    ...
    throws ResourceInitializationException
  {
    ...
    if (aggProps != null)
    {
        ...
        if (componentProps != null)
        {
              ...
              !componentProps.isMultipleDeploymentAllowed())
          {
            throw new ResourceInitializationException(ResourceInitializationException.INVALID_MULTIPLE_DEPLOYMENT_ALLOWED,
                new Object[] {getAnalysisEngineMetaData().getName(), md.getName(), getAnalysisEngineMetaData().getSourceUrlString()});
          }
          if (!aggProps.getModifiesCas() &&
              componentProps.getModifiesCas())
          {
      ...
      {
        throw new ResourceInitializationException(ResourceInitializationException.INVALID_OUTPUTS_NEW_CASES,
            new Object[] {getAnalysisEngineMetaData().getName(), getAnalysisEngineMetaData().getSourceUrlString()});
      }
    }
  }
  
  /**
   * Returns the default FlowController declaration to be used if the 
   * aggregate AE descriptor does not specify one.
   */
  protected FlowControllerDeclaration getDefaultFlowControllerDeclaration()
  {
    ...
    {
      flowControllerSpecifier.setImplementationName(CapabilityLanguageFlowController.class.getName());
    }
    else
    {
  ...
  /** 
   * For an aggregate AnalysisEngine only, gets a Map from each component's key
   * to ProcessingResourceMetaData for that component.  This includes component
   * AEs as well as the FlowController.
   * 
  ...
  /** 
   * For an aggregate AnalysisEngine only, gets a Map from each component's key
   * to the specifier for that component.
   * 
  ...
  /**
   * Construct a ProcessTrace object that represnts the last excecution of
   * this AnalysisEngine.  This is used so that we can return a ProcessTrace
   * object from each process() call for backwards compatibility with version 1.x.
   */
  protected void buildProcessTraceFromMBeanStats(ProcessTrace trace)
  {
    if (isProcessTraceEnabled())
    {
      ProcessTraceEvent_impl procEvt = new ProcessTraceEvent_impl(getMetaData().getName(), "Analysis", "");
      ...
      ProcessTraceEvent_impl flowControllerEvent =
        new ProcessTraceEvent_impl(fcc.getMetaData().getName(),"Analysis","");
  ...
  /**
   * For an aggregate AnalysisEngine only, a Map from each component's
   * key to a ProcessingResourceMetaData object for that component.
   * This includes component AEs as well as the FlowController.
   */
  private Map mComponentMetaData;

  /**
   * For an aggregate AnalysisEngine only, the ASB used to communicate with the 
   * delegate AnalysisEngines.
   */
  private ASB mASB;
