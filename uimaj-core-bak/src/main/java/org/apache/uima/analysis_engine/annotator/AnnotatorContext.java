...
/**
 * This interface is maintained only for backwards compatibility with annotators
 * developed in UIMA SDK v1.x.  New code should use {@link UimaContext} instead.
 */
public interface AnnotatorContext
{
   ...
   *  
   * @return  the value of the parameter with the given name.  The annotator
   *     is expected to know the data type of its parameters.  Returns <code>null</code>
   *     if the parameter does not exist or has not been assigned a value.
   *
   * @throws AnnotatorContextException if an internal failure has occurred
   *     in the <code>AnnotatorContext</code>.
   */ 
   ...
   * property will be used.  The search strategy can be specified in the 
  ...
  public Object getConfigParameterValue(String aGroupName, String aParamName)
   ...
   /**
	 ...
	 *    this annotator.  Returns an empty array if no groups are declared.
	 */
  ...
  /**
   * Gets the <code>Logger</code> to which this annotator should send its log 
   * output.  Annotators should use this facility rather than writing to their
   * own log files (or to stdout).
   *  
  ...
  /**
   * Gets the <code>InstrumentationFacility</code> that this annotator
   * can use to record information about its performance. 
   *  
   * @return  an instance of the instrumentation facility for use by this
   *     annotator.
   *
   * @throws AnnotatorContextException if an internal failure has occurred
   *     in the <code>AnnotatorContext</code>.
