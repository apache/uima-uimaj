...
/**
 * Container for a FlowController.  Manages configuration parameters,
 * resources, CAS interface conversions, and performance timing.  
 */
public class FlowControllerContainer extends ConfigurableResource_ImplBase
{
  ...
  public boolean initialize(ResourceSpecifier aSpecifier, Map aAdditionalParams) throws ResourceInitializationException
  {
    try
    {
      ...
      if (fwImpl == null || !(fwImpl.startsWith("org.apache.uima.java") || fwImpl.startsWith("JEDII")))
      {
        throw new ResourceInitializationException(
  ...
  /**
   * Invokes the FlowController's computeFlow method, returning a Flow
   * object that routes the given CAS through this aggregate.
   * This method makes sure to provide the FlowController with its required CAS
   * interface (e.g. JCas).
   * 
   * @param aCAS the CAS to pass to the FlowController
   * @return a Flow object that routes this CAS
   * 
   * @throws AnalysisEngineProcessException if the FlowController failed
   */
    ...
    throws ResourceInitializationException
  {
    ...
    if (!(userObject instanceof FlowController))
    {
      throw new ResourceInitializationException(
          ...
          new Object[] { flowControllerClassName, FlowController.class, aDescriptor.getSourceUrlString() });         
    }
