...
/**
 * A simple {@link ASB} implementation.  This implementation is not
 * specific to any transport technology.  It simply uses the 
 * {@link ResourceFactory} to acquire instances of its delegate AnalysisEngines 
 * and then communicates with these delegate AnalysisEngines through the 
 * {@link AnalysisEngine} interface.  Any communication with remote 
 * AnalysisEngine services is done through a 
 * {@link org.apache.uima.analysis_engine.service.AnalysisEngineServiceAdapter}
...
public class ASB_impl extends Resource_ImplBase implements ASB
{
   ...
   * 
   * @param aSpecifier describes how to create this ASB. 
   * @param aAdditionalParams parameters which are passed along to the
   *    delegate Analysis Engines when they are constructed
   *  
  ...
  /**
   * Called by the Aggregate Analysis Engine to provide this
   * ASB with information it needs to operate.
   * 
   * @param aSpecifiers the specifiers for all component AEs within
   *   this Aggregate.  The ASB will instantiate those AEs.
   * @param aParentContext UIMA context for the aggregate AE
   * @param aFlowControllerDeclaration declaration (key and specifier)
   *   of FlowController to be used for this aggregate.
      ...
      AnalysisEngineMetaData aAggregateMetadata) throws ResourceInitializationException
  {
    ...
    mAllComponentMetaDataMap.put(aFlowControllerDeclaration.getKey(), mFlowControllerContainer.getMetaData());
    ...
    throws ResourceInitializationException
  {
    ...
    FlowControllerContext ctxt =
      new FlowControllerContext_impl(aParentContext, key, sofamap,
    ...
    mFlowControllerContainer.initialize(aFlowControllerDeclaration.getSpecifier(), flowControllerParams);
  }
   ...
   *  Returns the same object as UimaContext.getManagementInterface() but 
   *  casted to the AnalysisEngineManagement type.
   */
  ...
  /** 
   * Map from String key to delegate AnalysisEngine for all component 
   * AnalysisEngines within this ASB.
   */
  private Map mComponentAnalysisEngineMap = new HashMap();

  /** 
   * Map from String key to delegate AnalysisEngineMetaData for all component 
   * AnalysisEngines within this ASB.
   */
  private Map mComponentAnalysisEngineMetaDataMap = new HashMap();

  /** 
   * Map from String key to component (AnalysisEngine or FlowController)
   * metadata.
   */
  private Map mAllComponentMetaDataMap = new HashMap();

  /** 
   * Initialization parameters passed to this ASB's initialize method.  They
   * will be passed along to the delegate AnalysisEngines.
   */
  ...
  class AggregateCasIterator implements CasIterator
  {
    ...
    /**
     * Stack, which holds StackFrame objects. A stack is necessary to handle CasMultipliers,
     * because when a CasMultiplier is invoked we need to save the state of processing of the
     * current CAS and start processing the output CASes instead.  Since CasMultipliers can be
     * nested, we need a stack.
     */
    Stack casIteratorStack = new Stack();
    
    /**
     * Set of CASes that are in circulation (that is, they have been passed
     * to FlowController and the FlowController hasn't yet returned a FinalStep for them). 
     * Needed so we can clean up on error.
     */
     ...
     *  The CasIterator will return all of the output CASes that this Aggregate AE generates
     *  when run on that input CAS, if any.
     * @param inputCas the CAS to be input to the Aggregate AE (this is the CAS that
     *    was passed to the Aggregate AE's processAndOutputNewCASes(CAS) method)
     * @throws AnalysisEngineProcessException if processing fails
     */
    ...
    /**
     * This is the main execution control method for the aggregate AE.  It is
     * called by the AggregateCasProcessorCasIterator.next() method.  
     * This runs the Aggregate, starting from its current state, until such time as the 
     * FlowController indicates a CAS should be returned to the caller.
     * The AggregateCasIterator remembers the state, so calling this method a second
     * time will continue processing from where it left off.
     * 
     * @return the next CAS to be output.  Returns null if the processing of the
     *    input CAS has completed. 
     * 
     * @throws ProcessingException if a failure occurs during processing
     */
  ...
  /**
   * A frame on the processing stack for this Aggregate AE.  Each time processing encounters
   * a CasMultiplier, a new StackFrame is created to store the state associated with the processing
   * of output CASes produced by that CasMultiplier.
   */
  static class StackFrame
  {
    ...
    /** The Flow object for the original CAS, so we can pick up processing from there
