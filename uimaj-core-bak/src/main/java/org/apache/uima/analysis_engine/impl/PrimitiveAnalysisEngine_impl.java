...
public class PrimitiveAnalysisEngine_impl extends AnalysisEngineImplBase implements AnalysisEngine
{
   ...
   * @param aAdditionalParams parameters passed to this AE's initialize method
   * 
      ...
      throws ResourceInitializationException
  {
    ...
    {
      throw new ResourceInitializationException(
          ResourceInitializationException.ANNOTATOR_CLASS_NOT_FOUND,
    ...
    try
    {
      ...
      {
        mAnalysisComponent = AnalysisComponentAdapterFactory.createAdapter(userObject, getAnalysisEngineMetaData(), aAdditionalParams);
      }  
    }
    ...
    getUimaContextAdmin().defineCasPool(
        mAnalysisComponent.getCasInstancesRequired(),getPerformanceTuningSettings(), mSofaAware);
  }  
   ...
   * 
   * @param aCAS CAS to be processed by annotator
   * @param aResultSpec result specification to be passed to annotator
   * @param aProcessTrace keeps track of time spent in each component
   */
      ...
      try
      {
        ...
        if (mResultSpecChanged || mLastTypeSystem != view.getTypeSystem())
        {
          ...
          ResultSpecification analysisComponentResultSpec =
             computeAnalysisComponentResultSpec(mCurrentResultSpecification,
  ...
  /**
   * Creates the ResultSpecification to be passed to the AnalysisComponent.
   * This is derived from the ResultSpec that is input to this AE
   * (via its setResultSpecification method) by intersecting with the
   * declared outputs of this AE, so that we never ask an AnalysisComponent
   * to produce a result type that it does not declare in its outputs.
   * 
   * @param currentResultSpecification the result spec passed to this AE's setResultSpecification method
   * @param capabilities the capabilities of this AE
   * 
   * @return a ResultSpecifciation to pass to the AnalysisComponent
   */
  protected ResultSpecification computeAnalysisComponentResultSpec(ResultSpecification inputResultSpec, Capability[] capabilities)
  {
   ...
   * 
   * @param aCAS the CAS to be affected
   * @param aEnable false to lock out functions, true to re-enable them
   */
  ...
  /**
   * If this is set it indicates that the AnalysisEngine is being constructed only to verify the validity of the descriptor.
   * The Annotator classes should not be instantiated in this case.
   */
  ...
  /**
   * Implements the iterator that steps through all outputs from
