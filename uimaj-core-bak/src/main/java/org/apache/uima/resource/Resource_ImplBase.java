...
/**
 * Implementation base class for {@link org.apache.uima.resource.Resource}s.  
 * Provides access to resource metadata and the UIMA Context, which in 
 * turn provides access to framework facilities such as logging and resource 
 * management.
 * 
 * 
 */
public abstract class Resource_ImplBase implements Resource {
	...
	public boolean initialize(
		...
		if (mInitialized) {
			throw new UIMA_IllegalStateException(
				UIMA_IllegalStateException.RESOURCE_ALREADY_INITIALIZED,
				new Object[] { name });
		}
		...
		if (aAdditionalParams != null) {
			mUimaContextAdmin =
				(UimaContextAdmin) aAdditionalParams.get(
					PARAM_UIMA_CONTEXT);
		}
        ...
        throw new ResourceInitializationException(e);
			...
			try {
				mUimaContextAdmin.getConfigurationManager().createContext(
					mUimaContextAdmin.getQualifiedContextName(),
					getMetaData());
				mUimaContextAdmin.getConfigurationManager().setSession(
					mUimaContextAdmin.getSession());
			...
			ResourceManagerConfiguration resMgrCfg =
				((ResourceCreationSpecifier) aSpecifier)
	...
	/**
	 * Sets the <code>ResourceMetaData</code> object associated with this
	 * <code>Resource</code>.  Any previously existing metadata will be
	 * replaced.
	 * <p>
	 * Resource subclasses should call this method during initialization in
	 * order to set the metadata before any calls to 
	 * {@link #getMetaData()} are made.
	 *  
	 * @param aMetaData metadata to assign to this <code>Resource</code>
	 */
    ...
    /**
     * Get the CasManager for this Resource.  The CasManager manages the
     * creation and pooling of CASes.
	...
	/**
	 * Whether this Resource's {@link #initialize(ResourceSpecifier,Map)} 
