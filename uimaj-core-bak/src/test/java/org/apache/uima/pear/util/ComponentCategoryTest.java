...
/**
 * The <code>ComponentCategoryTest</code> class provides JUnit test cases for 
 * the org.apache.uima.pear.util.UIMAUtil.identifyUimaComponentCategory() method.
 * The test cases are based on the sample XML descriptors located in the 
 * 'pearTests/componentCategoryTests' folder.
 * 
 * @author LK
 */
public class ComponentCategoryTest extends TestCase {
	...
	private static String 
			TEST_FOLDER = "pearTests/componentCategoryTests";
	...
	public void testAeDescriptor() throws Exception {
		File aeDescFile = 
			new File( _junitTestBasePath, TEST_FOLDER + "/" + AE_DESC_NAME );
		if( ! aeDescFile.isFile() )
			throw new FileNotFoundException( "AE descriptor not found" );
		Assert.assertTrue( UIMAUtil.ANALYSIS_ENGINE_CTG.equals( 
				UIMAUtil.identifyUimaComponentCategory( aeDescFile ) ) );
	}
	/**
	...
	public void testCcDescriptor() throws Exception {
		File ccDescFile = 
			new File( _junitTestBasePath, TEST_FOLDER + "/" + CC_DESC_NAME );
		if( ! ccDescFile.isFile() )
			throw new FileNotFoundException( "CC descriptor not found" );
		Assert.assertTrue( UIMAUtil.CAS_CONSUMER_CTG.equals( 
				UIMAUtil.identifyUimaComponentCategory( ccDescFile ) ) );
	}
	/**
	...
	public void testCiDescriptor() throws Exception {
		File ciDescFile = 
			new File( _junitTestBasePath, TEST_FOLDER + "/" + CI_DESC_NAME );
		if( ! ciDescFile.isFile() )
			throw new FileNotFoundException( "CI descriptor not found" );
		Assert.assertTrue( UIMAUtil.CAS_INITIALIZER_CTG.equals( 
				UIMAUtil.identifyUimaComponentCategory( ciDescFile ) ) );
	}
...
//	}
	...
	public void testCrDescriptor() throws Exception {
		File crDescFile = 
			new File( _junitTestBasePath, TEST_FOLDER + "/" + CR_DESC_NAME );
		if( ! crDescFile.isFile() )
			throw new FileNotFoundException( "CR descriptor not found" );
		Assert.assertTrue( UIMAUtil.COLLECTION_READER_CTG.equals( 
				UIMAUtil.identifyUimaComponentCategory( crDescFile ) ) );
	}
//	/**
