...
public class UnambiguousIteratorTest extends TestCase {
    ...
    public void testUnambiguous() throws Exception {
        ...
        try {
             ...
             * XMLInputSource in = new XMLInputSource(this.tafDataDir +
             * "specifiers/jtok.xml"); // Parse the specifier. ResourceSpecifier
             * specifier = UIMAFramework.getXMLParser()
             * .parseResourceSpecifier(in); // Create the Text Analysis Engine.
             * tae = UIMAFramework.produceTAE(specifier, null, null); // Create
             * a new TCAS. TCAS cas = tae.newTCAS(); // Set the document text on
             * the CAS. cas.setDocumentText(text);
             * cas.setDocumentLanguage("en"); // Process the sample document.
            ...
            LowLevelIndex annotIdx = llc.ll_getIndexRepository().ll_getIndex(
                    TCAS.STD_ANNOTATION_INDEX, tokType);
            ...
            xcasDeserializer = new XCASDeserializer(cas
                    .getTypeSystem());
            xcasFile = new File(xcasDir, sampleXcas2FileName);
            parser.parse(xcasFile, xcasDeserializer.getXCASHandler(cas));
            
            annotIdx = llc.ll_getIndexRepository().ll_getIndex(
                    TCAS.STD_ANNOTATION_INDEX, tokType);            
