...
import org.apache.uima.impl.util.IntVector;
...
public class IteratorTest extends TestCase {
    ...
    public void setUp() {
        ...
        this.tokenTypeFeat = this.ts
                .getFeatureByFullName(CASTestSetup.TOKEN_TYPE_FEAT_Q);
        assertTrue(this.tokenTypeFeat != null);
        this.lemmaFeat = this.ts
                .getFeatureByFullName(CASTestSetup.LEMMA_FEAT_Q);
        assertTrue(this.lemmaFeat != null);
        this.sentLenFeat = this.ts
                .getFeatureByFullName(CASTestSetup.SENT_LEN_FEAT_Q);
        assertTrue(this.sentLenFeat != null);
        this.tokenFloatFeat = this.ts
                .getFeatureByFullName(CASTestSetup.TOKEN_FLOAT_FEAT_Q);
        assertTrue(this.tokenFloatFeat != null);
        this.startFeature = this.ts
    ...
    public void testIterator() {
        for (int i = 0; i < 10; i++) {
            this.cas.getIndexRepository().addFS(
                    this.cas.createAnnotation(this.annotationType, i * 2,
                            (i * 2) + 1));
            this.cas.getIndexRepository().addFS(
                    this.cas.createAnnotation(this.sentenceType, i * 2,
                            (i * 2) + 1));
            this.cas.getIndexRepository().addFS(
                    this.cas.createAnnotation(this.tokenType, i * 2,
                            (i * 2) + 1));
            this.cas.getIndexRepository().addFS(
                    this.cas.createAnnotation(this.tokenType, i * 2,
                            (i * 2) + 1));
        ...
        FSIndex bagIndex = this.cas.getIndexRepository().getIndex(
                CASTestSetup.ANNOT_BAG_INDEX);
        FSIndex setIndex = this.cas.getIndexRepository().getIndex(
                CASTestSetup.ANNOT_SET_INDEX);
        FSIndex sortedIndex = this.cas.getIndexRepository().getIndex(
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            it.moveToFirst();
            // an = (AnnotationFS) it.get();
            // System.out.println(
            // "Annotation "
            // + an.hashCode()
            // + " of type "
            // + an.getType().getShortName()
            // + " from "
            // + an.getBegin()
            // + " to "
            // + an.getEnd());
            it.moveTo((FeatureStructure) list.get(i));
            // an = (AnnotationFS) list.get(i);
            // System.out.println(
            // "Annotation "
            // + an.hashCode()
            // + " of type "
            ...
            // + " to "
            // + an.getEnd());
            assertTrue(((AnnotationFS) it.get()).getBegin() == ((AnnotationFS) list
                    .get(i)).getBegin());
            assertTrue(((AnnotationFS) it.get()).getEnd() == ((AnnotationFS) list
                    .get(i)).getEnd());
        }
