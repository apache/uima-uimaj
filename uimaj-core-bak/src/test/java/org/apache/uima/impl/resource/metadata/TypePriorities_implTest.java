...
public class TypePriorities_implTest extends TestCase
{
  ...
  public void testBuildFromXmlElement() throws Exception
  {
  	try
  	{
  		//simple type priorties (backwards compatibility check)
			...
			TypePriorities pri =
			  UIMAFramework.getXMLParser().parseTypePriorities(new XMLInputSource(descriptor));
  		...
  		descriptor = JUnitExtension.getFile("TypePrioritiesImplTest/TestTypePriorities.xml");
			...
			assertEquals("TypePrioritiesImportedByLocation.xml", 
			             imports[0].getLocation());
  ...
  public void testResolveImports() throws Exception
  {
      ...
      assertNotNull(ex);
      assertEquals(1, pri.getPriorityLists().length); //should be no side effects when exception is thrown

      //set data path correctly and it should work
      ResourceManager resMgr = UIMAFramework.newDefaultResourceManager();
      resMgr.setDataPath(JUnitExtension.getFile("TypePrioritiesImplTest/dataPathDir").getAbsolutePath()); 
  ...
  public void testClone() throws Exception
  {
		...
		{
		  File descriptor = JUnitExtension.getFile("TypePrioritiesImplTest/TestTypePriorities.xml");    
	    TypePriorities pri = UIMAFramework.getXMLParser().parseTypePriorities(new XMLInputSource(descriptor));
