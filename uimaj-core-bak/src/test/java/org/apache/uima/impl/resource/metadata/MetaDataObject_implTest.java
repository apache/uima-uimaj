...
package org.apache.uima.impl.resource.metadata;
...
public class MetaDataObject_implTest extends TestCase
{
  ...
  protected void setUp() throws Exception
  {
    ...
    apple1.setAttributeValue("commonUses",
      new String[]{"baking","snack"});
    ...
    apple2.setAttributeValue("commonUses",
      new String[]{"baking","snack"});
    ...
    orange.setAttributeValue("commonUses",
      new String[]{"snack","juice"});
  ...
  * {@link MetaDataObject#buildFromXMLElement(Element,XMLParser)} methods.
  * These also sufficiently excercise the
  * {@link MetaDataObject#getAttributeValue(String)} and
  ...
  */
  public void testXMLization() throws Exception
  {
    try
    {
      ...
      Document apple1xmlDoc = docBuilder.parse(
        new ByteArrayInputStream(apple1xml.getBytes()));
      Document apple2xmlDoc = docBuilder.parse(
        new ByteArrayInputStream(apple2xml.getBytes()));
      Document orangeXmlDoc = docBuilder.parse(
        new ByteArrayInputStream(orangeXml.getBytes()));
      Document fruitBagXmlDoc = docBuilder.parse(
        new ByteArrayInputStream(fruitBagXml.getBytes()));
          
      //construct new objects from the XML
      XMLParser xmlp = UIMAFramework.getXMLParser();
      MetaDataObject_impl newApple1 = (MetaDataObject_impl)
        unknownFruit.clone();
      newApple1.buildFromXMLElement(apple1xmlDoc.getDocumentElement(),xmlp);  
      MetaDataObject_impl newApple2 = (MetaDataObject_impl)
        unknownFruit.clone();
      newApple2.buildFromXMLElement(apple2xmlDoc.getDocumentElement(),xmlp);
      MetaDataObject_impl newOrange = (MetaDataObject_impl)
        unknownFruit.clone();
      ...
      Document xmlDoc = docBuilder.parse(
