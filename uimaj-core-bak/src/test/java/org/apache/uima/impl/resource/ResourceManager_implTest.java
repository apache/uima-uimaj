...
package org.apache.uima.impl.resource;
...
import org.apache.uima.impl.resource.metadata.ExternalResourceBinding_impl;
import org.apache.uima.impl.resource.metadata.ResourceManagerConfiguration_impl;
...
{
  private final File TEST_DATA_FILE = JUnitExtension.getFile("ResourceTest/ResourceManager_implTest_tempDataFile.dat");

  
  private final String TEST_DATAPATH_WITH_SPACES = JUnitExtension.getFile("ResourceTest/spaces in dir name").toString(); 
  
  private final String TEST_FILE_IN_DATAPATH = "file:Test.dat"; 

  private final String TEST_STRING = "This is a test.  This is only a test."; //contents of test data file
   ...
   * @param arg0
   */
  public ResourceManager_implTest(String arg0)
	throws IOException
  {
	super(arg0);
  }
  ...
  protected void setUp() throws Exception
  {
	try
	{
      ...
      mManager.setDataPath(TEST_DATAPATH_WITH_SPACES);
			...
			langSpec.setFileUrlPrefix(JUnitExtension.getFile("ResourceTest/FileLanguageResource_implTest_data_").toURL().toString());
			...
			cfg.setExternalResources(new ExternalResourceDescription[] {desc, desc2, desc3, desc4, desc5});
			...
			cfg.setExternalResourceBindings(new ExternalResourceBinding[] {binding1, binding2, binding3, binding4, binding5});
	...
	throws Exception
  {
	...
	{   
	  ...
	  TestResourceInterface r2 = 
		(TestResourceInterface)mManager.getResource(TEST_CONTEXT_NAME + "myCustomObjectKey");
	  Assert.assertEquals(TEST_STRING, r2.readString());
      
	  DataResource en_r = (DataResource)mManager.getResource(TEST_CONTEXT_NAME + "myLanguageResourceKey",
		new String[]{"en"});
	  Assert.assertTrue(en_r.getUrl().toString().endsWith(
	   "FileLanguageResource_implTest_data_en.dat"));
  
	  DataResource de_r = (DataResource)mManager.getResource(TEST_CONTEXT_NAME + "myLanguageResourceKey",
		new String[]{"de"});
	  ...
	  DataResource enus_r = (DataResource)mManager.getResource(TEST_CONTEXT_NAME + "myLanguageResourceKey",
		new String[]{"en-US"});
	  Assert.assertTrue(en_r == enus_r);      
  
	  TestResourceInterface en_obj = (TestResourceInterface)
		mManager.getResource(TEST_CONTEXT_NAME + "myLanguageResourceObjectKey", new String[]{"en"});
	  Assert.assertEquals("English", en_obj.readString());
      
      //test spaces in datapath
      DataResource r3 = (DataResource)mManager.getResource(TEST_CONTEXT_NAME + "myResourceWithSpaceInPathKey");
  ...
  public void testResolveAndValidateDependencies() throws Exception
  {
	try
	{
	  ...
	  dep6.setKey("org/apache/uima/impl/resource/ResourceInClasspath.txt");
      
      
	  mManager.resolveAndValidateResourceDependencies(
		new ExternalResourceDependency[] {dep1, dep2, dep3, dep4, dep5, dep6}, TEST_CONTEXT_NAME);
