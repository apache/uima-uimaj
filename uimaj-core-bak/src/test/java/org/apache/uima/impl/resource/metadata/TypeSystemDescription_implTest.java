...
public class TypeSystemDescription_implTest extends TestCase
{
  	...
  	try
  	{
  		File descriptor = JUnitExtension.getFile("TypeSystemDescriptionImplTest/TestTypeSystem.xml");
			TypeSystemDescription ts = 
			  UIMAFramework.getXMLParser().parseTypeSystemDescription(new XMLInputSource(descriptor));
			...
			assertEquals("org.apache.uima.impl.resource.metadata.TypeSystemImportedByName", 
			             imports[0].getName());
  ...
  public void testResolveImports() throws Exception
  {
      ...
      resMgr.setDataPath(JUnitExtension.getFile("TypeSystemDescriptionImplTest/dataPathDir").getAbsolutePath()); 
      ts.resolveImports(resMgr); 
			...
			descriptor = JUnitExtension.getFile("TypeSystemDescriptionImplTest/TypeSystemImportedByLocation.xml");
			ts = UIMAFramework.getXMLParser().parseTypeSystemDescription(new XMLInputSource(descriptor));		
			assertEquals(2, ts.getTypes().length);
			ts.resolveImports();
			assertEquals(2, ts.getTypes().length);
			...
			TypeSystemDescription typeSystemWithResolvedImports = (TypeSystemDescription) typeSystemDescription.clone();
  ...
  public void testInvalidTypeSystem() throws Exception
  {
    ...
    TypeSystemDescription tsDesc =  UIMAFramework.getXMLParser().parseTypeSystemDescription(new XMLInputSource(file));
    try
    {
