...
//	private boolean isRunning = false;
	...
	/**
	 * Initialize the checkpoint with a reference to controlling cpe, the file where the
	 * checkpoint is to be stored, and the frequency of checkpoints.
	 * @param aCpm
	 * @param aFilename
...
//		isRunning = true;
		while (!stop)
		{
			...
			synchronized (sleepMonitor)
			{
				try
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_sleep__FINEST",
						        new Object[] {Thread.currentThread().getName()});
					}
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_wakeup__FINEST",
						        new Object[] {Thread.currentThread().getName()});
					}
				}
				catch (Exception e)
			...
			try
			{
				...
				if (pTrace != null)
				{
					if (synchPoint != null)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkpoint_with_synchpoint__FINEST",
							        new Object[] {Thread.currentThread().getName()});
						}
					...
					else
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkpoint_with_pt__FINEST",
							        new Object[] {Thread.currentThread().getName()});
						}
	 ...
	 * 
	 * @param aFilename - checkpoint file to rename
	 */
	 ...
	 * 
	 * @param lst List of ProcessEvent
	 * @param tCnt depth of this List in the Process Trace hierarchy
	 **/
		...
		try
		{
			...
			if (file.exists())
			{
				...
				if (stream != null)
				{
					...
					{
						ProcessTrace processTrace = (ProcessTrace) ((CheckpointData) anObject).getProcessTrace();
						printStats((ProcessTrace) processTrace);
					}
				}
			}
			...
			if (file.exists())
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_synchpoint_from_file__FINEST",
