 ...
 * Cas'es are checked out, used and checked back in when done.
 * 
 * 
 *
 * 
...
public class CPECasPool
{
	 ...
	 * Initialize the pool.
	 * 
	 * @param aNumInstances - max size of the pool
	 * @param aList - aggregate type system
	 * @param aResourceManager - resource manager, needed for JCAS class loading
	 * @throws ResourceInitializationException
	 */
	public CPECasPool(int aNumInstances, List aList,
	                  ResourceManager aResourceManager) throws ResourceInitializationException
	{
		mNumInstances = aNumInstances;
		fillPool(aList, UIMAFramework.getDefaultPerformanceTuningProperties(),
		         aResourceManager);
	}
	/**
	 * Initialize the pool
	   ...
	   throws ResourceInitializationException
	{
		...
		{
			CAS c = CasCreationUtils.createCas(aList, c0.getTypeSystem(), aPerformanceTuningSettings, aResourceManager);
			mAllInstances.add(c);
			mFreeInstances.add(c);
		}
	}
	/**
	 * Returns a Cas instance from the pool. This routine waits for a free instance of Cas
	 * a given amount of time. If free instance is not available this routine returns null.
	 * 
	 * @param aTimeout - max amount of time in millis to wait for CAS instance
			...
			if ( cas != null )
			{
				...
				if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_add_cas_to_checkedout_list__FINEST",
					        new Object[] {Thread.currentThread().getName(), String.valueOf(checkedOutInstances.size())});
	...
	public synchronized void releaseCas(CAS aCas)
	{
		...
		if (!mAllInstances.contains(aCas) || mFreeInstances.contains(aCas))
		{
			if ( UIMAFramework.getLogger().isLoggable(Level.WARNING) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_invalid_checkin__WARNING",
		...
		else
		{
			...
			if ( index != -1)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_removed_from_checkedout_list__FINEST",
					        new Object[] {Thread.currentThread().getName(), String.valueOf(checkedOutInstances.size())});
				}		
			}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_return_cas_to_pool__FINEST",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(checkedOutInstances.size())});
			}
		}
	 ...
	 * 
	 * @param aIndex - position of the CAS in the list
	 * 
