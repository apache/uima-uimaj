 ...
 * means of APIs. In both cases, the java-based CasProcessor to be launched by the CPE 
 * must be properly setup for launching. Its environment must be setup, command line,
 * and any jvm arguments.
 * 
 * 
 */
public class JavaApplication extends RunnableApplication
{
	 ...
	 * 
	 * @param aCasProcessorConfiguration - configuration for CasProcessor
	...
	{
		addApplicationInfo(aCasProcessorConfiguration, aJaxbCasProcessorConfig); //aJaxbCasProcessorConfig, aDescriptor,aDeploymentParameters );			
	}
	/**
	 ...
	 * @param argList - list of arguments set up in the CPE descriptor
	 * @param aExecutable - executable program
	 * 
	...
	protected String[] addApplicationCmdLineArguments(CasProcessorConfiguration aCasProcessorConfiguration, List argList, String aExecutable) //String[] cmdLine) //, String aDescriptor,List aDeploymentParameters )
	{
		...
		//	This is only used for local deployment so hardcode VNS information used later to launch the VNS.
		//	The actual port used here is a default port, just before launching the process this port may
		//	be changed to a different value since at this point we dont know what port the VNS will be using.
		//	The VNS starts up on port 9005 BUT if this port is not available the VNS will try to acquire a 
		//	different one.   
		...
		for (int i = 0; i < argList.size(); i++)
		{
			...
			if (argValue == null || argValue.trim().length() == 0 || argValue.startsWith("-DVNS_HOST") || argValue.startsWith("-DVNS_PORT"))
			{
				continue; // skip
			}
			...
			//	Special case: if arg=-cp <classpath> we need to split this string into 2 due to the way Java Runtime 
			//	object requirement that the command line params are seperate elements in the cmd array.
			if (argValue.trim().equalsIgnoreCase("-cp") && argValue.trim().length() > 3) // check if the -cp contains the classpath string
			{
				...
				//	Overwrite location where arg was found.Split -cp and its value (classpath) into two. First add the cp than add the classpath
				argList.add(i, "-cp");
				cmdArgs.add(arg);
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cmd_classpath__FINEST",
