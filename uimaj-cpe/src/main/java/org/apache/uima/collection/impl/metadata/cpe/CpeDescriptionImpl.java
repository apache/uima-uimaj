...
public class CpeDescriptionImpl extends MetaDataObject_impl implements CpeDescription
{
	...
	public CpeDescriptionImpl() {super();};
	
	/**
	 * This is needed for XMLParser.parseCpeDesription() to work.  Typically users should
	 * use CpeDescriptorFactory.produceDescriptor() instead. - APL
	 */
	public CpeDescriptionImpl(XMLInputSource aInput) throws InvalidXMLException
	{
		try
		{
			CpeDescription descriptor =
				CpeDescriptorFactory.produceDescriptor(aInput);
   ...
   */
	public CpeCollectionReaderCasInitializer addCasInitializer( String aInitializerDescriptorPath ) throws CpeDescriptorException
	{
		...
		{
			collectionReader.setCasInitializer( CpeDescriptorFactory.produceCollectionReaderCasInitializer(aInitializerDescriptorPath, this));
		}
		else
		{
			collectionReader.getCasInitializer().getDescriptor().getInclude().set(aInitializerDescriptorPath);
		}
  ...
  /* (non-Javadoc)
   * @see org.apache.uima.resource.metadata.impl.MetaDataObject_impl#readUnknownPropertyValueFromXMLElement(org.w3c.dom.Element, org.apache.uima.util.XMLParser, org.apache.uima.util.XMLParser.ParsingOptions, java.util.List)
   */
  ...
  static final private XmlizationInfo XMLIZATION_INFO =
	new XmlizationInfo("cpeDescription",
	  new PropertyXmlInfo[]{
