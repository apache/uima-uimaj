...
public class CasProcessorDeploymentParamsImpl extends MetaDataObject_impl implements CasProcessorDeploymentParams
{
	...
	public void buildFromXMLElement(Element aElement, XMLParser aParser, ParsingOptions aOptions) throws InvalidXMLException
	{
		...
		for (int i = 0; i < nodes.getLength(); i++)
		{
			...
			if (node instanceof Element)
			{
				...
				CasProcessorDeploymentParam p = new CasProcessorDeploymentParamImpl(paramName,paramValue, paramType);
				params.add(p);
			}
		}
	}
	public void toXML(ContentHandler aContentHandler, boolean aWriteDefaultNamespaceAttribute) throws SAXException
	{
		...
		{
			( (CasProcessorDeploymentParam)params.get(i)).toXML(aContentHandler, aWriteDefaultNamespaceAttribute);
		}
	...
	static final private XmlizationInfo XMLIZATION_INFO = 
		new XmlizationInfo("deploymentParameters", 
