...
public class CpeSofaMappingsImpl extends MetaDataObject_impl implements CpeSofaMappings
{
	...
	public void buildFromXMLElement(Element aElement, XMLParser aParser, ParsingOptions aOptions) throws InvalidXMLException
	{
		...
		for (int i = 0; i < nodes.getLength(); i++)
		{
			...
			if (node instanceof Element)
			{
				...
				if ( atts.getNamedItem( "componentSofaName") != null && atts.getNamedItem( "componentSofaName").getNodeValue() != null )
				{
					sofaMapping.setComponentSofaName(atts.getNamedItem( "componentSofaName").getNodeValue());
				}
				if ( atts.getNamedItem( "cpeSofaName") != null && atts.getNamedItem( "cpeSofaName").getNodeValue() != null)
				{
	...
	public void toXML(ContentHandler aContentHandler, boolean aWriteDefaultNamespaceAttribute) throws SAXException
	{
		...
		{
			( (CpeSofaMapping)sofaNameMappings.get(i)).toXML(aContentHandler, aWriteDefaultNamespaceAttribute);
		}
	...
	new XmlizationInfo("sofaNameMappings",
	new PropertyXmlInfo[]{
		new PropertyXmlInfo("sofaNameMapping",null),
	});
