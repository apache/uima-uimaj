...
//import org.apache.uima.impl.collection.metadata.casProcessor;
...
public class CPEDeployerDefaultImpl implements CasProcessorDeployer
{
	 ...
	 * 
	 * @param aCpeFactory - reference to CPE configuration
	 */
	...
	public ProcessingContainer deployCasProcessor(List aCasProcessorList, boolean redeploy) throws ResourceConfigurationException
	{
		...
		try
		{
			for (int i = 0; i < aCasProcessorList.size(); i++)
			{
				...
				if (processingContainer == null)
				{
					...
					CpeCasProcessor cpeCasProcessor = (CpeCasProcessor) cpeFactory.casProcessorConfigMap.get(metaData.getName());
					if ( engine != null )
					{
					...
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_checkpoint__FINEST",
						        new Object[] {Thread.currentThread().getName(), String.valueOf(casProcessorConfig.getBatchSize())});//Checkpoint().getBatch())});				
					}
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_deployment__FINEST",
					...
					if (name == null)
					{
						...
						throw new ResourceConfigurationException(InvalidXMLException.REQUIRED_ATTRIBUTE_MISSING, new Object[] { "name", "casProcessor" }, 
								new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_missing_attribute_from_xml_element__WARNING",new Object[] {Thread.currentThread().getName(),"n/a","name","casProcessor"}) ));
					}
				}
				else
				{
					...
					if (casProcessorConfig == null)
					{
						throw new ResourceConfigurationException(InvalidXMLException.ELEMENT_NOT_FOUND, new Object[] { "<casProcessor>", "<casProcessors>" }, 
								new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_bad_cpe_descriptor_no_cp__WARNING",new Object[] {Thread.currentThread().getName()}) ));
					}
				}
		...
		{
			e.printStackTrace();
			throw new ResourceConfigurationException(ResourceServiceException.RESOURCE_UNAVAILABLE, new Object[] {
			}, e);
		}
	 ...
	 * This routine is called when the CasProcessor fails and needs to be restarted.
	 * 
	 * @param aProcessingContainer - container managing Cas Processor
	 */
	public void deployCasProcessor(ProcessingContainer aProcessingContainer) throws ResourceConfigurationException
	{
		try
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_redeploying_cp__FINEST",
				        new Object[] {Thread.currentThread().getName(), name});				
			}
	 ...
	 * 
	 * @param aDescriptor - Cas Processor descriptor
	 * @return - instantiated CasProcessor
	 * 
	 * @throws ResourceConfigurationException
	 */
	private CasProcessor produceIntegratedCasProcessor(String aDescriptor) throws ResourceConfigurationException
	{
		CasProcessor casProcessor = null;
		try
		{
			if (aDescriptor != null)
			{
				...
				else if (resourceSpecifier instanceof CasConsumerDescription)
				{
					if (cpeFactory.isDefinitionInstanceOf(CasConsumer.class, resourceSpecifier, aDescriptor))
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_producing_cas_consumer__FINEST",
							        new Object[] {Thread.currentThread().getName()});				
						}
						casProcessor = UIMAFramework.produceCasConsumer(resourceSpecifier, this.cpeFactory.getResourceManager(), null);
					}
					else if (cpeFactory.isDefinitionInstanceOf(CasProcessor.class, resourceSpecifier, aDescriptor))
					{
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						...
						casProcessor = cpeFactory.produceCasDataConsumer(CasProcessor.class, resourceSpecifier, null);
					}
				}
				...
				if (casProcessor == null)
				{
					throw new ResourceConfigurationException(ResourceServiceException.RESOURCE_UNAVAILABLE, new Object[] {
							...
							"UIMA_CPM_EXP_instantiation_exception__WARNING",new Object[] {Thread.currentThread().getName(),"Integrated Cas Processor"}) ));
				}
			}
		}
		catch (ResourceConfigurationException e)
