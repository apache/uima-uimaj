...
import org.apache.uima.collection.base_cpm.container.CasProcessorController;
...
import org.apache.uima.collection.base_cpm.container.RunnableContainer;
...
import org.apache.uima.impl.collection.CasConsumerDescription_impl;
...
import org.apache.uima.impl.collection.cpm.utils.Filter;
import org.apache.uima.impl.collection.cpm.vinci.DATACasUtils;
import org.apache.uima.impl.resource.metadata.ConfigurationParameterSettings_impl;
...
import org.apache.uima.impl.util.StringUtils;

/**
 * Manages a pool of CasProcessor instances. Provides access to CasProcessor instance
 * to Processing Thread. Processing threads check out an instance of Cas Processor and
 * when done invoking its process() method return it back to pool. The container
 * aggregates counts and totals on behalf of all instances of Cas Processor. It also
 * manages error and restart thresholds for Cas Processors as a group. Errors are 
 * aggregated for all instances of Cas Processor as a group NOT individually. The container
 * takes appropriate actions when threshold are exceeded. What action is taken depends
...
//	private casProcessor casProcessorConfiguration = null;
  ...
  /**
	 ...
	 * 
	 * @param aCasProcessorConfig - CasProcessor configuration as defined in cpe descriptor
	 * @param aCasProcessorPool - pool of CasProcessor instances
	 */
	 ...
	 * 
	 * @param Capability - component capabilities
	 */
		...
		for (int j = 0; capabilities != null && j < capabilities.length; j++)
		{
			...
			if (tORf != null)
			{
				...
				for (int i = 0; i < tORf.length; i++)
				{
					...
					if (tORf[i].getName().indexOf(org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM) > -1)
					{
						newKey = StringUtils.replaceAll(tORf[i].getName(), org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM, org.apache.uima.impl.collection.cpm.Constants.LONG_DASH_TERM);
						modified = true;
					}
					if (tORf[i].getName().indexOf(org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM) > -1)
					{
						newKey = StringUtils.replaceAll(tORf[i].getName(), org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM, org.apache.uima.impl.collection.cpm.Constants.LONG_COLON_TERM);
						modified = true;
					}
					if (modified)
					{
	 ...
	 * 
	 * @param aDeployer - object responsible for deploying/launching CasProcessor
	 */
	...
	/** 
	 * Deploy Container specific logger to capture any exceptions occuring during CasProcessor(s) lifespan.   
	 * Each CasProcessor may have its own log. It is optional and available when there is a parameter
	 * called 'containerLogPath' in the <deploymentParameters> section of the cpe descriptor. If 
	 */
	private void deployLogger()
	{
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploy_logger__FINEST",
			        new Object[] {Thread.currentThread().getName(), getName()});
		}
		logPath = (String) casProcessorCPEConfiguration.getDeploymentParameter("containerLogPath");
		...
		else
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_logpath_not_defined__FINEST",
				        new Object[] {Thread.currentThread().getName(), getName()});
			}
		}
	 ...
	 * 
	 * @param abortedCasList - an arrar of Cas'es that could not be processed by this CasProcessor
	 * 
	 */
	public void logAbortedCases(Object[] abortedCasList)
	{
		...
		if (logger != null)
		{
			...
			for (int i = 0; i < abortedCasList.length; i++)
			{
				...
				{
					String value = DATACasUtils.getFeatureValueByType((CasData) abortedCasList[i], org.apache.uima.impl.collection.cpm.Constants.METADATA_KEY, org.apache.uima.impl.collection.cpm.Constants.DOC_ID);
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_log_docid__FINEST",
	...
	/**
	 * Resets batch counter. This counter is used to determine if the hosted CasProcessor should
	 * do special processing. A CasProcessor may buffer all Cas's in memory and only when its 
	 * batch size is reached, it does something usefull with them, like save them to a file, index
	 * them, etc  
	 *
	 */
	...
	/**
	 * Returns total number of bytes ingested so far by all CasProcessor instances managed by
	 * this container.
	 * 
	...
	/**
	 * Returns total number of bytes processed so far by all CasProcessor instances managed by
	 * this container.
	 * 
	 * @return - bytes processed
	 */
	...
	/**
	 * Increments number of CAS'es filtered by the CasProcessor. Filtered CAS'es dont
	 * contain required features. Features that are required by the Cas Processor to
	 * perform analysis. Dependant feateurs are defined in the filter expression in
	 * the CPE descriptor
	 * 
	 * @param aCount - number of filtered Cas'es
	 */
	...
	/**
	 * Returns number of entities still to be processed by the CasProcessor
	 * It is a delta of total number of entities to be processed by the CPE minus
	 ...
	 * 
	 * @param aRemainingCount - number of entities to process
	 */
	...
	/**
	 * Used when recovering from checkpoint, sets the total number of
	 * entities before CPE stopped. 
	 * 
	 ...
	 * 
	 * @param aTime - total time in process()
	 */
	...
	/**
	 * Returns true if maximum threshold for errors has been exceeded and the CasProcessor
	 * is configured to force CPE shutdown. It looks at the value of the action attribute
	 * of the <errorRateThreshold> element in the cpe descriptor.
	 * 
	 ...
	 * @param aThrowable - Exception to check for SocketTimeoutException
	 ...
	 * It interprets given exception and throws a new one according to configuration specified in
	 * the CPE descriptor. It examines provided thresholds and determines if the CPE should continue to
	 ...
	 * 
	 * @exception  AbortCPMException - force the CPE to stop processing
	 * @exception  AbortCasProcessorException - disables all instances of CasProcessor in this container
	 * @exception  ServiceConnectionException - forces the restart/relauch of the failed CasProcessor
	 * @exception  SkipCasException - disregard error, skip bad Cas'es and continue with the next Cas bundle
	 * 
	 */
	public synchronized void incrementCasProcessorErrors(Throwable aThrowable) throws Exception
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_increment_cp_errors__FINEST",
			        new Object[] {Thread.currentThread().getName(), getName()});
		}
		if (System.getProperty("DEBUG_EXCEPTIONS") != null)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cp_error_count__FINEST",
				        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(errorCounter), aThrowable.getCause().getMessage()});
			}
		...
		if (aThrowable instanceof ResourceProcessException)
		{
			...
			//	If the errors dont exceed configurable threshold already try to ServiceConnection exception. 
			else if ( (errorCounter < configuredErrorRate) &&  aThrowable.getCause() instanceof ServiceConnectionException && !isTimeout(aThrowable))
			{
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_connection_exception__SEVERE",
					        new Object[] {Thread.currentThread().getName(), getName()});
				}
				...
				if (restartCount > rC)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_max_restart_reached__FINEST",
						        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(rC)});
					}
					...
					if (Constants.TERMINATE_CPE.equals(actionOnMaxRestarts))
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_terminate_due_to_action__SEVERE",
							        new Object[] {Thread.currentThread().getName(), getName()});
						}
						throw new AbortCPMException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_EXP_configured_to_abort__WARNING",new Object[] {Thread.currentThread().getName(),getName()}) );
					}
					else if (Constants.DISABLE_CASPROCESSOR.equals(actionOnMaxRestarts))
					{
						...
						throw new AbortCasProcessorException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_EXP_configured_to_disable__WARNING",new Object[] {Thread.currentThread().getName(),getName()}) );
					}
					else if (Constants.KILL_PROCESSING_PIPELINE.equals(actionOnMaxRestarts))
					{
						...
						throw new KillPipelineException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_EXP_configured_to_kill_pipeline__WARNING",new Object[] {Thread.currentThread().getName(),getName()}) );
					}
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_force_reconnect__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName()});
				}
			...
			if (aThrowable.getCause() != null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_other_exception__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName(), aThrowable.getCause().getClass().getName()});
				}
			}
		...
		{
			//	Not ResourceException, so this most likely is an exception from the integrated CasProcessor. This is
			//	subject to error thresholds defined in the Cpe descriptor. CPE doesnt restart integrated CasProcessor
			//	since it is difficult to know what the nature of the failure is.
			if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_no_resource_exception__FINEST",
		...
		if (errorCounter > configuredErrorRate)
		{
			if (abortCPMOnError())
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_abort_exceeded_error_threshold__SEVERE",
					        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(configuredErrorRate)});
				}
			...
			if (isAbortable())
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_disable_exceeded_error_threshold__SEVERE",
					        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(configuredErrorRate)});
				}
				throw new AbortCasProcessorException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
						"UIMA_CPM_EXP_configured_to_disable__WARNING",new Object[] {Thread.currentThread().getName(),getName()}) );
	...
	/*
	 * Called after each entity set is processed and its purpose is to mark end of batch associated with this
	 * Container. At the end of batch a CasProcessor may perform a specific processing like writing to a store
	 * or do indexing.
	 * 
	 * @see org.apache.uima.collection.base_cpm.container.ProcessingContainer#isEndOfBatch(org.apache.uima.collection.base_cpm.CasProcessor, int)
	 */
	public synchronized boolean isEndOfBatch(CasProcessor aCasProcessor, int aProcessedSize) throws ResourceProcessException, IOException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cp_batch_size__FINEST",
			        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(batchSize)});
		}
		boolean eob = false;
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cp_doc_count__FINEST",
			        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(processed)});
		}
		...
		if (batchCounter > batchSize || batchCounter % batchSize == 0)
		{
			if (aCasProcessor == null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_is_null__SEVERE",
					        new Object[] {Thread.currentThread().getName(), getName()});
				}
				throw new ResourceProcessException(
						new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_EXP_invalid_reference__WARNING",new Object[] {Thread.currentThread().getName(),"NULL"}) ));
			}
      ...
      //ProcessTrace object not really needed any more except for backwards compatibility
			...
			//	This was used before to keep track of what was the last document id. This is no longer needed i think.
			if (!processedEntityIds.isEmpty())
			{
	 ...
	 * This routine checks for existance of dependent featues defined in the 
	 * filter expression defined for the CasProcessor in the cpe descriptor.
	 * Currently this is done on per bundle basis. Meaning that all Cas'es must
	 * contain required features. If even one Cas does not have them, the entire
	 * bundle is skipped. 
	 * 
	 * @param aCasList - bundle containing instances of CAS
	 */
	public boolean processCas(Object[] aCasList)
	{
		...
		{
			if (processCas((CasData) aCasList[i]))
			{
				doProcess = true;
				break;
			}
	...
	/**
	 * Used during filtering, determines if a given Cas has a required feature.
	 * Required featured are defined in the filter. Filtering is optional and
	 * if not present in the cpe descriptor this routine always returns true.
	 * 
	 * @param aCas - Cas instance to check 
	 * 
	...
	private boolean hasFeature(CasData aCas)
	{
		if (System.getProperty("DEBUG_FILTER") != null)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_check_filter__FINEST",
				        new Object[] {Thread.currentThread().getName(), getName()});
			}
			if (filterList == null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_filtering_disabled__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName()});
				}
			}
			else if (filterList.size() == 0)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_filter_empty__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName()});
				}
			}
		}
		for (int i = 0; filterList != null && i < filterList.size(); i++)
		{
			if (System.getProperty("DEBUG_FILTER") != null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_filter_enabled__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName()});
				}
			}
			...
			if (filterExpression.getRightPart() == null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_filter_enabled_no_right_operand__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName()});
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_filter_enabled_left_part__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName(),filterExpression.getLeftPart().get(), String.valueOf(exists )});
				}

				if ((filterExpression.getOperand() == null && filterExpression.getOperand().getOperand() == null && !exists) || // this means that the feature must exist in CAS
				 ("!".equals(filterExpression.getOperand().getOperand()) && exists)) // this means that the feature must not be present in CAS
				{
					if (System.getProperty("DEBUG_FILTER") != null)
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_filter_false__FINEST",
						        new Object[] {Thread.currentThread().getName(), getName()});
					}
					return false;
				}
	...
	/**
	 * Returns CasProcessor configuration object. This object represents xml configuration
	 * defined in the <casProcessor> section of the cpe descriptor.
	 * 
	...
	/**
	 * Returns available instance of the CasProcessor from the instance pool. It will wait indefinitely 
	 * until an instance is available.
	 */
	public CasProcessor getCasProcessor()
	{
		...
		while (isPaused())
		{
			...
			if (!isPaused)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_resuming_container__FINEST",
					        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(CONTAINER_SLEEP_TIME)});
				}
			}
		}
		...
		try
		{
			do
			{
				...
				if (processor == null)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_wait_no_processor__FINEST",
						        new Object[] {Thread.currentThread().getName(), getName()});
					}
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_failed_waiting_for_processor__SEVERE",
	 ...
	 * @see org.apache.uima.collection.base_cpm.container.ProcessingContainer#releaseCasProcessor(org.apache.uima.collection.base_cpm.CasProcessor)
	 *  
	 * @param aCasProcessor - an instance of CasProcessor to return back to the pool
	 */
	...
	{
		String action = casProcessorCPEConfiguration.getActionOnError(); //(String) getConfigParameterValue(Constants.ACTION_ON_MAX_ERRORS);
	 ...
	 * Abortable CasProcessor's action attribute in the <errorRateThreshold> element
	 * has a value of 'disable'.
	 * 
	...
	public boolean isAbortable()
	{
		...
		try
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cp_action_on_error__FINEST",
				        new Object[] {Thread.currentThread().getName(), getName(), casProcessorCPEConfiguration.getActionOnError()});
			}
			if (anAction == null)
			{
				if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_action_on_error_not_defined__SEVERE",
					        new Object[] {Thread.currentThread().getName(), getName()});
				}
			}
		}
	 ...
	 * Before destroying the instance, this method notifies it
	 * with CollectionProcessComplete so that the component finalizes
	 * its logic and does appropriate cleanup before shutdown.
	 * 
	 */
	public void destroy()
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cp_proxy_pool_size__FINEST",
			        new Object[] {Thread.currentThread().getName(), getName(), String.valueOf(casProcessorPool.getSize())});
		}
		try
		{
			synchronized (casProcessorPool)
			{
				while (casProcessorPool.getSize() > 0)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_destroy_processor__FINEST",
						        new Object[] {Thread.currentThread().getName(), getName()});
					}
	...
	public String getName()
	{
		...
		{
			if (metadata != null && Constants.DEPLOYMENT_INTEGRATED.equalsIgnoreCase(casProcessorCPEConfiguration.getDeploymentType()))
			{
	...
	/**
	 * Pauses the container until resumed. The CPM will pause to the Container while
	 * it is trying to re-connect to a shared remote service. While the Container is
	 * paused getCasProcessor() will not be allowed to return a new CasProcessor. All
	 * other methods are accessible and will function fine.
	 */
	public void pause()
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pause_container__FINEST",
			        new Object[] {Thread.currentThread().getName(), getName()});
		}
		isPaused = true;
	}
