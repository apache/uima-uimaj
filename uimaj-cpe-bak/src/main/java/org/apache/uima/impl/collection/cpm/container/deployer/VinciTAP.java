...
import org.apache.uima.impl.cas_data.CasDataImpl;
import org.apache.uima.impl.cas_data.FeatureStructureImpl;
import org.apache.uima.impl.cas_data.PrimitiveValueImpl;
import org.apache.uima.impl.cas_data.vinci.VinciCasDataConverter;
...
import org.apache.uima.impl.collection.cpm.vinci.Vinci;
...
/**
 *	Vinci Proxy to remote Cas Processor vinci service. This component is used
 *	for both local( managed) and remote ( unmanaged) Cas Processors. Its main
 *	purpose is to invoke remote APIs on Cas Processors running as vinci services.
 *	It serializes data contained in the Cas into XCAS and sends it to the service.
 *	It desiralizes data from XCAS returned from the service back into the Cas.
 */
...
public class VinciTAP
{
	...
	private VinciCasDataConverter vinciCasDataConverter = new VinciCasDataConverter(org.apache.uima.impl.collection.cpm.Constants.METADATA_KEY, org.apache.uima.impl.collection.cpm.Constants.DOC_ID, org.apache.uima.impl.collection.cpm.Constants.CONTENT_TAG, org.apache.uima.impl.collection.cpm.Constants.CONTENT_TAG_VALUE, contentTag, true);
	 ...
	 * 
	 * @param aContentTag - subject of analysis
	 */
	 ...
	 * 
	 * @param aKeys2Drop - array of types excluded from the request 
	 */
	...
	{
		int attemptCount = 0;
		int maxConnectRetryCount = org.apache.uima.impl.collection.cpm.Constants.CONNECT_RETRY_COUNT;
		...
		if ( System.getProperty("CONNECT_RETRY_COUNT") != null )
		{
			...
			{
				throw new ConnectException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
						"UIMA_CPM_EXP_invalid_retry_count__WARNING",new Object[] {Thread.currentThread().getName(), aHost, String.valueOf(aPort)}) );
			}
		}
		while (attemptCount++ < maxConnectRetryCount )
		{
			if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_set_service_timeout__FINEST",
			...
			try
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connect_to_service__FINEST",
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connected_to_service__FINEST",
					        new Object[] {Thread.currentThread().getName(), aHost, String.valueOf(aPort)});				
				}
				if (conn.isOpen())
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_testing_connection__FINEST",
					...
					try
					{
						VinciFrame query = query = new VinciFrame();
						...
						if ( resp.fgetString("vinci:STATUS") != null && resp.fgetString("vinci:STATUS").equals("OK"))
						{
							...
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_pid__FINEST",
				...
				else
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connection_closed__FINEST",
						        new Object[] {Thread.currentThread().getName()});				
					}
				}
				if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connection_validated__FINEST",
					        new Object[] {Thread.currentThread().getName(), aHost, String.valueOf(aPort)});				
				}
				return;
			}
				...
				if (e instanceof ConnectException)
				{
					if ( UIMAFramework.getLogger().isLoggable(Level.WARNING) )
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connection_not_established__WARNING",
						        new Object[] {Thread.currentThread().getName(), aHost, String.valueOf(aPort)});				
					}
					try
					{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connection_failed__WARNING",
			        new Object[] {Thread.currentThread().getName(), aHost, String.valueOf(aPort)});				
		}
		throw new ConnectException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
				"UIMA_CPM_EXP_unable_to_connect__WARNING",new Object[] {Thread.currentThread().getName(), aHost, String.valueOf(aPort)}) );
	}
	/**
	 * Define the max time in millis the proxy will wait for response from remote service
	 * 
	 * @param aTimeout - number of millis to wait
	 */
	...
	public void connect(String aServiceName) throws ServiceConnectionException
	{
		...
		if (getVNSHost() == null || getVNSPort() == null)
		{
			...
			if ( UIMAFramework.getLogger().isLoggable(Level.SEVERE) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_vns_not_provided__SEVERE",
				        new Object[] {Thread.currentThread().getName()});				
			}
			throw new ServiceConnectionException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
					"UIMA_CPM_EXP_vinci_vns_cfg_invalid__WARNING",new Object[] {Thread.currentThread().getName()}) );
		}
		...
		try
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_locating_service__FINEST",
				        new Object[] {Thread.currentThread().getName(),aServiceName, System.getProperty("VNS_HOST"), System.getProperty("VNS_PORT")});				
			}
			...
			if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
			{
				UIMAFramework.getLogger(this.getClass()).log(Level.FINEST,Thread.currentThread().getName() + " Connecting to::" + aServiceName + " VinciContext.getVNSHost():" + vctx.getVNSHost() + " VinciContext.getVNSPort():" + vctx.getVNSPort()); //getVNSHost());
			}
			...
			if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_set_service_timeout__FINEST",
				        new Object[] {Thread.currentThread().getName(),aServiceName+":"+String.valueOf(timeout)});				
			}
			serviceHost = conn.getHost();
			servicePort = String.valueOf(conn.getPort());
		...
		throw new ServiceConnectionException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
				"UIMA_CPM_EXP_unable_to_connect_toservice__WARNING",new Object[] {Thread.currentThread().getName(),aServiceName}) );
	}
	private void testAndReconnect() throws ServiceException, ServiceConnectionException
	{
		...
		if (conn == null || !conn.isOpen())
		{
			try
			{
				if (serviceName != null)
				{
					if ( UIMAFramework.getLogger().isLoggable(Level.INFO) )
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_down__INFO",
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_down_onhost__INFO",
			...
			catch (ConnectException ce)
			{
				if (serviceName != null)
				{
					throw new ServiceException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							"UIMA_CPM_EXP_unable_to_connect_toservice__WARNING",new Object[] {Thread.currentThread().getName(),serviceName}) );
			}
				else
				{
					throw new ServiceException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							"UIMA_CPM_EXP_unable_to_connect__WARNING",new Object[] {Thread.currentThread().getName(),serviceHost, servicePort}) );
	 ...
	 * 
	 * @param AFrame - Vinci Frame containing request
	 * 
	...
	public AFrame sendAndReceive(AFrame aFrame) throws ServiceException, ServiceConnectionException
	{
		...
		if ( UIMAFramework.getLogger().isLoggable(Level.FINE) )
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINE, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_sending_process_req__FINEST",
			        new Object[] {Thread.currentThread().getName(),serviceHost, servicePort, String.valueOf(currentTimeout)});				
		}
		try
		{
			...
			//	Dont test the connection, just try to send a message. If the send fails, go through the error handler
			...
			if ( System.getProperty("SHOW_MEMORY") != null )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_memory_before_call__FINEST",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(Runtime.getRuntime().totalMemory()/1024), String.valueOf(memStart/1024)});				
			}
			responseFrame = (AFrame) conn.sendAndReceive(aFrame, AFrame.getAFrameFactory(), timeout);
			if ( System.getProperty("SHOW_MEMORY") != null )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_memory_after_call__FINEST",
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_received_response__FINEST",
				        new Object[] {Thread.currentThread().getName(),serviceHost, servicePort});				
			}
		...
		catch (VNSException vnse)
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_failed_service_request__WARNING",
			        new Object[] {Thread.currentThread().getName(),conn.getHost(), String.valueOf(conn.getPort())});
		...
		catch (IOException e)
		{
			...
			throw new ServiceConnectionException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
					"UIMA_CPM_EXP_service_timeout__WARNING",new Object[] {Thread.currentThread().getName(),conn.getHost(),String.valueOf(conn.getPort()),String.valueOf(currentTimeout)}) );
		}
		catch (Exception e)
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
	 ...
	 * 
	 * @param dataCas - instance of CasData where the keys will be appended
	 * @param aFrame - source of keys (data)
	...
	public static CasData addKeysToDataCas(CasData dataCas, AFrame aFrame) throws Exception
	{
		try
		{
			...
			for (int i = 0; i < frameCount; i++)
			{
				...
				if (featureStructureType.indexOf(org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM) > -1)
				{
					featureStructureType = StringUtils.replaceAll(featureStructureType, org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM, org.apache.uima.impl.collection.cpm.Constants.LONG_COLON_TERM);
				}
				//	Convert WF keys from '-' representation to '_dash_'
				if (featureStructureType.indexOf(org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM) > -1)
				{
					featureStructureType = StringUtils.replaceAll(featureStructureType, org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM, org.apache.uima.impl.collection.cpm.Constants.LONG_DASH_TERM);
				}
	...
	/**
	 * Produces XCas from a given Cas. It selectively copies features from the Cas excluding those types
	 * that are defined in a given dropKeyList. 
	 * 
	 * @param aCasData - Cas for which XCAS is built
	 * @param dataFrame - XCas frame for data
	 * @param aDropKeyList - list of types to exclude from XCas
	 * 
	 * @throws Exception
	 */
	private void produceXCASRequestFrame(CasData aCasData, AFrame dataFrame, String[] aDropKeyList) throws Exception
	{
		...
		String ueid = DATACasUtils.getFeatureValueByType(aCasData, org.apache.uima.impl.collection.cpm.Constants.METADATA_KEY, org.apache.uima.impl.collection.cpm.Constants.DOC_ID);
		...
		try
		{
			...
			while (it.hasNext())
			{
				...
				if (type.indexOf(org.apache.uima.impl.collection.cpm.Constants.LONG_COLON_TERM) > -1)
				{
					type = StringUtils.replaceAll(type, org.apache.uima.impl.collection.cpm.Constants.LONG_COLON_TERM, org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM);
				}
				if (type.indexOf(org.apache.uima.impl.collection.cpm.Constants.LONG_DASH_TERM) > -1)
				{
					type = StringUtils.replaceAll(type, org.apache.uima.impl.collection.cpm.Constants.LONG_DASH_TERM, org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM);
				}
	 ...
	 *  
	 * @param feature - type to check
	 * 
	 * @return - true if SoFa, false otherwise
	 */
	private static boolean isText(String feature)
	{
		if ("Doc:SpannedText".equals(feature) || "Detag:Content".equals(feature) || "Detag:DetagContent".equals(feature) || "uima.cpm.DocumentText".equals(feature))
		{
			return true;
		}
		return false;
	 ...
	 * 
	 * @param aCas - Cas to analayze
	 * @param aPT - performance trace object for stats and totals
	 * @param aResourceName - name of the Cas Processor
	...
	public CasData analyze(CasData aCas, ProcessTrace aPT, String aResourceName) throws ServiceException, ServiceConnectionException
	{
		...
		try
		{
			...
			vinciCasDataConverter.appendVinciFrameToCasData(responseFrame.fgetAFrame("DATA").fgetAFrame("KEYS"), newCasData);
			aCas = newCasData;

			aPT.endEvent(aResourceName, "Vinci Response Frame to CAS", "");
			//Get the times reported by service and add to process trace
			int frameToCasTime = responseFrame.fgetVinciFrame("DATA").fgetInt(Constants.FRAME_TO_CAS_TIME);
			if (frameToCasTime > 0)
			{
			...
			int casToFrameTime = responseFrame.fgetVinciFrame("DATA").fgetInt(Constants.CAS_TO_FRAME_TIME);
			if (casToFrameTime > 0)
			{
		...
		if (aDropKeyList != null && aKeyFrame != null)
		{
			...
			for (int inx = 0; inx < keyList.size(); inx++)
			{
				if (System.getProperty("SHOWKEYS") != null)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_drop_key__FINEST",
	...
	/**
	 * Main routine that sends requests to remote vinci services. Each Cas in the list is
	 * placed in the request frame in its own DATA frame. For each Cas, this routine 
	 * create a seperate DATA frame. The DATA frame contains types and data that are
	 * required by the annotator. For efficiency, the dropKeyList array can be defined
	 * with types that will be omitted from the DATA frame. These keys are not required
	 * by the annotator thus it is waste of bandwidth to include them in the request.
	 * 
	 * @param aCasList - a list of Cas to send to service for analysis
	 * @param aPT - Process Trace object to aggrate time and stats
	 * @param aResourceName - name of the Cas Processor
	...
	public CasData[] analyze(CasData[] aCasList, ProcessTrace aPT, String aResourceName) throws ServiceException, ServiceConnectionException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_analyze_bundle__FINEST",
		...
		try
		{
			...
			for (int i = 0; i < aCasList.length && aCasList[i] != null; i++)
			{
				...
				if (DATACasUtils.isCasEmpty(aCasList[i]))
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_no_cas__FINEST",
						        new Object[] {Thread.currentThread().getName()});
					}
					continue;
				}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_send_casdata_to_service__FINEST",
				        new Object[] {Thread.currentThread().getName(), serviceName});
			}
			...
			{
				UIMAFramework.getLogger(this.getClass()).log(Level.INFO," responseFrame from service::" + serviceName + "\n" + responseFrame.toXML());
			}
			...
			int instanceCount = 0;
			//	Process response, DATA frame at a time. Each DATA frame corresponds to an instance of CasData
			...
			while (!(d.isEmpty()))
			{
				...
				try
				{
					...
					if (System.getProperty("WF_SERIALIZER") != null)
					{
						...
						vinciCasDataConverter.appendVinciFrameToCasData(dataFrame.fgetAFrame("KEYS"), newCasData);
					...
					{
						FeatureStructure vfs = new FeatureStructureImpl();
						vfs.setType(org.apache.uima.impl.collection.cpm.Constants.STAT_FEATURE);
				...
				catch (Exception e)
				{
					if ( UIMAFramework.getLogger().isLoggable(Level.FINER) )
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINER, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception__FINER",
						        new Object[] {Thread.currentThread().getName(), e.getMessage(),dataFrame.toXML()});
                        e.printStackTrace();
					}
					dataFrame.toXML();
				}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_analyzing_bundle__FINEST",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(aCasList.length)});
			}
			return aCasList;
		...
		catch (ServiceException e)
		{
			if ( UIMAFramework.getLogger().isLoggable(Level.SEVERE) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_exception__SEVERE",
				        new Object[] {Thread.currentThread().getName(), e.getMessage()});
			}
			e.printStackTrace();
			throw e;
	...
	*/
	public ProcessingResourceMetaData getAnalysisEngineMetaData() throws ResourceServiceException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_request_metadata__FINEST",
			        new Object[] {Thread.currentThread().getName(), serviceName, conn.getHost(), String.valueOf( conn.getPort())});
		}
		...
		try
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_return_meta__FINEST",
				        new Object[] {Thread.currentThread().getName(), serviceName, conn.getHost(), String.valueOf( conn.getPort())});
			}
			...
			ProcessingResourceMetaData metadata = (ProcessingResourceMetaData)
			  saxDeser.getObject();			
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_parsing_meta__FINEST",
				        new Object[] {Thread.currentThread().getName(), serviceName, conn.getHost(), String.valueOf( conn.getPort())});
			}
			return metadata;
		}
		...
		catch (Exception e)
		{
			if ("No Such Command supported".equals(e.getMessage()))
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_rejected_requested__WARNING",
					        new Object[] {Thread.currentThread().getName(), serviceName, resultFrame.toXML()});
				}
				return null;
			}
			if ( UIMAFramework.getLogger().isLoggable(Level.SEVERE) )
	...
	/**
	 * Let the remote service now that end of batch marker has been reached, 
	 * the notification is one-way meaning the CPE does not expect anything
	...
	public void batchProcessComplete() throws ResourceServiceException
	{
		...
		try
		{
			if (conn != null && conn.isOpen())
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_send_batch_complete__FINEST",
					        new Object[] {Thread.currentThread().getName(), conn.getHost(), String.valueOf(conn.getPort()), query.toXML()});
				}
	 ...
	 * Wait for response from the service before returning. This ensures that
	 * the request is accepted and the desired logic handling end of processing
	 * has completed. 
	 *  
	...
	public void collectionProcessComplete() throws ResourceServiceException
	{
		try
		{
			if (conn != null && conn.isOpen())
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_send_collection_complete__FINEST",
					        new Object[] {Thread.currentThread().getName(), conn.getHost(), String.valueOf(conn.getPort()), query.toXML()});
	...
	/**
	 * Conditionally sends the shutdown request to managed (local) vinci service. This routine should not
	 * terminate services deployed as remote (unmanaged). This routine does not verify that the service 
	 * shut itself down. It does not even wait for response. It is up to the service to clean itself up
	 * and terminate.
	 * 
	 * @param shutdownService - flag indicating if a shutdown command should be sent to the service
	 * @param aDoSendNotification - indicates whether or not to sent CollectionProcessComplete frame to service
	...
	public boolean shutdown(boolean shutdownService, boolean aDoSendNotification)
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_time_spent_serializing_xcas__FINEST",
			        new Object[] {Thread.currentThread().getName(), String.valueOf(totalSerializeTime)});
			
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_time_spent_deserializing_xcas__FINEST",
			        new Object[] {Thread.currentThread().getName(), String.valueOf(totalDeSerializeTime)});

			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_time_spent_in_transit__FINEST",
			        new Object[] {Thread.currentThread().getName(), String.valueOf(totalRoundTripTime)});
		}
		try
		{
					...
					try
					{
						...
						if ( UIMAFramework.getLogger().isLoggable(Level.INFO) )
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_stopping_service__INFO",
							        new Object[] {Thread.currentThread().getName(), conn.getHost(), String.valueOf(conn.getPort()), query.toXML()});
						}
						//	Send shutdown request to the TAE service
						conn.send(query);
	 ...
	 * terminate, sometimes it just lingers. Since the shutdown command sent in shuthdown() method
	 * is one-way call, it was immediately followed by the connection close. That caused an 
	 * exception on the service side, preventing it from clean exit. So here we just wait until
	 * the connection no longer exists and then close it on this side. 
	 *
	 */
	private void waitForServiceShutdown()
	{
		...
		do
		{
			try
			{
				...
				synchronized(monitor)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_waiting_for_service_shutdown__FINEST",
						        new Object[] {Thread.currentThread().getName(), String.valueOf(10-retry),String.valueOf(10)});
					}
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
		        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_shutdown_completed__FINEST",
		        new Object[] {Thread.currentThread().getName()});
		}
	}
	/**
	 ...
	 * 
	 * @param aVNSPort - vns port to use
	 */
	 ...
	 * 
	 * @param host - name of the VNS host
	 */
	 ...
	 * Currently, this must be one of the first two FSs in the CasData.
	 * 
	 * @param aCasData  CasData containing feature structures
