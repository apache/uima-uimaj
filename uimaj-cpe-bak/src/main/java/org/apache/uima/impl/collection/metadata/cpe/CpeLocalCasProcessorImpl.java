...
public class CpeLocalCasProcessorImpl extends CasProcessorCpeObject implements CpeLocalCasProcessor
{
	...
	private void addRunInSeperateProcess() throws CpeDescriptorException
	{
		...
		execArg.setArgValue("-DVNS_HOST=127.0.0.1");
		exe.addCasProcessorExecArg(execArg);

		execArg = CpeDescriptorFactory.produceCasProcessorExecArg();
		execArg.setArgValue("-DVNS_PORT=9904");
		exe.addCasProcessorExecArg(execArg);
	...
	/**
	 * 
	 * Adds a new env key to the list of env keys. If a kay with a given key name exists
	 * the new key value replaces the old.
	 */
