...
import org.apache.uima.collection.base_cpm.container.deployer.CasProcessorDeploymentException;
import org.apache.uima.collection.metadata.CpeCasProcessor;
import org.apache.uima.collection.metadata.CpeCasProcessors;
import org.apache.uima.collection.metadata.CpeConfiguration;
import org.apache.uima.collection.metadata.CpeDescription;
import org.apache.uima.impl.analysis_engine.service.vinci.util.Descriptor;
import org.apache.uima.impl.collection.EntityProcessStatusImpl;
import org.apache.uima.impl.collection.cpm.CPEConfig;
import org.apache.uima.impl.collection.cpm.CheckpointData;
import org.apache.uima.impl.collection.cpm.Constants;
import org.apache.uima.impl.collection.cpm.container.CPEFactory;
import org.apache.uima.impl.collection.cpm.container.deployer.DeployFactory;
import org.apache.uima.impl.collection.cpm.container.deployer.socket.ProcessControllerAdapter;
...
import org.apache.uima.impl.collection.cpm.utils.TimerFactory;
import org.apache.uima.impl.util.JavaTimer;
...
/**
 * Responsible for creating and initializing processing threads. This instance
 * manages the lifecycle of the CPE components. It exposes API for plugging in
 * components programmatically instead of declaratively. Running in its own
 * thread, this components creates seperate Processing Pipelines for Analysis
 * Engines and Cas Consumers, launches configured CollectionReader and attaches
 * all of those components to form a pipeline from source to sink. The Collection
 * Reader feeds Processing Threads containing Analysis Engines, and Analysis Engines
 * feed results of analysis to Cas Consumers.
 * 
 * 
 * 
 */
public class CPMEngine extends Thread
{
	...
	//	Component responsible for asynchronous read from the CollectionReader. It places Cas'es into work Queue
	...
	//	Instantiate a Processing Unit containing CasConsumers. There may be many Analysis Processing Units
	...
	private LinkedList initial_cp_list = new LinkedList(); // this list is used to hold Cas Processors 
	...
	/**
	 * Initializes Collection Processing Engine. Assigns this thread and all processing threads created by this component to
	 * a common Thread Group.
	 * 
	 * @param aThreadGroup - contains all CPM related threads
	 * @param aCpeFactory - CPE factory object responsible for parsing cpe descriptor and creating components
	 * @param aProcTr - instance of the ProcessTrace where the CPM accumulates stats
	 * @param aCheckpointData - checkpoint object facillitating restart from the last known point
	 */
	public CPMEngine(CPMThreadGroup aThreadGroup, CPEFactory aCpeFactory, ProcessTrace aProcTr, CheckpointData aCheckpointData) throws Exception
	{
		...
		if (cpeFactory.getCPEConfig() != null && cpeFactory.getCPEConfig().getDeployment().equalsIgnoreCase(SINGLE_THREADED_MODE))
		{
			if (UIMAFramework.getLogger().isLoggable(Level.CONFIG))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_single_threaded_mode__CONFIG",
		...
		else
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_multi_threaded_mode__CONFIG",
				        new Object[] {Thread.currentThread().getName()});
			}
		}
	 ...
	 * 
	 * @param aMap - map for runtime stats and totals
	 */
	 ...
	 * 
	 * @param aPause - true if pause is requested on exception, false otherwise
	 */
	...
	/**
	 * Defines the size of inputQueue. The queue stores this many entities read
	 * from the CollectionReader. Every processing pipeline thread will read its
	 * entities from this input queue. The CollectionReader is decoupled from the
	 * consumer of entities, and continuously replenishes the input queue.
	 * 
	...
	*/
	...
	/**
	 * Defines the size of outputQueue. The queue stores this many entities enqueued
	 * by every processing pipeline thread.The results of analysis are dumped into this queue
	 * for consumer thread to consume its contents.
	 * 
	 * @param aBatchSize the size of the batch.
	*/
	...
	*/
	 ...
	 * @param aListener - instance of {@link BaseStatusCallbackListener} to unregister
	 */
	...
	private void dumpState()
	{
		try
		{
			if (cpeFactory.getCPEConfig() != null && cpeFactory.getCPEConfig().getDeployment().equalsIgnoreCase(SINGLE_THREADED_MODE))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cr_state__INFO",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(this.readerState)});

				for (int i = 0; processingUnits != null && i < processingUnits.length; i++)
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_pu_state__INFO",
					        new Object[] {Thread.currentThread().getName(), String.valueOf(i), String.valueOf(processingUnits[i].threadState)});
				}
				if (casConsumerPU != null)
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cc_state__INFO",
					        new Object[] {Thread.currentThread().getName(), String.valueOf(casConsumerPU.threadState)});
				}
			...
			else
			{
				if (producer != null)
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cr_state__INFO",
					        new Object[] {Thread.currentThread().getName(), String.valueOf(producer.threadState)});
				}
				for (int i = 0; processingUnits != null && i < processingUnits.length; i++)
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
		...
		{ //ignore. This is called on stop() 
		}

	}
	/**
	 * Kill CPM the hard way. None of the entities in the queues will be processed. This methof simply empties
	 * all queues and at the end adds EOFToken to the work queue so that all threads go away.
	 *
	 */
	public void killIt()
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_killing_cpm__INFO",
			        new Object[] {Thread.currentThread().getName()});
		}
		if (workQueue != null)
		{
		...
		if (workQueue != null)
		{
			...
			UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_terminate_pipelines__INFO",
	...
	/**
	 * Returns if the CPE was killed hard. Soft kill allows the CPE to finish processing all in-transit CASes. Hard kill causes the CPM
	 * to stop processing and to throw away all unprocessed CASes from its queues.
	 * @return
	 */
		...
		new Thread()
		{
			public void run()
			{
				...
				if (!isRunning)
				{
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_already_stopped__FINEST",
						        new Object[] {Thread.currentThread().getName()});
					}
					//	Already stopped
					return;
				...
				try
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_consuming_queue__FINEST",
						        new Object[] {Thread.currentThread().getName(), workQueue.getName(), String.valueOf(cc)});
					}
					while (workQueue.getCurrentSize() > 0)
					{
						...
						if (System.getProperty("DEBUG") != null)
						{
							if (cc < workQueue.getCurrentSize())
							{
								...
								{
									UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
									        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_wait_consuming_queue__FINEST",
					...
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_consuming_queue__FINEST",
						        new Object[] {Thread.currentThread().getName(),workQueue.getName(), String.valueOf(outputQueue.getCurrentSize())});
					}
					while (outputQueue.getCurrentSize() > 0)
					{
						sleep(MAX_WAIT_ON_QUEUE);
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_consuming_queue__FINEST",
						        new Object[] {Thread.currentThread().getName()});
					}
					for (int i = 0; processingUnits != null && i < processingUnits.length; i++)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_stop_processors__FINEST",
	...
	{
		UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
		        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_stop_cpm__FINEST",
		        new Object[] {Thread.currentThread().getName(),String.valueOf(killed) });
		...
		try
		{
			...
			for (int i = 0; processingUnits != null && i < processingUnits.length && processingUnits[i] != null; i++)
			{
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
	 ...
	 * 
	 * 
	 * @param aDeployList - List of CasConsumers to be searched 
	 * @param aName - name of the CasConsumer we want to find
	 * 
	 * @return 0 - if a CasConsumer is not found in a list, else returns
	 * 			a position in the list where the CasConsumer can found
	 * 
	 */
	 ...
	 * 
	 * @param aDescPath - fully qualified path to a CP descriptor
	 * @param aCpName - name of the CP
	 * @param isConsumer - true if the CP is a Cas Consumer, false otherwise
	...
	private boolean isMultipleDeploymentAllowed(String aDescPath, String aCpName, boolean isConsumer) throws Exception
	{
		...
		throw new ResourceConfigurationException(ResourceInitializationException.NOT_A_CAS_PROCESSOR, new Object[] { aCpName, "<unknown>", aDescPath });

	}
	/**
	 * Get the full (absolute) path of the Cas Processor descriptor.
	 * 
	 * @param aCasProcessorCPEConfig - Cas Processor configuration from the CPE descriptor
	...
	private String getDescriptorPath(CpeCasProcessor aCasProcessorCPEConfig) throws Exception
	{
		...
		return CPMUtils.convertToAbsolutePath(System.getProperty("CPM_HOME"), CPEFactory.CPM_HOME, descAsString);
	}

	/**
	 * Determines if a given Cas Processor is parallelizable. Remote Cas Processors are by default parallelizable.
	 * For integrated and managed the CPM consults Cas Processor's descriptor to determine if it is parallelizable. 
	 * 
	 * @param aProcessor - Cas Processor being checked
	 * @param aCpName - name of the CP
	 * @return - true if CP is parallelizable, false otherwise
	 * 
	 * @throws Exception
	...
	public boolean isParallizable(CasProcessor aProcessor, String aCpName) throws Exception
	{
		...
		//	casProcessingConfigMap may not contain configuration for this Cas Processor if this CP has been
		...
		if (!cpeFactory.casProcessorConfigMap.containsKey(aCpName))
		{
			...
			if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_invalid_processor_configuration__SEVERE",
		...
		CpeCasProcessor casProcessorCPEConfig = (CpeCasProcessor) cpeFactory.casProcessorConfigMap.get(aCpName);
		...
		if (Constants.DEPLOYMENT_LOCAL.equalsIgnoreCase(casProcessorCPEConfig.getDeployment()))
		{
			...
			boolean is = isMultipleDeploymentAllowed(aResourceSpecifierPath, casProcessorCPEConfig.getName(), isConsumer);
			return is;
		}
		else if (Constants.DEPLOYMENT_INTEGRATED.equalsIgnoreCase(casProcessorCPEConfig.getDeployment()))
		{
	...
	/**
	 * Adds Cas Processor to a single-threaded pipeline. This pipeline is fed by the output queue and typicall
	 * contains Cas Consumers. AEs can alos be part of this pipeline. 
	 * 
	 * @param aProcessor - Cas Processor to add to single-threaded pipeline
	 * @param aCpName - name of the Cas Processor
	 * 
	 * @throws Exception
	 */
	private void addCasConsumer(CasProcessor aProcessor, String aCpName) throws Exception
	{
		if (consumers.containsKey(aCpName))
		{
			if (UIMAFramework.getLogger().isLoggable(Level.CONFIG))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_add_consumer_to_list__CONFIG",
				        new Object[] {Thread.currentThread().getName(),aCpName });
			}
	...
	/**
	 * Add Cas Processor to a list of CPs that are to be run in the parallelizable pipeline. The fact that the CP
	 * is in parallelizable pipeline does not mean that there will be instance per pipeline of CP. Its allowed to
	 * have a single instance, shareable CP running in multi-threaded pipeline.  
	 * 
	 * @param aProcessor - CP to add to parallelizable pipeline
	 * @param aCpName - name of the CP
	 * 
	 * @throws Exception
	 */
	private void addParallizableCasProcessor(CasProcessor aProcessor, String aCpName) throws Exception
	{
		...
		if (analysisEngines.containsKey(aCpName))
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_add_pcp_to_existing_list__CONFIG",
			        new Object[] {Thread.currentThread().getName(),aCpName });
			}
	...
	/**
	 * Classify based on Cas Processor capability to run in parallel. Some Cas Processors need to run as single
	 * instance only. It scans the list of Cas Processors backwords and moves those Cas Processors that are not
	 * parallelizable to a separate single-threade pipeline. This process of moving CPs continues until the first
	 * parallelizable Cas Processor is found. Beyond this all Cas Processors are moved to a parallelizable pipeline.
	 * If the non-parallelizable CP is in the parallelizable pipeline there simply will be a single instance of it
	 * that will be shared by all processing threads.
	 * 
	 * @throws Exception
	 */
	private void classifyCasProcessors() throws Exception
	{
		...
		//	Walk the list of Cas Processor backwards. The list of Cas Processors is actually a list of lists.
	...
	/** 
	 *
	 * Adds a CASProcessor to the processing pipeline. If a CasProcessor already exists and its status=DISABLED
	 * this method will re-enable the CasProcesser.
	 * 
	 ...
	 * 
	 * @param aProcessor CASProcessor to be added to the processing pipeline
	 * @param aIndex - insertion point for a given CasProcessor
	 **/
	 ...
	 * 
	 * @param aCasProcessorIndex - CasProcessor position in processing pipeline
	 **/
	...
	public void disableCasProcessor(int aCasProcessorIndex)
	{
		...
		if (pc != null)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_diabled_cp__FINEST",
	...
	private void deployConsumers() throws AbortCPMException
	{
		...
		for (int i = consumerDeployList.size(); i > 0; i--)
		{
			try
			{
				...
				{
					//
					//	Get a deployer for this type of CasProcessor. The type of deployer is determined from the
					//	CPE Configuration. Specifically from the deployment model for this CasProcessor.  
					//	 
					CpeCasProcessor casProcessorType = (CpeCasProcessor) cpeFactory.casProcessorConfigMap.get(name);
	...
	public void redeployAnalysisEngine(ProcessingContainer aProcessingContainer) throws Exception
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploying_cp__FINEST",
	...
	/**
	 * Deploys All Analysis Engines. Analysis Engines run in a replicated processing units
	 * seperate from Cas Consumers.
	 * 
	 * @throws AbortCPMException
	 */
	private void deployAnalysisEngines() throws AbortCPMException
	{
		...
		for (int i = 0; i < annotatorDeployList.size(); i++)
		{
			try
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploying_new_cp__CONFIG",
				...
				if (cpList.size() > 0)
				{
					...
					//
					//	Get a deployer for this type of CasProcessor. The type of deployer is determined from the
					//	CPE Configuration. Specifically from the deployment model for this CasProcessor. The first 
					//	 
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_lookup_cp__FINEST",
						        new Object[] {Thread.currentThread().getName(), name});
					}
					if (!cpeFactory.casProcessorConfigMap.containsKey(name))
					{
						...
						throw new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_EXP_missing_cp__WARNING",new Object[] {Thread.currentThread().getName(),name}) );
					}
					CpeCasProcessor casProcessorCPEConfig = (CpeCasProcessor) cpeFactory.casProcessorConfigMap.get(name);
					if (casProcessorCPEConfig == null)
					{
						if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
						{
							...
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_configuration_not_defined__SEVERE",
							        new Object[] {Thread.currentThread().getName(), name});
						}
						throw new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_EXP_missing_cp__WARNING",new Object[] {Thread.currentThread().getName(),name}) );
					}
					else if (casProcessorCPEConfig.getDeployment() == null || casProcessorCPEConfig.getDeployment().trim().length() == 0)
					{
						if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_deployment_mode_not_defined__SEVERE",
							        new Object[] {Thread.currentThread().getName(), name});
						}
						throw new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_Exception_invalid_deployment__WARNING",new Object[] {Thread.currentThread().getName(),name,casProcessorCPEConfig.getDeployment()}) );
					}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_failed_to_start__SEVERE",
				        new Object[] {Thread.currentThread().getName(), e.getMessage()});
	...
	/**
	* Starts CASProcessor containers one a time. During this phase the container 
	* deploys a TAE as local,remote, or integrated CasProcessor. 
	*  
	*/
	 ...
	 * 
	 * @param component - component name to restore named event for
	 * @param aEvType - event to restore
	 */
	 ...
	 * 
	 * @param aEvType - event type
	 * @param aList - list of events to copy
	 * @param aPTr - 
	 * @throws IOException
	 */
	...
	public void pauseIt()
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pause_cpe__FINEST",
			        new Object[] {Thread.currentThread().getName()});
		}
		synchronized (monitor)
		{
	...
	public void resumeIt()
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_resume_cpe__FINEST",
			        new Object[] {Thread.currentThread().getName()});
		}
		synchronized (monitor)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_notify_engine__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}
			monitor.notifyAll();
		}
	 ...
	 * 
	 * @param BaseCollectionReader aCollectionReader
	 */
	public void setCollectionReader(BaseCollectionReader aCollectionReader)
	{
		collectionReader = aCollectionReader;

		if (collectionReader.getProcessingResourceMetaData().getConfigurationParameterSettings().getParameterValue("fetchSize") != null)
		{
			try
			{
				readerFetchSize = ((Integer) collectionReader.getProcessingResourceMetaData().getConfigurationParameterSettings().getParameterValue("fetchSize")).intValue();
			}
		...
		if (checkpointData != null && checkpointData.getSynchPoint() != null && collectionReader != null)
		{
	 ...
	 * 
	 * @param aClassName - name of a class that extends ProcessingUnit
	 * 
	 ...
	 * 
	 * @param aQueueSize - max size of the queue
	 ...
	 * 
	 * @param e - en exception to be sent to listeners
	 */
	...
	/**
	 * Callback method used to notify the engine when a processing pipeline is killed due to excessive errors. This method 
	 * is only called if the processing pipeline is unable to acquire a connection to remote service and when configuration
	 * indicates 'kill-pipeline' as the action to take on excessive errors. When running with multiple pipelines, routine 
	 * decrements a global pipeline counter and tests if there are no more left. When all pipelines are killed as described
	 * above, the CPM needs to terminate. Since pipelines are prematurely killed, there are artifacts (CASes) in the work
	 * queue. These must be removed from the work queue and disposed of (released) back to the CAS pool so that the Collection
	 * Reader thread properly exits.  
	 * 
	 * @param aPipelineThreadName - name of the pipeline thread exiting from its run() method
	 */
	public synchronized void pipelineKilled(String aPipelineThreadName)
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pipeline_terminated__FINEST",
			...
			while (workQueue != null && workQueue.getCurrentSize() > 0)
			{
				...
				try
				{
					...
					if (anObject != null && anObject instanceof CAS[])
					{
						...
						notifyListeners(0, (CAS[]) anObject, procTr, new Exception("CPM Releases CAS before processing it due to premature CPM shutdown."));
				...
				catch (Exception e)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_while_consuming_cases__SEVERE",
						        new Object[] {Thread.currentThread().getName(), e.getMessage()});
					}
	...
	/**
	 * Using given configuration creates and starts CPE processing pipeline. It is either single-threaded or
	 * a multi-threaded pipeline. Which is actually used depends on the configuration defined in the CPE
	 * descriptor.
	 * In multi-threaded mode, the CPE starts number of threads:
	 * 1) ArtifactProducer Thread - this is a thread containing a Collection Reader. It runs asynchronously and 
	 *    it fills a WorkQueue with CASes.
	 * 2) CasConsumer Thread - this is an optional thread. It is only instantiated if there Cas Consumers in the
	 *    pipeline
	 * 3) Processing Threads - one or more processing threads, configured identically, that are performing analysis
	 *    How many threads are started depends on configuration in CPE descriptor
	 * 
	 * All threads started here are placed in a ThreadGroup. This provides a catch-all mechanism for errors that
	 * may occur in the CPM. If error is thrown, the ThreadGroup is notified. The ThreadGroup than notifies all
	 * registers listeners to give an application a chance to report the error and do necessary cleanup. This routine
	 * manages all the threads and makes sure that all of them are cleaned up before returning. The ThreadGroup must
	 * cleanup all threads under its control otherwise a memory leak occurs. Even those threads that are not started
	 * must be cleaned as they end up in the ThreadGroup when instantiated. The code uses number of state variables to
	...
	public void run()
	{
		...
		//	Single-threaded mode is enabled in the CPE descriptor. In the CpeConfig element check for the value of deployAs
		...
		if (singleThreadedCPE)
		{
			...
			catch (Throwable t)
			{
				...
				t.printStackTrace();
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_in_single_threaded_cpm__SEVERE",
		...
		try
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_starting_cpe__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}
			...
			//	When the CPE is configured to run exclusively with CasDataProcessor type components (no CasObjectProcessors) 
			//	there is no need to instantiate TCAS objects. These would never be used and woud waste memory.
			if (mixedCasProcessorTypeSupport)
			{
				...
				try
				{
					...
					{
						poolSize = readerFetchSize * (inputQueueSize + outputQueueSize) * cpeFactory.getProcessingUnitThreadCount() + 3;
						//	 This is a hack to limit # of CASes. In WF env where the WF Store decides the size of readerFetchSize
						//	 we have a problem with memory. If the store decides to return 1000 entities we will need a LOT of 
						//	memory to handle this. So for WF limit the pool size to something more reasonable
						if (poolSize > 100)
						{
							System.err.println("CPMEngine.run()-CAS PoolSize exceeds hard limit(100). Redefining size to 60.");
							UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_redefine_pool_size__CONFIG",
							        new Object[] {Thread.currentThread().getName()});
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cas_pool_size__CONFIG",
						        new Object[] {Thread.currentThread().getName(), String.valueOf(poolSize)});
					}
					casPool = new CPECasPool(poolSize, mdList, mPerformanceTuningSettings, cpeFactory.getResourceManager());
					callTypeSystemInit();
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_create_producer__CONFIG",
				        new Object[] {Thread.currentThread().getName()});
			...
			if (consumerList != null && consumerList.size() > 0)
			{
				//	Create a CasConsumer Processing Unit if there is at least one CasConsumer configured in a CPE descriptor
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_create_pus__CONFIG",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(workQueue.getCurrentSize())});
			}

			//	Adjust number of pipelines. Adjustment may be necessary in deployments using exclusive service access.  The adjustment is 
			//	based on number of available services that the CPM will connect to. If a static configuration calls for 5 processing 
			//	pipelines but only three services are available (assuming exclusive access ), the CPM will reduce number of processing
			//	pipelines to 3.
			for (int indx = 0; indx < annotatorList.size(); indx++)
			{
				...
				if (serviceAccess != null && serviceAccess.equalsIgnoreCase("exclusive"))
				{
					if (prContainer.getPool() != null)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_no_proxies__SEVERE",
							        new Object[] {Thread.currentThread().getName(), prContainer.getName()});
							return;
						}
						if (totalInstanceCount < concurrentThreadCount)
						{
							...
							UIMAFramework.getLogger(this.getClass()).logrb(Level.CONFIG, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_reduce_pipelines__CONFIG",
							        new Object[] {Thread.currentThread().getName(), prContainer.getName()});
						}
					}
				}
			...
			synchronized (this)
			{
				activeProcessingUnits = concurrentThreadCount; //keeps track of how many threads are still active. -Adam
			}
			...
			for (int i = 0; i < concurrentThreadCount; i++)
			{
				...
				//	If there are no consumers in the pipeline, instruct the pipeline to release a CAS at the end of processing
				if (consumerList == null || consumerList.size() == 0)
				{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pipeline_impl_class__FINEST",
					        new Object[] {Thread.currentThread().getName(), processingUnits[i].getClass().getName()});
				}
				//	Add tracing instance so that performance and stats are globally aggregated for all processing pipelines
				...
				//	pass initialized list of cases to processing units in case cas conversion is required between
			...
			//	Start the ArtifactProducer thread and the Collection Reader embedded therein. The Collection Reader begins
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_started_pipelines__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}

			//	==============================================================================================
			//	Now, wait for ALL CPE threads to finish. Join each thread created and wait for each to finish.
			//	==============================================================================================

			if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_join_threads__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cr_thread_completed__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}
			...
			for (int i = 0; i < concurrentThreadCount; i++)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_join_pu__FINEST",
					        new Object[] {Thread.currentThread().getName(), processingUnits[i].getName(), String.valueOf(i)});
				}
				processingUnits[i].join();
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_join_pu_complete__FINEST",
					        new Object[] {Thread.currentThread().getName(), processingUnits[i].getName(), String.valueOf(i)});
				}
			}
			...
			if (casConsumerPU != null)
			{
				...
				try
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_notifying_queue__FINEST",
						        new Object[] {Thread.currentThread().getName(), outputQueue.getName() });
					}
				}
				catch (Exception e)
				{
					e.printStackTrace();
					UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_adding_eof__SEVERE",
					        new Object[] {Thread.currentThread().getName(), e.getMessage()});
					notifyListenersWithException(e);
				}
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_join_cc__FINEST",
					        new Object[] {Thread.currentThread().getName() });
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_join_cc_completed__FINEST",
					        new Object[] {Thread.currentThread().getName() });
				}
			}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cc_completed__FINEST",
				        new Object[] {Thread.currentThread().getName(), workQueue.getName(), String.valueOf( workQueue.getCurrentSize()) });
			}
			boolean empty = false;
			...
			while (!empty && outputQueue != null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pus_completed__FINEST",
					        new Object[] {Thread.currentThread().getName(), outputQueue.getName(), String.valueOf( outputQueue.getCurrentSize()) });
				}
				synchronized (outputQueue)
				{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cleaning_up_pus__FINEST",
				        new Object[] {Thread.currentThread().getName() });
			...
			if (casConsumerPU != null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_stop_ccs__FINEST",
					        new Object[] {Thread.currentThread().getName()  });
				}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_engine_stopped__FINEST",
				        new Object[] {Thread.currentThread().getName()  });
			}
			if (dbgCtrlThread != null)
			{
		...
		catch (Exception e)
		{
			...
			//	threads are never garbage collected. If this is the state of the CPE (!isStarted) go through
			//	a cleanup cycle checking each thread and starting those that have not been started. All 
			//	CPE threads in their run() method MUST check the state of the CPE by calling cpe.isRunning()
			...
			if (!isStarted)
			{
				...
				if (producer != null && !producer.isRunning())
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cr_exception__SEVERE",
				...
				if (casConsumerPU != null && !casConsumerPU.isRunning())
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cc_exception__SEVERE",
						        new Object[] {Thread.currentThread().getName(), ex1.getMessage() });
		...
		finally
		{
			...
			Thread threadGroupDestroyer = new Thread(group.getParent(), "threadGroupDestroyer")
			{
				public void run()
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_destroy_thread_group__FINEST",
						        new Object[] {Thread.currentThread().getName() });
					}
				...
				private void showThreads(Thread[] aThreadList)
				{
					for (int i = 0; aThreadList != null && i < aThreadList.length; i++)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_thread__FINEST",
							        new Object[] {Thread.currentThread().getName(), String.valueOf(i),aThreadList[i].getName() });
						}
					}
				}
			};
	...
	/**
	 *  Return timer to measure performace of the cpm. The timer can optionally be configured
	 * in the CPE descriptor. If none defined, the method returns default timer.
	 * 
	...
	private ArrayList getMetaDataList() throws Exception
	{
		...
		{
			CasDataInitializer casIni = ((CasDataCollectionReader) collectionReader).getCasDataInitializer();
			if (casIni != null && casIni.getMetaData() != null)
			{
				mdList.add(casIni.getMetaData());
			}
		...
		for (int i = 0; i < consumerList.size(); i++)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkout_cp_from_container__FINEST",
				        new Object[] {Thread.currentThread().getName(), container.getName() });
			}
	...
	public void stopCasProcessors(boolean kill) throws CasProcessorDeploymentException
	{
		...
		for (int i = 0; annotatorList != null && i < annotatorList.size(); i++)
		{
			...
			synchronized (container)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_stop_container__FINEST",
					        new Object[] {Thread.currentThread().getName(), container.getName() });
				}
	 ...
	 * 
	 * @param aCASList - cas list to release
	 */
	public void releaseCASes(CAS[] aCASList)
	{
		for (int i = 0; i < aCASList.length; i++)
		{
			...
			else
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_release_tcas__FINEST",
					        new Object[] {Thread.currentThread().getName() });
				}
			}
		}
	...
	/**
	 * Overrides the default performance tuning settings for this CPE.  This affects things
	 * such as CAS sizing parameters.
	 * 
	...
	/**
	 * Called from the ProcessingUnits when they shutdown due to having received the EOFToken.
	 * When all ProcessingUnits have shut down, we put an EOFToken on the output queue so that
	 * The CAS Consumers will also shut down. -Adam
	 */
	...
	private Object getCasWithSOFA(Object entity, ProcessTrace pTrTemp)
	{
		...
		try
		{
			if (collectionReader instanceof CollectionReader)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_get_cas__FINEST",
					        new Object[] {Thread.currentThread().getName() });
				}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_get_cas_returns_null_FINEST",
					        new Object[] {Thread.currentThread().getName(), String.valueOf((casList[0] == null)) });
				}
				if (this.isRunning() == false)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_in_shutdown_state__FINEST",
						        new Object[] {Thread.currentThread().getName() });
					}
					readerState = 1010;
					return null;
          ...
          CAS view = casList[0].getView(CAS.NAME_DEFAULT_SOFA);
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_cr_next__FINEST",
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_cr_next_finished__FINEST",
						        new Object[] {Thread.currentThread().getName(), "TCAS" });
					}
				}
		...
		catch (Exception e)
		{
			...
			handleException(e, (Object[]) casList, pTrTemp);
	...
	private boolean needsTCas()
	{
		if (definedCapabilities == null)
		{
			...
			{
				definedCapabilities = ((CollectionReader) collectionReader).getProcessingResourceMetaData().getCapabilities();
			}
	...
	{
		ArrayList mdList = getMetaDataList();
		casPool = new CPECasPool(getPoolSize(), mdList, mPerformanceTuningSettings, cpeFactory.getResourceManager());
		callTypeSystemInit();
	 ...
	 * 
	 * @param entity - container for CAS
	 * @return
	 */
	...
	public void runSingleThreaded() throws Exception
	{
		...
		while (isRunning)
		{
			try
			{
				...
				{
					notifyListeners(CAS_PROCESSED_MSG, (Object[]) entity, pTrTemp, new SkipCasException("Skipping Document Due To Dropped Cas in a Sequence"));
	 ...
	 * 
	 * @param entityCount - number of documents processed so far
	 * 
	 ...
	 * @param entity - CAS container
	 * @param aPTrace - process trace 
	 */
	private void handleException(Throwable t, Object[] entity, ProcessTrace aPTrace)
	{
		...
		notifyListeners(CAS_PROCESSED_MSG, (Object[]) entity, aPTrace, t);
	}
	/**
	 * 
	 * @param aMsgType
			...
			if (statCL != null)
			{
				...
				else
				{
					for (int i = 0; i < entity.length; i++)
					{
						...
						{
							((StatusCallbackListener) statCL).entityProcessComplete((CAS) (CAS) entity[i], eps);
						}
						else
						{
							((StatusCallbackListener) statCL).entityProcessComplete(null, eps);
