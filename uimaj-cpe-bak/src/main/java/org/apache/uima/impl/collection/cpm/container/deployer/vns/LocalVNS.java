...
package org.apache.uima.impl.collection.cpm.container.deployer.vns;
...
/**
 * 
 * LOCAL Vinci Naming Service. Used by locally deployed TAEs. Locally, meaning TAEs running on the same
 * machine but in different JVM. This VNS is primarily used by TAEs to advertise their availability after
 * succesfull startup.
 * 
 */
...
public class LocalVNS extends VinciServableAdapter implements Runnable
{
	 ...
	 * 
	 * @param aProcessor - CASProcessor instantiating this VNS (the owner)
	 * @param aStartPort - a starting port # for clients (services)
	 * @param aEndPort - an ending port # for clients( services)
	 * @param aVNSPort - port on which this VNS will listen on
	 */
	public LocalVNS(String aStartPort, String aEndPort, String aVNSPort) throws PortUnreachableException
	{
				...
				while (!vnsPortAvailable)
				{
					if ( UIMAFramework.getLogger().isLoggable(Level.INFO) )
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_test_vns_port__INFO",
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_activating_vns_port__INFO",
			        new Object[] {Thread.currentThread().getName(), String.valueOf(vnsPort)});				
		}
		onport = startport;
	}
	 ...
	 * A given port is tested first for availability. If it is not available the next port
	 * is tested, until one is found to be available.
	 * 
	 * @param aStartPort - starting port number used
	 * @param aEndPort - end port number. Together with StartPort defines the range of ports (port pool)
	 * @param aVNSPort - port on which this VNS will listen for requests
	 * 
		...
		while (!vnsPortAvailable)
		{
			...
			if (currentRetryCount > 100)
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_vns_port_not_available__SEVERE",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(vnsPort)});				
	 ...
	 * 
	 * @param pQueue - queue where allocated ports will be added
	 */
	...
	/**
	 * Determines if a given port is free. It establishes a short lived connection to 
	 * the port and if successful returns false. 
	 * 
		...
		finally
		{
			if (socket != null)
			{
				try
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_test_local_port__INFO",
	...
	public synchronized int getPort() throws PortUnreachableException
	{
		...
		while (!portAvailable)
		{
			...
			if (retryCount > 3)
			{
				throw new PortUnreachableException("Unable to aquire any of the ports in configured range:[" + startport + ".." + maxport + "]");
			}
		}
		return onport;
	}
	 ...
	 * "serveon" request to VNS and waits for assigned port. The VNS looks up its cahce of ports
	 * and returns to the service one that has not yest allocated. 
	 * 
	 */
	public synchronized Transportable eval(Transportable in) throws ServiceException
	{
		...
		try
		{
			...
			if (cmd.equals("shutdown"))
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deactivating_vns_port__INFO",
			...
			else if (cmd.equals("serveon"))
			{
				if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_vns_process_serveon__FINEST",
					        new Object[] {Thread.currentThread().getName()});				
				}
				...
				try
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_assign_service_port__FINEST",
						        new Object[] {Thread.currentThread().getName(), String.valueOf(port)});				
					}
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_assign_service_port_complete__FINEST",
						        new Object[] {Thread.currentThread().getName(), String.valueOf(port)});				
					}
				}
				catch (Exception e)
			...
			else if (cmd.equals("resolve"))
			{
				...
				if ( publicVNSHost == null || publicVNSHost.trim().length() == 0 || 
				     publicVNSPort == null || publicVNSPort.trim().length() == 0)
				{
					if ( UIMAFramework.getLogger().isLoggable(Level.WARNING) )
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_unknown_vns_command__WARNING",
						        new Object[] {Thread.currentThread().getName()});				
					}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_vns_redirect__FINEST",
	...
	public void shutdown()
	{
		if ( UIMAFramework.getLogger().isLoggable(Level.INFO) )
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_vns_shutdown__INFO",
			        new Object[] {Thread.currentThread().getName()});				
		}
		try
		{
			...
			if (server != null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_vns_stopped_serving__INFO",
					        new Object[] {Thread.currentThread().getName()});				
				}
			}
		}
	...
	public void run()
	{
		...
		while (!done)
		{
			try
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_launching_local_vns__INFO",
					        new Object[] {Thread.currentThread().getName(), String.valueOf(vnsPort)});				
				}
			...
			catch (Exception e)
			{
				...
				if ( UIMAFramework.getLogger().isLoggable(Level.SEVERE) )
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_launching_local_vns_failed__SEVERE",
