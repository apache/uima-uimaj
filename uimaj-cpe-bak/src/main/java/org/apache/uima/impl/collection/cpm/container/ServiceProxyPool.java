...
/**
 * Pool containing and managing instances of CasProcessors. Managed by the container
 * the pool facilitates check out and check in of Cas Processors.  
 * 
 * 
 */
public class ServiceProxyPool
{
	 ...
	 * 
	 * @return a Resource for use by the client.  Returns <code>null</code>
	 *   if none are available (in which case the client may wait on this object
		...
		if (!mFreeInstances.isEmpty())
		{
			if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checking_out_cp_from_pool__FINEST",
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cp_pool_size__FINEST",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(mAllInstances.size()), String.valueOf(mFreeInstances.size())});
			}
			return r;
		}
		else
		{
			if ( UIMAFramework.getLogger().isLoggable(Level.WARNING) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_pool_empty__WARNING",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(mAllInstances.size()), String.valueOf(mFreeInstances.size())});
			}
			return null;
		}
	...
	/**
	 * Checks in a Resource to the pool.  Also notifies other Threads that
	 * may be waiting for available instance.
	 *
	 * @param aResource - instance of the CasProcessor to check in 
	 */
	public synchronized void checkIn(CasProcessor aResource)
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checking_in_cp_to_pool__FINEST",
			        new Object[] {Thread.currentThread().getName(),String.valueOf(mAllInstances.size()), String.valueOf(mFreeInstances.size())});
		}
		...
		if (!mAllInstances.contains(aResource) || mFreeInstances.contains(aResource))
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checking_in_invalid_cp_to_pool__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}
			if (!mAllInstances.contains(aResource) )
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_not_in_pool__FINEST",
					        new Object[] {Thread.currentThread().getName()});
				}
			}
			else
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_already_checked_in__FINEST",
					        new Object[] {Thread.currentThread().getName()});
				}
			}
		}
	...
	/**
	 * Checks out a Resource from the pool.  If none is currently available,
	...
	/**
	 * Gets the size of this pool (the total number of instances 
	 * that it can hold).
	 * 
	...
	public void addCasProcessor(CasProcessor aCasProcessor)
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_add_cp_to_pool__FINEST",
			        new Object[] {Thread.currentThread().getName(), aCasProcessor.getProcessingResourceMetaData().getName()});
		}
	...
	/**
	 * Utility method used in the constructor to fill the pool with Resource
	 * instances.
	 *
	 * @param aResourceSpecifier specifier that describes how to create the
	 *    Resource instances for the pool
	 * @param aResourceClass class of resource to instantiate
	 * @param aResourceInitParams initialization parameters to be passed to the
	 ...
	 *    created
	 */
	protected void fillPool(BoundedWorkQueue portQueue, Map initParams) throws ResourceInitializationException
	{
