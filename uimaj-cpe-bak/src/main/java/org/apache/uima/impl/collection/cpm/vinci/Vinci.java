...
public class Vinci
{
	 ...
	 * 
	 * @param errorMsg - error message to place in the error frame
	 * 
	 ...
	 * 
	 * @param {@link org.apache.vinci.transport.VinciFrame} containing result of thsi service operation
	 * 
	...
	public static String extractKEYSAsString(AFrame frame)
	{
		...
		{
			keys = frameAsString.substring(frameAsString.indexOf("KEYS") + 5, frameAsString.indexOf("</KEYS>"));
		}
		return keys;
	}
	...
	public static String getContentFromDATACas(CasData aCas)
	{
		...
		{
			FeatureStructure fs = (FeatureStructure) it.next();
			if (org.apache.uima.impl.collection.cpm.Constants.CONTENT_TAG.equals(fs.getType()))
			{
				return ((PrimitiveValue) fs.getFeatureValue(org.apache.uima.impl.collection.cpm.Constants.CONTENT_TAG_VALUE)).toString();
			}
		}
		return "";
	}
