...
package org.apache.uima.impl.collection.cpm.utils;
...
{
    public static final String CPM_LOG_RESOURCE_BUNDLE = "org.apache.uima.impl.collection.cpm.cpm_messages";
	
	private static UimaTimer timer = null;
	/**
	 * Currently, this returns initialized array of Strings. 
	 * 
	 * @param aKeyDropMapFile - a file containing a list of features that should
	 *     be removed from CAS being sent to Cas Processor. Currently not used. 
	 * 
	 ...
	 * 
	 * @param aPT - Event container
	 * @param eventName - name of the event for which the time is needed
	...
	/**
	 * Finds an occurance of the ##CPM_HOME in a value parameter and returns it with
	 * an expanded form (ie.c:/cpm/...) based on the env variable CPM_HOME.
	 * 
	 */
	 ...
	 * 
	 * @param path String - XPath path to a node
	...
	private static String extractText(Node aNode) throws Exception
	{
		...
		{
			Node achild = (Node) children.item(i);
	 ...
	 */
	private static ConfigurableFeature getConfigurableFeature(Node entityNode) throws ConfigurationException //SITHException
	{
			...
			for (int i = 0; i < children.getLength(); i++)
			{
				...
				if (achild.getNodeType() == Node.ELEMENT_NODE && achild.getNodeName().equals("name"))
				{
					...
					for (int j = 0; j < atts.getLength(); j++)
					{
						...
						Node attribute = (Node) atts.item(j);
	...
	public static File findDeployDirectory(String aServiceName) throws Exception
	{
		...
		{
			throw new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_service_not_defined__WARNING",new Object[] {Thread.currentThread().getName(),"NULL"}) );
		}
	...
	private static File[] getDirectories() throws Exception
	{
		...
		{
			throw new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_not_directory__WARNING",new Object[] {Thread.currentThread().getName(),appRoot}) );
		}
	...
	public static int getFeatureAsInt(CAS aCas, Feature aFeature, String aName ) throws Exception
	{
			...
			FeatureStructure documentMetaData = aCas.getView(CAS.NAME_DEFAULT_SOFA).getDocumentAnnotation().getFeatureValue(aFeature);
			return documentMetaData.getIntValue(seqNo2);
	}
	/**
	 * Returns a value associated with a given feature
	 * 
	 * @param aCas - Cas containing data to extract
	 * @param aFeature - feature to locate in the CAS
	 ...
	 * 
	 * @param aCas - Cas to extract chunk metadata from
	...
	public static synchronized ChunkMetadata getChunkMetadata(CAS aCas)
	{
		...
		if ( feat != null )
		{
			try
			{
				...
				ChunkMetadata cm = new ChunkMetadata(String.valueOf(docId), sequenceNo, isCompleted == 1 ? true : false);
			...
			catch( NullPointerException e)
			{
				...
				{
					Exception newE = new CPMException("Possible misconfiguration. CPM configured to use chunking but chunk metadata is not present in the CAS. Check if the CAS has been properly initialized by the CollectionReader.");
