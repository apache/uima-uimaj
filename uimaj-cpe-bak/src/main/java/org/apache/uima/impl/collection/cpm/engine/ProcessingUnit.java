...
import org.apache.uima.collection.base_cpm.container.KillPipelineException;
import org.apache.uima.collection.base_cpm.container.ProcessingContainer;
import org.apache.uima.collection.base_cpm.container.ServiceConnectionException;
import org.apache.uima.collection.base_cpm.container.deployer.CasProcessorDeployer;
import org.apache.uima.collection.metadata.CpeConfiguration;
import org.apache.uima.impl.collection.CasConverter;
import org.apache.uima.impl.collection.EntityProcessStatusImpl;
import org.apache.uima.impl.collection.cpm.Constants;
import org.apache.uima.impl.collection.cpm.container.CasObjectNetworkCasProcessorImpl;
import org.apache.uima.impl.collection.cpm.container.NetworkCasProcessorImpl;
import org.apache.uima.impl.collection.cpm.container.ProcessingContainer_Impl;
import org.apache.uima.impl.collection.cpm.utils.CPMUtils;
import org.apache.uima.impl.collection.cpm.utils.ChunkMetadata;
import org.apache.uima.impl.collection.cpm.utils.CpmLocalizedMessage;
import org.apache.uima.impl.collection.cpm.vinci.DATACasUtils;
import org.apache.uima.impl.util.ProcessTrace_impl;
...
/**
 * This component executes the processing pipeline. Running in a seperate thread
 * it continuously reads bundles of Cas from the Work Queue filled by {@link ArtifactProducer}
 * and sends it through configured CasProcessors. The sequence in which CasProcessors are
 * invoked is defined by the order of Cas Processor listing in the cpe descriptor.
 * The results of analysis produced be Cas Processors is enqueued onto an output queue
 * that is shared with Cas Consumers.  
 * 
 * 
 */
public class ProcessingUnit extends Thread
{
	 ...
	 * Initialize the PU
	 * 
	 * @param acpm - component managing life cycle of the CPE
	 * @param aInputQueue - queue to read from
	 * @param aOutputQueue - queue to write to
	 */
	public ProcessingUnit(CPMEngine acpm, BoundedWorkQueue aInputQueue, BoundedWorkQueue aOutputQueue)
	{
		...
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_initialize_pipeline__FINEST",
			        new Object[] {Thread.currentThread().getName(),workQueue.getName(), String.valueOf(workQueue.getCurrentSize())});
		}
	}
	 ...
	 * 
	 * @param acpm - reference to the contrlling engine
	 */
	...
	/**
	 * Null out fields of this object.  Call this only when this object is
	 * no longer needed. 
	 */
	public void cleanup()
	{
	 ...
	 * 
	 * @param aDoNotify  - true if notification is required, false otherwise
	 */
	 ...
	 * 
	 * @param aListener - {@link org.apache.uima.collection.base_cpm.BaseStatusCallbackListener} instance
	 */
	 ...
	 * 
	 * @return - lif of {@link org.apache.uima.collection.base_cpm.BaseStatusCallbackListener} instances 
	 */
	 ...
	 * 
	 * @param aListener - object to remove from the list
	 */
	 ...
	 * 
	 * @param aProcessingUnitProcessTrace - object to compile stats
	 */
	 ...
	 * 
	 * @param aTimer - custom timer to use
	 */
	public void setUimaTimer(UimaTimer aTimer)
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_timer_class__FINEST",
			        new Object[] {Thread.currentThread().getName(),timer.getClass().getName()});
		}
	}
	/** 
	 * Plugs in a list of Cas Processor containers. During processing Cas Processors in this list
	 * are called sequentially. Each Cas Processor is contained in the container that is managing
	 * errors, counts and totals, and restarts.  
	 * 
	 * @param aProcessor CASProcessor to be added to the processing pipeline
	 **/
	...
	/** 
	 *
	 * Disable a CASProcessor in the processing pipeline. Locate it by provided index.
	 * The disabled Cas Processor remains in the Processing Pipeline, however it is 
	 * not used furing processing. 
	 * 
	 * @param aCasProcessorIndex - location in the pipeline of the Cas Processor to delete
	 **/
	public void disableCasProcessor(int aCasProcessorIndex)
	{
		...
		if (pc != null)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_disabled_cp__FINEST",
	 ...
	 * 
	 * @param aCasProcessorName - a name of the Cas Processor to disable
	 **/
	 ...
	 * 
	 * @param aCasProcessorName - name of the Cas Processor to enable
	 **/
	public void enableCasProcessor(String aCasProcessorName)
	{
		for (int i = 0; i < processContainers.size(); i++)
		{
			...
			if (pc.getName().equals(aCasProcessorName))
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_enabled_cp__FINEST",
					        new Object[] {Thread.currentThread().getName(),pc.getName()});
				}
	...
	private ProcessTrace getProcessTrace()
	{
		...
		if (timer != null)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_use_custom_timer__FINEST",
				        new Object[] {Thread.currentThread().getName(),timer.getClass().getName()});
			}
		...
		else
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_use_default_timer__FINEST",
	...
	/**
	 * Handles EOFToken. This object is received when the CPM terminates. This token is passed to each running processing
	 * thread and cas consumer thread to allow orderly shutdown. The EOFToken may be generated by ArtifactProducer if
	 * end of collection is reached, or the CPM itself can place it in the Work Queue to force all processing threads to
	 * stop. 
	 * 
	 * @throws Exception
	 */
	private void handleEOFToken() throws Exception
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_got_eof_token__FINEST",
			        new Object[] {Thread.currentThread().getName()});
		}
		//	Add EOF Token back to the work queue so that the next processing thread (if there is one) can terminate. There will be more
		...
		if (!isCasConsumerPipeline())
		{
			...
			if ( outputQueue != null )
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_placed_eof_in_queue__FINEST",
			...
			if ( outputQueue != null )
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_placed_eof_in_queue__FINEST",
					        new Object[] {Thread.currentThread().getName(), outputQueue.getName()});
				}
			}
		}
	...
	/**
	 * Release CAS back to the CAS Pool. This method is only used when chunk-aware queue is used. When a document is chunked
	 * each chunk represents a portion of the document. These chunks are ingested in sequential order by the Cas Consumer.
	 * The delivery of chunks in the correct sequence ( chunk seg 1 before chunk sequence 2) is guaranteed. Since chunks are
	 * processed asynchronously ( if multi pipeline configuration is used), they may arrive in the queue out of sequence.
	 * If this happens the Cas Consumer will wait for an expected chunk sequence. If such chunk does not arrive in configured
	 * interval the entire sequence ( all related chunks (CASes) ) are invalidated. Invalidated in the sense that they are 
	 * marked as timed out. Each CAS will be released back to the CAS Pool.
	 *  
	 * @param artifact - an array of CAS instances
	 */
	...
	private void isCpmPaused()
	{
		...
		if (cpm.isPaused())
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pausing_pp__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_resuming_pp__FINEST",
				        new Object[] {Thread.currentThread().getName()});
			}
		}
	...
	/**
	 * 	Starts the Processing Pipeline thread. This thread waits for an artifact to arrive on configured Work Queue.
	 * 	Once the CAS arrives, it is removed from the queue and sent through the analysis pipeline. 
	 */
	public void run()
	{
		if ( !cpm.isRunning() )
		{
			...
			UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cpm_not_running__WARNING",
			        new Object[] {Thread.currentThread().getName()});
			return;
		}
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_start_pp__FINEST",
			        new Object[] {Thread.currentThread().getName(), workQueue.getName(), String.valueOf(workQueue.getCurrentSize())});
		}
		...
		while (run)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_dequeue_artifact__FINEST",
				        new Object[] {Thread.currentThread().getName(), workQueue.getName(), String.valueOf(workQueue.getCurrentSize())});
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_maxwait_for_artifact__FINEST",
				        new Object[] {Thread.currentThread().getName(), String.valueOf(maxWaitTimeForEntity)});
			}
			...
			try
			{
				if (entity instanceof WorkUnit)
				{
					...
					if (((WorkUnit) entity).isTimedOut() && artifact instanceof CAS[])
					{
						...
						for (int i = 0; i < artifact.length && artifact[i] != null; i++)
						{
							...
							if (meta != null)
							{
								EntityProcessStatusImpl enProcSt = new EntityProcessStatusImpl(processingUnitProcessTrace);
				...
				if (artifact instanceof Object[])
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_processNext__FINEST",
						        new Object[] {Thread.currentThread().getName()});
					}
					/* *********** EXECUTE PIPELINE ************ */
					...
					if (System.getProperty("DEBUG_EVENTS") != null)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_dump_events__FINEST",
							        new Object[] {Thread.currentThread().getName()});
						}
				...
				else
				{
					if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
					{
						if (artifact != null)
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_unexpected_artifact_type__SEVERE",
							        new Object[] {Thread.currentThread().getName(),artifact.getClass().getName()});
						}
						else
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_artifact_null__SEVERE",
							        new Object[] {Thread.currentThread().getName()});
						}
					}
				}
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exit_pp__FINEST",
			        new Object[] {Thread.currentThread().getName(), workQueue.getName(), String.valueOf(workQueue.getCurrentSize())});
		}
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pp_terminated__FINEST",
			        new Object[] {Thread.currentThread().getName()});
		}
		isRunning = false;
	...
	/**
	 *	Releases all CAS instances from the Cache back to the Cas Pool. Cas Cache is used as optimization to store CAS in case 
	 *	it is needed for conversion. Specifically, in configurations that use XCAS and CAS based AEs.  
	 *
	 */
		...
		if (casCache != null)
		{
			for (int index = 0; index < casCache.length; index++)
			{
				if (casCache[index] != null)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_release_cas_from_cache__FINEST",
						        new Object[] {Thread.currentThread().getName() });
					}
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_release_cas_from_cache_done__FINEST",
						        new Object[] {Thread.currentThread().getName() });
					}
				}
			}
	...
	public boolean consumeQueue()
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_dequeued_artifact__FINEST",
			        new Object[] {Thread.currentThread().getName(),workQueue.getName() });
		}
		if (artifact != null)
		{
			try
			{
				...
				if (artifact instanceof Object[])
				{
					...
					if (oList[0] != null && !(oList[0] instanceof EOFToken))
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_processNext_done__FINEST",
							        new Object[] {Thread.currentThread().getName()});
						}
						synchronized (processingUnitProcessTrace)
						{
			...
			{
				UIMAFramework.getLogger(this.getClass()).log(Level.FINEST,"", e);
			}
		}
		return false;
	}
	/**
	 * Executes the processing pipeline. Given bundle of Cas instances is processed by each Cas Processor
	 * in the pipeline. Conversions between different types of Cas Processors
	 * is done on the fly. Two types of Cas Processors are currently supported:
	 * 
	 * <li> CasDataProcessor</li> 
	 * <li> CasObjectProcessor</li>
	 * 
	 * The first operates on instances of CasData the latter operates on instances of CAS.
	 * The results produced by Cas Processors are added to the output queue.
	 * 
	...
	throws ResourceProcessException, IOException, CollectionException, AbortCPMException, KillPipelineException
	{
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_start_analysis__FINEST",
			        new Object[] {Thread.currentThread().getName()});
		}
		//		String lastDocId = "";
		...
		if (aCasObjectList == null || aCasObjectList[0] == null)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_invalid_cas_reference__SEVERE",
				        new Object[] {Thread.currentThread().getName()});
			}
			return false;
		}
		Object[] casObjects = null;
		//	Determine if the Cas'es contained in the CasList are of type CAS. Samples the first CAS in the list.
		//	The list contains CASes of the same type ( either CasData or CAS ). Mixed model not supported.
		boolean isCasObject = aCasObjectList[0] instanceof CAS;
		//		String docid = "";
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_entering_pipeline__FINEST",
			        new Object[] {Thread.currentThread().getName()});
		}
		...
		for (int i = 0; processContainers != null && i < processContainers.size(); i++)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_retrieve_container__FINEST",
			...
			synchronized (container)
			{
				...
				if (!isProcessorReady(container.getStatus()))
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_container_not_ready__FINEST",
			...
			if (!isCasObject && !container.processCas(aCasObjectList))
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_skip_CAS__FINEST",
					        new Object[] {Thread.currentThread().getName(), container.getName() });
				}
			...
			do
			{
				if (System.getProperty("SHOW_MEMORY") != null)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_memory__FINEST",
						        new Object[] {Thread.currentThread().getName(), String.valueOf(Runtime.getRuntime().totalMemory() / 1024), String.valueOf(Runtime.getRuntime().freeMemory() / 1024)});
					}
				}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkout_cp_from_container__FINEST",
					        new Object[] {Thread.currentThread().getName(), container.getName()});
				}
				...
				if (processor == null)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkout_null_cp_from_container__SEVERE",
						        new Object[] {Thread.currentThread().getName(), container.getName()});
					}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkedout_cp_from_container__FINEST",
					        new Object[] {Thread.currentThread().getName(), container.getName(), processor.getClass().getName()});
				}
				try
				{
					if (processor instanceof CasDataProcessor)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cas_data_processor__FINEST",
							        new Object[] {Thread.currentThread().getName(), container.getName(), processor.getClass().getName()});
						}
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_process__FINEST",
							        new Object[] {Thread.currentThread().getName(), container.getName(), processor.getClass().getName()});

							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_casObjects_class__FINEST",
						...
						if (!(casObjects instanceof CasData[]))
						{
							...
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_expected_casdata__FINEST",
								        new Object[] {Thread.currentThread().getName(),  casObjects.getClass().getName()});
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_process_completed__FINEST",
							        new Object[] {Thread.currentThread().getName(),  container.getName(), processor.getClass().getName()});
						}
					...
					else if (processor instanceof CasObjectProcessor)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_casobject_processor__FINEST",
							        new Object[] {Thread.currentThread().getName(), container.getName(), processor.getClass().getName()});
						...
						for (int casIndex = 0; casIndex < aCasObjectList.length; casIndex++)
						{
							...
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_initialize_cas__FINEST",
								        new Object[] {Thread.currentThread().getName(), container.getName() });
							}
							if (aCasObjectList[casIndex] == null)
							{
								...
								{
									UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
									        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_casobjectlist_is_null__SEVERE",
									        new Object[] {Thread.currentThread().getName(), container.getName(), String.valueOf(casIndex) });
								}
								break;
							}
							if (isCasObject == false)
							{
								//	The following may be true if the CollectionReader is CasData based and this is the first CasObject based annotator in the chain.
								if (casCache == null || casCache[casIndex] == null)
								{
									...
									while (casList[casIndex] == null)
									{
										...
										{
											UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
											        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_get_cas_from_pool__FINEST",
											        new Object[] {Thread.currentThread().getName(), container.getName()});
										}
										...
										{
											UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
											        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_got_cas_from_pool__FINEST",
											        new Object[] {Thread.currentThread().getName(), container.getName()});
										}
									}
									if (casList[casIndex] != null)
									{
										...
										{
											UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
											        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_call_cas_reset__FINEST",
											        new Object[] {Thread.currentThread().getName(), container.getName() });
										}
								...
								else
								{
									...
									{
										UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
										        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_nullify_cas__FINEST",
										        new Object[] {Thread.currentThread().getName(), container.getName() });
									}
									//	Cas is used up
					...
					if ((releaseCAS) && (i == (processContainers.size() - 1)))
					{
						...
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							...
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process", CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_notify_listeners__FINEST", new Object[] { Thread.currentThread().getName() });
						}
						threadState = 2007;
						...
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process", CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_notify_listeners__FINEST", new Object[] { Thread.currentThread().getName() });

							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process", CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_releasing_cases__FINEST", new Object[] { Thread.currentThread().getName(), container.getName(), String.valueOf(releaseCAS), String.valueOf(true) });
						}
						if (casCache != null)
						{
							clearCasCache();
						...
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process", CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_releasing_cases__FINEST", new Object[] { Thread.currentThread().getName(), container.getName() });
						}
					}
					...
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						...
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pipeline_completed__FINEST",
						        new Object[] {Thread.currentThread().getName()});
					}
					retry = false;
					//	On successfull processing reset the restart counter. Restart counter determines how many times to restart Cas Processor on the same CAS
					//	Do this conditionally. If the CAS is to be dropped on Exception this restart counter scope extends to the entire collection not just one CAS
				...
				catch (Exception e)
				{
					...
					if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
					{
						...
						UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pipeline_exception__SEVERE",
						        new Object[] {Thread.currentThread().getName(),container.getName(), e.getMessage()});
						
						UIMAFramework.getLogger(this.getClass()).log(Level.SEVERE, Thread.currentThread().getName(), e);

						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pipeline_exception__FINEST",
						        new Object[] {Thread.currentThread().getName(),container.getName(), String.valueOf(container.isPaused())});
					}
					...
					//	Check the policy to determine what to do with the CAS on exception. Return the CAS back to the pool
					//	and stop the processing chain if required. The policy for what to do with the CAS on exception is
					...
					if (cpm.dropCasOnException())
					{
						...
						UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_drop_cas__WARNING",
						        new Object[] {Thread.currentThread().getName(), container.getName() , processor.getClass().getName()});
					}
					else
					{
					...
					if (processor instanceof CasObjectNetworkCasProcessorImpl && container.isPaused())
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_container_paused_do_retry__FINEST",
							        new Object[] {Thread.currentThread().getName(), container.getName() });
						}

						//	Do conditional release of CAS instances. The release occurs only if the CasProcessor is the last one
						//	in processing chain. This only releases instances of CAS checked out from the Cas Pool. If not done, the 
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_container_paused__FINEST",
							        new Object[] {Thread.currentThread().getName(), container.getName() });
						}
						...
						if (processor != null)
						{
							...
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_ok_released_cp__FINEST",
								        new Object[] {Thread.currentThread().getName(), container.getName() });
							}
							processor = null;
						}
					...
					if (pauseContainer(container, e, threadId)) //container.isRemote() && container.isSingleFencedService() && threadId == null )
					{
						threadState = 2011;
						...
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							...
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_pausing_container__FINEST",
							        new Object[] {Thread.currentThread().getName(), container.getName() });
						}
					...
					try
					{
						...
						if (cpm.dropCasOnException())
						{
							//	Cas has already been returned to the CAS pool. The policy requires to stop the processing chain for this CAS and
					...
					catch (KillPipelineException ex)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_on_pipeline_kill__WARNING",
							        new Object[] {Thread.currentThread().getName(), container.getName(), innerE.getMessage() });
						}
//						finally
//						{
...
//						}
					...
					catch (AbortCPMException ex)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_on_cpm_kill__WARNING",
							        new Object[] {Thread.currentThread().getName(), container.getName(), innerE.getMessage() });
						}
//						finally
//						{
//							throw new AbortCPMException("Aborting CPM. CasProcessor::" + container.getName() + " Configured to Abort the CPM.");
//						}
						throw ex;
							...
							if ( rpe.getCause() != null && rpe.getCause() instanceof KillPipelineException )
							{
								...
								catch( Exception excep) 
								{
									...
									if (UIMAFramework.getLogger().isLoggable(Level.WARNING))
									{
										UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
										        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_on_pipeline_kill__WARNING",
										        new Object[] {Thread.currentThread().getName(), container.getName(), excep.getMessage() });
									}
								}
							}
							pTrTemp.endEvent(container.getName(), "Process", "failure");
					...
					catch (SkipCasException ex)
					{
						try
						{
							...
							handleSkipCasProcessor(container, aCasObjectList, (i == (processContainers.size() - 1)));
						}
						catch (Exception sEx)
						{
					...
					catch (Exception ex)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINER, this.getClass().getName(), "process",
					...
					//	CAS may have already been dropped (released) due to dropCasOnException policy defined in the CPE Descriptor
					if (!cpm.dropCasOnException())
					{
						//	Do conditional release of CAS instances. The release occurs only if the CasProcessor is the last one
						//	in processing chain. This only releases instances of CAS checked out from the Cas Pool. If not done, the 
				...
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					...
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_end_of_batch__FINEST",
					        new Object[] {Thread.currentThread().getName(),  container.getName(), processor.getClass().getName() });
				}
			...
			if (processor != null)
			{
				...
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_release_cp__FINEST",
				        new Object[] {Thread.currentThread().getName(), container.getName(), processor.getClass().getName(), String.valueOf(casCache == null)});
				...
				{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_ok_release_cp__FINEST",
						        new Object[] {Thread.currentThread().getName(), container.getName(), processorClassName, String.valueOf(casCache == null)});
				}
			}
	...
	/**
	 * Notifies application listeners of completed analysis and stores results of analysis (CAS) in the Output Queue that
	 * this thread shares with a Cas Consumer thread. 
	 * 
	 * @param aCasObjectList - List of Artifacts just analyzed
	 * @param isCasObject - determines the types of CAS just analyzed ( CasData vs CasObject)
	 * @param casObjects 
	 * @param aProcessTr - ProcessTrace object holding events and stats
	 * @param doneAlready - flag to indicate if the last Cas Processor was released back to its container
	 * 
	 * @throws Exception
	 */
	private void postAnalysis(Object[] aCasObjectList, boolean isCasObject, Object[] casObjects, ProcessTrace aProcessTr, boolean doneAlready) throws Exception
	{
		try
		{
			...
			if (!doneAlready && notifyListeners)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_notify_listeners__FINEST",
					        new Object[] {Thread.currentThread().getName()});
				}
				threadState = 2013;
				//	Notif Listeners
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_notify_listeners__FINEST",
					        new Object[] {Thread.currentThread().getName()});
				}
			}
			//	enqueue CASes. If the CPM is in shutdown mode due to hard kill dont allow enqueue of CASes
			if (outputQueue != null && (cpm.isRunning() == true || (cpm.isRunning() == false && cpm.isHardKilled() == false)))
			{
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_add_cas_to_queue__FINEST",
					        new Object[] {Thread.currentThread().getName(),outputQueue.getName(), String.valueOf(outputQueue.getCurrentSize())});
				}
		...
		finally
		{
			if (outputQueue == null && casObjects != null && casObjects instanceof CasData[])
			{
				if (System.getProperty("DEBUG_RELEASE") != null)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_with_cas__FINEST",
						        new Object[] {Thread.currentThread().getName(),String.valueOf(Runtime.getRuntime().freeMemory() / 1024)});
					}
				}
				for (int i = 0; i < casObjects.length; i++)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_local_cache__FINEST",
						        new Object[] {Thread.currentThread().getName(),String.valueOf(casCache == null)});
					}
					casObjects[i] = null;
				...
				if (System.getProperty("DEBUG_RELEASE") != null)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_total_memory__FINEST",
						        new Object[] {Thread.currentThread().getName(),String.valueOf(Runtime.getRuntime().freeMemory() / 1024)});
					}
				}
			}
	...
	/**
	 * Performs end of batch processing. It delegates the processing to Cas Processor container. The container
	 * using configuration determines if its time to call Cas Processor's batchProcessComplete() method. 
	 *  
	 * @param aContainer - container performing end of batch processing
	 * @param aProcessor - Cas Processor to call on end of batch
	 * @param aProcessTr - Process Trace to use for aggregating events
	 * @param aCasObjectList - CASes just analyzed
	 */
	private void doEndOfBatchProcessing(ProcessingContainer aContainer, CasProcessor aProcessor, ProcessTrace aProcessTr, Object[] aCasObjectList)
	{
		...
		try
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_end_of_batch_completed__FINEST",
		...
		catch (Exception ex)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_end_of_batch_exception__SEVERE",
				        new Object[] {Thread.currentThread().getName(), aContainer.getName() , ex.getMessage()});
			}
	...
	private void handleSkipCasProcessor(ProcessingContainer aContainer, Object[] aCasObjectList, boolean isLastCP) throws Exception
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_skipping_cas__FINEST",
			        new Object[] {Thread.currentThread().getName(), aContainer.getName()});
		}
			...
			finally
			{
				...
				catch (Exception ex2)
				{
					if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
					{
						...
						UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_releasing_cas__SEVERE",
						        new Object[] {Thread.currentThread().getName(), aContainer.getName()});
						UIMAFramework.getLogger(this.getClass()).log(Level.SEVERE, Thread.currentThread().getName(), ex2);
						throw ex2;
					}
				}
			}
	 ...
	 * 
	 * @param aContainer - container managing CasProcessor that failed
	 * @param aProcessor - failed CasProcessor
	 * @param aProcessTr - ProcessTrace object holding events
	...
	private void handleServiceException(ProcessingContainer aContainer, CasProcessor aProcessor, ProcessTrace aProcessTr, Exception ex) throws Exception
	{
		...
		if (aContainer.isRemote() && aContainer.isSingleFencedService())
		{
			if (Thread.currentThread().getName().equals(threadId))
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_connection_exception__FINEST",
					        new Object[] {Thread.currentThread().getName(), aContainer.getName(), aProcessor.getClass().getName() });
				}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_redeploy_cp__FINEST",
					        new Object[] {Thread.currentThread().getName(), aContainer.getName(), aProcessor.getClass().getName() });
				}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_redeploy_cp_done__FINEST",
					        new Object[] {Thread.currentThread().getName(), aContainer.getName(), aProcessor.getClass().getName() });
				}
			}
	 ...
	 * 
	 * @param aContainer - a container that manages the current Cas Processor. 
	 * @param aProcessor - a Cas Processor to be disabled
	...
	private void handleAbortCasProcessor(ProcessingContainer aContainer, CasProcessor aProcessor) throws Exception
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_disable_due_to_action__FINEST",
			        new Object[] {Thread.currentThread().getName(), aContainer.getName()});
		}
		if (aContainer.isPaused())
		{
	...
	private void handleAbortCPM(ProcessingContainer aContainer, CasProcessor aProcessor) throws Exception
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_abort_cpm__SEVERE",
			        new Object[] {Thread.currentThread().getName(), aContainer.getName()});
		...
		if (!cpm.dropCasOnException())
		{
			...
			catch (Exception exc)
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_on_cpm_kill__WARNING",
				        new Object[] {Thread.currentThread().getName(), aContainer.getName(), exc.getMessage()});
				
				UIMAFramework.getLogger(this.getClass()).log(Level.SEVERE, Thread.currentThread().getName() , exc);
			}
		}
		throw new AbortCPMException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_configured_to_abort__WARNING",new Object[] {Thread.currentThread().getName(),aContainer.getName()}) );
	...
	private void handleKillPipeline(ProcessingContainer aContainer) throws Exception
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_kill_pipeline__FINEST",
			        new Object[] {Thread.currentThread().getName(), aContainer.getName()});
		}
	...
	/**
	 * Determines if the thread should be paused. Pausing container effectively pauses ALL Cas Processors that 
	 * are managed by the container. The pause is needed when there are multiple pipelines shareing a common
	 * service. If this service dies (Socket Down), only one thread should initiate service restart. While the
	 * service is being restarted no invocations on the service should be done. Containers will be resumed on
	 * successfull service restart.  
	 * 
	 * @param aContainer - a container that manages the current Cas Processor. 
	 * @param aProcessor - a Cas Processor to be disabled
	 * @param aThreadId - id of the current thread
	 * 
	 * @throws Exception - exception 
	 */
	private boolean pauseContainer(ProcessingContainer aContainer, Exception aException, String aThreadId)
	{
		if (aContainer.isRemote() && aContainer.isSingleFencedService() && aException.getCause() instanceof ServiceConnectionException && aThreadId == null)
		{
			return true;
		}
	...
	/**
	 * Conditionally, releases CASes back to the CAS pool. The release only occurs if the Cas Processor is
	 * the last in the processing chain.
	 * 
	 * @param aCasList - list of CASes to release
	 * @param lastProcessor - determines if the release takes place
	 * @param aContainer - current container
	 */
	private void releaseCases(Object aCasList, boolean lastProcessor, String aName) //ProcessingContainer aContainer)
	{
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_releasing_cases__FINEST",
			        new Object[] {Thread.currentThread().getName(), aName, String.valueOf(releaseCAS), String.valueOf(lastProcessor)});
		}
		if (aCasList == null)
		{
			return;
		...
		if (releaseCAS && lastProcessor)
		{
			if (aCasList instanceof CAS[])
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_done_releasing_cases__FINEST",
			...
			else
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_casobject_class__FINEST",
					        new Object[] {Thread.currentThread().getName(), aName, aCasList.getClass().getName()});
				}
			}
		}
	 ...
	 * 
	 * @param aCas - object containing an array of OR a single instance of Cas
	 * @param isCasObject - true if instance of Cas is of type Cas, false otherwise
	 * @param aEntityProcStatus - status object that may contain exceptions and trace
	 */
	...
	/**
	 * Notifies all configured listeners. Makes sure that appropriate type of Cas is sent to
	 * the listener. Convertions take place to ensure compatibility.  
	 *  
	 * @param aCas - Cas to pass to listener
	 * @param isCasObject - true is Cas is of type CAS 
	 * @param aEntityProcStatus - status object containing exceptions and trace info
	 */
		...
		for (int j = 0; j < statusCbL.size(); j++)
		{
			...
			else if (statCL instanceof StatusCallbackListener)
			{
				...
				if (isCasObject == false)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_converting_CAS__WARNING",
						        new Object[] {Thread.currentThread().getName() });
					}
					casObjectCopy = conversionCas;
				}
				//	Notify the listener that the Cas has been processed
				 ((StatusCallbackListener) statCL).entityProcessComplete((CAS) casObjectCopy, aEntityProcStatus);
	...
	/**
	 *	Called by the CPMEngine during setup to indicate that this thread is supposed to release a CAS at the end of 
	 *	processing. This is typically done for Cas Consumer thread, but in configurations not using Cas Consumers 
	 *	The processing pipeline may also release the CAS.
	 * 
	 * @param aFlag - true if this thread should release a CAS when analysis is complete
	 */
	 ...
	 * 
	 * @param kill - true if CPE has been stopped before finishing processing during external stop
	 *
	 */
	public void stopCasProcessors(boolean kill)
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_stop_containers__FINEST",
			        new Object[] {Thread.currentThread().getName()});
		}
		...
		for (int i = 0; processContainers != null && i < processContainers.size(); i++)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_container_time__FINEST",
				        new Object[] {Thread.currentThread().getName(), container.getName(), String.valueOf(container.getTotalTime())});
			}
			synchronized (container)
			{
				...
				if (kill || (!cpm.isRunning() && isProcessorReady(container.getStatus())))
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_kill_cp__FINEST",
						        new Object[] {Thread.currentThread().getName(), container.getName()});
					}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_container_status__FINEST",
					        new Object[] {Thread.currentThread().getName(), container.getName(), String.valueOf(container.getStatus())});
				}
				...
				try
				{
					...
					if (deployer != null)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_undeploy_cp_instances__FINEST",
							        new Object[] {Thread.currentThread().getName(), container.getName(), deployer.getClass().getName()});
						}
						deployer.undeploy();
					}
					container.destroy();
				...
				catch (Exception e)
				{
					...
					UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_exception_during_cp_stop__WARNING",
					        new Object[] {Thread.currentThread().getName(), container.getName(), e.getMessage()});
				}
				finally
				{
	 ...
	 *  
	 * @param aCount - running total of documents processed so far
	 * 
	...
	private boolean containerDisabled(ProcessingContainer aContainer)
	{
		synchronized (aContainer)
		{
			...
			if (!isProcessorReady(aContainer.getStatus()))
			{
				...
				//	Skip any CasProcessor that is not ready to process. Cas Processors may be disabled during processing
				return true;
			}
		}
		return false;
	 ...
	 * 
	 * @param aCasObjectList - a list of CASes to analyze
	 * @param pTrTemp - process trace where statistics are added during analysis
	 * 
	 */
	protected boolean analyze(Object[] aCasObjectList, ProcessTrace pTrTemp) throws Exception // throws ResourceProcessException, IOException, CollectionException, AbortCPMException
	{
		long t1 = 0; 
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
		...
		for (int i = 0; processContainers != null && i < processContainers.size(); i++)
		{
			...
			container = (ProcessingContainer) processContainers.get(i);
			//	container can be disabled in multi-processing pipeline configurations. The container is disabled
			//	when one of the processing threads is in the process of restarting/reconnecting to a shared
			//	fenced service. Shared, meaning that all processing pipelines use the same service for invocations.
			...
			do //	Retry
			{
				...
				try
				{
					...
					//	Check to see if the CasProcessor is available for processing
					//	The CasProcessor may have been disabled due to excessive errors and error policy defined
					...
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkedout_cp_from_container__FINEST",
						        new Object[] {Thread.currentThread().getName(), containerName, processor.getClass().getName()});
					}
					t1 = System.currentTimeMillis();	
					//	*************************  P E R F O R M    A N A L Y S I S *************************
					if (processor instanceof CasDataProcessor)
					{
						invokeCasDataCasProcessor(container, processor, aCasObjectList, pTrTemp, isCasObject, retry);
						isCasObject = false;
					}
					else if (processor instanceof CasObjectProcessor)
					{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_analysis_successfull__FINEST",
						        new Object[] {Thread.currentThread().getName(), containerName, processor.getClass().getName()});
					}
					retry = false;
					//	On successfull processing reset the restart counter. Restart counter determines how many times to restart Cas Processor on the same CAS
				...
				finally
				{
					...
					if (processor != null)
					{
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_release_cp__FINEST",
							        new Object[] {Thread.currentThread().getName(), containerName, processor.getClass().getName(), String.valueOf(casCache == null)});
						}
						t1 = System.currentTimeMillis();	
	 ...
	 * 
	 * @param e - exception in the main processing loop
	 * @param aContainer - current container of the Cas Processor 
	 * @param aProcessor - current Cas Processor
	 * @param aProcessTrace - an object containing stats for this procesing loop
	 * @param aCasObjectList - list of CASes being analyzed
	 * @param isCasObject - determines type of CAS in the aCasObjectList ( CasData or CasObject)
	 * @return boolean
	 * @throws Exception
	 */
	private boolean handleErrors(Throwable e, ProcessingContainer aContainer, CasProcessor aProcessor, ProcessTrace aProcessTrace, Object[] aCasObjectList, boolean isCasObject) throws Exception
	{
		...
		if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
		{
			UIMAFramework.getLogger(this.getClass()).log(Level.SEVERE, Thread.currentThread().getName() , e);
			UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_handle_exception__SEVERE",
			        new Object[] {Thread.currentThread().getName(), containerName , aProcessor.getClass().getName(),  e.getMessage()});
		}
		...
		if (cpm.dropCasOnException())
		{
			...
			UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_drop_cas__WARNING",
			        new Object[] {Thread.currentThread().getName(), containerName , aProcessor.getClass().getName()});
		...
		catch (AbortCPMException ex)
		{
			...
			throw new AbortCPMException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_configured_to_abort__WARNING",new Object[] {Thread.currentThread().getName(),containerName}) );
		} //	check if the CasProcessor is to be disabled due to excessive errors
		catch (AbortCasProcessorException ex)
		{
			retry = false;
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_disable_cp__SEVERE",
				        new Object[] {Thread.currentThread().getName(), aProcessor.getClass().getName() });
			}
			if (aContainer.isPaused())
			{
	...
	private void invokeCasObjectCasProcessor(ProcessingContainer container, CasProcessor processor, Object[] aCasObjectList, ProcessTrace pTrTemp, boolean isCasObject) throws Exception
	{
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			...
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_invoke_cp_process__FINEST",
			        new Object[] {Thread.currentThread().getName(), container.getName(), processor.getClass().getName() });
		}
		casList = new CAS[aCasObjectList.length];
		for (int casIndex = 0; casIndex < aCasObjectList.length; casIndex++)
