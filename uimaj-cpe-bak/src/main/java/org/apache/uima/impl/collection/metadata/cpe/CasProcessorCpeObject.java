...
import org.apache.uima.impl.collection.metadata.CpeDefaultValues;
 ...
 * Provides support for getting and setting common configuration settings shared by all
 * CpeCasProcessor objects
 * 
 * 
 */
public class CasProcessorCpeObject extends MetaDataObject_impl implements CpeCasProcessor 
{
	...
	/**
	 * Associates deployment type with for this CasProcessor. Three types are currently
	 * supported:
	 * <ul>
	 ...
	 * </ul>
	 * @param aDeployMode - String identifying deployment type
	 * @throws CpeDescriptorException - if invalid deployment type is provided
	 */
	public void setDeployment(String aDeployMode) throws CpeDescriptorException
	{
		...
		throw new CpeDescriptorException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_Exception_invalid_deployment__WARNING",new Object[] {Thread.currentThread().getName(),"", aDeployMode}) );
	}
	/**
	 ...
	 * 
	 * @param aDescriptorPath - path to the descriptor
	...
	/**
	 * Returns the {@link CpeComponentDescriptor} instance associated with this Cas Processor.
	 * That object contains a path to the component descriptor.
	 * 
	...
	/**
	 * Associates a filter string with this CasProcessor. A filter provides a mechanism that facilitates efficient 
	 * routing of Cas's to the CasProcessor.  
	 * 
	 * @param aFilterExpression - String containing a filter
	 */
	...
	public String getCasProcessorFilter()
	{
		...
		if (filter != null)
		{
			if (filterString.indexOf(org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM) > -1)
			{
				filterString = StringUtils.replaceAll(filterString, org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM, org.apache.uima.impl.collection.cpm.Constants.LONG_COLON_TERM);
			}
			if (filterString.indexOf(org.apache.uima.impl.collection.cpm.Constants.LONG_DASH_TERM) > -1)
			{
				filterString = StringUtils.replaceAll(filterString, org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM, org.apache.uima.impl.collection.cpm.Constants.LONG_DASH_TERM);
			}
		}
		return filterString;
	}
	 ...
	 * 
	 * @param aBatchSize - batch size of this CasProcessor
	 */
	...
	/**
	 * Deletes a given param from a param list if it exists. Returns a position in the current Param List for 
	 * a given 'aParamName'.
	 * 
	 * @param aParamName - name of the param to find.
	 * 
	...
	/**
	 * Adds a given deployment param to the param list. If a param with a given name exists in the list
	 * its value will be over-written.
	 * 
	 * @param aParamName - name of the new parameter
	 * @param aParamValue - value of the new parameter
	 * 
	 * @throws CpeDescriptorException
	 */
	public void addDeployParam(String aParamName, String aParamValue) throws CpeDescriptorException
	{
		...
		if ( !found )
		{
			deploymentParameters.add(new CasProcessorDeploymentParamImpl(aParamName, aParamValue, "string"));
		}
	}
	/**
	 * 
	 ...
	 * 
	 * @return @see org.apache.uima.collection.metadata.CasProcessorDeploymentParams instance
	 */
	 ...
	 * 
	 * 
	 * @param aErrorCount - max error tolerance
	 */
	...
	/**
	 * Returns error sample size. The value is used to determine the max error tolerance for 
	 * this CasProcessor. Error thresholds are defines as quotients. Error Count /  Sample Size
	 * for example, 3/1000, which means 3 errors per thousand.
	 *  
	 ...
	 * 
	 * @param aAction - action as string
	 * 
	...
	/**
	 * Associates action in the event CasProcessor restarts exceed max tolerance. In such case, the action
	 * determines appropriate strategy ( terminate, continue, disable).
	 * 
	 * @param aAction - action string
	 */
	 ...
	 * 
	 * @param aRestartCount - max number of restarts 
	 */
	...
	/**
	 * Associates timeout in terms of ms, with this CasProcessor. It is the max number of millis
	 * to wait for response.
	 *  
	 * @param aTimeoutValue - millis to wait for response
	 */
	...
	public void setParameterSettings(ConfigurationParameterSettings settings)
	{
		...
		{
			configurationParameterSettings = new CasProcessorConfigurationParameterSettingsImpl(parameterSettings);
		}
	}
	/**
	 * @param settings
	...
	public void buildFromXMLElement(Element aElement, XMLParser aParser, ParsingOptions aOptions) throws InvalidXMLException
	{
		...
		catch( Exception e)
		{
			throw new InvalidXMLException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
					"UIMA_CPM_EXP_missing_attribute_from_xml_element__WARNING",new Object[] {Thread.currentThread().getName(),"casProcessor",field,"casProcessor"});
	...
	static final private XmlizationInfo XMLIZATION_INFO = 
		new XmlizationInfo("casProcessor", new PropertyXmlInfo[] 
			{   
				new PropertyXmlInfo("cpeComponentDescriptor", null), 
