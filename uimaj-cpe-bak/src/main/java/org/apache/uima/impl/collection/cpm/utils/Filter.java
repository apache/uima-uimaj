...
public class Filter
{
		...
		for( int i=0; i < expressionList.size(); i++ )
		{
			...
			if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_expression__FINEST",
	 ...
	 * Parses tokens
	 * 
	 * @param aTokenizer - tokenized filter expression
	 * 
	 * @throws ParseException
	 */
	...
	public class Expression
	{
		...
		{
			//	Already defined as AND filter. Currently filtering is either AND or OR. No mixing is supported
		...
		{
			//	Already defined as OR filter. Currently filtering is either AND or OR. No mixing is supported
