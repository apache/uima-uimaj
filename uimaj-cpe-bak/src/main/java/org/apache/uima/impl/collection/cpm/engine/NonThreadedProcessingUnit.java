...
public class NonThreadedProcessingUnit
{
	...
	private void invokeCasDataCasProcessor(ProcessingContainer container, CasProcessor processor, Object[] aCasObjectList, ProcessTrace pTrTemp, boolean isCasObject, boolean retry) throws Exception
	{
		...
		if (!retry)
		{
			for (int casIndex = 0; casIndex < aCasObjectList.length; casIndex++)
			{
				byteCount = getBytes((CasData) aCasObjectList[casIndex]);
		...
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        ...
			        new Object[] {Thread.currentThread().getName(), container.getName(), casObjects.getClass().getName() });
		}
		if (!(casObjects instanceof CasData[]))
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_expected_casdata_class__FINEST",
				        new Object[] {Thread.currentThread().getName(), container.getName(), casObjects.getClass().getName() });
			}
		}
		...
		for (int j = 0; j < statusCbL.size(); j++)
		{
			...
			else if (statCL instanceof StatusCallbackListener)
			{
				...
				if (conversionCas != null)
				{
					...
					conversionCas = null;
					if (casCache != null && casCache[0] != null && casCache[0] instanceof CAS)
					{
						((CAS) casCache[0]).reset();
					}
				}
	...
	private void releaseCases(Object aCasList, boolean lastProcessor, String aName) //ProcessingContainer aContainer)
	{
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        ...
			        new Object[] {Thread.currentThread().getName(), aName, String.valueOf(relaseCAS), String.valueOf(lastProcessor)});
		}
		if (aCasList == null)
		{
			return;
