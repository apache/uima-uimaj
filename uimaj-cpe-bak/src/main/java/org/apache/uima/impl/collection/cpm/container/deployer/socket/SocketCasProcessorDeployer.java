...
package org.apache.uima.impl.collection.cpm.container.deployer.socket;
...
/**
 * Reference implementation of the {@link CasProcessorDeployer} component responsible for
 * launch and termination of the fenced CasProcessor. It uses a plug-in 
 * {@link ProcessControllerAdapter} object to delegate launch requests from the CPM
...
public class SocketCasProcessorDeployer implements CasProcessorDeployer
{
	...
	private URL[] serviceUrls = null; //changed to URL[] since there are multiple instances of service -Adam
	...
	public ProcessingContainer deployCasProcessor(List aCasProcessorList, boolean redeploy) throws ResourceConfigurationException
	{
		...
		if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploy_services__FINEST",
			        Thread.currentThread().getName());				
		}
		if ( controller == null )
		{
			throw new ResourceConfigurationException(ResourceInitializationException.CONFIG_SETTING_ABSENT, 
		...
		try
		{
		  ...
		  ServiceProxyPool casProcessorPool = new ServiceProxyPool();
			...
			for (int i = 0; i < aCasProcessorList.size(); i++)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploying_service__FINEST",
					        new Object[] {Thread.currentThread().getName(), name});				
				}
				//	Launch one instance of the fenced CasProcessor
//				URL[] urls = controller.deploy(name, 1);  //Commented out by Adam -- deployment is now done outside of loop
				if (cProcessor instanceof CasObjectNetworkCasProcessorImpl)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_deployed__FINEST",
						        new Object[] {Thread.currentThread().getName(), name});				
					}
				}
		...
		try
		{
			if (aProcessingContainer.isSingleFencedService())
			{
				...
				if ( pool == null )
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_no_service_proxy__SEVERE",
						        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName()});				
					}
					throw new ResourceConfigurationException( CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
							"UIMA_CPM_no_service_proxy__SEVERE", new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName()});
				...
				while (totalPoolSize != pool.getSize())
				{
					try
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_wait_for_service_proxy_pool__FINEST",
							        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName()});				
						}
						pool.wait(50);
					}
	...
	public void undeploy(URL aURL) throws CasProcessorDeploymentException
	{
		try
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_undeploying_service__FINEST",
				        new Object[] {Thread.currentThread().getName(), aURL.getHost(), String.valueOf(aURL.getPort())});				
