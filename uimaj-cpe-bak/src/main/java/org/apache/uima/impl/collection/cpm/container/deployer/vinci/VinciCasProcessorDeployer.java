...
package org.apache.uima.impl.collection.cpm.container.deployer.vinci;
...
import org.apache.uima.impl.collection.cpm.container.CasProcessorConfigurationJAXBImpl;
...
import org.apache.uima.impl.collection.cpm.container.ServiceProxyPool;
import org.apache.uima.impl.collection.cpm.container.deployer.VinciTAP;
import org.apache.uima.impl.collection.cpm.container.deployer.vns.LocalVNS;
import org.apache.uima.impl.collection.cpm.container.deployer.vns.VNSQuery;
import org.apache.uima.impl.collection.cpm.container.deployer.vns.VinciServiceInfo;
import org.apache.uima.impl.collection.cpm.engine.BoundedWorkQueue;
import org.apache.uima.impl.collection.cpm.engine.CPMEngine;
...
/**
 * Reference implementation of {@link org.apache.uima.collection.base_cpm.container.deployer.CasProcessorDeployer}
 * This component enables the CPE to deploy Cas Processors running as a Vinci service. Two deployment models
 * are supported in the current implementation:
 * 
 * <li>managed deployment (aka local)</li>
 * <li>unmanaged deployment (aka remote)</li>
 * 
 * Managed deployment gives the CPE control over the life cycle of the Cas Processor. The CPE starts, restarts and
 * shuts down the Cas Processor running as a Vinci service. This service is launched on the same machine as the 
 * CPE but in a seperate process.
 * 
 * Unmanaged deployment does not provide the CPE control over the Cas Processor that may be running on a remote
 * machine. The CPE assumes that such Cas Processor is managed by a separate application and is always available.
 * 
 * For the managed deployment the CPE uses its internal VNS (Vinci Naming Service) to which Cas Processor 
 * must connect to.  The VNS issues a port for the Cas Processor to run on and creates a proxy to it.
 * 
 * For unmanaged Cas Processor the CPE creates a proxy to remote service. The remote Cas Processor service
 * is discovered with help of VNS running on a host and port defined in the Cpe descriptor for this Cas Processor. 
 * 
 */
public class VinciCasProcessorDeployer implements CasProcessorDeployer
{
	...
	// Local VNS is a shared instance across all instances of VinciCasProcessorDeployer. It uses shared
	//	portQueue (defined below). VNS and the queue are instantiated once. 
	private static LocalVNS vns = null;
	private int restartCount = 0;
	//	Pool for port numbers assigned to Cas Processors running as vinci services deployed as managed Cas Processors
	...
	/**
	 * Deploys integrated Cas Processor. Number of instances this routine actually deploys 
	 * depends on number of processing threads defined in the CPE descriptor. There is one
	 * instance per processing thread created here. The <i>aCasProcessorList</i> contains
	...
	/**
	 * Deploys CasProcessor using configuration from provided container. This method is used
	 * for re-launching failed Cas Processor.
	 * 
	 * @param aProcessingContainer - container for deployed CasProcessor.  	 
	 */
	public void deployCasProcessor(ProcessingContainer aProcessingContainer) throws ResourceConfigurationException
	{
		try
		{
			...
			if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_reeploying_service__FINEST",
				        new Object[] {Thread.currentThread().getName(),name});				
			}
			//	re-deploy failed Cas Processor according to mode defined in the cpe descriptor. true - stands for re-launch or restart 
		...
		{
			throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_unable_to_deploy_service__SEVERE",
					new Object[] {Thread.currentThread().getName(), aProcessingContainer.getCasProcessorConfiguration().getName()} );
		}
	}
	/**
	 * Deploys CasProcessors in a provided List. The List contains instances of Cas Processors that
	 * are not yet bound to a vinci service. To do anything usefull, the Cas Processor must be deployed  
	 * first. The process of deploying a proxy depends on the deployment mode defined in the cpe descriptor.
	 * In case of managed Cas Processor, the deployment consists of launching the vinci service and creating a 
	 * connection to it. For un-managed Cas Processor the CPE establishes the connection.
	 * 
	 * @param aCasProcessorList- list of CasProcessors to deploy
	 * @param redeploy - true if intent is to redeploy failed service
	...
	public ProcessingContainer deployCasProcessor(List aCasProcessorList, boolean redeploy) throws ResourceConfigurationException
	{
		...
		try
		{
			...
			for (int i = 0; i < aCasProcessorList.size(); i++)
			{
				...
				//	Container may have already been instantiated. This will be the case if the CPM is configured for concurrent 
				//	processing ( more than one processing pipeline). There is only one container per CasProcessor type. 
				//	So each instance of the same CasProcessor will be associated with a single container. Inside the
				//	container instances are pooled. When deploying the very first CasProcessor of each type, the
				//	container will be created and initialized. Any subsequent deployments of this CasProcessor will 
				...
				if (processingContainer == null)
				{
					...
					CpeCasProcessor casProcessorType = (CpeCasProcessor) cpeFactory.casProcessorConfigMap.get(metaData.getName());
					//	Create a pool to hold instances of CasProcessors. Instances are managed by a container through 
					...
					if (name == null)
					{
						if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_unable_to_read_meta__SEVERE",
							        Thread.currentThread().getName());				
						}
						throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
								"UIMA_CPM_casprocessor_no_name_found__SEVERE",new Object[] {Thread.currentThread().getName()});
					}
				}
			...
			{
				throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
						"UIMA_CPM_invalid_container__SEVERE",new Object[] {Thread.currentThread().getName()});
			}
			...
			{
				throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
						"UIMA_CPM_container_configuration_not_found__SEVERE",new Object[] {Thread.currentThread().getName()});
			}
	 ...
	 *  
	 * @param aProcessingContainer - container managing instances of CasProcessor
	 * @param aCasProcessorConfig - CasProcessor configuration
	 * @param redeploy - flag indicating if this re-deployment of CasProcessor that failed
	 * 
	 * @throws ResourceConfigurationException
	 */
	private void deployBasedOnModel(ProcessingContainer aProcessingContainer, CasProcessorConfiguration aCasProcessorConfig, boolean redeploy) 
	throws ResourceConfigurationException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploy_with_mode__FINEST",
		...
		else
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_unsupported_deploy_mode__SEVERE",
				        new Object[] {Thread.currentThread().getName(),name, deployModel});				
			}
			throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_unsupported_deploy_mode__SEVERE",
			        new Object[] {Thread.currentThread().getName(),name, deployModel});				
		}
	...
	/**
	 * Deploys CasProcessor as a Local Vinci Service. The CPE will launch the Cas Processor in a separate process but on
	 * the same machine. The CPE will manage the life-cycle of the Cas Processor including shutdown. Shutdown command will
	 * be sent when the CPE completes processing. In this deployment mode the CPE uses its own VNS. The VNS information
	 * (host and port) will be added as command line arguments to the program being launched (Cas Processor application). 
	 * It is the responsibility of the Cas Processor to to read this information and use it to locate VNS the CPE is managing.
	 *   
	 * @param aProcessingContainer - container object that will hold proxies to Cas Processor when it is launched 
	 * @param redeploy - true if intent is to redeploy failed service
	 * @throws ResourceConfigurationException
	 */
	private void deployLocal(ProcessingContainer aProcessingContainer, boolean redeploy) throws ResourceConfigurationException
	{
		//	Cas Processor configuration is associated with the container. There is a container per Cas Processor type.
		//	All Cas Processors that are replicated are manages by a single container. The configuration is shared accross
		...
		try
		{
			if (casProcessorConfig.runInSeparateProcess())
			{
				...
				if (redeploy == false)
				{
					//	Get number of instances of the Cas Processors to launch. This is defined in the CPE descriptor by
					...
					//	08/16/04 commented out to facilitate sharing of single queue across multiple instances of this class.
					//	LocalVNS is also shared.
					//	For local Vinci services we must have a local vns service. This component determines what ports
					...
					if (vns == null)
					{
						...
						//						08/16/04 added to facilitate sharing of single queue across multiple instances of this class.
					...
					if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploy_with_mode__FINEST",
						        new Object[] {Thread.currentThread().getName(),casProcessorConfig.getName(),casProcessorConfig.getDeploymentType() });				
					}
					...
					launchLocalService(aProcessingContainer, casProcessorConfig, redeploy, concurrentThreadCount);
				}
				else
				{
			...
			{
				throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
						"UIMA_CPM_invalid_cpe_descriptor__SEVERE",new Object[] {Thread.currentThread().getName(),name,"runInSeparateProcess"});
			}
		}
		catch (Exception e)
		{
	 ...
	 * 
	 * @param aProcessingContainer - container where the metadata will be saved
	 */
	...
	/**
	 * Launches an application as a seperate process using java's Process object. Actually, it launches
	 * as many copies of the application as given in the <i>howMany</i> parameter.
	 * 
	 * @param casProcessorConfig - Configuration containing start up command
	 * @param redeploy - true if intent is to redeploy failed application
	 * @param howMany - how many seperate process to spawn
	...
	private void launchLocalService(ProcessingContainer aProcessingContainer, CasProcessorConfiguration casProcessorConfig, boolean redeploy, int howMany) throws CasProcessorDeploymentException
	{
		try
		{
			...
			//	The command line has been established, now make sure that we tell the Cas Processor about the
			//	VNS port in use. In this logic the VNS is querried for its port number and this will the port 
			//	provided to the Cas Processor. The Cas Processor must read this argument during its start up
			...
			{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_vns_started__FINEST",
			        new Object[] {Thread.currentThread().getName(), casProcessorConfig.getName()});				

			
			}
			int serviceCount = howMany;
			//	Spawn off as many processes as defined in CPE descriptor. The exact number of instances is equal to
			...
			while (serviceCount-- > 0)
			{
				...
				for (int i = 0; cmd != null && i < cmd.length; i++)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_launching_with_service_cmd__FINEST",
						        new Object[] {Thread.currentThread().getName(), casProcessorConfig.getName(), String.valueOf(i), cmd[i]});				
					}
				}
				...
				if ((env = exec.getEnvironment()) != null && env.length > 0)
				{
					for (int i = 0; i < env.length; i++)
					{
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
							        ...
							        new Object[] {Thread.currentThread().getName(), casProcessorConfig.getName(), String.valueOf(i), env[i]});				
						}
					}
					...
					if (System.getProperty("os.name").equalsIgnoreCase("linux"))
					{
						for (int i = 0; execCommand != null && i < execCommand.length; i++)
						{
							...
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_launching_with_service_exec__FINEST",
								        new Object[] {Thread.currentThread().getName(), casProcessorConfig.getName(), String.valueOf(i), execCommand[i]});				
							}
						}
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connecting_to_services__FINEST",
				        new Object[] {Thread.currentThread().getName(), casProcessorConfig.getName()});				
			}
	...
	/**
	 * No-op for integrated deployment. Integrated CasProcessors are instantiated using
	 * UIMA framework factory in the CPEFactory.
	 * 
	...
	/**
	 * Deploys CasProcessor as a remote/network service. In this case, the CPE does not manage life-cycle
	 * of the Cas Processor. The CPE in this case simply creates a proxy to the remote Cas Processor
	 * via provided VNS. The exact VNS used here is defined on per Cas Processor in the cpe descriptor.
	 * 
	 * @param aProcessingContainer - container that will manage instances of the CasProcessor 
	 * @param redeploy - flag indicating if this redeployment of failed CasProcessor
	 * @throws ResourceConfigurationException
	 */
	private void deployRemote(ProcessingContainer aProcessingContainer, boolean redeploy) throws ResourceConfigurationException
	{
		...
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploy_with_mode__FINEST",
			        new Object[] {Thread.currentThread().getName(), name,casProcessorConfig.getDeploymentType()});				
		}
				...
				for (int i = 0; i < poolSize; i++)
				{
					...
					if (processor instanceof NetworkCasProcessorImpl)
					{
						if (((NetworkCasProcessorImpl) processor).getProxy() == null)
						{
							...
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_reducing_cp_instance_count__FINEST",
								        new Object[] {Thread.currentThread().getName(), name,casProcessorConfig.getDeploymentType()});				
							}
							processor = null;
						}
	 ...
	 *  
	 * @param redeploy - true if the connection is made as part of the recovery due to previous service crash or termination.
	 * @param aServiceUri - servie uri, this is a vinci service name
	 * @param howMany - how many proxies to create and connect 
	 * @param aProcessingContainer - hosting container for the proxies. Proxies are assigned to a pool managed by the container.
	 * 
	...
	{
		VNSQuery vnsQuery = null;
		VinciServiceInfo serviceInfo = null;
		
		//	Retrieve configuration information for the CasProcessor. This is the configuration from the CPE descriptor
		CasProcessorConfiguration casProcessorConfig = aProcessingContainer.getCasProcessorConfiguration();
		ArrayList serviceList = null;
		//	Check if this Cas Processor has exclusive service access. Meaning it requires service per proxy.
		boolean exclusiveAccess = false;
		String serviceAccess = casProcessorConfig.getDeploymentParameter("service-access");
		if (serviceAccess != null && serviceAccess.equalsIgnoreCase("exclusive"))
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_cp_with_exclusive_access__FINEST",
				        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), casProcessorConfig.getDeploymentType()});				
			}
			exclusiveAccess = true;
			//	The following is true if the service just crashed and we are trying to recover. The container in this case
			...
			if ( ((ProcessingContainer_Impl)aProcessingContainer).failedCasProcessorList.size() > 0)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_retrieve_cp_from_failed_cplist__FINEST",
					        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), casProcessorConfig.getDeploymentType()});				
				}
				CasProcessor processor = (CasProcessor)((ProcessingContainer_Impl)aProcessingContainer).failedCasProcessorList.get(0);
				//	Extract old (stale) proxy from the CAS Processor
				VinciTAP tap = ((NetworkCasProcessorImpl)processor).getProxy();
				if ( tap != null )
				{
					//	Since the new service may have started on the same machine and the same port, make sure that the
					...
					//	Go through all services in the in-use service list and try to find one that matches host and port
			...
			{
				throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
						"UIMA_CPM_no_service_in_vns__FINEST",new Object[] {Thread.currentThread().getName(),
																			aServiceUri,
			...
			//	If this is the first time through here, make sure the service list is cached. This is the current service list.
		...
		for (int i = 0; i < howMany; i++)
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_activating_service__FINEST",
				        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), String.valueOf(i), String.valueOf(howMany)});				
			}
			int restartCount = 0;
			...
			do
			{
				try
				{
					if (exclusiveAccess)
					{
						serviceInfo = (VinciServiceInfo) getNextAvailable(serviceList);
						if (serviceInfo == null)
						{
							if (redeploy == false)
							{
					...
					if (sleepBetweenRetries)
					{
						try
						{
							synchronized (locMonitor)
							{
								locMonitor.wait(maxTimeToWait);
							}
						}
					...
					if (exclusiveAccess)
					{
						activateProcessor(casProcessorConfig, serviceInfo.getHost(), serviceInfo.getPort(), aProcessingContainer, redeploy);
					...
					if (ex.getCause() instanceof ServiceException || ex.getCause() instanceof ServiceDownException)
					{
						if (UIMAFramework.getLogger().isLoggable(Level.WARNING))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_activating_service__WARNING",
	 ...
	 * 
	 * @param aServiceUri - named service endpoint
	 ...
	 * 
	 *  
	 * @param aProcessingContainer - container holding CasProcessor configuration
	 * 
	 * @throws ResourceConfigurationException
	 */
	private void handleMaxRestartThresholdReached(ProcessingContainer aProcessingContainer) throws ResourceConfigurationException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_restart_exceeded__WARNING",
			        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), String.valueOf(casProcessorConfig.getMaxRestartCount())});				
		}
		...
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_restart_exceeded__WARNING",
			        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), casProcessorConfig.getActionOnMaxRestart()});				
		}
		...
		if (Constants.TERMINATE_CPE.equals(casProcessorConfig.getActionOnMaxRestart()))
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_terminate_onerrors__INFO",
				        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName()});				
			}
		...
		if (Constants.KILL_PROCESSING_PIPELINE.equals(casProcessorConfig.getActionOnMaxRestart()))
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_terminate_pipeline__INFO",
				        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName()});				
			}
		...
		else if (Constants.DISABLE_CASPROCESSOR.equals(casProcessorConfig.getActionOnMaxRestart()))
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_disable_cp__INFO",
				        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName()});				
			}
			return;
		}
	 ...
	 * 
	 * @param aCasProcessorConfig - CasProcessor configuration containing service descriptor path 
	...
	private String getServiceUri(CasProcessorConfiguration aCasProcessorConfig) throws ResourceConfigurationException
	{
		...
		{
			throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
					"UIMA_CPM_invalid_deployment__SEVERE",new Object[] {Thread.currentThread().getName(),descriptor,uri });
		}
	...
	{
		String descriptorPath = CPMUtils.convertToAbsolutePath(System.getProperty("CPM_HOME"), CPEFactory.CPM_HOME, aDescriptor);
		ResourceSpecifier resourceSpecifier = getSpecifier(descriptorPath);

		if (!(resourceSpecifier instanceof URISpecifier)) 
		{
			throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
					"UIMA_CPM_invalid_deployment__SEVERE",new Object[] {Thread.currentThread().getName(),descriptorPath, null });
		}
	 ...
	 * 
	 * @param aFileName - path of the descriptor
	...
	private ResourceSpecifier getSpecifier(String aFileName) throws ResourceConfigurationException
	{
		...
		catch (Exception e)
		{
			e.printStackTrace();
			throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
					"UIMA_CPM_invalid_deployment__SEVERE",new Object[] {Thread.currentThread().getName(),aFileName, null });
		}
	}
	/**
	 * Deploys internal VNS for use with local CasProcessor deployments
	 * 
	 * @param casProcessorConfig - CasProcessor configuration
	 * @param redeploy - flag indicating if VNS being redeployed
	 * @throws CasProcessorDeploymentException
	 */
	private void deployVNS(CasProcessorConfiguration casProcessorConfig, boolean redeploy) throws CasProcessorDeploymentException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_deploy_vns_redeploy__FINEST",
			        new Object[] {Thread.currentThread().getName(), String.valueOf(redeploy), String.valueOf(restartCount)});				
		}
	...
	/**
	 * Creates a proxy and connects it to Vinci service running on a given port. Once connected the proxy
	 * is associated with Cas Processor. 
	 *  
	 * @param aCasProcessorConfig - CasProcessor configuration
	 * @param port - port wher vinci service is listening
	 * @return Connected proxy to service
	 * 
	 * @throws ResourceInitializationException
	 * @throws ResourceConfigurationException
	 */
	private synchronized boolean activateProcessor(CasProcessorConfiguration aCasProcessorConfig, String aHost, int aPort) throws ResourceInitializationException, ResourceConfigurationException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_activating_service_on_port__FINEST",
		...
		while (tryAgain)
		{
			...
			catch (Exception e)
			{
				if (maxCount-- == 0)
				{
					...
					throw new ResourceConfigurationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, 
							"UIMA_CPM_no_service_connection__SEVERE",new Object[] {Thread.currentThread().getName(), String.valueOf(aPort), aHost,aCasProcessorConfig.getName()});
				}
				try
				{
		...
		//	At this point there is a connected proxy (tap), now we need to add it to container that will menage it.
	 ...
	 * @throws ResourceConfigurationException
	 */
	private synchronized boolean activateProcessor(CasProcessorConfiguration aCasProcessorConfig, String aHost, int aPort, ProcessingContainer aProcessingContainer, boolean redeploy) 
	throws Exception
	{
		//	Instantiate proxy from given configuration
		VinciTAP tap = getTextAnalysisProxy(aCasProcessorConfig);
	 ...
	 * 
	 * @param aCasProcessorConfig - CasProcees configuration
	 * @param aService - name of the vinci service
	 * @return - connected Proxy
	 * 
	 * @throws ResourceInitializationException
	 * @throws ResourceConfigurationException
	 */
	private synchronized boolean activateProcessor(CasProcessorConfiguration aCasProcessorConfig, String aService,ProcessingContainer aProcessingContainer, boolean redeploy) //throws ResourceInitializationException, ResourceConfigurationException
	throws Exception
	{
		VinciTAP tap = getTextAnalysisProxy(aCasProcessorConfig);
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		...
		while (tryAgain)
		{
			try
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connecting_to_service__FINEST",
					        new Object[] {Thread.currentThread().getName(), aService});				
				}
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_connection_established__FINEST",
					        new Object[] {Thread.currentThread().getName(), aService});				
				}
			}
			catch (Exception e)
			{
				if (maxCount-- == 0)
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_max_connect_retries_exceeded__FINEST",
						        new Object[] {Thread.currentThread().getName(), aService, String.valueOf(maxCount)});				
					}
					throw new ResourceInitializationException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,new Object[] {Thread.currentThread().getName(), aService, String.valueOf(maxCount)}, new ServiceConnectionException("Unable to connect to service :::" + aService));
				}
			}
	...
	private void bindProxyToNetworkCasProcessor(VinciTAP aTap)
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_get_cp_from_pool__FINEST",
			        new Object[] {Thread.currentThread().getName(), aTap.getServiceHost(),  String.valueOf(aTap.getServicePort()), String.valueOf(casProcessorPool.getSize())});				
		}
		synchronized (casProcessorPool)
		{
			...
			CasProcessor processor = null;
			//	There are as many Cas Processors as there are Procesing Threads. All of them have been already created and exist
			//	in the cas processor pool. Cas Processor may or may not already have an associated proxy.  
			//	So here we cycle through all Cas Processors until we find one that has not yet have a proxy and
			//	add it.
			for (int i = 0; i < casProcessorPool.getSize(); i++)
			{
				try
				{
					...
					if (processor instanceof NetworkCasProcessorImpl)
					{
						...
						if (netProcessor.getProxy() != null && netProcessor.getProxy().isConnected())
						{
							...
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_already_allocated__FINEST",
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_assign_cp_to_service__FINEST",
							        new Object[] {Thread.currentThread().getName(), String.valueOf(i),aTap.getServiceHost(),  String.valueOf(aTap.getServicePort())});				
						}
				...
				finally
				{
					if (processor != null)
					{
						...
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checkin_cp_to_pool__FINEST",
							        new Object[] {Thread.currentThread().getName(), String.valueOf(i),aTap.getServiceHost(),  String.valueOf(aTap.getServicePort())});				
						}
						casProcessorPool.checkIn(processor);
						if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checked_in_cp_to_pool__FINEST",
							        new Object[] {Thread.currentThread().getName(), String.valueOf(i),aTap.getServiceHost(),  String.valueOf(aTap.getServicePort())});				
						}
					}
				}
	...
	private void bindProxyToNetworkCasProcessor(VinciTAP aTap, ProcessingContainer aProcessingContainer, boolean redeploy) throws Exception
	{
		if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_checked_in_cp_to_pool__FINEST",
			        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName() ,aTap.getServiceHost(),  String.valueOf(aTap.getServicePort())});				
		}
		CasProcessor processor = null;
		...
		try
		{
			if (redeploy)
			{
				...
				{
					throw new ResourceProcessException(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_no_cp_instance_in_failed_list__FINEST",
							new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName() ,aTap.getServiceHost(),  String.valueOf(aTap.getServicePort())},
							new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_cp_not_in_failed_list__WARNING",new Object[] {Thread.currentThread().getName(),aProcessingContainer.getName()})) );
				...
				for (int i = 0; i < cpPoolSize; i++)
				{
					...
					if (processor == null)
					{
						if (UIMAFramework.getLogger().isLoggable(Level.SEVERE))
						{
							UIMAFramework.getLogger(this.getClass()).logrb(Level.SEVERE, this.getClass().getName(), "initialize",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_get_cp_from_pool_error__SEVERE",
							        new Object[] {Thread.currentThread().getName(), aTap.getServiceHost(),  String.valueOf(aTap.getServicePort()),String.valueOf(casProcessorPool.getAllInstanceCount())});				
						}
						continue;
					}
					//	Add proxy only to instances of NetworkCasProcessorImpl
	...
	private VinciTAP getTextAnalysisProxy(CasProcessorConfiguration aCasProcessorConfig) throws ResourceConfigurationException
	{
		...
		if (aCasProcessorConfig.getDeploymentType() != null && Constants.DEPLOYMENT_LOCAL.equalsIgnoreCase(aCasProcessorConfig.getDeploymentType()))
		{
	...
	/**
	 * Returns a value for a named VNS parameter (either VNS_HOST or VNS_PORT). The parameter is resolved
	 * with the following priority:
	 * 1) Find the parameter in the Service descriptor
	 * 2) Find the parameter in the CPE descriptor
	 * 3) Find the parameter as System.property (using -D on the command line)
	 * 4) Use Hardcoded default ( VNS_HOST=localhost  VNS_PORT=9000)
	 * 
	 * 
	 * @param aVNSParamKey - name of the VNS parameter for which the value is sought 
	 * @param aCasProcessorConfig - CPE descriptor settings
	 * @param aDefault - default value for the named parameter if parameter is not defined
	 * 
	 * @return - value for a named VNS parameter
	 */
	private String getVNSSettingFor( String aVNSParamKey, CasProcessorConfiguration aCasProcessorConfig, String aDefault )
	throws ResourceConfigurationException
	{
	...
	/**
	 * This method is used during a launch of the local or managed Cas Processor. Here 
	 * connections to Cas Processors running as vinci services are made. Connections
	 * are established to services whose ports are provided in the portQueue. Whenever
	 * a managed Cas Processor starts up, it contacts local vns and is assigned a port
	 * to run on. The same port is added to the port queue and used here for establishing
	 * a connection.
	 * 
	 * @param casProcessorConfig - CasProcessor configuration
	 * @param redeploy - flag indicating if if this is restart
	 * @param howMany - indicates how many connections to make
	 * @throws ConnectException - unable to establish connection to Cas Processor
	 */
	private void connectToServices(ProcessingContainer aProcessingContainer, CasProcessorConfiguration casProcessorConfig, boolean redeploy, int howMany) throws ConnectException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_thread_count__FINEST",
			        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), String.valueOf(howMany)});				
		}
		//	Create as many proxies as there are concurrent processing pipelines
		//	The assumption is that the services have started up and retrieved ports to run on from the local VNS
		//	managed by the CPE.
		for (int i = 0; i < howMany; i++)
		{
			if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_activating_service_on_port2__FINEST",
				        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), String.valueOf(i), String.valueOf(howMany)});				
			}
			...
			catch (NumberFormatException e)
			{
				...
				{
					throw new ConnectException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_invalid_service_port__WARNING",new Object[] {Thread.currentThread().getName(),(String) portObject}) );
				}
				else
				{
			...
			try
			{
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_activating_service_on_port__FINEST",
					        new Object[] {Thread.currentThread().getName(), aProcessingContainer.getName(), String.valueOf(port)});				
				}
	...
	/**
	 * Used during a launch of the managed (local) Cas Processor this method returns 
	 * a port number on which the Cas Processor is waiting for requests. Each Cas Processor
	 * was a given a port by the local vns where it is expected to accept requests from clients.
	 * The ports assigned to Cas Processors are managed by the local instance of the VNS
	 * and available in the queue <i>portQueue</>.
	 * 
	 * @param portQueue - queue containing ports assigned to services by local VNS
	...
	private String getPort(BoundedWorkQueue portQueue) throws TimeLimitExceededException
	{
		...
		try
		{
			waitCount = System.getProperty(CONN_RETRY_COUNT) != null ? Integer.parseInt(System.getProperty(CONN_RETRY_COUNT)) : MAX_WAIT_TRIES;
		}
		catch (NumberFormatException e)
		{
			if (UIMAFramework.getLogger().isLoggable(Level.INFO))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_invalid_retry_count__INFO",
				        new Object[] {Thread.currentThread().getName()});				
			}
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_got_port_from_queue__INFO",
			        new Object[] {Thread.currentThread().getName(), ""});				
		}
		while (portQueue.getCurrentSize() == 0)
		{
			synchronized (localMonitor)
			{
				try
				{
					...
					{
						UIMAFramework.getLogger(this.getClass()).logrb(Level.INFO, this.getClass().getName(), "initialize",
						        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_port_not_allocated__INFO",
				...
				if (waitCount-- <= 0)
				{
					throw new TimeLimitExceededException(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_timeout_no_service_port__WARNING",new Object[] {Thread.currentThread().getName(),String.valueOf(waitCount * WAIT_TIME + " millis")}) );
