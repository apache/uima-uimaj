...
/**
 * 
 * Connects to and querries a given VNS server for a list a services bound to 
 * a given name. 
 *
 */
...
public class VNSQuery
{
	 ...
	 * 
	 * @param aVnsHost - VNS host name
	 * @param aVnsPort - VNS port number
	 * 
	 * @throws Exception - when unable to connect to VNS
	 */
	 ...
	 * 
	 * @throws Exception - unable to get a list
	 */
	 ...
	 * It diffs the current list and a new list as returned from the VNS.
	 * 
	 * @param aName - name of the service
	 * @param assignedServices - a list of services currently in use
	 * @return - ArrayList of {@link VinciServiceInfo} instances
	 * 
	 * @throws Exception - unable to get a list
	 */
     ...
     * @param oldList - current (in-use) list of services
     * @param newList - new list of services
     * 
    ...
    /**
     * Checks if a service identified by {@link VinciServiceInfo} instance is in use.
     * If a service exists in the service list but is not assigned, that means that
     * is available. If the service does not exist in the list it is also considered
     * available.   
     * 
     * @param aService - {@link VinciServiceInfo} instance to locate in the list
     * @param oldList - list of current (in-use) services
     * 
    ...
    private static boolean newService( VinciServiceInfo aService, ArrayList oldList)
    {
		for( int i=0; i < oldList.size(); i++)
		{
			...
			if ( aService.getHost().equals( service.getHost() ) &&
			     aService.getPort() == service.getPort() && 
   	...
   	public class VinciVNSQuery
	{
    	 ...
    	 * 
    	 * @param aVnsHost - name of the host where the VNS is running
    	 * @param aVnsPort - port on which the VNS is listening
    	 * @throws Exception - unable to connect to VNS
    	 */
    	...
    	/**
    	 * Returns a list of services bound to a given name. It ONLY returns those services
    	 * that are actually running. The VNS may return services that are stale. Those will
    	 * be filtered out.
    	 * 
    	 * @param aVinciServiceName - name of the service
    	 * @return - list of services bound to a given name.
    	 * 
    	 * @throws Exception - error while looking up the service
    	 */
    			...
    			for (int i=0; i < serviceFrames.size(); i++ )
    			{
    				...
    				if ( serviceInfo != null )
    				{
						...
						try
						{
							...
							//	Its not an error if the service does not respond. The retry logic is done elsewhere.
							...
							if ( client.isOpen() )
							{
								if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
								{
									UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
									        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_active_on_port__FINEST",
									        new Object[] {Thread.currentThread().getName(), serviceInfo.getServiceName(), serviceInfo.getHost(), String.valueOf(serviceInfo.getPort())});				
								}
						...
						catch( ConnectException ce )
						{
							if (UIMAFramework.getLogger().isLoggable(Level.WARNING))
							{
								UIMAFramework.getLogger(this.getClass()).logrb(Level.WARNING, this.getClass().getName(), "initialize",
								        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_service_not_active_on_port__WARNING",
								        new Object[] {Thread.currentThread().getName(), serviceInfo.getServiceName(), serviceInfo.getHost(), String.valueOf(serviceInfo.getPort())});				
							}
						}
						finally
    	 ...
    	 * 
    	 * @param aServiceFrame - Vinci frame containing service info
    	 * 
