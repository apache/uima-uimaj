...
package org.apache.uima.impl.collection.cpm.vinci;
...
import org.apache.uima.impl.cas_data.CasDataToXCas;
...
import org.apache.uima.impl.collection.cpm.utils.ConfigurableFeature;
import org.apache.uima.impl.collection.cpm.utils.FeatureMap;
import org.apache.uima.impl.collection.cpm.utils.Filter;
import org.apache.uima.impl.collection.cpm.vinci.cas_data.VinciPrimitiveValue;
...
public class DATACasUtils
{
	...
	public static boolean shouldAnalyzeCAS(CasData aCAS, LinkedList aFilterList)
	{
		...
		for (int i = 0; aFilterList != null && i < aFilterList.size(); i++)
		{
			...
			if (filterExpression.getRightPart() == null)
			{
				...
				if ((filterExpression.getOperand() == null && featureValue == null || featureValue.trim().length() == 0 ) || // this means that the feature must exist in CAS
				 ("!".equals(filterExpression.getOperand().getOperand()) && featureValue != null) // this means that the feature must not be present in CAS
				)
				{
					return false;
				}
	...
	public static boolean isValidType(String aKey, String[] typeList)
	{
		...
		if ( aKey.indexOf(org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM) > -1 )
		{
			aKey = StringUtils.replaceAll(aKey,org.apache.uima.impl.collection.cpm.Constants.SHORT_DASH_TERM,
				org.apache.uima.impl.collection.cpm.Constants.LONG_DASH_TERM);
		}
		if ( aKey.indexOf(org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM) > -1 )
		{
			aKey = StringUtils.replaceAll(aKey,org.apache.uima.impl.collection.cpm.Constants.SHORT_COLON_TERM,
				org.apache.uima.impl.collection.cpm.Constants.LONG_COLON_TERM);
		}
	...
	public static boolean hasFeatureStructure(CasData aCAS, String aName)
	{
		...
		while (it.hasNext())
		{
			...
			{
				FeatureStructure fs = (FeatureStructure) object;
				String type = StringUtils.replaceAll(fs.getType(),Constants.LONG_COLON_TERM, Constants.SHORT_COLON_TERM);
	...
	public static void dumpFeatures(CasData aCAS)
	{
		...
		while (it.hasNext())
		{
			...
			if (object instanceof FeatureStructure)
			{
				...
				{
					UIMAFramework.getLogger(DATACasUtils.class).logrb(Level.FINEST, DATACasUtils.class.getName(), "process",
				...
				for (int i = 0; names != null && i < names.length; i++)
				{
					...
					if (fValue != null)
					{
						...
						{
							UIMAFramework.getLogger(DATACasUtils.class).logrb(Level.FINEST, DATACasUtils.class.getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cas_fs_value__FINEST",
							        new Object[] {Thread.currentThread().getName(), names[i], fValue.toString()});
						}
					}
				}
			}
	...
	public static String getFeatureValueByType(CasData aCAS, String featureName)
	{
		...
		while (it.hasNext())
		{
			...
			if (object instanceof FeatureStructure)
			{
				...
				{
					UIMAFramework.getLogger(DATACasUtils.class).logrb(Level.FINEST, DATACasUtils.class.getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_search_cas_by_value__FINEST",
				...
				if ( featureName.equals( fs.getType() ) )
				{
					...
					for ( int i=0; names != null && i < names.length; i++ )
					{
						...
						{
							UIMAFramework.getLogger(DATACasUtils.class).logrb(Level.FINEST, DATACasUtils.class.getName(), "process",
							        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_type_value__FINEST",
...
//								fs.removeFeature(featureNameList[i]);
    ...
    public static NameValuePair[] getCasDataFeatures( CasData aCasData, String aFeatureStructureName)
    {
		...
		{
			Object object = it.next();
			if (object instanceof FeatureStructure && ((FeatureStructure)object).getType().equals(aFeatureStructureName) )
			{
				FeatureStructure fs = (FeatureStructure) object;
