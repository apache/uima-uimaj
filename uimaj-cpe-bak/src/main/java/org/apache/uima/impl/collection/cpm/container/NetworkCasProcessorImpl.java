...
import org.apache.uima.impl.resource.metadata.OperationalProperties_impl;
import org.apache.uima.impl.resource.metadata.ProcessingResourceMetaData_impl;
...
/**
 * Implementation of the {@link CasDataProcessor} interface used for both Local and Remote CasDataProcessors. 
 * The CPE delegates analysis of entities to this instance. Each instance of this class has a proxy to extenal service. 
 * 
 * 
 *
 */
public class NetworkCasProcessorImpl implements CasDataProcessor
{
	 ...
	 * 
	 * @param aTap - proxy to remote service
	 */
	public void setProxy(VinciTAP aTap)
	{
		...
		//		if ( casProcessorType.getContentTag() != null && casProcessorType.getContentTag().trim().length() > 0 )
		//		{
	...
	/**
	 * Main method used during analysis. The ProcessingUnit calls this method to initiate analysis
	 * of the content in the CasData instance. This handles one Cas at a time processing mode.
	 * 
	 * @param CasData aCas - instance of CasData to analyze
	...
	public CasData process(CasData aCas) throws ResourceProcessException
	{
		if (textAnalysisProxy == null)
		{
			throw new ResourceProcessException(
					new Exception(Thread.currentThread().getName()+
							CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
									"UIMA_CPM_EXP_no_proxy__WARNING",new Object[] {Thread.currentThread().getName()})) );
		}
		...
		catch (ServiceException e)
		{
			...
			if (textAnalysisProxy.isConnected() == false)
			{
				...
				throw new ResourceProcessException(new ServiceConnectionException(Thread.currentThread().getName()+
						CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
								"UIMA_CPM_EXP_service_down__WARNING",new Object[] {Thread.currentThread().getName(),name}) ));
			}
	...
	/**
	 * Main method used during analysis. The ProcessingUnit calls this method to initiate analysis
	 * of the content in the CasData instance. This handles processing of multiple Cas'es at a time.
	 * 
	 * @param CasData[] aCas - array of CasData instances to analyze
	...
	public ProcessingResourceMetaData getProcessingResourceMetaData()
	{
		if (textAnalysisProxy == null)
		{
			if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_use_default_metadata__FINEST",
				        new Object[] {Thread.currentThread().getName(), name});
			}
			if (metadata == null)
			{
				...
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_no_metadata__FINEST",
					        new Object[] {Thread.currentThread().getName(), name});
				}
			}
			else if (metadata.getConfigurationParameterSettings().getParameterValue(Constants.CAS_PROCESSOR_CONFIG) == null)
			{
				if (UIMAFramework.getLogger().isLoggable(Level.FINEST))
				{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_no_cp_configuration_in_metadata__FINEST",
					        new Object[] {Thread.currentThread().getName(), name});
				}
			}
			return metadata;
		...
		try
		{
			...
			if (textAnalysisProxy.isConnected() == false)
			{
				...
				throw new ResourceProcessException(new ServiceConnectionException("Service::" + name + " appears to be down"));
			}
	...
	/**
	 * Notifies Network AE that end-of-batch marker has been reached. The notification can be disabled in the
	 * Cpe descriptor by setting batch=0 in the <checkpoint> element.  
	 * 
	...
	public void collectionProcessComplete(ProcessTrace aTrace) throws ResourceProcessException, IOException
	{
		...
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "process",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_stopping_cp__FINEST",
			        new Object[] {Thread.currentThread().getName(), name});
		}
		//	Send shutdown command to Local services, meaning services started by the CPM on the same
		//	machine in a different JVM. Remote services will not be shutdown since these may be started for
	...
	/**
	 * Determines if CasProcessor wants to be notified when end-of-batch and end-of-processing
	 * events happen.
	 * 
	...
	{
		//	Checks if notification is to be sent by looking at the value of batch attribue of the <checkpoint> element 
		//	in the Cpe descriptor for this Cas Processor.
		if (casProcessorType.getCheckpoint() != null && casProcessorType.getCheckpoint().getBatchSize() == 0)
		{
			return false;
		}
		return true;
	...
	/**
	 * Inspect descriptors to determine if the network CAS processor is
	 * an AnalysisEngine or a CasConsumer.  Sets the isCasConsumer field
	 * appropriately.
	 */
	private void checkNetworkCasProcessorType() throws IOException, InvalidXMLException
	{
		...
		if (desc != null)
		{
			String descriptorPath = CPMUtils.convertToAbsolutePath(System.getProperty("CPM_HOME"), CPEFactory.CPM_HOME, desc);
			if (Constants.DEPLOYMENT_REMOTE.equals(casProcessorType.getDeployment()))
			{
				//parse the descriptor, expecting it to be a URISpecifier, and get the resource type from there 
				ResourceSpecifier resourceSpecifier = UIMAFramework.getXMLParser().parseResourceSpecifier(new XMLInputSource(descriptorPath));
			...
			else if (Constants.DEPLOYMENT_LOCAL.equals(casProcessorType.getDeployment()))
			{
				...
				ResourceSpecifier resourceSpecifier = UIMAFramework.getXMLParser().parseResourceSpecifier(new XMLInputSource(componentDescPath));
