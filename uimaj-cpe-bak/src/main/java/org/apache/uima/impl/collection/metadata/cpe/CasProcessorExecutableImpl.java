...
public class CasProcessorExecutableImpl extends MetaDataObject_impl implements CasProcessorExecutable
{
	  ...
	  for (int i=0; i < nodes.getLength(); i++)
	  {
		...
		if (node instanceof Element)
		{
			if ( "arg".equals( node.getNodeName()) )
			{
				...
				CasProcessorExecArg arg = (CasProcessorExecArg)aParser.buildObject((Element)node, aOptions);
				args.add(arg);
			}
			else if ( "env".equals( node.getNodeName()) )
			{
				//assumes all children are CasProcessor elements
				CasProcessorRuntimeEnvParam env = (CasProcessorRuntimeEnvParam)aParser.buildObject((Element)node, aOptions);
				envs.add(env);
			}
		}
	  }
	...
	public void toXML(ContentHandler aContentHandler,
		...
		throws SAXException
	  {
		...
		{
			((CasProcessorRuntimeEnvParam)envs.get(i)).toXML(aContentHandler, aWriteDefaultNamespaceAttribute);
		}         
	...
	new XmlizationInfo("exec",
	new PropertyXmlInfo[]{
		new PropertyXmlInfo("arg"),
		new PropertyXmlInfo("env"),
	});
