...
import org.apache.uima.impl.collection.cpm.container.deployer.JavaApplication;
import org.apache.uima.impl.collection.cpm.container.deployer.NonJavaApplication;
...
/**
 * A wrapper containing Cas Processor configuration. An instance of this class is associated with each
 * Cas Processor.
 *  
 * 
 */
public class CasProcessorConfigurationJAXBImpl implements CasProcessorConfiguration
{
	...
	/**
	 * Initializes instance and copies configuation from cpe
	 * descriptor.
	 *   
	 * @param aJaxbCasProcessorConfig - - configuration object containing Cas Processor configuration
	 *  
	 * @throws ResourceConfigurationException
	 */
	public CasProcessorConfigurationJAXBImpl(CpeCasProcessor aCasProcessorConfig) throws ResourceConfigurationException
	{
		if (aCasProcessorConfig == null)
		{
			throw new ResourceConfigurationException(InvalidXMLException.ELEMENT_NOT_FOUND, new Object[] { "<casProcessor>", "<casProcessors>" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_bad_cpe_descriptor__WARNING",
		...
		if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
		{
			UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
			        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_max_restart_action__FINEST",
			        new Object[] {Thread.currentThread().getName(), name, aCasProcessorConfig.getErrorHandling().getMaxConsecutiveRestarts().getAction()});
		}
		...
		if (!isJavaProcess())
		{
			...
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_config_non_java_service__FINEST",
				        new Object[] {Thread.currentThread().getName(), name});
			}
			nonJavaApp = new NonJavaApplication(this, aCasProcessorConfig); //aJaxbCasProcessorConfig, getDescriptor(), getDeploymentParameters());
		}
		else
		{
			if ( UIMAFramework.getLogger().isLoggable(Level.FINEST) )
			{
				UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
				        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_config_java_service__FINEST",
				        new Object[] {Thread.currentThread().getName(), name});
			}
	...
	{
		isJavaProcess = false;
		CasProcessorRunInSeperateProcess runInProcessType = 
			aCasProcessorConfig.getRunInSeparateProcess();
	...
	private void addErrorHandling(CpeCasProcessor aCasProcessorConfig) throws ResourceConfigurationException
	{
		...
		{
			throw new ResourceConfigurationException(InvalidXMLException.ELEMENT_NOT_FOUND, new Object[] { "errorHandling", "casProcessor" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_EXP_missing_xml_element__WARNING",
		...
		if (maxRestarts == null)
		{
			throw new ResourceConfigurationException(ResourceConfigurationException.MANDATORY_VALUE_MISSING, new Object[] { "maxConsecutiveRestarts", "CPE" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_missing_xml_element__WARNING",
							new Object[] {Thread.currentThread().getName(), name,"<maxConsecutiveRestarts>"})
					));
		}
		...
		{
			throw new ResourceConfigurationException(ResourceConfigurationException.MANDATORY_VALUE_MISSING, new Object[] { "action", "CPE" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_bad_action_string__WARNING",
							new Object[] {Thread.currentThread().getName(), name, "<maxConsecutiveRestarts>",maxRestarts.getAction()})
					));
		}
		...
		{
			throw new ResourceConfigurationException(ResourceConfigurationException.MANDATORY_VALUE_MISSING, new Object[] { "errorRateThreshold", "CPE" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_missing_xml_element__WARNING",
		...
		if (!validActionOnError(errorRateThresholdType.getAction()))
		{
			throw new ResourceConfigurationException(ResourceConfigurationException.MANDATORY_VALUE_MISSING, new Object[] { "action", "CPE" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,"UIMA_CPM_EXP_bad_action_string__WARNING",
							new Object[] {Thread.currentThread().getName(), name, "<errorRateThreshold>",maxRestarts.getAction()})));
	...
	/**
	 * Copies deployment parameters associated with this Cas Processor
	 * These parameters are used to construct appropriate command line for launching the Cas Processor
	 * in external process
	 *  
	 * @param aJaxbCasProcessorConfig - configuration object containing Cas Processor configuration
	 */
	private void addDeploymentParameters(CpeCasProcessor aCasProcessorConfig) throws ResourceConfigurationException
	{
		...
		CasProcessorDeploymentParams deployParams = 
			aCasProcessorConfig.getDeploymentParams();
		...
		if (deployParams != null)
		{
			...
			for (int i = 0; parameters != null && i < parameters.length; i++)
			{
				try
				{
					...
					{
					UIMAFramework.getLogger(this.getClass()).logrb(Level.FINEST, this.getClass().getName(), "initialize",
					        CPMUtils.CPM_LOG_RESOURCE_BUNDLE, "UIMA_CPM_show_cp_deploy_params__FINEST",
					        new Object[] {Thread.currentThread().getName(), name, parameters[i].getParameterName(), parameters[i].getParameterValue()});
					}
					NameValuePair nvp = new NameValuePair_impl();
					nvp.setName(parameters[i].getParameterName());

					//	If a value is a path to a file and IS a relative path convert to absolute path using CPE_HOME variable
					String value = CPMUtils.convertToAbsolutePath(System.getProperty("CPM_HOME"), CPEFactory.CPM_HOME, parameters[i].getParameterValue());
					nvp.setValue(value);
		...
		if ((rsp = aCasProcessorConfig.getRunInSeparateProcess()) != null && rsp.getExecutable() != null)
		{
		...
		if (deployType == null || deployType.trim().length() == 0)
		{
			throw new ResourceConfigurationException(InvalidXMLException.REQUIRED_ATTRIBUTE_MISSING, new Object[] { "deployment", "casProcessor" }, 
					new Exception( CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							...
							new Object[] {Thread.currentThread().getName(),aCasProcessorConfig.getName(), "deployment", "<casProcessor>"})));
		}
	...
	private void addBatchSize(CpeCasProcessor aCasProcessorConfig) throws ResourceConfigurationException
	{
		...
		if (checkpoint == null)
		{
			throw new ResourceConfigurationException(InvalidXMLException.ELEMENT_NOT_FOUND, new Object[] { "<checkpoint>", "<casProcessor>" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							"UIMA_CPM_EXP_missing_xml_element__WARNING",new Object[] {Thread.currentThread().getName(),aCasProcessorConfig.getName(),"<checkpoint>"})));
		}
		...
		{
			throw new ResourceConfigurationException(InvalidXMLException.REQUIRED_ATTRIBUTE_MISSING, new Object[] { "batch", "<checkpoint>" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							"UIMA_CPM_EXP_missing_attribute_from_xml_element__WARNING",
							new Object[] {Thread.currentThread().getName(),aCasProcessorConfig.getName(), "batch", "<checkpoint>"})));
		}
	 ...
	 *  
	 * @param aJaxbCasProcessorConfig - configuration object holding path to the descriptor
	 * 
	 * @throws ResourceConfigurationException
	 */
	private void addDescriptor(CpeCasProcessor aCasProcessorConfig) throws ResourceConfigurationException
	{
		...
		descriptor = aCasProcessorConfig.getDescriptor();
		if (descriptor == null || descriptor.trim().length() == 0)
		{
			throw new ResourceConfigurationException(InvalidXMLException.ELEMENT_NOT_FOUND, new Object[] { "href", "include" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							"UIMA_CPM_EXP_missing_attribute_from_xml_element__WARNING",
							new Object[] {Thread.currentThread().getName(),aCasProcessorConfig.getName(), "href", "<include>"})));
		}
	 ...
	 * <p>
	 *  
	 * @param aActionOnError - action string to verify
	 * 
	...
	/**
	 * Returns an action as String to identify an action to take in case of 
	 * excessive Cas Processor errors.
	 * <p>
	 * <li>continue</li>
	 * <li>terminate</li>
	 * <li>disable</li>
	...
	/**
	 * Returns an action as String to identify an action to take in case of 
	 * excessive Cas Processor restarts.
	 * <p>
	 * <li>continue</li>
	 * <li>terminate</li>
	 * <li>disable</li>
	...
	public LinkedList getFilter() throws ResourceConfigurationException
	{
		...
		{
			throw new ResourceConfigurationException(InvalidXMLException.INVALID_ELEMENT_TEXT, new Object[] { "filter" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							"UIMA_CPM_EXP_missing_xml_element__WARNING",new Object[] {Thread.currentThread().getName(),name,"filer"})) );
		}
		return null;
	}
	/**
	 * Returns an array of types that should not be sent to Cas Processor. The drop types
	 * are defined in the cpe descriptor.
	 */
	public String[] getKeysToDrop() throws ResourceConfigurationException
	{
		try
		{
			//	Now extract the file containing features to be dropped from the CAS when communicating with the
			...
			{
				String descriptorPath = CPMUtils.convertToAbsolutePath(System.getProperty("CPM_HOME"), CPEFactory.CPM_HOME, dropFeatureFile);
				try
				{
		...
		{
			throw new ResourceConfigurationException(InvalidXMLException.ELEMENT_NOT_FOUND, new Object[] { "parameter" }, 
					new Exception(CpmLocalizedMessage.getLocalizedMessage(CPMUtils.CPM_LOG_RESOURCE_BUNDLE,
							"UIMA_CPM_EXP_missing_xml_element__WARNING",new Object[] {Thread.currentThread().getName(),name,"parameter"})) );
		}
		return null;
	}
	...
	/**
	 * Returns descriptor path associated with this Cas Processor
	 * 
	...
	public String getDeploymentParameter(String aDeployParameter)
	{
		...
		for (int i = 0; i < deploymentParameters.size(); i++)
		{
			...
			{
				descriptor = (String) nvp.getValue();
				break;
			}
		}
		return descriptor;
	...
	/**
	 * Returns true it the Cas Processor is written in java and will be
	 * run with java jvm.
	 * 
	 */
	...
	/**
	 * Returns executable section of the Cas Processor configuration. It contains
	 * the name of executable program to be used when launching a seperate process
	 * with Cas Processor running as vinci service.
	 * 
