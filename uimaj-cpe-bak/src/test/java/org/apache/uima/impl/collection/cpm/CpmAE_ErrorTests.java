...
import org.apache.uima.impl.collection.cpm.utils.DescriptorMakeUtil;
import org.apache.uima.impl.collection.cpm.utils.FunctionErrorStore;
import org.apache.uima.impl.collection.cpm.utils.TestStatusCallbackListener;
 ...
 * <ul>
 * 	<li>
 * 		Function tests with errorhandling of the methods defined in 
 * 		{@link org.apache.uima.analysis_engine.annotator.BaseAnnotator}
 * 		and {@link org.apache.uima.analysis_engine.annotator.JTextAnnotator.
 * 	</li>
 * 	<li>
 * 		CPM function tests concerning errorhandling
 * 	</li>
 * </ul>
 * <p>
 * The first section of tests analyse the behaviour of the different methods implemented 
 * with the {@link JTextAnnotator} Interface while throwing predefined exceptions. Therefore
 * special helper classes located in the {@link org.apache.uima.impl.collection.cpm.utils}
 * are used. For instance {@link DecriptorMakeUtil} generates the customized descriptors which 
 * throw the predefined Exceptions. {@link FunctionErrorStore} is the class where all data about
 * method calls and counts are kept. That's just to point out some important once.
 * </p>
 * <p>
 ...
 * <ul>
 * 	<li>
 * 		generate the descriptors, with fit to the testcase. For instance an
 * 		annotator which throws a (runtime) exception after every 5th document.
 * 	</li>
 * 	<li>
 * 		create the cpe an set all needed values (error handling params f. instance)
 * 	</li>
 * 	<li>
 * 		[optional] add some mechanism to stop the cpm if it crashes in the test (timeouts or so)
 * 	</li>
 * 	<li>
 * 		run the test and check for the results
 * 	</li>
 * </ul>
 * 
 * Also have a look at <br>
 * 
 * @see org.apache.uima.impl.collection.cpm.CpmCasConsumer_ErrorTests
 * @see org.apache.uima.impl.collection.cpm.CpmCollectionReader_ErrorTests
 */
public class CpmAE_ErrorTests extends TestCase {
	 ...
	 * the cpm should regular finish after processing all documents. No error and no abort should occur.
	 * 
	 * @throws Exception
	 */
	public void testProcessWithAnnotatorProcessException() throws Exception{
		...
		assertEquals("The cpm is still working or the collectionProcessComplete-method of the listener was not called.", true, listener.isFinished());
		assertEquals("The cpm propably didn't finish correctly! The aborted method of the listener was called.", false, listener.isAborted());
		assertEquals("There are not as much exceptions as expected! ", countExceptions(documentCount,exceptionSequence),FunctionErrorStore.getCount());
	...
	public void testProcessWithOutOfMemoryException() throws Exception{
		...
		assertEquals("The cpm is still working or the collectionProcessComplete-method of the listener was not called.", true, listener.isFinished());
		assertEquals("The cpm propably didn't finish correctly! The aborted method of the listener was called.", false, listener.isAborted());
		assertEquals("There are not as much exceptions as expected! ", (documentCount/exceptionSequence),FunctionErrorStore.getCount());
		// that's it.
	}
	 ...
	 * <b>expected behaviour:</b><br>
	 * The cpm should not finish. Instead, the exception is passed back to the testscript.
	 * Neither the collectionProcessComplete-, nor the aborted- method of the listener is called. 
	 * 
	 * @throws Exception
	 */
	public void testInitializeWithNullPointerException() throws Exception{
		...
		assertEquals("The cpm called the listener, that the cpm has finished - which normally could not be.", false, listener.isFinished());
		assertEquals("The aborted-method of the listener was called. (new behaviour?)", false, listener.isAborted());
	 ...
	 * <b>expected behaviour:</b><br>
	 * the cpm should regular finish, but don't process the documents. No error and no abort should occur.
	 * the collectionProcessComplete -method of the listener is called. 
	 * 
	 * @throws Exception
	 */
	public void testInitializeWithOutOfMemoryException() throws Exception{
		...
		ManageOutputDevice.setAllSystemOutputToDefault();
		// check the results, if everything worked as expected
		assertEquals("The cpm is still working or the collectionProcessComplete-method of listener was not called.", true, listener.isFinished());
		assertEquals("The cpm propably didn't finish correctly! The aborted method of the listener was called.", false, listener.isAborted());
		assertEquals("There are not as much exceptions as expected! ", 1, FunctionErrorStore.getCount());
		// that's it.
	}
	 ...
	 * <b>expected behaviour:</b><br>
	 * The cpm should not finish. Instead, the exception (a NullPointerException) is passed back to the testclass.
	 * Neither the collectionProcessComplete-, nor the aborted- method of the listener is called. 
	 * 
	 * @throws Exception
	 */
...
//	}
	...
	public void testAeProcessingUnitThreadCount() throws Exception{
		...
		assertEquals("There are not as much ae's running as expected ", count, FunctionErrorStore.getAnnotatorCount());
		// that's it.
	}
	
	/**
	 * <b>testcase:</b> set the errorRateThreshold to terminate and leave the default value at 100/1000. Every 5th
	 * document an AnnotatorProcessException is thrown. <code>&lt;errorRateThreshold action="terminate" value="100/1000"/&gt;</code><br>
	 * <b>expected behaviour:</b><br>
	 * The cpm should not finish. Instead, after 100 Exceptions + 1 the aborted -method is called by the cpm. The cpm 
	 * shut down.
	 * 
	 * @throws Exception
	 */
	public void testAeErrorRateThresholdTerminateDefault() throws Exception{
		...
		Object[] objs =
			 setupConfigurableCpm(
				 documentCount, exceptionName, exceptionSequence, functionName);
		...
		assertEquals("There are not as much exceptions as expected! ", (100+1), FunctionErrorStore.getCount());
		// that's it.
	}
	
	/**
	 * <b>testcase:</b> set the errorRateThreshold to action:terminate and set the value to 5/1000. Every 5th
	 * document an AnnotatorProcessException is thrown. <code>&lt;errorRateThreshold action="terminate" value="5/1000"/&gt;</code><br>
	 * <b>expected behaviour:</b><br>
	 * The cpm should not finish. Instead, after 5 Exceptions + 1 the aborted -method is called by the cpm. The cpm 
	 * shut down.
	 * 
	 * @throws Exception
	 */
	public void testAeErrorRateThresholdTerminateModified1() throws Exception{
		...
		assertEquals("The cpm called the listener, that the cpm has finished - which normally could not be.", false, listener.isFinished());
		assertEquals("The aborted-method of the listener wasn't called.", true, listener.isAborted());
		assertEquals("There are not as much exceptions as expected! ", (5+1), FunctionErrorStore.getCount());
	}
	
	/**
	 * <b>testcase:</b> set the errorRateThreshold to action:terminate and set the value to 5/10. Every 5th
	 * document an AnnotatorProcessException is thrown. <code>&lt;errorRateThreshold action="terminate" value="5/10"/&gt;</code><br>
	 * <b>expected behaviour:</b><br>
	...
	public void testAeErrorRateThresholdTerminateModified2() throws Exception{
		...
		// check the results, if everything worked as expected
		assertEquals("The cpm is still working or the collectionProcessComplete-method of the listener was not called.", true, listener.isFinished());
		assertEquals("The cpm didn't finish correctly! Abort in the listener was called.", false, listener.isAborted());
		assertEquals("There are not as much exceptions as expected! ", countExceptions(documentCount, exceptionSequence), FunctionErrorStore.getCount());
		// that's it.
	}

	/**
	 * <b>testcase:</b> set the errorRateThreshold to action:continue and set the value to 5/15. Every 2nd
	 * document an AnnotatorProcessException is thrown. <code>&lt;errorRateThreshold action="continue" value="5/15"/&gt;</code><br>
	 * <b>expected behaviour:</b><br>
	...
	public void testAeErrorRateThresholdContinue() throws Exception{
		...
		//assertEquals("There are not as much exceptions as expected! ", 44, FunctionErrorStore.getCount());
	}
	
	/**
	 * <b>testcase:</b> set the errorRateThreshold to action:disable and set the value to 5/15. Every 2nd
	 * document an AnnotatorProcessException is thrown. <code>&lt;errorRateThreshold action="disable" value="5/15"/&gt;</code><br>
	 * <b>expected behaviour:</b><br>
	...
	public void testNumToProcess() throws Exception{
		...
		assertEquals("There is a difference between the expected and the processed document number. ", 20, FunctionErrorStore.getAnnotatorProcessCount());
	}
	 ...
	 * 
	 * @param totalCount all documents that should be processed
	 * @param errorSequence iteration rate of occuring errors 
	 * 
	 ...
	 * @param exceptionName the exception to be thrown
	 * @param exceptionSequence the iteration rate of the exceptions
	 * @param functionName the name of the function/method that throws the exception
	 *  
	 * @return CollectionProcessingEngine - initialized cpe
	 */
	private CollectionProcessingEngine setupCpm(
		int documentCount,
		String exceptionName,
		int exceptionSequence,
		String functionName) {
		CpeDescription cpeDesc = null;
		CollectionProcessingEngine cpe = null;

		try {
			String colReaderBase =
				junitTestBasePath
					+ "CpmTests"
					+ FS
					+ "ErrorTestCollectionReader.xml";
			String taeBase =
				junitTestBasePath + "CpmTests" + FS + "ErrorTestAnnotator.xml";
			String casConsumerBase =
				junitTestBasePath + "CpmTests" + FS + "ErrorTestCasConsumer.xml";

			// first, prepare all descriptors as needed
			String colReaderDesc =
				DescriptorMakeUtil.makeCollectionReader(colReaderBase, documentCount);
			String taeDesc = DescriptorMakeUtil.makeAnalysisEngine(taeBase, true, functionName,	exceptionSequence, exceptionName);
			String casConsumerDesc =
				DescriptorMakeUtil.makeCasConsumer(casConsumerBase);
			...
			CpeIntegratedCasProcessor integratedProcessor =
				CpeDescriptorFactory.produceCasProcessor("ErrorTestAnnotator");
			integratedProcessor.setDescriptor(taeDesc);

			CpeIntegratedCasProcessor casConsumer =
				CpeDescriptorFactory.produceCasProcessor("ErrorTest CasConsumer");
	 ...
	 * 
	 * @param document number of how many documents are been read.
	 * 
	 ...
	 * {@link org.apache.uima.impl.collection.cpm.utils.ErrorTestAnnotator} class, which
	 * producess the expected errors.
	 * 
	 * @param documentCount number of documents that are processed
	 * @param exceptionName name of the the exception that should be thrown
	 * @param exceptionSequence the iteration rate of the given exception or error
	 * @param functionName the function in which the exception should be thrown
	 * 
	 * @return Object[] with the cpe-Descriptor at index 0 and the integratedProcessor at index 1
	 */
	private Object[] setupConfigurableCpm(int documentCount,	String exceptionName, int exceptionSequence, String functionName) {
		CpeDescription cpeDesc = null;
		...
		try {
			...
			if(exceptionName == null || exceptionSequence <= 0 || functionName == null){
				...
				taeDesc = DescriptorMakeUtil.makeAnalysisEngine(taeBase, true, functionName,	exceptionSequence, exceptionName);
			}
			String casConsumerDesc =
				DescriptorMakeUtil.makeCasConsumer(casConsumerBase);
	...
	class CollectionReaderStatusCallbackListener
		extends TestStatusCallbackListener {
