 ...
 * 
 * <p>
 * The TestCase aims to test the important methods normally used within the CasConsumer
 * (initialize and processCas). In each function different Exceptions are thrown to test
 * the behaviour of the system in such a situation. Therefore special helper classes 
 * located in the {@link org.apache.uima.impl.collection.cpm.utils} package are 
 * used. For instance {@link DecriptorMakeUtil} generates the customized descriptors which 
 * throws the predefined Exceptions. {@link FunctionErrorStore} is the class where all data 
 * about methodcalls and counts are kept. That's just to point out some important classes. 
 * </p>
 * <p>
 ...
 * 		annotator which throws a (runtime) exception every 5th document.
 * 	</li>
 * 	<li>
 * 		[optional] add some mechanism to handle errors in the tests (timeouts or try-catch blocks)
 * 	</li>
 * 	<li>
 ...
 * 
 * @see org.apache.uima.impl.collection.cpm.CpmAE_ErrorTests
...
public class CpmCasConsumer_ErrorTests extends TestCase {
	...
	public void testProcessCasWithIOException() throws Exception{
		...
		assertEquals("The aborted-method of the listener was called. (new behaviour?)", false, listener.isAborted());
		assertEquals("There are not as much exceptions  thrown as expected! ", ((documentCount)/exceptionSequence), FunctionErrorStore.getCount());
		assertEquals("The CAS which causes the error wasn't given to the process methode. Null was returned.", false, null == listener.getLastCas());
	}
	...
	public void testProcessCasWithResourceProcessException() throws Exception{
		...
		// check the results, if everything worked as expected
		ManageOutputDevice.setAllSystemOutputToDefault();
		assertEquals("The cpm did not call the listener, that the cpm has finished.", true, listener.isFinished());
	 ...
	 * called and the error is passed to the EntityProcessStatus value. In this methode, the cpe.kill()
	 * methode is invoked, which organises the shut down of the cpm. In the end, the abort-methode is
	 * called, to comunicate the status of the cpm to everyone who is listening for errors.
	 * 
	 * @throws Exception
	 */
	...
	private CollectionProcessingEngine setupCpm(
		...
		try {
			...
			String casConsumerDesc =
				DescriptorMakeUtil.makeCasConsumer(casConsumerBase,true, functionName, exceptionSequence, exceptionName);
	...
	/**
	 * Special Listener for reacting on the different Exceptions and Errors to ensure a
	 * secure shut down during the whole test.  
	 */
