 ...
 * (initialize, getNext, hasNext and getProgress). In each function different Exceptions are thrown to test
 * the behaviour of the system in such a situation. 
 * </p>
 * <p>
 ...
 * <ul>
 * 	<li>
 * 		generate the descriptors, with fit to the testcase. For instance a
 * 		CollectionReader which throws a (runtime) exception every 5th document.
 * 	</li>
 * 	<li>
...
public class CpmCollectionReader_ErrorTests extends TestCase {
	 ...
	 * The cpm notify the entityProcessComplete methode of the listener and
	 * propagate the error in the EntityProcessStatus. After that, the cpm is 
	 * shut down and the abort methode is called.
	 * 
	 * @throws Exception
	 */
	 ...
	 * <b>expected behaviour:</b><br>
	 * The cpm notifies the entityProcessComplete methode of the listener and
	 * propagate the error in the EntityProcessStatus. After that, the cpm is 
	 * shut down and the abort methode is called.
	 * 
	 * @throws Exception
	...
	public void testHasNextWithNullPointerException() throws Exception{
		...
		assertEquals("The cpm didn't finish correctly! Finish was not called.", true, listener.isFinished());
	...
	public void testInitializeWithResourceInitializationException() throws Exception{
		...
		TestStatusCallbackListener listener = null; // listener with which the statusinformation are made avaiable
		...
		try {
			...
			exceptionThrown = true;
				...
				assertEquals("The cpm called the listener, that the cpm has finished - which normally could not be.", false, listener.isFinished());
				assertEquals("There are not as much exceptions as expected! ", 1, FunctionErrorStore.getCount());
				assertEquals("The expected ResourceInitializationException was not fiven back to the programm. ", true, exceptionThrown);
				// that's it.
		}
	}
	...
	public void testInitializeWithOutOfMemoryError() throws Exception{
		...
		int exceptionSequence = 1; // the sequence in which errors are produced
		ManageOutputDevice.setAllSystemOutputToNirvana();
		CollectionReaderStatusCallbackListener listener = null; // listener with which the statusinformation are made avaiable
	...
	public void testGetProcessWithOutOfMemoryError() throws Exception{
		...
		// check the results, if everything worked as expected
		assertEquals("Abort was not called.", true, listener.isAborted());
		assertEquals("The collectionProcessComplete-method of the listener was called. - Unexpected!", false, listener.isFinished());
		assertEquals("There are not as much exceptions as expected! ", 1, FunctionErrorStore.getCount());
		checkForOutOfMemoryError(listener);
	}
...
//	}
	...
	private void checkForOutOfMemoryError(CollectionReaderStatusCallbackListener listener){
		...
		assertEquals("No Error was thrown (and no Exception) - expected was an OutOfMemoryError.", true, listener.hasError());
		assertEquals("The expected null for the failed cas is missing.", true, (listener.getLastCas() == null));
		assertEquals("There are not as much exceptions as expected! ", 1, FunctionErrorStore.getCount());
		assertEquals("The cpm called the listener, that the cpm has finished - which normally could not be.", false, listener.isFinished());
	...
	private CollectionProcessingEngine setupCpm(
		...
		try {
			...
			String colReaderDesc =
				DescriptorMakeUtil.makeCollectionReader(
					colReaderBase,
					true,
					functionName,
					exceptionSequence,
