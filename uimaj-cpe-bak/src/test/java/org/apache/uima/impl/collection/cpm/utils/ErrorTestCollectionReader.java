...
public class ErrorTestCollectionReader extends CollectionReader_ImplBase {
	...
	public void initialize() throws ResourceInitializationException {
		...
		if (errorConfig.containsKey(FUNC_INITIALIZE_KEY)) {
			((FunctionErrorStore) errorConfig.get(FUNC_INITIALIZE_KEY))
				.methodeCalled5();
		}
	}
	...
	public boolean hasNext() {
		...
		if (errorConfig.containsKey(FUNC_HASNEXT_KEY)) {
				((FunctionErrorStore) errorConfig.get(FUNC_HASNEXT_KEY))
					.methodeCalled8();
		}
	...
	public void getNext(CAS aCAS) throws IOException, CollectionException {
		...
		if (errorConfig.containsKey(FUNC_GETNEXT_KEY)) {
			((FunctionErrorStore) errorConfig.get(FUNC_GETNEXT_KEY))
				.methodeCalled6();
		}
	}
	...
	public void close() throws IOException {
		...
		if (errorConfig.containsKey(FUNC_CLOSE_KEY)) {
			((FunctionErrorStore) errorConfig.get(FUNC_CLOSE_KEY))
				.methodeCalled7();
		}
	}
	...
	public Progress[] getProgress() {
		logger.log(LOG_LEVEL, "getProgress() was called");
				if (errorConfig.containsKey(FUNC_GETPROCESS_KEY)) {
					((FunctionErrorStore) errorConfig.get(FUNC_GETPROCESS_KEY))
						.methodeCalled8();
				}
		return new Progress[] {
			 new ProgressImpl(documentsCounted, documentCount, Progress.ENTITIES)};
	}
	...
	/**
	 * @param function name of the methode where the error should be added
	 * @param obj the error object - @see FunctionErrorStore
	 */
