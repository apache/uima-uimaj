...
package org.apache.uima.impl.analysis_engine;
...
import org.apache.uima.impl.analysis_engine.asb.ASB_impl;
import org.apache.uima.impl.analysis_engine.asb.FlowControllerContainer;
import org.apache.uima.impl.analysis_engine.metadata.SofaMapping_impl;
...
public class SofaNamingInAggregateTest extends TestCase {
	...
	protected void setUp() throws Exception {
		try {
			...
			aeDescriptor = UIMAFramework.getXMLParser()
			...
			flowController = ((ASB_impl) aggregateAE._getASB())
			...
			AnalysisEngineDescription aeDescriptor2 = UIMAFramework
					.getXMLParser().parseAnalysisEngineDescription(in2);
	...
	/**
	 * Checks if sofa name mappings set in an aggregate AE descriptor get passed
	 * to UimaContext of the delegate AE.
	 * 
	 */
	public void testGetSofaMappings() throws Exception {
		try {
			SofaID[] sofamappings = delegateAE.getUimaContext()
					.getSofaMappings();
			Assert.assertEquals(2, sofamappings.length);
			Assert.assertEquals("EnglishDocument", sofamappings[0]
					.getComponentSofaName());
			Assert.assertEquals("GermanDocument", sofamappings[1]
					.getComponentSofaName());

			sofamappings = flowController.getUimaContext().getSofaMappings();
			Assert.assertEquals(1, sofamappings.length);
			Assert.assertEquals("OriginalDocument", sofamappings[0]
					.getComponentSofaName());
		} catch (Exception e) {
			JUnitExtension.handleException(e);
		}
	...
	public void testGetUimaContextMapToSofaID() throws Exception {
		try {
			SofaID sofaid1 = delegateAE.getUimaContext().mapToSofaID(
					"EnglishDocument");
			Assert.assertEquals("SourceDocument", sofaid1.getSofaID());
			SofaID sofaid2 = delegateAE.getUimaContext().mapToSofaID(
					"GermanDocument");
			Assert.assertEquals("OutputTranslator1", sofaid2.getSofaID());

			SofaID sofaid3 = flowController.getUimaContext().mapToSofaID(
					"OriginalDocument");
			Assert.assertEquals("SourceDocument", sofaid3.getSofaID());

			// now try the second aggregate (With the sofa-unaware mapping)
			sofaid1 = delegateAE2.getUimaContext().mapToSofaID(
					CAS.NAME_DEFAULT_TEXT_SOFA);
			Assert.assertEquals("SourceDocument", sofaid1.getSofaID());
			sofaid2 = delegateAE2.getUimaContext()
					.mapToSofaID("GermanDocument");
			Assert.assertEquals("OutputTranslator1", sofaid2.getSofaID());
	...
	public void testMapRootSofaNameToSofaID() throws Exception {
		try {
			SofaID sofaid1 = delegateAE.getUimaContext().mapToSofaID(
					"EnglishDocument.1.txt");
			Assert.assertEquals("SourceDocument.1.txt", sofaid1.getSofaID());
			SofaID sofaid2 = delegateAE.getUimaContext()
					.mapToSofaID("SomeID.1");
	...
	public void testGetAnnotatorContextMapToSofaID() throws Exception {
		try {
			AnnotatorContext context = new AnnotatorContext_impl(delegateAE
					.getUimaContextAdmin());
	...
	/**
	 * Test the whether input sofa specified in the AE descriptar are in the AE
	 * meta data.
	 * 
	 */
	public void testGetInputSofas() throws Exception {
		try {
			Capability[] capabilities = aggregateAE.getAnalysisEngineMetaData()
					.getCapabilities();
	...
	/**
	 * Test whether the output sofa specified in the AE descriptor are in the AE
	 * meta data.
	 * 
	 */
	...
	/**
	 * Tests programmatically specifying the sofa name mapping in a aggregate
	 * AE.
	 * 
	 */
	public void testSetSofaNameMappingInAggregateDescriptor() throws Exception {
		try {
			...
			AnalysisEngine delegateAE1 = (PrimitiveAnalysisEngine_impl) aggregateAE
					._getASB().getComponentAnalysisEngines().get("Translator1");
			Assert.assertEquals(2, delegateAE1.getUimaContext()
					.getSofaMappings().length);
			Assert.assertEquals("SourceDocument", delegateAE1.getUimaContext()
					.mapToSofaID("EnglishDocument").getSofaID());

			// get the second delegate AE
			AnalysisEngine delegateAE2 = (PrimitiveAnalysisEngine_impl) aggregateAE
					._getASB().getComponentAnalysisEngines().get("Translator2");
			Assert.assertEquals(2, delegateAE2.getUimaContext()
					.getSofaMappings().length);
			Assert.assertEquals("SourceDocument", delegateAE2.getUimaContext()
					.mapToSofaID("EnglishDocument").getSofaID());
