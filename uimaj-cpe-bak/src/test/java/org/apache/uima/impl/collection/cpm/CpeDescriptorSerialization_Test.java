 ...
 * 
 * After generating a new Descriptor with the modified data, the generated descriptor file 
 * must be equivalent to the referenced file.
 */
public class CpeDescriptorSerialization_Test extends TestCase {
	...
	/**
	 * test to be sure, that first reading and then writing a discriptor produces 
	 * the same file as the original.(old configuration file format)
	 * 
    * @throws Exception
    */
   public void testReadDescriptor() throws Exception {
	...
	/**
    * test to be sure, that first reading and then writing a discriptor produces 
	 * the same file as the original (new configuration file format).
	 *
	 * @throws Exception
    */
    ...
    * given descriptor from a file. Write the new descriptor back to a file and compare this with 
   ...
   public void testAddRemoteCasProcessor() throws Exception {
		...
		CpeCasProcessor casProcessor = CpeDescriptorFactory.produceRemoteCasProcessor("myTestCasPocessor");
		//set specifier path
   ...
   public void testAddLocalCasProcessor() throws Exception {
		...
		CpeLocalCasProcessor localProcessor = CpeDescriptorFactory.produceLocalCasProcessor("DupShingle Miner", "Detag:DetagContent");
	...
	/**
    * Create a integrated CasProcesser via API and add him to an existing cpe configuration read from a
    * given descriptor from a file. Write the new descriptor back to a file and compare this with 
    * an expected descriptor configuration.
	 *
	 * @throws Exception
   ...
   public void testAddIntegratedCasProcessor() throws Exception {
		...
		// add a new iCpe
		CpeIntegratedCasProcessor integratedProcessor = CpeDescriptorFactory.produceCasProcessor("WF Writer_X");
   ...
   public void equal(String aReferenceDescriptorFilePath, CpeDescription aGeneratedDescriptor)
   {
	   try
	   {
				...
				else
				{
					...
					if ( readers[0].getCollectionIterator() != null )
					{
						assertTrue(isCollectionIteratorTheSame(readers[0].getCollectionIterator(),
								aGeneratedDescriptor.getAllCollectionCollectionReaders()[0].getCollectionIterator()) );
					}
				
					if ( readers[0].getCasInitializer() != null )
					{
						assertTrue(isCasInitializerTheSame(readers[0].getCasInitializer(),
								aGeneratedDescriptor.getAllCollectionCollectionReaders()[0].getCasInitializer()) );
			...
			if ( refCasProcessors != null )
			{
				...
				{
					compareCasProcessors( referenceDesc.getCpeCasProcessors(), aGeneratedDescriptor.getCpeCasProcessors());
				}
			}
	   }
	   catch( Exception e)
   ...
   public void compareCasProcessors( CpeCasProcessors aRefCasProcessors, CpeCasProcessors aGenCasProcessors)
   {
	   ...
	   try
	   {
		   ...
		   {
			   fail( "Generated CPE Descriptor does not have any Cas Processors. Expected <"+expectedCasProcessorCount+"> Cas Processors");
		   }
		   else
		   {
			   assertEquals("Generated CPE Descriptor does not have expected number of Cas Processors. " +
			   		"Expected <"+expectedCasProcessorCount+"> Cas Processors. Got <"+aGenCasProcessors.getAllCpeCasProcessors().length+">",expectedCasProcessorCount,aGenCasProcessors.getAllCpeCasProcessors().length);
		   }
		   ...
		   for( int i=0; i < aRefCasProcessors.getAllCpeCasProcessors().length; i++)
		   {
			   ...
			   {
				   compareIntegratedCasProcessors( refCasProcessorList[i],aGenCasProcessors.getAllCpeCasProcessors()[i]);
			   }
			   else if ( refCasProcessorList[i].getDeployment().equals("remote"))
			   {
				   compareRemoteCasProcessors( refCasProcessorList[i],aGenCasProcessors.getAllCpeCasProcessors()[i]);
			   }
			   else if ( refCasProcessorList[i].getDeployment().equals("local"))
			   {
				   compareLocalCasProcessors( refCasProcessorList[i],aGenCasProcessors.getAllCpeCasProcessors()[i]);
			   }
		   }
	   }
	   catch( Exception e)
	   {
		   fail( "Exception::"+e.getMessage()+" thrown while retrieving Cas Processors From CPE Descriptor");
	   }
   }
   public void compareCommonElements(CpeCasProcessor aRefCasProcessor, CpeCasProcessor aGenCasProcessor)
   {
	   ...
	   if ( aRefCasProcessor.getDeploymentParams() == null )
	   {
		   ...
		   {
			   fail( aRefCasProcessor.getDeployment()+" Cas Processor with name="+aGenCasProcessor.getName()+" contains <deploymentParameters> element. Expected to find empty <deploymentParameters> element");
		   }
	   }
	   if ( aRefCasProcessor.getCasProcessorFilter() == null )
	   {
		   ...
		   {
			   fail( aRefCasProcessor.getDeployment()+" Cas Processor with name="+aGenCasProcessor.getName()+" contains <filter> element. Expected to find empty <filter> element");
		   }
	   }
	   if ( aRefCasProcessor.getErrorHandling() != null )
	   {
		   compareErrorHandling( aRefCasProcessor.getErrorHandling(), aGenCasProcessor.getErrorHandling(), aRefCasProcessor.getName());
	   }
	   if ( aRefCasProcessor.getCheckpoint() != null )
	   {
		   if ( aGenCasProcessor.getCheckpoint() == null )
		   {
			   fail( aRefCasProcessor.getDeployment()+" Cas Processor with name="+aGenCasProcessor.getName()+" is missing <checkpoint> element.");
		   }
		   else
		   {
			  assertEquals("Generated Cas Processor:"+aGenCasProcessor.getName()+" batch size does match expected value",aRefCasProcessor.getCheckpoint().getBatchSize(), aGenCasProcessor.getCheckpoint().getBatchSize()); 	
		   }
	   }
	   
   }
   public void compareLocalCasProcessors( CpeCasProcessor aRefCasProcessor, CpeCasProcessor aGenCasProcessor)
   {
	   compareCommonElements( aRefCasProcessor, aGenCasProcessor);
	   
   }
   public void compareRemoteCasProcessors( CpeCasProcessor aRefCasProcessor, CpeCasProcessor aGenCasProcessor)
   {
	   compareCommonElements( aRefCasProcessor, aGenCasProcessor);
   }
   public void compareIntegratedCasProcessors( CpeCasProcessor aRefCasProcessor, CpeCasProcessor aGenCasProcessor)
   {
	   compareCommonElements( aRefCasProcessor, aGenCasProcessor);
   }
   public void compareErrorHandling( CasProcessorErrorHandling aRefCasProcessorErrorHandling, CasProcessorErrorHandling aGenCasProcessorErrorHandling, String aComponentName)
   {
	   if ( aGenCasProcessorErrorHandling == null )
	   {
		   fail( "Integrated Cas Processor with name="+aComponentName+" is missing <errorHandling> element.");
	   }
	   else
	   {
		   if ( aRefCasProcessorErrorHandling.getErrorRateThreshold() != null )
		   {
			   ...
			   {
				   fail( "Expected element <errorRateThreshold> is missing from the CPE Descriptor for Cas Processor with name:"+aComponentName);
			   }
			   else
			   {
				   assertEquals(aRefCasProcessorErrorHandling.getErrorRateThreshold().getAction(), aGenCasProcessorErrorHandling.getErrorRateThreshold().getAction());
				   assertEquals( aRefCasProcessorErrorHandling.getErrorRateThreshold().getMaxErrorCount(), aGenCasProcessorErrorHandling.getErrorRateThreshold().getMaxErrorCount());
				   assertEquals(aRefCasProcessorErrorHandling.getErrorRateThreshold().getMaxErrorSampleSize(), aGenCasProcessorErrorHandling.getErrorRateThreshold().getMaxErrorSampleSize());
			   }
		   }
		   if ( aRefCasProcessorErrorHandling.getMaxConsecutiveRestarts() != null )
		   {
			   ...
			   {
				   fail( "Expected element <maxConsecutiveRestarts> is missing from the CPE Descriptor for Cas Processor with name:"+aComponentName);
			   }
			   else
			   {
				   assertEquals(aRefCasProcessorErrorHandling.getMaxConsecutiveRestarts().getAction(), aGenCasProcessorErrorHandling.getMaxConsecutiveRestarts().getAction());
				   assertEquals( aRefCasProcessorErrorHandling.getMaxConsecutiveRestarts().getRestartCount(), aGenCasProcessorErrorHandling.getMaxConsecutiveRestarts().getRestartCount());
				   assertEquals(aRefCasProcessorErrorHandling.getMaxConsecutiveRestarts().getWaitTimeBetweenRetries(), aGenCasProcessorErrorHandling.getMaxConsecutiveRestarts().getWaitTimeBetweenRetries());
			   }
		   }
		   if ( aRefCasProcessorErrorHandling.getTimeout() != null )
		   {
			   ...
			   {
				   fail( "Expected element <timeout> is missing from the CPE Descriptor for Cas Processor with name:"+aComponentName);
			   }
			   else
			   {
				   assertEquals(aRefCasProcessorErrorHandling.getTimeout().get(), aGenCasProcessorErrorHandling.getTimeout().get());
			   }
		   }
	   }
   ...
   public void isConfigTheSame(CpeConfiguration aRefCpeConfig, CpeConfiguration aGenCpeConfig )
   {
	  try
	  {
		  ...
		  else
		  {
			  ...
			  {
				  assertTrue( "Generated Cpe Descriptor has an invalid value for <startAt> in <cpeConfig>.Expected no value OR empty string. Instead got value of:"+aGenCpeConfig.getStartingEntityId(),aGenCpeConfig.getStartingEntityId().trim().length()==0);
			  }
		  }
		  if ( aRefCpeConfig.getCheckpoint() != null )
		  {
			  ...
			  else
			  {
				  if ( aRefCpeConfig.getCheckpoint().getFilePath() != null )
				  {
					  ...
					  {
						  assertEquals(aRefCpeConfig.getCheckpoint().getFilePath(),aGenCpeConfig.getCheckpoint().getFilePath());
					  }
				  }
				  assertEquals(aRefCpeConfig.getCheckpoint().getBatchSize(), aGenCpeConfig.getCheckpoint().getBatchSize());
				  assertEquals(aRefCpeConfig.getCheckpoint().getFrequency(),aGenCpeConfig.getCheckpoint().getFrequency());
			  }
		  }
	  }
	   catch( Exception e)
	   {
		   fail( "Exception::"+e.getMessage()+" thrown comparing configuration <cpeConfig> in CPE Descriptor");
	   }
   }
   public void compareDescriptors( CpeComponentDescriptor aRefDescriptor, CpeComponentDescriptor aGenDescritptor, String aComponentName )
   {
		if ( aRefDescriptor.getInclude() != null )
		{
			if ( aGenDescritptor.getInclude() == null )
			{
				fail("Component Descriptor of "+aComponentName+" missing from a generated CPE Descriptor");
			}
			else
			{
				...
				if ( refFilePath != null )
				{
					assertEquals( "Component Descriptor in Generated CPE Descriptor has unexpected value. Expected:"+refFilePath+" Got:"+genFilePath+" Instead.", refFilePath, genFilePath);
				}
			}
		}
   }
	public boolean isCollectionIteratorTheSame(CpeCollectionReaderIterator aRefIterator, CpeCollectionReaderIterator aGenIterator)
	{
	...
	public boolean isCasInitializerTheSame(CpeCollectionReaderCasInitializer aRefCasInitializer, CpeCollectionReaderCasInitializer aGenCasInitializer)
	{
