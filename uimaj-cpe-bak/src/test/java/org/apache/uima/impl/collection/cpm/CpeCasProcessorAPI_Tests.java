 ...
 * 
 * <p>
 * The general workflow of the TestCases is producing a default cpm and then adding
 * a new CasProcessor to this configuration. After that, the new configuration is read via 
 * API and checked for correctness.
 * </p>
 * <b>Attention!!!</b><br>
 * All parameters set in the script are only dummy values. They do not have any
 * corresponding files, values or something else. The parameters are only set to be
 * comparable for unit testing.
 *
 */
public class CpeCasProcessorAPI_Tests extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		CpeDescriptorFactory.produceCollectionReader(confLocation + "collectionReaders/WFReader.xml", cpe);
		//	Crate CasInitializer and associate it with the CollectionReader
		CpeDescriptorFactory.produceCollectionReaderCasInitializer(confLocation + "casinitializers/WFReaderInitializer.xml", cpe);
		...
		CpeRemoteCasProcessor remoteProcessor = CpeDescriptorFactory.produceRemoteCasProcessor("Detag Miner");
		...
		localProcessor.setExecutable("/home/cwiklik/cpm/wfcp/annotators/wfminers/detagger/bin/runDetagger.sh");
		...
		CpeIntegratedCasProcessor integratedProcessor = CpeDescriptorFactory.produceCasProcessor("WF Writer");
   ...
   public void testAddLocalCasProcessor() throws Exception {
		...
		assertEquals("DeploymentParam", "Value1x", ((localProcessor.getDeploymentParams()).get("Parm1")).getParameterValue());
		assertEquals("DeploymentParam", "Host1x", ((localProcessor.getDeploymentParams()).get("vnsHost")).getParameterValue());
		assertEquals("CasProcessorFilter", "where Detag_colon_DetagContent", localProcessor.getCasProcessorFilter());
   ...
   public void testAddIntegratedCasProcessor() throws Exception {
		...
		assertEquals("Descriptor()", confLocation + "consumers/bla.xml", integratedProcessor.getDescriptor());
		assertEquals("DeploymentParam", "V1.0", ((integratedProcessor.getDeploymentParams()).get("testversion")).getParameterValue());
