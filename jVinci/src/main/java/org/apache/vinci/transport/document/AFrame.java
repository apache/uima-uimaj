 ...
 * "fadd" methods of the Frame class, except they return an empty set of attributes that can then
 * be populated. See the "main()" method for an example of how to use this class to easily create
...
public class AFrame extends VinciFrame {
  ...
  /**
   * Override the createFrameLeaf to create an AFrameLeaf so that leaf values can
   * have attributes.
   *
   * @pre array != null
   */
   ...
   * @exception ClassCastException if the value was not of type AFrame.
  ...
  /**
   * Get the attributes associated with a particular key. If there is more than one matching
   * key, then the attributes of only the first matching key are returned.
   * 
   ...
   * 
   * @param key The key to be associated with the value.
   * @param val The data to be encoded and added to this frame.
   * @return The (empty) set of attributes associated with the added key.
   * 
   * @pre key != null
   */
   ...
   *
   * NOTE: This method should only be used when performance hit of Base64 encoding binary data
   * [as performed by aadd(String, byte[])] is unacceptable. 
   * 
   * @param key The key to be associated with the value.
   ...
   *
   * @throws IllegalStateException if VNS_HOST is not specified.
   */
  static public AFrame rpc(Transportable in, String service_name) throws IOException, ServiceException,
   ...
   * @throws IllegalStateException if VNS_HOST is not specified.
   */
  static public AFrame rpc(Transportable in, String service_name, int socket_timeout) throws IOException,
   ...
   * @throws IllegalStateException if VNS_HOST is not specified.
   */
  static public AFrame rpc(Transportable in, String service_name, int socket_timeout, int connect_timeout)
      throws IOException, ServiceException, ServiceDownException, VNSException {
    return (AFrame) VinciClient.sendAndReceive(in, service_name, getAFrameFactory(), socket_timeout, connect_timeout);
  }

  /*	public static void main(String[] args) {
   // EXAMPLE of creating a document with attributes.
   AFrame f = new AFrame();
   f.setAttributes((Attributes)new Attributes().fadd("foo", "bar"));
   // Once we create the empty AFrame, we can call "aadd" to add a key/value pair and
   // retrieve an empty set of attributes that we can then populate with attributes that will
   // be associated with the just-added key. Note that the attribute list object is itself a
   // special type of VinciFrame, thus we can use the standard "fadd()" methods to create the
   // attribute list.
   f.aadd("KEY", "value1") // Add XML element and retrieve the attribute list object
   .fadd("attr1", "aval1") // Add an attribute for the tag.
   .fadd("attr2", "aval2") // Add another attribute for the tag.
   .fadd("attr3", "you get the point");
   ...
   f.aadd("VALUELESS_KEY") // Add a valueless tag that has a couple of attributes
   .fset("foo", "bar")
   .fset("foo2", 2)
   .fset("foo", "replaced bar");

   // Here's how to create a nested AFrame: 
   AFrame nested = new AFrame();
   nested.aadd("NESTED_KEY", 1234.567)
   .fadd("nested", "attribute");
   f.aadd("NESTED_FRAME", nested)
   .fadd("attributename", new int[] {1,2,3});

   // We can use either fadd() or aadd() to add attributeless tags, though fadd is
   // preferrable since it avoids creating the attributes object.
   nested.fadd("ATTRIBUTELESS_KEY", "done with fadd()");
   nested.aadd("ANOTHER_ATTRIBUTELESS_KEY", "done with aadd()");

   System.out.println(f.toXML());
   System.out.println();

   // Querying key values:
   String foo_value = f.aget("VALUELESS_KEY").fgetString("foo");
