 ...
 * attributes or processing instructions.  Applications which require attributes 
 * should use the org.apache.vinci.transport.document.AFrame document model instead.
 *
 ...
 * [getKeyValuePair(int)]. The Frame descendent QueryableFrame provides additional getter
 * methods. QueryableFrame should be extended instead of Frame if this more powerful query support
 * is necessary.
 *
 ...
 *
 * Typically you will use VinciFrame, a concrete descendent of the (Queryable)Frame class, for
 * most of your code. You may however also wish to implement specialized Frame descendents for
 * optimized and/or type-safe handling of particular queries. For example, see ResolveResult and
 * ServeonResult (in package org.apache.vinci.transport.vns.client), which do this for the two most
 * common VNS queries. Automated stub generators may also wish to extend Frame to generate Java
 * object to Vinci XML document adapters.
 *
 */
public abstract class Frame extends FrameComponent implements Transportable {
  ...
  /**
   * This method lets you replace the default XTalk marshaller with another one, for example if
   * you want to use a different wire format (such as XML/SOAP), or if you want to easily
   * exploit an optimized/native implementation.
   *
   * @param transporter The new marshaller to plug in. 
   */
  ...
  /**
   * Add a tagged value to this frame (value is either a Frame or FrameLeaf).  Frames that
   * support marshalling from a stream must implement this method. 
   * 
   * While the method is conceptually abstract, I provide a default error-only implementation in
   * cases where marshalling the document to a stream is not necessary (e.g. the document is
   * used as an input source only).
   *
   * @param tag The tag name with which to associate the value.
   * @param val The (Frame | FrameLeaf) value to associate with the tag. 
  ...
  /**
   * Return the specified KeyValue pair. Frames that support marshalling to (but not from) a
   * stream via the Transportable interface must implement this method. We chose this simple
   * iteration method over providing an interator object since it doesn't require allocating any
   * new objects.
   * 
   * While conceptually abstract, I provide a default error-only implementation to avoid
   * the need to define it in cases where alternative getters are provided for
   * querying the document, and the object is never marshalled to a stream.
   *
   * @param which The index of the KeyValuePair to retrieve.
   * @throws UnsupportedOperationException
  ...
  /**
   * Return the number of key/value pairs within this frame. Frames that support
   * marshalling to (but not from) a stream via the Transportable interface must implement
   * this method.
   * 
   * While conceptually abstract, I provide a default error-only implementation to avoid
   * the need to define it in cases where alternative getters are provided for
   ...
   * @throws IOException Can come from the underlying input stream.
   * @throws UnsupportedOperationException if this document model does not support key addition.
   ...
   *
   * @param os The stream to where the document is written.
   * @throws IOException Can come from the underlying output stream. 
   * @throws UnsupportedOperationException if this document model does not support key iteration.
   * 
   * @pre os != null
   */
  ...
  /**
   * Convert the document to XML. Performs limited pretty printing -- if you don't want any
   * pretty printing, use toRawXML().
   *
   ...
   * Convert the document to XML without any pretting printing.
   *
   * @param buf The StringBuffer to append the document text to.
   ...
   *
   * @param buf The StringBuffer where the document is appended.
  ...
  /**
   * Factory method used by fromStream when it needs to create a frame leaf. Default
   * implementation creates a regular FrameLeaf.
   *
  ...
  /**
   * Factory method used by fromStream when it needs to create a sub-frame. Default
   * implementation creates a subframe of the same type as the current frame.
   *
   * @throws UnsupportedOperationException if the getClass().newInstance() call on this object results in an exception.
   ...
   * @return This frame
   * @throws UnsupportedOperationException if this document model doesn't support key addition.
   *
   * @pre key != null
   */
  ...
  /**
   * Decorator method for adding double-array valued tags. Adding a null array results in a
   * no-op.
   * 
   * @param key The key to be associated with the value.
   * @param val The array to add. The array is immediately converted to string representation.
   * @return This frame 
   * @throws UnsupportedOperationException if this document model doesn't support key addition.
   *
   * @pre key != null
  ...
  /**
   * Decorator method for adding String-array valued tags. Adding a null value results
   * in a no-op.
   *
   * This implementation will use the '#' char as the string separator. If the '#' char is used
   * in some string, then the implementation generates a separator that is not contained by any
   * string and prepends the separator to the string for extraction.  The generated separator
   * always will begin and end with the character '#'. This allows arbitrary string arrays to be
   * sent as a single string without separator conflicts.
   * 
   * @param key The key to be associated with the value.
   * @param val The string to add.
   * @throws UnsupportedOperationException if this document model doesn't support key addition.
   ...
   *         $assert(val[i] != null, "array elements are non-null"); 
   * }
   */
  public Frame fadd(String key, String[] val) {
    if (val != null) {
  ...
  /**
   * Decorator method for adding binary valued tags. Encodes the data in Base64. Adding a null
   * value results in a no-op.
   * 
   * @param key The key to be associated with the value.
   * @param val The binary data to add (will be Base64 encoded).
   * @return This frame.  
   * @throws UnsupportedOperationException if this document model doesn't support key addition.
   *
   * @pre key != null
   ...
   * 
   * @param key The key to be associated with the value.
   * @param val The sub-frame to add. Note this frame is not copied.
   * @return This frame.
   * @throws UnsupportedOperationException if this document model doesn't support key addition.
   * 
   * @pre key != null
   ...
   * @return This frame.
   * @throws UnsupportedOperationException if this document model doesn't support key addition, or creation of empty sub-frames.
   *
   * @pre key != null
   */
   ...
   * XTalk-1.0 compatible which formally requires only UTF-8, but it still works. Binary data
   * added using this method can be retrieved using QueryableFrame/VinciFrame getter method
   * fgetTrueBinary(String). Adding a null value results in a no-op.
   *
   * WARNING: if the default XTalk parser is replaced with another one, applications that depend
   * on this method may break!
   *
   * WARNING #2: This method should only be used when performance hit of Base64 encoding binary
   * data [as performed by fadd(String, byte[])] is unacceptable.
   *
   * WARNING #3: The byte array is NOT copied, thus it is up to the caller to ensure that the
   * byte array cannot be modified by external code after passed in to this object.
   * 
   * @param key The key to be associated with the value.
   * @param val The byte array to be added to the frame. Note the array is NOT copied or
