...
public class TypeSystemSelectionDialog extends TwoPaneElementSelector {
  ...
  /*
   * A string comparator which is aware of obfuscated code (type names starting with lower case
   * characters).
   */
  private static class StringComparator implements Comparator {
    public int compare(Object left, Object right) {
      ...
      if (isLowerCase(leftString.charAt(0)) && !isLowerCase(rightString.charAt(0)))
        return +1;

      if (isLowerCase(rightString.charAt(0)) && !isLowerCase(leftString.charAt(0)))
        return -1;
