...
public class PrimitiveSection extends AbstractSection {
  ...
  private Button outputsNewCASes;
	 ...
	 * Only enabled if annotator is primitive
   * Also has the operational parameters
	 */
    ...
    if (isAeDescriptor() || isCasConsumerDescriptor() || isFlowControllerDescriptor()) {
      multipleDeploymentAllowed = newCheckBox(
          sectionClient,
	...
	public void handleEvent(Event event) {
			...
			try {
				...
				if ("TAF".equals(implKind) || "org.apache.uima.cpp".equals(implKind)) {
					...
					SelectionDialog typeDialog = JavaUI.createTypeDialog(
							getSection().getShell(), editor.getEditorSite()
									.getWorkbenchWindow(),
			  ...
			  ops.setModifiesCas(
			  		setValueChangedBoolean(modifiesCas.getSelection(), ops.getModifiesCas()));
		} else if (event.widget == multipleDeploymentAllowed) {
		  ops.setMultipleDeploymentAllowed(
		  		setValueChangedBoolean(multipleDeploymentAllowed.getSelection(), ops.isMultipleDeploymentAllowed()));
      ...
      ops.setOutputsNewCASes(
          setValueChangedBoolean(outputsNewCASes.getSelection(), ops.getOutputsNewCASes()));
			...
			editor.getAeDescription().setAnnotatorImplementationName(
					setValueChanged(implName.getText(), editor
							.getAeDescription()
