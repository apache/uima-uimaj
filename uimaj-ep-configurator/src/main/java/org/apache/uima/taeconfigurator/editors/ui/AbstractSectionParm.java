...
public abstract class AbstractSectionParm extends AbstractSection {
  ...
  public final static String NOT_IN_ANY_GROUP = Messages.getString("AbstractSectionParm.notInAnyGroup"); //$NON-NLS-1$
  public final static String COMMON_GROUP = Messages.getString("AbstractSectionParm.common"); //$NON-NLS-1$
  // maintainers note: names below have extra trailing blanks to get them to approximately line up where possible
  protected final static String DELEGATE_HEADER = Messages.getString("AbstractSectionParm.delegateKeyName"); //$NON-NLS-1$
  protected final static String FLOWCTLR_HEADER = "Flow Controller Key Name: ";
  protected final static String GROUP_HEADER    = Messages.getString("AbstractSectionParm.headerGroupNames"); //$NON-NLS-1$
  protected final static String COMMON_GROUP_HEADER = Messages.getString("AbstractSectionParm.headerCommon"); //$NON-NLS-1$
  protected final static String NOT_IN_ANY_GROUP_HEADER = Messages.getString("AbstractSectionParm.headerNotInAnyGroup"); //$NON-NLS-1$
  protected final        String OVERRIDE_HEADER = Messages.getString("AbstractSectionParm.overrides");  // nonstatic for easy ref in subclass //$NON-NLS-1$
   ...
   */
  public AbstractSectionParm(MultiPageEditor aEditor, Composite parent,
  ...
  /**
   * Two modes:  
   *   settingsDisplayMode - if true, shows groups one name at a time, and
   *                         puts all <common> parms in other groups
   */
  protected void clearAndRefillTree(boolean usingGroups) {
    ...
    try {
      ...
      for (int i = 0; i < groups.length; i++) {
        ...
        if (null == cps)
          groups[i]
              .setConfigurationParameters(cps = configurationParameterArray0);
        fillGroup(groups[i].getConfigurationParameters(), groups[i].getNames(),
            groups[i]);
      }
  ...
  /**
   * called by refresh() for <Common>  and refresh() for named,  
   * via another path with names as array first converted to concatenated string.
   * @param parms
   * @param names
   */
  private void fillGroup(ConfigurationParameter[] parms, 
                           String names,
  ...
  return (String []) items.toArray(stringArray0);
	...
	/**
	 * Takes an existing model parm and fills a pre-allocated treeItem.
	 * 3 callers:  addNewConfigurationParameter,
	 *             alterExistingConfigurationParamater (editing),
	 *             fill (bulk update from refresh)
	 * @param item
	 * @param parm
	 */
...
//    if (item.getParent() == parameterSectionTree)
    ...
    return
    ((parm.isMultiValued()) ? MULTI_VALUE_INDICATOR : SINGLE_VALUE_INDICATOR) + 
	 ...
	 * @param newCP
	 * @param group - is never null.  May be <Not in any group>, indicate no groups; may be the "<Common>" group; or 
	 * may be a regular group with a set of group names
	 */
	...
	protected boolean isParameter(TreeItem item) {
	  ...
	  return ( ! isGroup(item) &&
	           ! s.startsWith(DELEGATE_HEADER) &&
             ! s.startsWith(FLOWCTLR_HEADER) &&    
	...
	protected boolean isGroup(TreeItem item) {
	  String s = item.getText();
	  return s.startsWith(GROUP_HEADER) ||
	         s.startsWith(COMMON_GROUP_HEADER) ||
	...
	protected boolean isDelegate(TreeItem item) {
	  return item.getText().startsWith(DELEGATE_HEADER) || 
           item.getText().startsWith(FLOWCTLR_HEADER);
	}
  ...
  protected String getName(TreeItem item) {
	 ...
	 * We don't use any relative index offsets.
	...
	protected TreeItem [] getSettingsParameter (TreeItem containingGroup, String sourceItemName) {
	  ...
	  if (null != containingGroup && isNOT_IN_ANY_GROUP(containingGroup))
	    return new TreeItem [] {
	  		findMatchingParm(settingsTree.getItems()[0], sourceItemName)};
	  	  
	  TreeItem [] groups = getSettingsGroups(
	  		(null == containingGroup) 
	  		   ? tree.getItems()[1]   // use common group, will return all groups in settings pg
	  		   : containingGroup);
	...
	/**
	 * get set of settings group from settingsTree that correspond to
	 * parmsection group 
	 * @param group
	 * @return
	 */
	protected TreeItem[] getSettingsGroups(TreeItem group) {
	  ...
	  if (groupNamesArray.length == 1 &&
	      groupNamesArray[0].equals(COMMON_GROUP)) {
	...
	protected ConfigurationParameter getCorrespondingModelParm(TreeItem item) {
	  if ( ! isParameter(item)) throw new InternalErrorCDE("invalid argument"); //$NON-NLS-1$
			  return (ConfigurationParameter)item.getData();
	}
	
	protected ConfigGroup getCorrespondingModelGroup(TreeItem item) {
	  if ( ! isGroup(item)) throw new InternalErrorCDE("invalid argument"); //$NON-NLS-1$
	 ...
	 * If settings page is shown, also update the GUI. 
	 * @param treeItem in ParameterSection of parameter belonging to (multiple) groups
	 */
	public void removeParmSettingFromMultipleGroups(TreeItem parmItem, boolean removeFromGUI) {
	  if ( ! isParameter(parmItem)) throw new InternalErrorCDE("invalid argument"); //$NON-NLS-1$
	  ...
	  if ( ! groupName.equals(NOT_IN_ANY_GROUP)) {
	    String [] groupNames = (getName(parent.getText()).equals(COMMON_GROUP))
	                ? getAllGroupNames()
	...
	public void updateParmInSettingsGUI( 
	    ConfigurationParameter existingCP, 
	    TreeItem existingTreeItem,
	    String prevName) {
	  if (null != settings) {
      TreeItem [] settingsTreeParms = getSettingsParameter(existingTreeItem.getParentItem(), prevName);
      for (int i = 0; i < settingsTreeParms.length; i++) {
...
//      }
      for (int j = 0; j < commonParms.length; j++) {
        modelSettings.setParameterValue(allGroupNames[i], commonParms[j].getName(),
            null);   
      }
    }
	...
	/**
	 * Remove some of the parameter settings associated with this particular group,
	 * not all the parameters for that group name (some parameters may
	 * be associated with other instances of a particular group name.)
	 * If no other group-set contains a particular individual group name, 
	 * in the Settings: remove the common parameters, and remove the individual group itself.
	 * Remove the particular group-set definition. Note that a group
	 * may be defined in more than one group-set.
	 * 
	 * Method: for the group-set, get the parms.
	 * Remove just those parms from all groups.  
	 * Remove the group on the settings page (together with common parms for it) if
	 *   no other group-set has this group name
	...
	/**
	 * 
	 * @param treeItem in ParameterSection of items an array of tree items to remove
	 * Can be all items under a particular group, or
	 * a set of items from different groups
	 */
...
//    modelSettings.setParameterValue()
    ...
    if (null != settings) {
			...
			if (groupName.equals(COMMON_GROUP) ||
					groupName.equals(NOT_IN_ANY_GROUP)) {
    ...
    if (null != item && isParameter(item)) {
      setToolTipText(tree, 
          getCorrespondingModelParm(item).getDescription());           
    }
    else
