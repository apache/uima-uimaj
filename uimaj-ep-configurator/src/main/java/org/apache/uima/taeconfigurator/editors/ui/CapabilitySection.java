...
public class CapabilitySection extends AbstractSection {
  ...
  public void initialize(IManagedForm form) {
    ...
    addLangButton = newPushButton(buttonContainer, "Add Language", "Click here to add a Language Capability to the selected set.");
    addTypeButton = newPushButton(buttonContainer, "Add Type", "Click here to add a Type to the selected capability set.");
    addSofaButton = newPushButton(buttonContainer, "Add Sofa", "Click here to add a Subject of Analysis (Sofa) to the selected capability set.");
    addEditFeatureButton = newPushButton(buttonContainer, "Add/Edit Features", "Click here to specify the features of a selected type as input or output");
    editButton = newPushButton(buttonContainer, S_EDIT, "Edit the selected item. You can also double-click the item to edit it.");
    removeButton = newPushButton(buttonContainer, "Remove", "Remove the selected item.  You can also press the Delete key to remove an item.");
  }
	  ...
	  if (null != inputs) {
	    for (int i = 0; i < inputs.length; i++) {
          ...
          tc.isInputType = true;
	        ...
	        tc = getTypeCapability(getTypeNameFromFullFeatureName(name)); // create a typecapability if one doesn't exist
...
//      throw new InternalErrorCDE("invalid state");
    ...
    case SOFA_ITEM: {
      ...
      if (dialog.isInput == existingIsInput &&
      		dialog.sofaName.equals(existingSofaName))
      	return;
      ...
      for (int i = 0; i < cSets.length; i++) {
        ...
        if (wasRemoved) {
					if (dialog.isInput) {
						cSets[i].setInputSofas(stringArrayAdd(cSets[i].getInputSofas(),
								dialog.sofaName));
					} else {
						cSets[i].setOutputSofas(stringArrayAdd(cSets[i].getOutputSofas(),
								dialog.sofaName));
					}
				}
      }
...
//      setGuiSofaName(editItem, dialog.sofaName, dialog.isInput);
  ...
  private boolean anyCapabilitySetDeclaresSofa(String name, boolean isInput) {
  	final Capability [] cSets = 
  		getAnalysisEngineMetaData().getCapabilities();
  	for (int i = 0; i < cSets.length; i++) {
  		final String [] sofaNames = isInput? 
  				cSets[i].getInputSofas() : cSets[i].getOutputSofas();
    ...
    switch (itemKind) {
    case CS : {
      if (Window.CANCEL == Utility.popOkCancel(
          "Confirm Remove",
    ...
    case SOFA_ITEM: {
      if (Window.CANCEL == Utility.popOkCancel(
        ...
        return;
   	  ...
   	  if (isInput)
   	  	c.setInputSofas((String [])Utility.removeElementFromArray(
   	  			c.getInputSofas(), sofaName, String.class));
   	  else
   	  	c.setOutputSofas((String [])Utility.removeElementFromArray(
   	  			c.getOutputSofas(), sofaName, String.class));
   	  removeItem.dispose();
    ...
    case TYPE: {
      if (Window.CANCEL == Utility.popOkCancel(
          "Confirm Removal of Type", 
					"This action will remove this type as a capability.  Please confirm.", MessageDialog.WARNING)) {
        table.setSelection(table.getSelectionIndex() + 1);
        return;
      }
	...
	private void removeFeature(Capability c, TableTreeItem removeItem) {
	  ...
	  if (shortFeatureName.equals(ALL_FEATURES)) {
	    if (isInput(removeItem)) {
	      TypeOrFeature tfItem = getTypeOrFeature(
	          c.getInputs(), getFullyQualifiedName(removeItem.getParentItem()));
	      tfItem.setAllAnnotatorFeatures(false);
	    }
	    if (isOutput(removeItem) /*|| isUpdate(removeItem)*/) {
	      TypeOrFeature tfItem = getTypeOrFeature(
	          c.getOutputs(), getFullyQualifiedName(removeItem.getParentItem()));
	      tfItem.setAllAnnotatorFeatures(false);
	    }
	  } else {
    ...
    Capability c = getCapabilityFromTableTreeItem(selItem.getParentItem());
		...
		c.setLanguagesSupported(
		    stringArrayAdd(c.getLanguagesSupported(), dialog.getValue()));
  ...
  private void handleAddEditFeature(TableTreeItem selItem, int itemKind) {
	...
	private void addOrEditFeature(
	    AddCapabilityFeatureDialog dialog,
  ...
  /**
   * Given a current list of inputs/ outputs, made up of "Types" and "features",
   * make a new list keeping all the types,
   * and keeping all the features that belong to other types,
   * and adding the features that are passed in for one particular type in the
   * "features" parameter
   * @param items
   * @param type A string representing the fully qualified type name
   * @param features - associated with the type
   * @return
   */
  private TypeOrFeature [] replaceFeaturesKeepingTypes(TypeOrFeature [] items, String typeName, List features) {
    ...
    if (null != items) 
      for (int i = 0; i < items.length; i++) {
        if (items[i].isType() ||
            !items[i].getName().startsWith(typeName))
      ...
      for (int i = 0; i < languages.length; i++) {
        if (languages[i].equals(dialog.getValue())) {
          Utility.popMessage("Language spec already defined",
              ...
              + "\n\nLanguage: "
              + dialog.getValue(), MessageDialog.ERROR);
