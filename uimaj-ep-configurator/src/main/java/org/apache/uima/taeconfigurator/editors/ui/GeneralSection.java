...
public class GeneralSection extends AbstractSection {
  ...
  private Button tafButton;
	...
	/**
	 * Creates a section to edit general information
	 * like primitive or aggregate and TAF or JEDII
  ...
  */  
   ...
   */
    ...
    if (isAeDescriptor() || isCasConsumerDescriptor()) {
      ...
      jediiButton.setSelection("org.apache.uima.java".equals(implType)
          || "JEDII".equals(implType)); // JEDII is deprecated
    }
	}
...
//      				"Cas Consumers, Cas Initializers, Collection Readers, and Flow Controllers cannot be Aggregates.",
//							MessageDialog.ERROR);
...
//      	}
    ...
    if (event.widget == jediiButton || event.widget == tafButton) {
      ...
      if (tafButton.getSelection()) {
        editor.getAeDescription().setFrameworkImplementation(
        		setValueChanged("org.apache.uima.cpp",
        				editor.getAeDescription().getFrameworkImplementation()));
      } else {
        editor.getAeDescription().setFrameworkImplementation(
            setValueChanged("org.apache.uima.java",
            		editor.getAeDescription().getFrameworkImplementation()));
      }
      if (!valueChanged)
      	return;
    ...
    // reset the flow to fixed flow with null as the set
      FlowConstraints flowConstraints = UIMAFramework.getResourceSpecifierFactory().createFixedFlow();
    ...
    //reset the parm settings
		ConfigurationParameterSettings configParmSettings = 
    ...
    // reset resources
		editor.getAeDescription()
		    .setExternalResourceDependencies(new ExternalResourceDependency [0]);
