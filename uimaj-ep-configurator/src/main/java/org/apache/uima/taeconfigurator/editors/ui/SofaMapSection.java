...
public class SofaMapSection extends AbstractSection {
  ...
  private static final String titleMsg = "This section shows all defined Sofas for an Aggregate and their mappings to the component Sofas.\n" +
	...
	public SofaMapSection(MultiPageEditor aEditor, Composite parent) {
		super(aEditor, parent, "Sofa Mappings (Only used in aggregate Descriptors)",
		    titleMsg);
	}
   ...
   */  
	...
	private String [] getAggrSofas( 
			                 String [] inputCapabilityNames,
			                 String [] outputCapabilityNames) {
  ...
  private void fillBindings(TreeItem parent, String aggrSofa) {
    ...
    SofaMapping [] sofaMappings = getSofaMappings();
		if (null == sofaMappings) return stringArray0;
			...
			if (sofaMapping.getAggregateSofaName().equals(aggrSofa))
				if (null != sofaMapping.getComponentSofaName() &&
				  ...
				  bindings.add(sofaMapping.getComponentKey() + '/' + 
						         sofaMapping.getComponentSofaName());
				else
					bindings.add(sofaMapping.getComponentKey());
		}
	...
	private void editAggrMap(TreeItem selected) {
  	...
  	EditSofaBindingsDialog dialog = new EditSofaBindingsDialog(
  			this, aggrSofa, availAndBoundSofas);
  ...
  private void addSofasToAllComponentSofaMap (
      Map allComponentSofas,
      String key,
   ...
   */
	private Map getAvailAndBoundSofas(String aggrSofa, boolean availOnly) {
		...
		for (Iterator it = editor.getResolvedDelegates().entrySet().iterator(); 
		     it.hasNext();) {
      Map.Entry entry = (Map.Entry)it.next();
      addSofasToAllComponentSofaMap(
          allComponentSofas, (String)entry.getKey(), (ResourceSpecifier) entry.getValue(), isInput);     
		}
		// pick up any sofa info from flow controller
    FlowControllerDeclaration fcd = editor.getResolvedFlowControllerDeclaration();
    if (null != fcd) {
      addSofasToAllComponentSofaMap(
          allComponentSofas, fcd.getKey(), fcd.getSpecifier(), isInput);     
    }
	...
	private void removeAggr(String aggrSofa) {
		Comparator comparator = new Comparator() {
			public int compare(Object AggrSofaName, Object o2) {
				...
				if (sofaMapping.getAggregateSofaName().equals((String)AggrSofaName))
					return 0;
				else 
					return -1;
			}	};
		editor.getAeDescription().setSofaMappings(
				(SofaMapping []) Utility.removeElementsFromArray(
	  		    getSofaMappings(), 
	  		    aggrSofa, 
	  		    SofaMapping.class, 
	  		    comparator));	
   ...
   * Removes from the sofaMappings, any and all mappings associated with 
   *   the delegate.
   * @param componentSofa
   */
  public static void removeSofaMappings(
  		String componentKey, ResourceSpecifier delegate, MultiPageEditor pEditor) {
  	if (delegate instanceof AnalysisEngineDescription ||
        delegate instanceof CasConsumerDescription) {
