...
public class ParameterDelegatesSection extends AbstractSectionParm {
   ...
   */  
	...
	public void refresh() {
      ...
      for (Iterator it = editor.getResolvedDelegates().entrySet().iterator(); it
          .hasNext();) {
...
//	  }
  ...
  private void addDelegateToGUI(String key, ResourceSpecifier delegate) {
    if (delegate instanceof AnalysisEngineDescription || 
      ...
      TreeItem d = new TreeItem(tree, SWT.NONE);
      d.setText(((delegate instanceof FlowControllerDescription) ? 
                  FLOWCTLR_HEADER : DELEGATE_HEADER) + 
                  key);
      d.setData(key);
      addDelegateGroupsToGUI(d, (ResourceCreationSpecifier)delegate);
      d.setExpanded(true);
...
//	  	throw new InternalErrorCDE("Invalid state");
  ...
  private void addNewParameter(ConfigurationParameter parm,
       ConfigGroup delegateGroup, 
       String key) {
    ...
    if (null != (overrideParmName = getOverridingParmName(override, cpd))) {
      Utility.popMessage("Only one override allowed",
        "This delegate parameter already is being overridden by '" +
        overrideParmName + "'.  To override " +
    ...
    if (null != (parmInGroup = getSameNamedParmInGroup(parm, group))) {
      if (( ! createNonSharedOverride) && parmSpecMatches(parm, parmInGroup)) {
    	...
    	if (ParameterSection.parameterNameAlreadyDefinedNoMsg(
    			  parm.getName(), getConfigurationParameterDeclarations())){
  ...
  public static String getOverridingParmName(String override, ConfigurationParameterDeclarations cpd) {
    ...
    if (null != groups) 
      for (int i = 0; i < groups.length; i++){
        if (null != (result = getOverridingParmName(override, groups[i].getConfigurationParameters())))
          return result;
      }
    return null;
  }
