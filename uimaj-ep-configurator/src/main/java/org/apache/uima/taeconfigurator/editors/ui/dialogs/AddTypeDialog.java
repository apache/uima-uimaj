...
//  private boolean seenThisAlready = false;
	...
	protected Control createDialogArea(Composite parent) {
		...
		createWideLabel(mainArea, "Type names must be globally unique, unless you are intentionally redefining another type.");
  ...
  public ArrayList getTypeSystemInfoList() {
	  ...
	  if (null != existingTd) {
	  	hasFeatures = ((null != existingTd.getFeatures()) &&
	  			           (existingTd.getFeatures().length > 0));
	...
	public boolean isValid() {
	  ...
	  if (!typeContainedInTypeSystemInfoList(supertypeName, allTypesList)) {
	    setErrorMessage("SuperType '" + supertypeName + "' is unknown. If this is intended, please define it first.");
	    return false;
	  }
