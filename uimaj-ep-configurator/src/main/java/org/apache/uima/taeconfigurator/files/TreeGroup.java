...
public class TreeGroup implements ICheckStateListener, SelectionListener,
   ...
   * item which should be white-checked will result in a <code>true</code> answer from this
   * method. To determine whether a tree item should be white-checked use method
    ...
    if (state) {
      if (checkedListItems == null) {
        ...
        for (int i = 0; i < listTableItems.length; i++) {
          if (listTableItems[i].getData() != listElement && listTableItems[i].getChecked()) {
  ...
  /**
   * Handle the selection of an item in the tree viewer Call-back for selectionChangedListener for
   * the tree viewer
   * 
   * @param selection
   *          ISelection
   ...
   * 
   * @param Map
   *          with keys of Object (the tree element) and values of List (the selected list
   *          elements).
   */
  public void updateSelections(final Map items) {
    ...
    BusyIndicator.showWhile(treeViewer.getControl().getDisplay(), new Runnable() {
      public void run() {
        ...
        // Update the store before the hierarchy to prevent updating parents before all of the
        // children are done
