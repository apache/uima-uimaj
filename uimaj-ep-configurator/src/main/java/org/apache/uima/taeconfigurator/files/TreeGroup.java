 ...
 * org/eclipse/ui/internal/dialogs/ResourceTreeAndListGroup.java version 2.1
 ...
 */
public class TreeGroup implements ICheckStateListener, SelectionListener, ISelectionChangedListener, ITreeViewerListener {
	Object			root;
	Object			currentTreeSelection;
	private	List			expandedTreeNodes = new ArrayList();
	Map				checkedStateStore = new HashMap(9);
	...
	int lastTime = 0; // hack to filter double selection events coming from gtx linux
...
public TreeGroup(
  ...
  commonSetup(parent, rootObject, treeContentProvider, treeLabelProvider,
...
public TreeGroup (
  	...
  	ITreeContentProvider treeContentProvider,ILabelProvider treeLabelProvider,
  	IStructuredContentProvider listContentProvider,ILabelProvider listLabelProvider,
  	int style,int width,int height, boolean aSingleSelectionMode) {
...
private void commonSetup(	Composite parent, Object rootObject,
	...
	treeLabelProvider = aTreeLabelProvider;
	listLabelProvider = aListLabelProvider;
 ...
 */
void addToHierarchyToCheckedStore(Object treeElement) {
...
//			  System.out.println("Calling from checkStateChangedEvent.");
...
protected void createListViewer(Composite parent, int width, int height) {
	listViewer = CheckboxTableViewer.newCheckList(parent, SWT.BORDER /*| SWT.FULL_SELECTION*/);
	...
	listViewer.getTable().addSelectionListener(this);
}
/**
 *	Create this group's tree viewer.
 */
protected void createTreeViewer(Composite parent, int width, int height) {
	Tree tree = new Tree(parent, ((singleSelectionMode)? 0 : SWT.CHECK) | SWT.BORDER);
	GridData data = new GridData(GridData.FILL_BOTH);
	data.widthHint = width;
	data.heightHint = height;
...
public void widgetSelected(SelectionEvent event) {
  if (event.detail == SWT.CHECK)
    return;  // trigger just on selection event
  if (lastTime == event.time) // dupl sel event on linux
  	return;
  lastTime = event.time;
...
//  System.out.println("WidgetSelected: Item before action was checked = " + item.getChecked());
  table.setSelection(-1); 
  item.setChecked(!item.getChecked());
  listItemChecked(listViewer.getElementAt(selectionIndex), item.getChecked(), true);
  notifyCheckStateChangeListeners(new CheckStateChangedEvent(listViewer, listViewer.getElementAt(selectionIndex), item.getChecked()));
}
...
protected void listItemChecked(
	Object listElement,  // is instance of "File"
	boolean state,
	...
	if (state) {
		if (checkedListItems == null) {
		  ...
		  // reset other checked item in the list - mode means to only pick one
		  TableItem [] listTableItems = getListTable().getItems();
		  for (int i = 0; i < listTableItems.length; i++) {
		    if (listTableItems[i].getData() != listElement &&
		        listTableItems[i].getChecked()) {
		      listTableItems[i].setChecked(false);
		      checkedListItems.remove(listTableItems[i].getData());
		      }
		  }	  
		}
 ...
 *  Call-back for selectionChangedListener for the tree viewer
...
/**
 * Select or deselect all of the elements in the tree depending on the value of
