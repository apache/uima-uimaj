...
//		autoAddToFlowButton.setBackground(null);
	...
	public void update() {
		...
		ResourceSpecifier rs = (ResourceSpecifier) m_delegateComponentDescriptions.get(nSelectedAeIndex);
...
//	}
  ...
  public void copyValuesFromGUI() {
		...
		for(int i = 0, j = 0; i < selIndices.length; i++) {
			if(!delegateComponentListGUI.getItem(selIndices[i]).endsWith("*")) {
				m_selectedDelegateComponentDescriptors[j] = (String) m_DelegateComponentDescriptors.get(selIndices[i]);
			}
		}
