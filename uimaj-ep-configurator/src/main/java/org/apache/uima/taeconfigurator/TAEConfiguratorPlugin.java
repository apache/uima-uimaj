...
/*
 * Plugin not yet converted to OSGi Bundle (3.0 design) - 
 * therefore will require the compatibility interface.
 * 
 * The descriptor editor is a Multi-page editor, and 
 * implements the Model View Controller pattern (MVC).
 *   Model: Maintain data, basic logic plus one or more data sources
 *   View: Display all or a portion of the data.  
 *         Implements the GUI that displays information about the model to the user 
 *   Controller: Handle events that affect the model or view.
 *               The flow-control mechanism means by which the user interacts with the application 
 *
 *   View: in packages ...editors, editors.ui, and editors.ui.dialogs 
 *   Observer links: View updates from model when update() is called on the 
 *     page that is showing.  This happens when which page is showing changes, or
 *     when the model changes.
 * 
 * 
 * Life Cycle: 
 *   On first activation - does almost nothing.  Anything that could be done
 *     here is done lazily, on first use/need.
 *   On shutdown: shutdown call not done (it's deprecated.) Instead, we
 *     use the stop method.  It releases SWT resources. 
 */
...
public class TAEConfiguratorPlugin extends AbstractUIPlugin {
		...
		try {
			resourceBundle =
				ResourceBundle.getBundle(
					"org.apache.uima.taeconfigurator.taeconfigurator");
	...
	public static String getResourceString(String key) {
		ResourceBundle bundle =
			TAEConfiguratorPlugin.getDefault().getResourceBundle();
		try {
	...
	/**
	 * On first call, gets a formColors instance; 
	 * on subsequent calls, returns that instance.
	 * @param display
	 * @return
	 */
