...
public class PickOverrideKeysAndParmName 	extends LimitedResourceSelectionDialog {
  ...
  // rootElement is the map returned from getDelegateAnalysisEngineSpecifiers()
	...
	public PickOverrideKeysAndParmName(
	    Shell parentShell, 
	    Object rootElement, // is editor.getResolvedDelegates(), a Map, key = keyname, value = delegate description object
	    String message, 
	    ConfigurationParameter aCp,
	    ConfigurationParameterDeclarations aCpd,
	    boolean aAdding) {
	...
	protected TreeGroup createTreeGroup(Composite composite, Object rootObject) {
		return new TreeGroup(
				composite,
				rootObject,
				(keyTreeProvider = new KeyTreeProvider()),
				new KeyTreeLabelProvider(),
				new ParmNameProvider(),
				new ParmNameLabelProvider(),
				SWT.NONE,
        -1, -1,
	...
	class KeyTreeProvider implements ITreeContentProvider {
	  ...
	  /**
	   * for a given map of delegates, return an array of maps representing the those delegates having children.
	   * Not called for the top element, but called for subsequent layers
	   */
    ...
    /**
     * returns an array of Map.Entry elements: Key and AE or flow ctlr Description.
     * Called only for the top element
     */
    ...
    public Object[] getElements(Object inputElement) {
      ...
      if (rs instanceof AnalysisEngineDescription ||
      		rs instanceof CasConsumerDescription  ||
          rs instanceof FlowControllerDescription) {
      	 ConfigurationParameterDeclarations delegateCpd =
     ...
     *   (Group match not required)  
     */
    private void addSelectedParms(ConfigurationParameter [] parms, AbstractList items, String keyName) {
      boolean isMultiValued = (null != parameterDialog) ? 
          parameterDialog.multiValueUI.getSelection() :
          cp.isMultiValued();
      String type = (null != parameterDialog) ?
          parameterDialog.parmTypeUI.getText() :
          cp.getType();
      ...
      if (null != parms) {
        for (int i = 0; i < parms.length; i++) {
          ...
          if (adding && 
          		null != ParameterDelegatesSection.getOverridingParmName(override, cpd))
          	continue;
