...
public class AddCapabilityTypeDialog extends AbstractDialogMultiColTable {
  ...
  private static final int NAMESPACE = 3;
	...
	public AddCapabilityTypeDialog(AbstractSection aSection, Capability c) {
		...
		capability = c;
		enableCol1 = !aSection.isCasInitializerDescriptor() &&
		             !aSection.isCollectionReaderDescriptor();
    ...
    if (null == existing) {
      ...
      for (int i = 0; i < allTypes.length; i++) {
        if (!excludedTypes.contains(allTypes[i])
            && !hasType(capability.getInputs(), allTypes[i])
      ...
      item
          .setText(NAMESPACE, existing.getText(CapabilitySection.NAMESPACE_COL));
      TypeOrFeature tof = CapabilitySection.getTypeOrFeature(capability
          .getInputs(), capabilitySection.getFullyQualifiedName(existing));
      setChecked(item, 1, null != tof);
      tof = CapabilitySection.getTypeOrFeature(capability.getOutputs(),
  ...
  public void copyValuesFromGUI() {
    ...
    for (int i = table.getItemCount() - 1; i >= 0; i--) {
      ...
      if (item.getText(INPUT).equals(checkedIndicator(INPUT)) || 
        ...
        names.add(capabilitySection.getFullyQualifiedName(item.getText(NAMESPACE), item.getText(NAME)));
	...
	public boolean isValid() {
	  for (int i = 0; i < types.length; i++) {
	    if ( ! inputs[i]) {
	      ...
	      if ( ! outputs[i] &&
	          someFeatureOnType(types[i], OUTPUT) &&
	           ! inputs[i] ) {
	...
	private boolean someFeatureOnType(String typeName, int IO) {
	  ...
	  TypeOrFeature [] tofs = (IO == INPUT)? 
	      capability.getInputs() : capability.getOutputs();
    ...
    // even if it has the all-features flag on.
	  ...
	  for (int i = 0; i < tofs.length; i++) {
	    if ( ! tofs[i].isType() && 
	      tofs[i].getName().startsWith(typeNamePlusColon))
	      return true;
	  }
	  return false;
	}
