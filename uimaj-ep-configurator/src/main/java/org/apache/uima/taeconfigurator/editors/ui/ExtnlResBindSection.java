...
public class ExtnlResBindSection extends AbstractSection {
  ...
  public ExtnlResBindSection(MultiPageEditor aEditor, Composite parent) {
    super(aEditor, parent, "Resources Needs, Definitions and Bindings",
 	...
 	public void initialize(IManagedForm form) {
	  ...
	  // set up Composite to hold widgets in the section
		...
		addButton = newPushButton(buttonContainer, S_ADD, "Click to add a new External Resource definition" );
		editButton = newPushButton(buttonContainer, S_EDIT, "Click to edit an External Resource definition");
		removeButton = newPushButton(buttonContainer, "Remove", "Click to remove selected binding or External Resource");
		bindButton = newPushButton(buttonContainer, "Bind", "Click to bind selected dependency with selected Resource");
    ...
    initialFormWidth = getSection().getSize().x;
		((GridData)tree.getLayoutData()).widthHint = initialFormWidth - 
        buttonContainer.getSize().x;
  ...
  private void addExternalResourceDescriptionToGUI(
      ExternalResourceDescription xrd, ExternalResourceBinding [] bindings) {
  ...
  private void fillXrdItem(TreeItem item, ExternalResourceDescription xrd) {
    ...
    if (rs instanceof FileLanguageResourceSpecifier) {
      ...
      text.append("  URL_Prefix: ").append(flrs.getFileUrlPrefix())
          .append("  URL_Suffix: ").append(flrs.getFileUrlSuffix());
    }
  ...
  private void fillBindings(TreeItem parent, 
      ExternalResourceDescription xrd, ExternalResourceBinding [] bindings) {
    if (null != bindings) {
  ...
  /**
   * Bindings can't be edited.  They can be removed or new ones bound.
   * External resources can be edited.
   * Edit button is disabled unles an editable thing is selected.
   * But user could double click an illegal entry
   *
   */
  private void handleEdit() {
    ...
    if (!((rs instanceof FileResourceSpecifier) ||
          (rs instanceof FileLanguageResourceSpecifier))) {
  ...
  private void alterExistingXRD(AddExternalResourceDialog dialog, 
      ExternalResourceDescription xrd, TreeItem item) {
    valueChanged = false;
    ...
    xrd.setImplementationName(setValueChanged(dialog.xrImplementation, xrd.getImplementationName()));
  ...
  /**
   * Bind button - enabled only when one dependency is selected,
   * and one External Resource, not already bound to this key, is selected
   */
  ...
  /**
   * remove either a binding or an external resource.
   * Removing the resource removes all bindings associated with it
   *
   */
