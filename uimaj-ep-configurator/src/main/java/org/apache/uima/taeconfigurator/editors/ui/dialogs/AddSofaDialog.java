...
public class AddSofaDialog extends AbstractDialogKeyVerify {
  ...
  private Button outputButton;
    ...
    outputButton = newButton(io, SWT.RADIO, "Output", "Click here to specify this Sofa as an output.");
	 ...
	 *   Duplicates are OK for sofas belonging to other capability sets,
	...
	public boolean isValid() {
	  ...
	  if ( ! sofaName.equals(originalSofa) ||  // true for adding new sofa, or sofa name changed on edit
	  		 isInput != existingIsInput) { // true if input / output switched for editing sofa, not changed name
			...
			if (null != sofaNames)
				for (int j = 0; j < sofaNames.length; j++) {
					if (sofaName.equals(sofaNames[j]))
						return "This name exists as an " +
						(isInput? "output" : "input") +
