...
public class AddExternalResourceDialog extends AbstractDialogKeyVerify {
  ...
  public  String     xrImplementation;
	...
	public AddExternalResourceDialog(AbstractSection aSection) {
		super(aSection, "Add an External Resource Definition",
		    "Define and name an external resource");
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    xrDescriptionUI = newDescription(twoCol, 
    ...
    if (null != existingXRD) {
      ...
      if (rs instanceof FileResourceSpecifier)
        ...
        Utility.popMessage("Unknown resource type",
          "The resource type '" + rs.getClass().getName() + "' is unknown.  Editing should be done by hand in the source view.",
    ...
    if (event.keyCode == SWT.CR ||
	  ...
	  if ((event.widget == xrUrlUI ||
	      event.widget == xrUrlSuffixUI) && 
	      (event.character == '/' ||
	       event.character == ':'))
	      return true;  
    return false;
	...
	public boolean isValid() {
	  if (xrName.length() == 0) 
	    return false;
	  if ( ! xrName.equals(originalXrName)
	      && extnlResBindSection.resourceNameAlreadyDefined(xrName))
	    return false;
	  if (xrUrl.length() == 0)
	    return false;
	  return true;
	...
	public void enableOK() {
	  copyValuesFromGUI();
		okButton.setEnabled(xrName.length() > 0 &&
		                    xrUrl.length() > 0);
	}
}
