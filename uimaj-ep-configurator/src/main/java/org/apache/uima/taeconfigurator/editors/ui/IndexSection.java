...
public class IndexSection extends AbstractSection {
  ...
  private IndexImportSection indexImportSection;
	...
	public IndexSection(MultiPageEditor editor, Composite parent) {
	 ...
	 public void initialize(IManagedForm form) {
	    ...
	    addKeyButton = newPushButton(buttonContainer, "Add Key", "Click here to add a new key for the selected index.");
    ...
    item.setText(ASC_DES_COL, key.getComparator() == FSIndexComparator.STANDARD_COMPARE ? "Standard" : "Reverse");
    item.setData(key);
	}
	...
	public FsIndexDescription getBuiltInIndexDescription() {
		if(m_builtInIndexDescription == null) {
			m_builtInIndexDescription = UIMAFramework.
				getResourceSpecifierFactory().createFsIndexDescription();
  ...
  private boolean notAllowed(String message) {
  	if (isIndexDescriptor() && !editor.getIsContextLoaded()) {
  		Utility.popMessage("Not Allowed", "Editing or Adding Indexes can't be done here because the information about the type system is missing.",
	...
	public void handleEvent(Event event) {
		...
		if (event.widget == addIndexButton) {
			...
			FsIndexDescription id = 
			  UIMAFramework.getResourceSpecifierFactory().createFsIndexDescription();
	    ...
	    AddIndexKeyDialog dialog = new AddIndexKeyDialog(
	        this, fsid.getTypeName(), handleDefaultIndexKind(fsid.getKind()), getAlreadyUsedFeatures(fsid));
		  ...
		  if (o instanceof FsIndexDescription) {
		    ...
		    if (Window.CANCEL == Utility.popOkCancel(
		        "Confirm Remove", 
		        "Do you want to remove this key?", 
		        MessageDialog.WARNING )) 
		      return;
		  ...
		  if (o instanceof FsIndexDescription) {
		    ...
		    FsIndexKeyDescription key = (FsIndexKeyDescription)o;
			  ...
			  FsIndexDescription fsid = getFsIndexDescriptionFromTableTreeItem(parent);
		    AddIndexKeyDialog dialog = new AddIndexKeyDialog(
		        this, fsid.getTypeName(), handleDefaultIndexKind(fsid.getKind()), getAlreadyUsedFeatures(fsid), key);
		    valueChanged = false;
	...
	public void addFsIndexDescription(FsIndexDescription fsid) {
		FsIndexDescription [] oldFsIndexes = 
		  getAnalysisEngineMetaData().getFsIndexes();
    ...
    fsid.setKeys((FsIndexKeyDescription [])Utility.removeElementFromArray(
    		fsid.getKeys(), key, FsIndexKeyDescription.class));
	}
  ...
  public FsIndexKeyDescription addOrEditIndexKey(AddIndexKeyDialog dialog,
    ...
    notBuiltInSelected &= !(null != parent && parent == tt.getItems()[0]); 
	  ...
	  addKeyButton.setEnabled(notBuiltInSelected && 
        (null != parent || /*null == parent && */ !"bag".equals(item.getText(INDEX_KIND_COL))));
