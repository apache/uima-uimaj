...
/**
 * An unsafe String class based on a publicly accessible character array. This
 * class aims to provide similar functionality as java.lang.String, but without
 * the overhead of copying at creation time. Consequently, you should only use
 * this class if you have complete control over the underlying memory (i.e.,
 * char array).
 * 
 * 
...
public class CharArrayString {
    ...
    /**
     * Returns a substring. The position parameters are interpreted relative to
     * the string represented in this object, not the underlying char array.
     * Note that the substring is NOT a copy, but uses the same underlying char
     * array.
     * 
     * @param startPos
     ...
     */
    public CharArrayString substring(int startPos, int endPos)
            throws IndexOutOfBoundsException {
        if (startPos >= this.len || startPos > endPos || startPos < 0) {
            throw new IndexOutOfBoundsException();
        }
        return new CharArrayString(this.chars, this.start + startPos, endPos
                - startPos);
    }
     ...
     * @return The last position of the character, or <code>-1</code> if the
     ...
     * @return <code>true</code> iff <code>string</code> is a suffix of
     *         this.
     * @pre string != null
     */
     ...
     * @param c
     *            The char we're looking for.
     * @return The position of the char, or <code>-1</code> if the char
     *         couldn't be found.
     */
    ...
    /**
     * A static method to compute the hash code for a character range in an
     * array.
     * 
     * @param charArray
     * @param startPos
