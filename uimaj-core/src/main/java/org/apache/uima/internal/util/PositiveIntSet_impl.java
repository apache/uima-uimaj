...
public class PositiveIntSet_impl implements PositiveIntSet {
  ...
  // Extra space in hashset for initial capacity - when creating from bitmap set - multiplier of
  // existing size
  private static final int HASH_SET_OVERALLOCATE_DIVIDER_SHIFT = 1; // bit shift right = divide by 2
                                                                    // = 50% of existing capacity

  private PositiveIntSet intSet; // one of 3 representations: IntBitSet, IntHashSet, IntSet (based
                                 // on IntVector)
   ...
   * 
   * @param initialSize
   *          - if 0, don't allocate yet, wait for first add. If isBitSetDense, then this number is
   *          interpreted as the first int to be added, typically with an offset. The next two
   *          params are used only if initialSize is not 0.
   * @param estMin
   *          - the estimated minimum int value to be added
   * @param estMax
   ...
   * 
   * So we just set the offset to 0, and either convert the whole thing to a inthashset or copy all
   * the bits to a new bit set.
   * 
   * @param key
   */
  private void adjustBitSetForLowerOffset(int key) {
    ...
    final int hashSetSpaceNeeded = getHashSetSpace(size, largestInt, key); // computes using load
                                                                           // factor,
  ...
  private static int getHashSetOffset(int estMax, int estMin) {
    ...
    assert (range >= 0);
    return (range > HASH_SET_SHORT_MAX_SIZE) ? Integer.MIN_VALUE : // signal to force use of 4 byte
                                                                   // ints
   ...
   * 
   * If IntBitSet (spaceUsed != 0) compute what its size() (capacity) would be if the key were added
   * (causing doubling); include the overhead of the intbitset class.
   * 
   * long words required = larger of double the current size, or the number of long words required
   * 
   * @param adjKey
   *          - the range of bits needed
   * @param spaceUsed
   *          - the number of bits currently allocated (if currently a bit set), else 0
   ...
   * 
   * @param size
   *          including new element to be added
   * @return number of words including overhead to store that many elements unless is bigger than
   *         INT_SET_MAX_SIZE, then return MAX_VALUE
   */
   ...
   *          -
   * @param estMax
   *          - the largest int to be stored
   * @param estMin
   *          - the smallest int to be stored
  ...
  /**
   * When converting from bitset to hash set, the initial hash set should be big enough so that it
   * takes a while before it is doubled-expanded.
   * 
  ...
  private void handleBitSet(int newKey) {
    ...
    if (intBitSet == null) {
      /*********
       * Size is 0, use this opportunity to set the offset unless that has been a bad strategy for
       * this instance already
       ********/
  ...
  private void handleHashSet(int newKey) {
    ...
    final int bitSetSpaceNeeded = getBitSetSpaceFromRange(BIT_SET_OVERALLOCATE + maxInt - offset,
            0);
   ...
   * 
   * @param estMax
   *          the largest int to store in the bit set - determines the initial size
   * @param estMin
   *          this is a lower bound on the value that can be in the bit set
   * @param offsetSpec
   *          this is the offset to use, or -1, to calculate the offset from the initial_size
   *          (assuming a small amount of adds will be for ints somewhat less than the initial_size
   *          entry (being treated as the first int to be added)
  ...
  private boolean allocateIntBitSet(int estMax, int estMin, int offsetSpec) {
    ...
    if (useOffset) {
      ...
      // initialize bitSetOffset to the key, minus some amount to allow some storage of previous
      // values
  ...
  private static int estimatedBitSetOffset(int estMin, int offsetSpec) {
    return // initialize bitSetOffset to the key, minus some amount to allow some storage of
           // previous values
           // a minimum of 63, a maximum of 512 == 16 words, when key is > 4K (128 words)
    (offsetSpec == -1) ? Math.max(0, estMin - Math.min(1023, Math.max(63, estMin >> 3)))
            : offsetSpec;
  }
   ...
   * 
   * @param size
   *          - space needed including new item
   * @param offset
   *          - for IntHashSet values kept as short, the offset subtracted from values before
   *          storing them offset == Integer.MIN_VALUE means use 4 byte ints
   */
   ...
   * 
   * @param size
   *          - the capacity - this many elements could be stored before swtiching
   * @param offset
   *          - used only when the size < 65K, and is a value subtracted from elements before
   *          storing them, in an attempt to fit the results into just "short"s instead of "int"s if
  ...
  public String toString() {
    return String.format(
            "PositiveIntSet_impl [%n  intSet=%s%n secondTimeShrinkable=%s, useOffset=%s]", intSet,
            secondTimeShrinkable, useOffset);
  }
