 ...
 * pass an InputStream to this class's {@link #initializeCas(Object, CAS)} method, and this
 * InputStream must point to a valid XML document.
 * <p>
 * This CAS Initializer has one optional parameter, <code>XmlTagContainingText</code>. If a value
 * is specified for this parameter, then only the portion of the XML document within the named tag
 * will be put into the CAS.
 * </p>
 * A SAX Parser is used to parse the document from the InputStream. This should correctly handle any
 * encoding that is specified properly in the XML header.
 */
public class SimpleXmlCasInitializer extends CasInitializer_ImplBase {
  ...
  /**
   * Parses and returns the descriptor for this CAS Initializer. The descriptor is stored in the
  ...
  /**
   * Inner class implementing the SAX ContentHandler interface. Parses the XML to extract the text
