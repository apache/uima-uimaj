 ...
 * 
 * This class makes use of CASImpl methods, but is only passed CAS objects, which may be CAS
 * Wrappers. To make this more feasible, the implementors of CAS Wrappers need to implement the
 ...
 * 
 * But if the FS being copied is a subtype of AnnotationBase, then the corresponding view for the
...
public class CasCopier {
  ...
  /**
   * true if the copyCasView api was used, and the target view name corresponding to the source view
   * name is changed
   */
   ...
   * Note that if you are merging data from multiple CASes, you must create a new CasCopier for each
   * source CAS.
   * 
   * Note: If the feature structure and/or feature is not defined in the type system of the
   * destination CAS, the copy will fail (in other words, the lenient setting is false, by default).
   * 
   * @param aSrcCas
   ...
   * Creates a new CasCopier that can be used to copy FeatureStructures from one CAS to another.
   * Note that if you are merging data from multiple CASes, you must create a new CasCopier for each
   * source CAS. This version of the constructor supports a "lenient copy" option. When set, the CAS
   * copy function will ignore (not attempt to copy) FSs and features not defined in the type system
  ...
  public CasCopier(CAS aSrcCas, CAS aDestCas, boolean lenient) {
    ...
    src2TgtFeat = (srcTsi == tgtTsi) ? null
            : new Int2ObjListMap<>(srcTsi.getNumberOfFeatures() + 1);
  ...
  /**
   * Does a complete deep copy of one CAS into another CAS. The contents of each view in the source
   * CAS will be copied to the same-named view in the destination CAS. If the view does not already
   * exist it will be created. All FeatureStructures that are indexed in a view in the source CAS
   * will become indexed in the same-named view in the destination CAS.
   * 
   * Note: If the feature structure and/or feature is not defined in the type system of the
   * destination CAS, the copy will fail (in other words, the lenient setting is false, by default).
   *
   ...
   *          the CAS to copy to
   * @param aCopySofa
   *          if true, the sofa data and mimeType of each view will be copied. If false they will
   *          not.
   */
   ...
   * CAS will be copied to the same-named view in the destination CAS. If the view does not already
   * exist it will be created. All FeatureStructures that are indexed in a view in the source CAS
   * will become indexed in the same-named view in the destination CAS. This version of the method
   * supports a "lenient copy" option. When set, the CAS copy function will ignore (not attempt to
   * copy) FSs and features not defined in the type system of the destination CAS, rather than
   * throwing an exception.
   *
   * @param aSrcCas
   *          the CAS to copy from
   * @param aDestCas
   *          the CAS to copy to; must be a completely different CAS than the source (that is, not
   *          an alternative "view" of the source)
   * @param aCopySofa
   *          if true, the sofa data and mimeType of each view will be copied. If false they will
   *          not.
   * @param lenient
  ...
  /**
   * Does a deep copy of the contents of one CAS View into another CAS's same-named-view If the
   * destination view already exists in the destination CAS, then it will be the target of the copy.
   * Otherwise, a new view will be created with that name and will become the target of the copy.
   * All FeatureStructures (except for those dropped because the target type system doesn't have the
   * needed type) that are indexed in the source CAS view will become indexed in the target view.
   * Cross-view references may result in creating additional views in the destination CAS; for these
   * views, any Sofa data in the source is *not* copied.
   * 
   * @param aSrcCasView
   *          the CAS to copy from. This must be a view in the src Cas set by the constructor
   * @param aCopySofa
  ...
  public void copyCasView(CAS aSrcCasView, boolean aCopySofa) {
    copyCasViewDifferentCASs(aSrcCasView,
            getOrCreateView(originalTgtCas, aSrcCasView.getViewName()), aCopySofa);
  }
   ...
   * needed type) that are indexed in the source CAS view will become indexed in the target view.
   * Cross-view references may result in creating additional views in the destination CAS; for these
   * views, any Sofa data in the source is *not* copied. Any views created because of cross-view
   * references will have the same view name as in the source.
   *
   * @param aSrcCasViewName
   *          the name of the view in the source CAS to copy from
   * @param aCopySofa
   *          if true, the sofa data and mimeType will be copied. If false they will not.
   */
  ...
  /**
   * Does a deep copy of the contents of one CAS View into another CAS view, with a possibly
   * different name. If the destination view already exists in the destination CAS, then it will be
   * the target of the copy. Otherwise, a new view will be created with that name and will become
   * the target of the copy. All FeatureStructures (except for those dropped because the target type
   * system doesn't have the needed type) that are indexed in the source CAS view will become
   * indexed in the target view. Cross-view references may result in creating additional views in
   * the destination CAS; for these views, any Sofa data in the source is *not* copied. Any views
   * created because of cross-view references will have the same view name as in the source.
   * 
   * @param aSrcCasView
   *          The view in the source to copy from
   * @param aTgtCasViewName
   *          The name of the view in the destination CAS to copy into
   * @param aCopySofa
   *          if true, the sofa data and mimeType will be copied. If false they will not.
   */
  ...
  /**
   * Does a deep copy of the contents of one CAS View into another CAS view, with a possibly
   * different name. All FeatureStructures (except for those dropped because the target type system
   * doesn't have the needed type) that are indexed in the source CAS view will become indexed in
   * the target view. Cross-view references may result in creating additional views in the
   * destination CAS; for these views, any Sofa data in the source is *not* copied. Any views
   * created because of cross-view references will have the same view name as in the source.
   * 
   * @param aSrcCasViewName
   *          The name of the view in the Source CAS to copy from
   * @param aTgtCasView
   *          The view in the destination CAS to copy into
   * @param aCopySofa
   *          if true, the sofa data and mimeType will be copied. If false they will not.
   */
   ...
   * created because of cross-view references will have the same view name as in the source.
   * 
   * If the source and target views are both views of the same CAS, then Feature Structures in the
   * view are effectively "cloned", with the following change: Subtypes of AnnotationBase in the
   * source whose sofaRef is for the source View are cloned with their sofaRefs changed to the new
   * targetView.
   * 
   * @param aSrcCasView
   ...
   * @param aCopySofa
   *          if true, the sofa data and mimeType will be copied. If false they will not. If true
   *          and the sofa data is already set in the target, will throw CASRuntimeException
   */
  public void copyCasView(CAS aSrcCasView, CAS aTgtCasView, boolean aCopySofa) {

    if (!casViewsInSameCas(aSrcCasView, originalSrcCas)) {
      throw new UIMARuntimeException(UIMARuntimeException.VIEW_NOT_PART_OF_CAS,
              new Object[] { "Source" });
    }
    ...
    try { // to support finally to reset the src/tgt view names
      ...
      if ((aSrcCasView == srcCasViewImpl.getBaseCAS())
              || (aTgtCasView == tgtCasViewImpl.getBaseCAS())) {
        throw new UIMARuntimeException(
                UIMARuntimeException.UNSUPPORTED_CAS_COPY_TO_OR_FROM_BASE_CAS);
      }
      ...
      // we don't clear the cas copier instance map "mFsMap" here, in order to skip actually copying
      // the
      ...
      for (final TOP fs : c) {
        ...
        if (!indexedFsAlreadyCopied.contains(fs._id())) {
          ...
          // if (0 == sofaRef) {
          // tgtCasViewImpl.ll_setRefValue(copyOfFs, mDestSofaFeatureCode,
          // tgtCasViewImpl.getSofaRef());
          // }
          // }
  ...
  /**
   * For long lists, and other structures, the straight-forward impl with recursion can nest too
   * deep, causing a Java failure - out of stack space.
   * 
   ...
   * 
   * The copying of a FS copies the FS without setting the slots; instead it queues the copied FS
   * together with its source instance on featureStructuresWithSlotsToSet for later processing.
   * 
   */

  /**
   * Copy 1 feature structure from the originalSrcCas to a new Cas. No indexing of the new FS is
   * done. If the FS has been copied previously (using this CasCopier instance) the same identical
   * copy will be returned rather than making another copy.
   * 
   * View handling: ignores the view of the targetCas
   * 
   * @param aFS
   *          the Feature Structure to copy
   * @param <T>
   *          the generic type of the returned Feature Structure
   * @return a deep copy of the Feature Structure - any referred to FSs will also be copied, or null
   *         if the target CAS doesn't define a corresponding type
   */
  ...
  /**
   * Copy one FS from the src CAS to the tgt CAS View context: The caller must set the
   * srcCasViewImpl and the tgtCasViewImpl
   * 
   * @param aFS
   *          a Feature Structure reference in the originalSrcCas
   * @return a Feature Structure reference to a copy in the target CAS, or null if the target CAS
   *         doesn't have a corresponding type definition
   */
   ...
   * 
   * The _casView is set to the target cas view, unless the FS being copied is a subtype of
   * Annotation Base. - In that case, it is set to the view associated with the sofa ref.
   * 
   * @param srcFs
  ...
  private TOP copyFsInner(TOP srcFs) {
    ...
    if (isDocumentAnnotation(srcFs)) {
      ...
      if (destDocAnnot != null) {
        destDocAnnot.removeFromIndexes(); // deleting this one, will be using the copy of the new
                                          // one
    ...
    if (srcFs instanceof CommonArrayFS) {
      ...
      if (copy != null) { // can be null if trying to copy MyFs[] and type doesn't exist in target
                          // type system
  ...
  /**
   * 
   * @return the view in the target corresponding by name (after sofa name mapping if any) to the
   *         source view
   */
  ...
  /**
   * There are two cases for getting target sofa name from the source one, depending on whether or
   * not the API which allows specifying a different target view name for the source view name, is
   * in use.
   * 
   * If so, then whenever the source sofa name is that src view name, replace it in the target with
   * the specified different target view name.
   * 
  ...
  private String getDestSofaId(String viewNameFromSrcSofaId) {
    return (isChangeViewName && viewNameFromSrcSofaId.equals(srcViewName)) ? tgtViewName
   ...
   * @param tgtFS
   *          FeatureStructure to copy to, which must not be in the index (index corruption checks
   *          skipped)
   */
    ...
    if (isEqualTypeSystems) {
      for (final FeatureImpl fi : ti.getFeatureImpls()) {
        ...
        if (fi.isInInt) {
          ...
          // unless it's the sofa feature for AnnotationBase - that's feature final, set when
          // created
  ...
  // tgtCasViewImpl.ll_setStringValue(tgtFS, tgtFeatCode,
  // srcCasViewImpl.ll_getStringValueFeatOffset(srcFS, srcFeatOffset));
  // break;
  // case FRC_INT_LIKE:
  // tgtCasViewImpl.ll_setIntValue(tgtFS, tgtFeatCode,
  // srcCasViewImpl.ll_getIntValueFeatOffset(srcFS, srcFeatOffset));
  // break;
  // case FRC_LONG:
  // tgtCasViewImpl.ll_setLongValue(tgtFS, tgtFeatCode,
  // srcCasViewImpl.ll_getLongValueFeatOffset(srcFS, srcFeatOffset));
  // break;
  // case FRC_DOUBLE:
  // tgtCasViewImpl.ll_setDoubleValue(tgtFS, tgtFeatCode,
  // srcCasViewImpl.ll_getDoubleValueFeatOffset(srcFS, srcFeatOffset));
  // break;
  // case FRC_REF:
   ...
   * because the target doesn't have that type. It also returns false for sofa FSs and the
   * documentAnnotation FS.
   * 
   * @param aFS
   *          a feature structure
  ...
  /**
   * Note: only for backwards compatibility Note: if lenient is in effect, this method will return
   * false for FSs which are not copied because the target doesn't have that type. It also returns
   * false for sofa FSs and the documentAnnotation FS.
   * 
   * @param aFS
   *          a feature structure
   * @return true if the given FS has already been copied using this CasCopier.
  ...
  private TOP copyArray(TOP srcFS) {
    ...
    // we do a direct recursive call here because the depth of this is limited to the depth of array
    // nesting
  ...
  /**
   * Determines whether the given FS is the DocumentAnnotation in the srcCasView. Supports subtypes
   * of DocumentAnnotation. Returns true if the FS is the actual one that is being used by the
   * source CAS as the Document Annotation instance.
   */
  ...
  /**
   * Change from https://issues.apache.org/jira/browse/UIMA-3112 : requires that the CAS returned
   * from the getLowLevelCAS() be castable to CASImpl
   * 
   * @param c1
   *          -
   * @param c2
