  ...
  implements FsIndexKeyDescription
{
  ...
  /**
   * Overridden to handle XML export of the <code>typePriority</code> and
   * <code>comparator</code> properties.
   * 
    ...
    throws SAXException
  {
    if ("typePriority".equals(aPropInfo.propertyName))
    {
      ...
      if (isTypePriority())
      {
        aContentHandler.startElement(getXmlizationInfo().namespace,
          "typePriority","typePriority", new AttributesImpl());
    ...
    else if (!isTypePriority()) //don't write other properties for a type priority key
    {
      if ("comparator".equals(aPropInfo.propertyName))
      {
        ...
        aContentHandler.startElement(getXmlizationInfo().namespace,
          "comparator","comparator", new AttributesImpl());
  ...
  /**
   * Overridden to handle XML import of the <code>typePriority</code> and
   * <code>comparator</code> properties.  
   * 
   * @see org.apache.uima.resource.impl.MetaDataObject_impl#readPropertyValueFromXMLElement(org.apache.uima.resource.impl.PropertyXmlInfo, org.w3c.dom.Element, org.apache.uima.util.XMLParser)
   */
  protected void readPropertyValueFromXMLElement(PropertyXmlInfo aPropXmlInfo, 
    Element aElement, XMLParser aParser, XMLParser.ParsingOptions aOptions)
  ...
  static final private XmlizationInfo XMLIZATION_INFO =
    new XmlizationInfo("fsIndexKey",
      new PropertyXmlInfo[]{
         new PropertyXmlInfo("typePriority"), //NOTE: custom XMLization
         new PropertyXmlInfo("featureName"),
         new PropertyXmlInfo("comparator") //NOTE: custom XMLization
      });
