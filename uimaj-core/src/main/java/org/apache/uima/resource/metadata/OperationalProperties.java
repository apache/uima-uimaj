...
public interface OperationalProperties
{
  ...
  /**
   * Sets whether multiple instances of this component can be run in parallel, 
   * each receiving a subset of the documents from a collection.
   * 
   * @param aMultipleDeploymentAllowed true if multiple instances can be run in parallel, 
   *     false if not
   */
  public void setMultipleDeploymentAllowed(boolean aMultipleDeploymentAllowed);

  /**
   * Gets whether this AnalysisEngine may output new CASes.  If this property is
   * set to true, an application can use the {@link AnalysisEngine#processAndOutputNewCASes(CAS)}
   * to pass a CAS to this this AnalysisEngine and then step through all of the output CASes 
   * that it produces.  For example, such an AnalysisEngine could segment a CAS into
   * smaller pieces, emitting each as a separate CAS. 
   * 
   ...
   * smaller pieces, emitting each as a separate CAS. 
   * 
   * @param aOutputsNewCASes true if this component may output new CASes, false if it does not
   */
