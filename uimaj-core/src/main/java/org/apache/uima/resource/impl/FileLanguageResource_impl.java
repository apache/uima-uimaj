...
/**
 * An implementation of {@link ParameterizedDataResource} for language-based
 * resources.  Initialized from a {@link FileLanguageResourceSpecifier}.
 * 
 * 
 */
  ...
  implements ParameterizedDataResource
{
    ...
    throws ResourceInitializationException
  {
    ...
    mResourceInitParams = (aAdditionalParams == null) ?
    ...
    throws ResourceInitializationException
  {
    ...
    while (resource == null && lang != null)
    {
      ...
      FileResourceSpecifier fileSpec = 
        UIMAFramework.getResourceSpecifierFactory().createFileResourceSpecifier();
      fileSpec.setFileUrl(urlString);

      try
      {
        resource = (DataResource)UIMAFramework.produceResource(
            DataResource.class, fileSpec, mResourceInitParams);
      }
      ...
      if (resource == null)
      {
        ...
        int truncateAt = 
          (lastUnderscore > lastHyphen) ? lastUnderscore : lastHyphen;
        if (truncateAt == -1)
        {
          lang = null;                            
        }
