 ...
 * 
 */
public class SimplePrecondition_impl extends MetaDataObject_impl
  implements SimplePrecondition
{
  ...
  /**
   * Determines whether the given String is a valid name a predicate defined
   * by this class.  Valid predicate names are legal arguments to the
   * {@link setPredicate(String)} method, and are defined by constants on
   * the {@link SimplePrecondition} interface.
   * 
   * @param aName an Object to test
   * 
   * @return true if and only if <code>aName</code> is a <code>String</code>
   *     that is a valid predicate name.
   */
  protected static boolean isValidPredicateName(Object aName)
  {
    return EQUAL.equals(aName) ||
            ELEMENT_OF.equals(aName) ||
            LANGUAGE_SUBSUMED.equals(aName);
  }
  ...
  static final private XmlizationInfo XMLIZATION_INFO =
    new XmlizationInfo("simplePrecondition",
      new PropertyXmlInfo[]{
