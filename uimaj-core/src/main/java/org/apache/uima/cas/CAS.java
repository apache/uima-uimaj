 ...
 * 
 * <p>
 * A <code>CAS</code> object provides the starting point for working with the
 * CAS. It provides access to the type system, to indexes, iterators and filters
 * (constraints). It also lets you create new annotations and other data
 * structures. You can create a <code>CAS</code> object using static methods
 * on the class {@link org.apache.uima.internal.util.CasCreationUtils}.
 * <p>
 * The <code>CAS</code> object is also the container that manages multiple
 * Subjects of Analysis or Sofas. A Sofa represents some form of an unstructured
 * artifact that is processed in a UIMA pipeline. The Java string called the
 * "DocumentText" used in a UIMA text processing pipeline is an example of a
 * Sofa. A Sofa can be analyzed independently using the standard UIMA
 * programming model or analyzed together with other Sofas utilizing the Sofa
 * programming model extensions.
 * <p> 
 * A Sofa is implemented as a built-in CAS type uima.cas.Sofa. Use
 * {@link org.apache.uima.cas.CAS#createSofa CAS.createSofa()} to instantiate a
 * Sofa feature structure. The {@link SofaFS SofaFS} class provides methods to
 * set and get the features of a SofaFS. Although Sofas are implemented as
 * standard feature structures, generic CAS APIs must never be used to create
 * Sofas or set their features.
 * <p>
 * Use {@link org.apache.uima.cas.CAS#getView(String)} or {@link org.apache.uima.cas.CAS#getView(SofaFS)}
 * to obtain a view of a particular Sofa in the CAS.  This view will provide access to 
 * the Sofa data (for example the document text) as well as the index repository, 
 * which contains metadata (annotations and other feature structures) about that Sofa.
 * <p>
 * Use {@link #getTypeSystem getTypeSystem()} to access the type system. With a
 * {@link TypeSystem TypeSystem} object, you can access the {@link Type Type}
 * and {@link Feature Feature} objects for the CAS built-in types. Note that
 * this interface also provides constants for the names of the built-in types
 * and features.
 * 
 * 
 * 
...
public interface CAS extends AbstractCas {
     ...
     * UIMA CAS name space.
     */
    static final String NAME_SPACE_UIMA_CAS = "uima"
            + TypeSystem.NAMESPACE_SEPARATOR + "cas";

    /**
     * UIMA CAS name space prefix to prepend to type names (adds an extra period
     * to the name space proper.
     */
    static final String UIMA_CAS_PREFIX = NAME_SPACE_UIMA_CAS
            + TypeSystem.NAMESPACE_SEPARATOR;
     ...
     * Integer array type.
     */
    static final String TYPE_NAME_INTEGER_ARRAY = UIMA_CAS_PREFIX
            + "IntegerArray";
     ...
     * String array type.
     */
    static final String TYPE_NAME_STRING_ARRAY = UIMA_CAS_PREFIX
            + "StringArray";
    /**
     * Boolean array type.
     */
    ...
    static final String TYPE_NAME_ANNOTATION_BASE =
      UIMA_CAS_PREFIX + "AnnotationBase";
     ...
     */
    static final String FEATURE_FULL_NAME_SOFANUM = TYPE_NAME_SOFA
            + TypeSystem.FEATURE_SEPARATOR + FEATURE_BASE_NAME_SOFANUM;

    /**
     * Qualified name of Sofa id feature.
     */
    static final String FEATURE_FULL_NAME_SOFAID = TYPE_NAME_SOFA
            + TypeSystem.FEATURE_SEPARATOR + FEATURE_BASE_NAME_SOFAID;

    /**
     * Qualified name of Sofa mime type feature.
     */
    static final String FEATURE_FULL_NAME_SOFAMIME = TYPE_NAME_SOFA
            + TypeSystem.FEATURE_SEPARATOR + FEATURE_BASE_NAME_SOFAMIME;

    /**
     * Qualified name of Sofa URI feature.
     */
    static final String FEATURE_FULL_NAME_SOFAURI = TYPE_NAME_SOFA
            + TypeSystem.FEATURE_SEPARATOR + FEATURE_BASE_NAME_SOFAURI;

    /**
     * Qualified name of Sofa string data feature.
     */
    static final String FEATURE_FULL_NAME_SOFASTRING = TYPE_NAME_SOFA
            + TypeSystem.FEATURE_SEPARATOR + FEATURE_BASE_NAME_SOFASTRING;

    /**
     * Qualified name of Sofa array fs data feature.
     */
    static final String FEATURE_FULL_NAME_SOFAARRAY = TYPE_NAME_SOFA
            + TypeSystem.FEATURE_SEPARATOR + FEATURE_BASE_NAME_SOFAARRAY;
     ...
     * @deprecated As of v2.0, this is replaced by {@link #NAME_DEFAULT_SOFA},
     *   and the value has changed.  In general, user code should not need to 
     *   refer to this name.
     */
     ...
     */
    static final String TYPE_NAME_NON_EMPTY_FS_LIST = UIMA_CAS_PREFIX
            + "NonEmptyFSList";

    /**
     * Empty feature structure list type.
     */
    static final String TYPE_NAME_EMPTY_FS_LIST = UIMA_CAS_PREFIX
            + "EmptyFSList";

    /**
     * Integer list type.
     */
    static final String TYPE_NAME_INTEGER_LIST = UIMA_CAS_PREFIX
            + "IntegerList";

    /**
     * Non-empty integer list type.
     */
    static final String TYPE_NAME_NON_EMPTY_INTEGER_LIST = UIMA_CAS_PREFIX
            + "NonEmptyIntegerList";

    /**
     * Empty integer list type.
     */
    static final String TYPE_NAME_EMPTY_INTEGER_LIST = UIMA_CAS_PREFIX
            + "EmptyIntegerList";
     ...
     */
    static final String TYPE_NAME_NON_EMPTY_FLOAT_LIST = UIMA_CAS_PREFIX
            + "NonEmptyFloatList";

    /**
     * Empty float type.
     */
    static final String TYPE_NAME_EMPTY_FLOAT_LIST = UIMA_CAS_PREFIX
            + "EmptyFloatList";
     ...
     */
    static final String TYPE_NAME_NON_EMPTY_STRING_LIST = UIMA_CAS_PREFIX
            + "NonEmptyStringList";

    /**
     * Empty string list type.
     */
    static final String TYPE_NAME_EMPTY_STRING_LIST = UIMA_CAS_PREFIX
            + "EmptyStringList";
    ...
    /**
     * Create a Subject of Analysis. The new sofaFS is automatically added to
     * the SofaIndex.
     * 
     * @return The sofaFS.
     * 
     ...
     * Constraints are described in the javadocs for {@link ConstraintFactory}
     * and related classes.
     * 
     * @param it
     *            The input iterator.
    ...
    /**
     * Get a constraint factory. A constraint factory is a simple way of
     * creating {@link org.apache.uima.cas.FSMatchConstraint FSMatchConstraints}.
     * 
     ...
     * 
     * @return The index repository, or <code>null</code> if none is
     *         available.
     */
    FSIndexRepository getIndexRepository();

    /**
     * Wrap a standard Java {@link java.util.ListIterator ListIterator} around
     * an FSListIterator. Use if you feel more comfortable with java style
     * iterators.
     * 
     * @param it
    ...
    /**
     * Reset the CAS, emptying it of all content. Feature structures and
     * iterators will no longer be valid. Note: this method may only be called
     * from an application. Calling it from an annotator will trigger a runtime
     * exception.
     * 
     ...
     * 
     * @return the TCas view for the default text Sofa. If it does not already
     *         exist, it will be created.
     *         
     * @deprecated  As of v2.0, all methods on the TCAS interface have been moved to 
     *   the CAS interface, making this method unnecessary.        
     */
    TCAS getTCAS();
     ...
     *  
     * @param aSofsFS a Sofa feature struture in this CAS
     *  
     * @return The TCas for the given Sofa.
     * 
     * @deprecated  As of v2.0, use {@link #getView(String)} to get a view.  All methods on 
     * the TCAS interface have been moved to the CAS interface, making this method unnecessary.
     */
     ...
     * Note: as of UIMA v2.0, can be replaced with getView(sofaFS).getJCas().
     * 
     * @param aSofa a Sofa feature structure in this CAS.
     * 
    ...
    /**
     * Create a JCas view for a Sofa.
     * Note: this is provided for convenience.  It is equivalent to
     * <code>getView(aSofaID).getJCas()</code>.
     * 
     * @param aSofaID the ID of a Sofa defined in this CAS
     * 
     * @return The view for the Sofa with ID <code>aSofaID</code>.
     * @throws CASRuntimeException if no Sofa with the given ID exists in this CAS
     * 
     * @deprecated As of v2.0, use {@link #getView(String)} followed by
     * {@link #getJCas()}.
     */
    JCas getJCas(SofaID aSofaID) throws CASException;
    
    /**
     * Create a view and its underlying Sofa (subject of analysis).  The view provides access to
     * the Sofa data and the index repository that contains metadata (annotations 
     * and other feature structures) pertaining to that Sofa.
     * 
     * @param localViewName the local name, before any sofa name mapping is done, for this view
     * (note: this is the same as the associated Sofa name).
     * 
     * @return The view corresponding to this local name.
     * @throws CASRuntimeException if a View with this name already exists in this CAS
     */
    CAS createView(String localViewName);
    
    /**
     * Get the view for a Sofa (subject of analysis).  The view provides access to
     * the Sofa data and the index repository that contains metadata (annotations 
     * and other feature structures) pertaining to that Sofa.
     * 
     * @param localViewName the local name, before any sofa name mapping is done, for this view
     ...
     * 
     * @return The view corresponding to this local name.
     * @throws CASRuntimeException if no View with this name exists in this CAS
     */
     ...
     * and other feature structures) pertaining to that Sofa.
     * 
     * @param aSofa a Sofa feature structure in the CAS
     * 
    ...
    /**
     * Get the view name.  The view name is the same as the name of the view's Sofa,
     * retrieved by getSofa().getSofaID(), except for the initial View before
     * its Sofa has been created.
     * 
    ...
    /**
     * Get an instance of the low-level CAS. Low-level and regular CAS can be
     * used in parallel, all data is always contained in both.
     * 
     * <p>
    ...
    /**
     * Set the document text.  Once set, Sofa data is immutable, and cannot
     * be set again until the CAS has been reset.
     * @param text The text to be analyzed.
     * @exception CASRuntimeException If the Sofa data has already been set.
     */
     ...
     * This value sets the language feature of the special instance of
     * DocumentAnnotation associated with this TCAS.
     * @param languageCode
    ...
    /**
     * Gets the language code for this document from the language feature of the special
     * instance of the DocumentationAnnotation associated with this TCAS.
     * @return language identifier
     */
    String getDocumentLanguage();

    /**
     * Set the Sofa data as an ArrayFS.  Once set, Sofa data is immutable, and cannot
     * be set again until the CAS has been reset.
     * @param array The ArrayFS to be analyzed.
     * @param mime The mime type of the data
     * @exception CASRuntimeException If the Sofa data has already been set.
     */
    ...
    /**
     * Set the Sofa data as a URI.  Once set, Sofa data is immutable, and cannot
     * be set again until the CAS has been reset.
     * @param uri The URI of the data to be analyzed.
     * @param mime The mime type of the data
     * @exception CASRuntimeException If the Sofa data has already been set.
     */
     ...
     * @param type The annotation type the index is restricted to.
     * @return The standard annotation index, restricted to <code>type</code>.
     */
    FSIndex getAnnotationIndex(Type type);

    /**
     * Create a new annotation.  Note that you still need to insert the annotation
     * into the index repository yourself.
     * @param type The type of the annotation.
     * @param begin The start of the annotation.
    ...
    /**
     * Get the document annotation.  The document has a string-valued feature
     * called "language" where the document language is specified.
    ...
    /**
     * Informs the CAS of relevant information about the component
     * that is currently procesing it.  This is called by the framework
     * automatically; users do not need to call it.
     * @param info information about the component that is currently
     *   processing this CAS.
     */
     ...
     * 
     * <p>
     * <b>Important</b>: after you have called <code>addFsToIndexes(...)</code> on a FS,
