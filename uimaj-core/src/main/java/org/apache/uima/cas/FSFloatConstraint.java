 ...
 * A float constraint contains 0 or more tests, the results of which are
 * "anded" together.  To set a test, use any of the methods on this
 * class (except "match").
 * <p>
 * To use the constraint, invoke its {@link #match(float)} method, passing 
...
public interface FSFloatConstraint extends FSConstraint {
   ...
   *  @param f Matched value must be equal to this.
   */
  void eq(float f);

  /** Require float value to be less than <code>f</code>.
   *  @param f Matched value must be less than this.
   */
  void lt(float f);

  /** Require float value to be less than or equal to <code>f</code>.
   *  @param f Matched value must be less than or equal to this.
   */
  void leq(float f);

  /** Require float value to be greater than <code>f</code>.
   *  @param f Matched value must be greater than this.
   */
  void gt(float f);

  /** Require float value to be greater than or equal to <code>f</code>.
   *  @param f Matched value must be greater than or equal to this.
   */
  void geq(float f);

  /** Check if float matches defined constraints.
   *  @param f The float to be checked.
