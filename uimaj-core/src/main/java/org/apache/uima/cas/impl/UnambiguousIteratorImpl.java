 ...
 * 
 * <p>
 * Warning: this implementation creates a copy of the collection, so changes in
 * the underlying collection are not reflected by this iterator.
 * 
 * 
 */
public class UnambiguousIteratorImpl extends FSIteratorImplBase {
     ...
     * 
     */
    public UnambiguousIteratorImpl(FSIterator it, final int start,
            final int end, final boolean strict) {
        super();
        initUnambiguousSubiterator(it, start, end, strict);
    }

    private void initUnambiguousSubiterator(FSIterator it, final int start,
            final int end, final boolean strict) {
        this.list = new ArrayList();
        it.moveToFirst();
