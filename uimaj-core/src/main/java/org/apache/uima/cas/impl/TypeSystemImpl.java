...
public class TypeSystemImpl implements TypeSystemMgr, LowLevelTypeSystem {
    ...
    static {
        arrayComponentTypeNameMap
                .put(CAS.TYPE_NAME_TOP, CAS.TYPE_NAME_FS_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_BOOLEAN,
                CAS.TYPE_NAME_BOOLEAN_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_BYTE,
                CAS.TYPE_NAME_BYTE_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_SHORT,
                CAS.TYPE_NAME_SHORT_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_INTEGER,
                CAS.TYPE_NAME_INTEGER_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_FLOAT,
                CAS.TYPE_NAME_FLOAT_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_LONG,
                CAS.TYPE_NAME_LONG_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_DOUBLE,
                CAS.TYPE_NAME_DOUBLE_ARRAY);
        arrayComponentTypeNameMap.put(CAS.TYPE_NAME_STRING,
                CAS.TYPE_NAME_STRING_ARRAY);
    }

    static {
        arrayTypeComponentNameMap
                .put(CAS.TYPE_NAME_FS_ARRAY, CAS.TYPE_NAME_TOP);
        arrayTypeComponentNameMap.put(CAS.TYPE_NAME_BOOLEAN_ARRAY,
                CAS.TYPE_NAME_BOOLEAN);
        arrayTypeComponentNameMap.put(CAS.TYPE_NAME_BYTE_ARRAY,
                CAS.TYPE_NAME_BYTE);
        arrayTypeComponentNameMap.put(CAS.TYPE_NAME_SHORT_ARRAY,
                CAS.TYPE_NAME_SHORT);
        arrayTypeComponentNameMap.put(CAS.TYPE_NAME_INTEGER_ARRAY,
                CAS.TYPE_NAME_INTEGER);
        arrayTypeComponentNameMap.put(CAS.TYPE_NAME_FLOAT_ARRAY,
                CAS.TYPE_NAME_FLOAT);
        arrayTypeComponentNameMap.put(CAS.TYPE_NAME_LONG_ARRAY,
                CAS.TYPE_NAME_LONG);
        arrayTypeComponentNameMap.put(CAS.TYPE_NAME_DOUBLE_ARRAY,
     ...
     * Default constructor.
     * 
     * @deprecated Use 0 arg constructor. Type Systems are shared by many CASes,
     *             and can't point to one. Change also your possible calls to
     *             ts.commit() - see comment on that method.
     */
    ...
    int ll_computeArrayParentFromComponentType(int componentType) {
        if (ll_isPrimitiveType(componentType)
                ...
                // in all cases
                (ll_getTypeForCode(componentType).getName()
                        .equals(CAS.TYPE_NAME_TOP))) {
    ...
    /**
     * Check if feature is appropriate for type (i.e., type is subsumed by
     * domain type of feature).
     */
     ...
     * @return A feature object, or <code>null</code> if no such feature
     *         exists.
     */
     ...
     * @return An feature object, or <code>null</code> if no such feature
     *         exists.
     */
     ...
     * @return The new type, or <code>null</code> if <code>typeName</code>
     *         is already in use.
     */
    public Type addType(String typeName, Type mother) throws CASAdminException {
        ...
        if (mother.isInheritanceFinal()) {
            CASAdminException e = new CASAdminException(
                    CASAdminException.TYPE_IS_INH_FINAL);
        ...
        final int typeCode = this.addType(typeName, ((TypeImpl) mother)
                .getCode());
    ...
    private void checkTypeSyntax(String name) throws CASAdminException {
        if (!TypeSystemUtils.isTypeName(name)) {
            CASAdminException e = new CASAdminException(
                    CASAdminException.BAD_TYPE_SYNTAX);
            e.addArgument(name);
            throw e;
        }
    }
    ...
    /**
     * Internal code for adding a new type. Warning: no syntax check on type
     * name, must be done by caller. This method is not private because it's
    ...
    int addType(String name, int superType, boolean isStringType) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            String feat = name + TypeSystem.FEATURE_SEPARATOR
                    + getFeature(featCode).getShortName();
    ...
    public Feature addFeature(String featureName, Type domainType,
            Type rangeType) throws CASAdminException {
     ...
     */
    public Feature addFeature(String featureName, Type domainType,
            Type rangeType, boolean multipleReferencesAllowed)
        ...
        if (domainType.isFeatureFinal()) {
            CASAdminException e = new CASAdminException(
                    CASAdminException.TYPE_IS_FEATURE_FINAL);
            e.addArgument(domainType.getName());
            throw e;
        }
        checkFeatureNameSyntax(featureName);
        final int featCode = this.addFeature(featureName,
                ((TypeImpl) domainType).getCode(), ((TypeImpl) rangeType)
                        .getCode(), multipleReferencesAllowed);
    ...
    private void checkFeatureNameSyntax(String name) throws CASAdminException {
        if (!TypeSystemUtils.isIdentifier(name)) {
            CASAdminException e = new CASAdminException(
                    CASAdminException.BAD_FEATURE_SYNTAX);
            e.addArgument(name);
            throw e;
        }
    }
    ...
    /**
     * Return the list of all types subsumed by the input type. Note: the list
     * does not include the type itself.
     * 
     * @param type
     *            Input type.
     ...
     * @param subType
     *            Subtype.
     * @return <code>true</code> iff <code>sub</code> inherits from
     *         <code>super</code>.
     */
    public boolean subsumes(Type superType, Type subType) {
        // assert(superType != null);
        // assert(subType != null);
        return this.subsumes(((TypeImpl) superType).getCode(),
                ((TypeImpl) subType).getCode());
    }
    ...
    /**
     * @return An offset <code>&gt;0</code> if <code>feat</code> exists;
     *         <code>0</code>, else.
     */
    int getFeatureOffset(int feat) {
        return ((IntVector) this.approp.get(this.intro.get(feat)))
                .position(feat) + 1;
    }
     ...
     */
    int addFeature(String shortName, int domain, int range,
            boolean multiRefsAllowed) {
        // Since we just looked up the domain in the symbol table, we know it
        // exists.
        String name = this.typeNameST.getSymbol(domain)
                + TypeSystem.FEATURE_SEPARATOR + shortName;
        ...
        for (int i = 0; i < max; i++) {
            String featureName = ((Type) typesLocal.get(i)).getName()
                    + FEATURE_SEPARATOR + shortName;
            if (this.featureMap.containsKey(featureName)) {
                ...
                CASAdminException e = new CASAdminException(
                        CASAdminException.DUPLICATE_FEATURE);
        ...
        for (int i = 0; i < max; i++) {
            this.featureMap.put(((Type) typesLocal.get(i)).getName()
                    + FEATURE_SEPARATOR + shortName, feat);
        }
    ...
    private boolean ll_isPrimitiveArrayType(int type) {
        return type == this.floatArrayTypeCode || type == this.intArrayTypeCode
                || type == this.booleanArrayTypeCode
                || type == this.shortArrayTypeCode
                || type == this.byteArrayTypeCode
                || type == this.longArrayTypeCode
                || type == this.doubleArrayTypeCode
                || type == this.stringArrayTypeCode;
    }
    ...
    public boolean ll_subsumes(int superType, int type) {
        ...
        if (type == this.fsArrayTypeCode) {
            return superType == this.top
                    || superType == this.arrayBaseTypeCode
        ...
        if (isSuperArray) {
            if (isSubArray) {
                ...
                return ll_subsumes(ll_getComponentType(superType),
                        ll_getComponentType(type));
            }
    ...
    /**
     * This writes out the type hierarchy in a human-readable form. Files in
     * this form can be read in by a {@link TypeSystemParser TypeSystemParser}.
     */
    public String toString() {
        ...
        for (int i = 2; i <= numTypes; i++) {
            t = this.getType(i);
            buf.append(getTypeString(t) + " < "
                    + getTypeString(this.getParent(t)) + ";\n");
        } // Print feature declarations.
        final int numFeats = this.featureNameST.size();
        Feature f;
        for (int i = 1; i <= numFeats; i++) {
            f = this.getFeature(i);
            buf.append(getFeatureString(f) + ": "
                    + getTypeString(f.getDomain()) + " > "
     ...
     */
    public Type addStringSubtype(String typeName, String[] stringList)
            throws CASAdminException {
    ...
    public int ll_getArrayType(int componentTypeCode) {
        ...
        return addArrayType(
                ll_getTypeForCode(componentTypeCode),
    ...
    int addArrayType(Type componentType, Type mother) {
        return ll_addArrayType(ll_getCodeForType(componentType),
                ll_getCodeForType(mother));
    }
    ...
    int ll_addArrayType(int componentTypeCode, int motherCode) {
        ...
        String arrayTypeName = getArrayTypeName(ll_getTypeForCode(
                componentTypeCode).getName());
