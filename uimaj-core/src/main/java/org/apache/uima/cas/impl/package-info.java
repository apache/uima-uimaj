 ...
 * <p>The Java Cover Objects are no longer cover objects; instead, these objects <b>are</b> the Feature Structures. &nbsp;The Java classes for these objects are in a hierarchy that corresponds to the UIMA type hierarchy. &nbsp;JCasGen continues to serve to generate (for user, not for built-in types) particular Java Classes for particular UIMA Types. &nbsp;And, as before, JCasGen'd classes are optional. &nbsp;If there was not a JCasGen'd class for "MyType" (assume a subtype of "Annotation"), then the most specific supertype of "MyType" which has a particular corresponding Java cover class, is used. &nbsp;(This is how it works in V2, also).&nbsp;</p>
 * <p>There is one definition of these objects per UIMA Type System. &nbsp;Support for PEARs having different "customizations" of the same JCas classname is not supported in v3.</p>
 * <ul>
 ...
 * <p>There is one definition of the class per type system. &nbsp;Type systems are often shared among multiple CASes. &nbsp;Each definition is loaded under a specific loader for that type system. &nbsp;</p>
 * <p>(Not implemented) <span style="text-decoration: line-through;">The loader is set up to delegate to the parent for all classes except the JCas types, and for those, it generates them using ASM byte code generation from the fully merged TypeSystem information and existing "customizations".</span></p>
 * <p>Each feature is stored in one of two arrays, kept per Java Object Feature Structure Instance: an "int" array, holding boolean/byte/short/int/long/float/double values, and a "Object" array holding strings/refs-to-other-FSs. &nbsp;Longs and Doubles take 2 int slots.</p>
 * <p>Built-in arrays have their array parts represented by native Java Arrays. &nbsp;Getters and Setters are provided as before. &nbsp;Constructors are provided as before.</p>
 * <p>Extra fields in the Feature Structure include both instance and class fields:</p>
 * <ul>
 * <li>(static class fields) a set of fields representing the int offset in the "int" and "object" arrays for all the features</li>
 * <li>(instance field) a reference to the TypeImpl for this class - initialized by a reference to a TypeSystemImpl thread local value, at load time. &nbsp;This is updatable to handle two edge cases.</li>
 * <li>(instance field) a reference to the CAS View used when this feature structure was created</li>
 * </ul>
 ...
 * <li>more: see <a href="../../../uimaj-tools/src/main/java/org/apache/uima/tools/jcasgen/package.html"> package.html for uimaj-tools jcasgen</a> (link only works if all sources checked out)</li>
 * </ul>
 * </li>
 * </ul>
 * <h3>UIMA Indexes</h3>
 ...
 * <p>There are 3 kinds of indexes: Sorted, Set, and Bag.&nbsp; The basic object type for an index is <code>FsIndex_singleType</code>. This has 3 subtypes, one for each of the index types:</p>
 * <ul>
 * <li>FsIndex_bag</li>
 ...
 * </ul>
 * <p>The FsIndex_singleType index is just for one type (and doesn't include entries for any subtypes).</p>
 * <p>The Set and Sorted implementations are combined; the only difference is in the comparator used. &nbsp;For sets, the comparator is what the index definition specifies. &nbsp;For sorted, the specified comparator is augmented with an least significant extra key which is the Feature Structure id.</p>
