...
public class FSBagIndex extends FSLeafIndexImpl {
    ...
    private class IntVectorIterator implements ComparableIntPointerIterator,
        ...
        public boolean isValid() {
            return ((this.itPos >= 0) && (this.itPos < FSBagIndex.this.index
                    .size()));
        }
    ...
    /*
     * public final boolean insert(int fs) { // First, check if we can insert at
     * the end. final int [] indexArray = this.index.getArray(); final int
     * length = this.index.size(); if (length == 0) { this.index.add(fs); return
     * true; } final int last = indexArray[length - 1]; if (compare(last, fs) <
     * 0) { index.add(fs); return true; } final int pos =
     * this.binarySearch(indexArray, fs, 0, length); if (pos >= 0) { return
     * false; } index.add(-(pos+1), fs); return true; }
     *  // public IntIteratorStl iterator() { // return new IntVectorIterator(); // }
     */
    ...
    /*
     * // Do binary search on index. private final int binarySearch(int []
     * array, int ele, int start, int end) { --end; // Make end a legal value.
     * int i; // Current position int comp; // Compare value while (start <=
     * end) { i = (start + end) / 2; comp = compare(ele, array[i]); if (comp ==
     * 0) { return i; } if (start == end) { if (comp < 0) { return (-i)-1; }
     * else { // comp > 0 return (-i)-2; // (-(i+1))-1 } } if (comp < 0) { end =
     * i-1; } else { // comp > 0 start = i+1; } } // This means that the input
     * span is empty. return (-start)-1; }
     */
