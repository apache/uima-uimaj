...
/**
 * XCAS serializer. Create a serializer from a type system, then encode
...
public class XCASSerializer {
    ...
    private class XCASDocSerializer {
         ...
         * @return KEY_AND_VALUE_MATCH iff we've seen (address, value) before.
		 *         KEY_NOT_FOUND  iff the address has not been seen before.
		 *         KEY_ONLY_MATCH iff the address has been seen before with a different value.
         */
        ...
        /**
         * Version of serialize which also includes OutOfTypeSystemData
         * (obtained from previous deserialization) in the produced XCAS.
         * 
         * @throws XMLException
         * @throws IOException
         * @throws SAXException
        ...
        private void serialize(boolean encodeDoc,
                OutOfTypeSystemData outOfTypeSystemData) throws IOException,
                SAXException {
        ...
        private boolean isValidXmlChar(char c) {
            return (c >= 0x20 && c < 0xFFFE) || c == 0x09 || c == 0x0A
                    || c == 0x0D;
        }

        private void addAttribute(AttributesImpl attrs, String attrName,
                String attrValue) {
        ...
        private void startElement(String tag, Attributes attrs, int num)
                throws SAXException {
            numChildren = num;
        ...
        private void enqueueIndexed() {
            FSIndexRepositoryImpl ir = (FSIndexRepositoryImpl) cas.getBaseCAS()
         ...
         * @throws SAXException
         */
        private void encodeFS(int addr, IntVector indexRep) throws IOException,
                SAXException {
            ++fsCount;
            workAttrs.clear();
            ...
            if (indexRep != null) {
				if (indexRep.size() == 1) {
					...
					addAttribute(workAttrs, INDEXED_ATTR_NAME, Integer
							.toString(indexRep.get(0)));
				} else {
        ...
        private void encodePrimitiveTypeArrayFS(String[] data,
        		String typeName,
        ...
        private void encodeFSArray(int addr, AttributesImpl attrs)
                throws SAXException {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (heapVal == CASImpl.NULL && mOutOfTypeSystemData != null) {
                    ...
                    // FS.
                    List ootsElems = (List) mOutOfTypeSystemData.arrayElements
                            .get(new Integer(addr));
                    if (ootsElems != null) {
                                                ...
                                                // for large arrays
                        {
                            ...
                            if (ootsElem.index == i) {
                                val = (String) mOutOfTypeSystemData.idMap
                                        .get(ootsElem.value);
                                break;
                            }
                        }
        ...
        private void encodeFeatures(int addr, AttributesImpl attrs) {
            ...
            for (int i = 0; i < feats.length; i++) {
                ...
                if (!cas.isFSRefType(ts.range(feats[i]))) {
                	...
                	if (nameMapping && featName.equals(CAS.FEATURE_BASE_NAME_SOFAID)
                        && uimaContext != null) {
                        // map absolute SofaID to that expected by Component
                        attrValue = uimaContext
                                .mapSofaIDToComponentSofaName(attrValue);
                    }
                } else {
         ...
         * @param addr
         */
        private void encodeOutOfTypeSystemFeatures(int addr,
                AttributesImpl attrs) {
            List attrList = (List) mOutOfTypeSystemData.extraFeatureValues
                    .get(new Integer(addr));
            if (attrList != null) {
                ...
                while (it.hasNext()) {
                    ...
                    if (attr[0].startsWith("_ref_")) {
                        if (attr[1].startsWith("a")) { // reference to OOTS FS
                                                        // - remap
                            attr[1] = (String) mOutOfTypeSystemData.idMap
                                    .get(attr[1]);
                        }
                    }
                ...
                while (attrIt.hasNext()) {
                    ...
                    if (attrName.startsWith("_ref_")) {
                        if (attrVal.startsWith("a")) {
                            ...
                            // in the XCAS
                            attrVal = (String) mOutOfTypeSystemData.idMap
                                    .get(attrVal);
                        }
                    }
    ...
    /**
     * Gets the XCAS element name for a CAS type name. The element name is
     * usually the same as the type name, but the sequences _colon_ and _dash_
     * are translated to the characters : and -, respectively.
     * 
     * @param aCasTypeName
     *            CAS type name
    ...
    private String getXCasElementName(String aTagName) {
        if (aTagName.indexOf(':') == -1 && aTagName.indexOf('-') == -1) {
            ...
            return StringUtils.replaceAll(StringUtils.replaceAll(aTagName, ":",
                    "_colon_"), "-", "_dash_");
        }
    }
     ...
     *            The SAX content handler the data is written to.
     * @param encodeDoc
     *            If set to false, no uima.tcas.Document structure will be
     *            created, and the document text will not be serialized.
     * @throws IOException
     * @throws SAXException
     */
    public void serialize(CAS cas, ContentHandler contentHandler,
            boolean encodeDoc) throws IOException, SAXException {
     ...
     *            data not part of the CAS type system, which should be inserted
     *            into the XCAS output
     * 
     * @throws IOException
     * @throws SAXException
     */
    public void serialize(CAS cas, ContentHandler contentHandler,
            boolean encodeDoc, OutOfTypeSystemData outOfTypeSystemData)
            throws IOException, SAXException {
        contentHandler.startDocument();
        XCASDocSerializer ser = new XCASDocSerializer(contentHandler,
                ((CASImpl) cas).getBaseCAS());
    ...
    /**
     * Gets the name of the type representing the document. This will become the
     * name of the XML element that will hold the document text.
     * 
    ...
    /**
     * Gets the name of the type representing the document. This will become the
     * name of the XML element that will hold the document text. If not set,
     * defaults to {@link #DEFAULT_DOC_TYPE_NAME}.
     * 
     * @param aDocTypeName
    ...
    /**
     * Gets the name of the feature holding the documeng text. This will become
     * the value of the _content attribute on the document element.
     * 
    ...
    /**
     * Sets the name of the feature holding the documeng text. This will become
     * the value of the _content attribute on the document element. If not set,
     * defaults to {@link #DEFAULT_DOC_TEXT_FEAT}. If set to null, no _content
     ...
     */
    public static void serialize(CAS aCAS, OutputStream aStream,
            boolean isFormattedOutput) throws SAXException, IOException {
