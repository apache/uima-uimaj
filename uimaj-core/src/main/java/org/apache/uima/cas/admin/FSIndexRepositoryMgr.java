...
public interface FSIndexRepositoryMgr extends FSIndexRepository {
    ...
    /**
     * Create a new index. Note: if you creata a BAG_INDEX, the comparator will
     * be ignored.
     * 
     * @param comp
     ...
     * @return <code>false</code> iff an index with the same<code>label</code>
     *         already exists.
     * @throws CASAdminException
     *             If the repository is locked (after calling
     *             {@link #commit() commit()}).
     */
    boolean createIndex(FSIndexComparator comp, String label,
            int indexingStrategy) throws CASAdminException;
