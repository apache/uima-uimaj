...
class FSListIteratorImpl implements ListIterator {
    ...
    /**
     * Move the iterator so that the next call to {@link #previous() previous()}
    ...
    /**
     * Move the iterator so that the next call to {@link #next() next()} will
     * return the first element in the index (for a non-empty index).
     */
