...
public class ArrayFSImpl extends FeatureStructureImplC implements ArrayFS {
     ...
     */
    public void set(int i, FeatureStructure fs)
            throws ArrayIndexOutOfBoundsException {
        this.casImpl.setArrayValue(this.addr, i, this.getCASImpl().ll_getFSRef(
                fs));
    }
     ...
     *      int)
     */
    public void copyFromArray(FeatureStructure[] src, int srcOffset,
        ...
        for (int i = 0; i < length; i++) {
            ...
            this.casImpl.heap.heap[destOffset] = this.getCASImpl().ll_getFSRef(
                    src[srcOffset]);
            ++destOffset;
            ++srcOffset;
        }
     ...
     *      int)
     */
    public void copyToArray(int srcOffset, FeatureStructure[] dest,
            int destOffset, int length) throws ArrayIndexOutOfBoundsException {
        ...
        for (int i = 0; i < length; i++) {
            dest[destOffset] = this.casImpl
                    .createFS(this.casImpl.heap.heap[srcOffset]);
            ++destOffset;
            ++srcOffset;
        }
    }
