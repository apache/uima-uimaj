 ...
 * index is a sorted index. In a sorted index, FSs that are committed (added to the indexes) are
 * added, unless they are duplicates of already existing FSs in the index. Multiple different
 * instances of FSs which compare equal may all be in the index, individually.
 * 
 * <p>
 * The index is sorted in the sense that iterators will output FSs in sorted order according to the
 * comparator for that index. The order of FSs that are equal with respect to the comparator is
 * arbitrary but fixed. That is, if you iterate over the same index several times, you will see the
 * same relative order of FSs every time. We also guarantee that reverse iterators will produce
 * exactly the reverse sequence of forward iteration.
 * 
 * <p>
 ...
 * equal with respect to the indexing comparator, only the first one will be entered into the index.
 * Note that you can still have duplicates with respect to the indexing order if they are of a
 * different type; different types are never "equal". A set index is not in any particular sort
 * order.
 * 
 * <p>
 * A bag index finally simply stores everything, without any guaranteed order. Operations that
 * depend on comparison, such as find(FeatureStructure fs) or FSIterator.moveTo(FeatureStructure fs)
 * only compare for equality using FeatureStructure identity, since no ordering relationship is
 * possible with bag indexes.
 * 
 * <p>
 * Indexes have a top-most type; let's call it T. They store only instances of that type or its
 * subtypes. A given index definition specifies this top-most type. The APIs for obtaining an index
...
public interface FSIndex<T extends FeatureStructure> extends Collection<T> {
   ...
   * comparators defined for this index. For bag indexes (which have no comparators), the equality
   * test means the identical feature structure. Note that this is in general not the same as
   * feature structure identity.
   * </p>
   * 
   * <p>
   * The element is used as a template, and may be a supertype of the type of the index, as long as
   * the keys specified for this index can be accessed.
   * 
   * @param fs
   ...
   * specified for this index. Note that this is in general not the same as feature structure
   * identity. For BAG indexes, it is identity, for others it means the found feature structure
   * compares equal with the parameter in terms of the defined comparators for the index.
   * 
   * If there are multiple different FSs in the index which compare equal with the given feature
   * structure, an arbitrary one is returned. This differs from the moveTo(fs) operation which
   ...
   * 
   * @param fs1
   *          the first Feature Structure to compare
   * @param fs2
   *          the second Feature Structure to compare
   * @return <code>-1</code> if <code>fs1 &lt; fs2</code>; <code>0</code> if <code>fs1 = fs2</code>;
   *         <code>1</code> else.
   */
   ...
   * greater than or equal to <code>fs</code>, and any previous FS is less than <code>FS</code> (the
   * iterator is positioned at the earliest of equal values). If no such position exists, the
   * iterator will be invalid.
   * 
   * @param fs
   *          A feature structure template (may be a supertype of T) having keys used in the index
   *          compare function, specifying where to initially position the iterator.
   ...
   * 
   * @return a light-weight copy of this FSIndex, configured such that any iterator created using it
   *         will be a snapshot iterator - one where a snapshot is made of the state of the index at
   *         the time the iterator is created, and where subsequent modifications to the underlying
   *         index are allowed, but don't affect the iterator (which iterates over the read-only
   *         snapshot). Iterators produced with this won't throw ConcurrentModificationExceptions.
   */
