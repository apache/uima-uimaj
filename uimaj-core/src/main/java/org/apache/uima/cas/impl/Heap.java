...
/**
 * A heap for CAS. Actually provides two heaps: a temporary one for per-document
 * structures, and a permanent one. The space in the temporary heap can be
 * reused by calling <code>resetTempHeap()</code>. Note though that the two
 * heaps share an address space.
 * 
 * <p>
 ...
 * 
 * <p>
 * Finally, a word of caution. This class works internally with pages of a
 * certain size (see {@link Heap#DEFAULT_PAGE_SIZEDEFAULT_PAGE_SIZE}). It will
 * not accept structures larger than this size. You can set a larger value by
 * using the appropriate constructor. Use a smaller value only if you know
...
public final class Heap {
    ...
    /**
     * Default size of internal pages. Currently set to <code>500000</code>(2
     * MB).
     */
    ...
    /**
     * Constructor lets you set page size. Use only if you know what you're
     * doing.
     * 
     * @param pageSize
     *            The page size. If this is smaller than the
     *            {@link #MIN_PAGE_SIZE MIN_PAGE_SIZE}, the default will be
     *            used instead.
     */
    ...
    /**
     * Re-init the heap without metadata (i.e., most likely from TAF). Note that
     * we have no good way of determining page size. We just use the size of the
     * incoming heap, unless it's smaller than our minimum. Something more
     * sophisticated could be implemented, but let's see how we do with this
     * simple strategy first.
     * 
     * @param shortHeap
     */
    private void reinitNoMetaData(int[] shortHeap) {
        this.PAGE_SIZE = (shortHeap.length < MIN_PAGE_SIZE) ? MIN_PAGE_SIZE
                : shortHeap.length;
        this.availablePages = new SortedIntSet();
        this.tempPages = new SortedIntSet();
     ...
     * Just use the size of the incoming heap, unless it's smaller than our minimum.
     * It is expected that the caller will then fill in the new heap up to newSize.
     * 
     * @param newSize
     */
    void reinitSizeOnly(int newSize) {
        this.PAGE_SIZE = (newSize < MIN_PAGE_SIZE) ? MIN_PAGE_SIZE
                : newSize;
        this.availablePages = new SortedIntSet();
        this.tempPages = new SortedIntSet();
    ...
    int[] getMetaData() {
        final int arSize = AVST_POS + this.availablePages.size()
                + this.tempPages.size();
    ...
    private int newPage() {
        ...
        this.heap = IntArrayUtils.ensure_size(this.heap,
                start + this.PAGE_SIZE, 2, this.PAGE_SIZE);
        return start;
    }
...
//        }
     ...
     * @param fs
     *            The input structure.
     * @return The position where the structure was added, i.e., a pointer to
    ...
    /**
     * Reserve space for <code>len</code> items on the temp heap and set the
     * first item to <code>val</code>. The other items are set to
     * <code>0</code>.
     * 
     * @param len
