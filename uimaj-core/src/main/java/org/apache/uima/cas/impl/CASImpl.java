 ...
 * 
 */
public class CASImpl extends AbstractCas_ImplBase implements TCAS, CASMgr,
		LowLevelCAS {
	...
	private static String[] nonCreatableTypes = { CAS.TYPE_NAME_INTEGER,
			CAS.TYPE_NAME_FLOAT, CAS.TYPE_NAME_STRING,
			CAS.TYPE_NAME_ARRAY_BASE, CAS.TYPE_NAME_FS_ARRAY,
			CAS.TYPE_NAME_INTEGER_ARRAY, CAS.TYPE_NAME_FLOAT_ARRAY,
			CAS.TYPE_NAME_STRING_ARRAY, CAS.TYPE_NAME_SOFA, CAS.TYPE_NAME_BYTE,
			CAS.TYPE_NAME_BYTE_ARRAY, CAS.TYPE_NAME_BOOLEAN,
			CAS.TYPE_NAME_BOOLEAN_ARRAY, CAS.TYPE_NAME_SHORT,
			CAS.TYPE_NAME_SHORT_ARRAY, CAS.TYPE_NAME_LONG,
			CAS.TYPE_NAME_LONG_ARRAY, CAS.TYPE_NAME_DOUBLE,
	...
	private final void initTypeVariables() {
		...
		this.stringArrayType = (TypeImpl) this.ts
				.getType(TYPE_NAME_STRING_ARRAY);
		...
		this.endFeat = (FeatureImpl) this.ts
				.getFeatureByFullName(TCAS.FEATURE_FULL_NAME_END);
		...
		this.booleanArrayType = (TypeImpl) this.ts
				.getType(TYPE_NAME_BOOLEAN_ARRAY);
		...
		this.doubleArrayType = (TypeImpl) this.ts
				.getType(TYPE_NAME_DOUBLE_ARRAY);
	...
	private final void initTypeCodeVars() {
		...
		this.sofaArrayFeatCode = ll_getTypeSystem()
				.ll_getCodeForFeature(
						this.sofaType
								.getFeatureByBaseName(FEATURE_BASE_NAME_SOFAARRAY));
		this.annotSofaFeatCode = ll_getTypeSystem().ll_getCodeForFeature(
				this.annotBaseType
						.getFeatureByBaseName(TCAS.FEATURE_BASE_NAME_SOFA));
		this.startFeatCode = ll_getTypeSystem().ll_getCodeForFeature(
				this.annotType
						.getFeatureByBaseName(TCAS.FEATURE_BASE_NAME_BEGIN));
		this.endFeatCode = ll_getTypeSystem()
				.ll_getCodeForFeature(
						this.annotType
								.getFeatureByBaseName(TCAS.FEATURE_BASE_NAME_END));

	}

	private void checkInternalCodes(CASMgrSerializer ser)
			throws CASAdminException {
		if ((ser.topTypeCode > 0)
				&& (ser.topTypeCode != ((TypeImpl) this.ts.getTopType())
						.getCode())) {
		...
		for (int i = 1; i < ser.featureOffsets.length; i++) {
			if (ser.featureOffsets[i] != this.featureOffset[i]) {
				throw new CASAdminException(
						CASAdminException.DESERIALIZATION_ERROR);
			}
		}
	}
	...
	public FeatureStructure createFS(Type type) {
		...
		if (!isCreatableType(typeCode)) {
			CASRuntimeException e = new CASRuntimeException(
					CASRuntimeException.NON_CREATABLE_TYPE);
			e.addArgument(type.getName());
			e.addArgument("CAS.createFS()");
			throw e;
		}
		final int addr = createTempFS(typeCode);
		final boolean isAnnot = this.ts.subsumes(this.annotBaseTypeCode,
				typeCode);
		...
		if (isAnnot) {
			...
			getLowLevelCAS().ll_setIntValue(llsofa, this.annotSofaFeatCode,
					this.getSofaRef());
		}
		return newFS;
	}
	...
	public final void checkArrayPreconditions(int len)
			throws CASRuntimeException {
		// Check array size.
		if (len < 0) {
			throw new CASRuntimeException(
					CASRuntimeException.ILLEGAL_ARRAY_SIZE);
		}
	}
	...
	public boolean isBackwardCompatibleCas() {
		...
		final int llsofa = getLowLevelCAS().ll_getFSRef(
				this.getInitialView().getSofa());

		// check for mime type exactly equal to "text"
		String sofaMime = getLowLevelCAS().ll_getStringValue(llsofa,
				this.sofaMimeFeatCode);
		if (!"text".equals(sofaMime))
			return false;
		// check that sofaURI and sofaArray are not set
		String sofaUri = getLowLevelCAS().ll_getStringValue(llsofa,
				this.sofaUriFeatCode);
		if (sofaUri != null)
			return false;
		int sofaArray = getLowLevelCAS().ll_getRefValue(llsofa,
				this.sofaArrayFeatCode);
		if (sofaArray != CASImpl.NULL)
			return false;
		// check that name is NAME_DEFAULT_SOFA
		String sofaname = getLowLevelCAS().ll_getStringValue(llsofa,
				this.sofaIdFeatCode);
	...
	protected FSIndexRepository getSofaIndexRepository(int aSofaRef) {
		return (FSIndexRepositoryImpl) this.sofa2indexMap.get(new Integer(
				aSofaRef));
	}

	protected void setSofaIndexRepository(SofaFS aSofa,
			FSIndexRepository indxRepos) {
		setSofaIndexRepository(aSofa.getSofaRef(), indxRepos);
	}

	protected void setSofaIndexRepository(int aSofaRef,
			FSIndexRepository indxRepos) {
	...
	public void addSofa(FeatureStructure sofa, String sofaName, String mimeType) {
		...
		if (0 == getLowLevelCAS().ll_getIntValue(llsofa, this.sofaNumFeatCode)) {
			getLowLevelCAS().ll_setIntValue(llsofa, this.sofaNumFeatCode,
					++this.baseCAS.sofaCount);
		}
		getLowLevelCAS().ll_setStringValue(llsofa, this.sofaIdFeatCode,
				sofaName);
		getLowLevelCAS().ll_setStringValue(llsofa, this.sofaMimeFeatCode,
				mimeType);
	...
	private SofaFS getSofa(String sofaName) {
		...
		while (iterator.isValid()) {
			...
			if (sofaName.equals(getStringValue(sofa.hashCode(),
					this.sofaIdFeatCode))) {
				return sofa;
			}
			iterator.moveToNext();
	...
	protected SofaFS getSofa(int sofaRef) {
		...
		if (aSofa == null) {
			CASRuntimeException e = new CASRuntimeException(
					CASRuntimeException.SOFAREF_NOT_FOUND);
			throw e;
		}
		return aSofa;
	}
	...
	public FSIterator getSofaIterator() {
		FSIndex sofaIndex = this.baseCAS.indexRepository
				.getIndex(CAS.SOFA_INDEX_NAME);
	...
	public InputStream getSofaDataStream(SofaFS aSofa) {
		try {
			...
			if (null != aSofa.getLocalStringData()) {
				ByteArrayInputStream bis = new ByteArrayInputStream(aSofa
						.getLocalStringData().getBytes("UTF-8"));
				return bis;
			} else if (null != aSofa.getLocalFSData()) {
				FeatureStructureImpl fs = (FeatureStructureImpl) aSofa
						.getLocalFSData();
				...
				if (type.getName().equals(CAS.TYPE_NAME_INTEGER_ARRAY)) {
					...
					for (int i = arrayStart; i < arrayStart + arraySize; i++) {
						floatArray[i - arrayStart] = Float
								.intBitsToFloat(this.heap.heap[i]);
					}
					...
					arrayStart = this.heap.heap[getArrayStartAddress(fs
							.getAddress())];
					...
					for (int i = arrayStart; i < arrayStart + arraySize; i++) {
						doubleArray[i - arrayStart] = Double
								.longBitsToDouble(this.longHeap.heap[i]);
					}
			...
			CASRuntimeException e = new CASRuntimeException(
					CASRuntimeException.SOFADATASTREAM_ERROR);
	...
	public FSIterator createFilteredIterator(FSIterator it,
	...
	private void createIndexRepository() {
		if (!this.getTypeSystemMgr().isCommitted()) {
			throw new CASAdminException(
					CASAdminException.MUST_COMMIT_TYPE_SYSTEM);
		}
	...
	public void resetNoQuestions() {
		...
		if (this.jcas != null) {
			try {
				...
				CASAdminException cae = new CASAdminException(
						CASAdminException.JCAS_ERROR);
	...
	public void reinit(CASSerializer ser) {
		...
		reinit(ser.getHeapMetadata(), ser.getHeapArray(), ser.getStringTable(),
				ser.getFSIndex(), ser.getByteArray(), ser.getShortArray(), ser
						.getLongArray());
	}
	...
	public CAS getCAS() {
		...
		throw new CASAdminException(
				CASAdminException.MUST_COMMIT_INDEX_REPOSITORY);
	}
	...
	private void initCreatableTypeTable() {
		...
		for (int i = 0; i < nonCreatableTypes.length; i++) {
			typeCode = ((TypeImpl) this.ts.getType(nonCreatableTypes[i]))
					.getCode();
    ...
    this.indexRepository = casCompSer.getCASMgrSerializer()
        .getIndexRepository(this);
    ...
    this.baseCAS.sofaCount = 1;
		...
		reinit(casSer.getHeapMetadata(), casSer.getHeapArray(), casSer
				.getStringTable(), casSer.getFSIndex(), casSer.getByteArray(),
				casSer.getShortArray(), casSer.getLongArray());
	...
	void reinit(int[] heapMetadata, int[] heapArray, String[] stringTable,
			int[] fsIndex, byte[] byteHeapArray, short[] shortHeapArray,
			long[] longHeapArray) {
	 ...
	 * 4 1 Version (currently = 1) 4 1 size of 32-bit FS Heap array = s32H 4
	 * s32H 32-bit FS heap array 4 1 size of 16-bit string Heap array = sSH 2
	 * sSH 16-bit string heap array 4 1 size of string Ref Heap array = sSRH 4
	 * 2*sSRH string ref offsets and lengths 4 1 size of FS index array = sFSI 4
	 * sFSI FS index array
	 * 
	 * 4 1 size of 8-bit Heap array = s8H 1 s8H 8-bit Heap array 4 1 size of
	 * 16-bit Heap array = s16H 2 s16H 16-bit Heap array 4 1 size of 64-bit Heap
	 * array = s64H 8 s64H 64-bit Heap array
	 * ---------------------------------------------------------------------
	 * 
	 * This reads in and deserializes CAS data from a stream. Byte swapping may
	 * be needed if the blob is from TAF -- TAF blob serialization writes data
	...
	public void reinit(InputStream istream) throws CASRuntimeException {
		...
		try {
			...
			this.stringHeap.refHeap = new int[StringHeap.FIRST_CELL_REF
					+ refheapsz];

			dis.readInt(); // 0
			for (int i = this.stringHeap.refHeapPos; i < this.stringHeap.refHeap.length; i += StringHeap.REF_HEAP_CELL_SIZE) {
				if (swap) {
					this.stringHeap.refHeap[i
							+ StringHeap.CHAR_HEAP_POINTER_OFFSET] = swap4(dis,
							bytebuf);
					this.stringHeap.refHeap[i
							+ StringHeap.CHAR_HEAP_STRLEN_OFFSET] = swap4(dis,
							bytebuf);
				} else {
					this.stringHeap.refHeap[i
							+ StringHeap.CHAR_HEAP_POINTER_OFFSET] = dis
							.readInt();
					this.stringHeap.refHeap[i
							+ StringHeap.CHAR_HEAP_STRLEN_OFFSET] = dis
							.readInt();
				}
	...
	private void reinitIndexedFSs(int[] fsIndex) {
		...
		FSIterator iterator = this.baseCAS.getSofaIterator();
		final Feature idFeat = getTypeSystem().getFeatureByFullName(
	 ...
	 * reverse is not supported by public APIs (i.e., there is currently no way
	 * to get at the address of a FeatureStructure. Maybe we will need to change
	 * that.
	 * 
	 * @param addr
	 *            The address of the feature structure to be created.
	 * @return A FeatureStructure object. Note that no checking whatsoever is
	 *         done on the input address. There is really no way of finding out
	 *         which addresses in the valid address space actually represent
	 *         feature structures, and which don't.
	 */
	...
	public int createTempStringArray(int type, int len) {
		...
		final int addr = this.heap
				.addToTempHeap(arrayContentOffset + len, type);
	...
	void setArrayValueFromString(final int addr, final int index,
			final String value) {
	 ...
	 */
	void copyToArray(int addr, int sourceOffset, int[] dest, int destOffset,
			int length) {
		// Get the length of this array.
		final int arraySize = this.heap.heap[addr + arrayLengthFeatOffset];
		...
		System.arraycopy(this.heap.heap, offset + sourceOffset, dest,
				destOffset, length);
	}
	 ...
	 */
	void copyFromArray(int[] src, int srcOffset, int addr, int destOffset,
			int length) {
		// Get the length of this array.
		final int arraySize = this.heap.heap[addr + arrayLengthFeatOffset];
		...
		// Compute the offset into the heap where the array starts.
		final int offset = addr + arrayContentOffset;
		System.arraycopy(src, srcOffset, this.heap.heap, offset + destOffset,
				length);
	}
	...
	void copyFeatures(int trgAddr, int srcAddr) throws CASRuntimeException {
		...
		if (typeCode != getHeapValue(srcAddr)) {
			...
			e
					.addArgument("Type of source and target feature structures do not match");
			throw (e);
		}
		// get features to copy
		int[] featcodes = getTypeSystem().getLowLevelTypeSystem()
				.ll_getAppropriateFeatures(typeCode);
		for (int i = 0; i < featcodes.length; i++) {

			// get range type of this feature
			Feature feature = getTypeSystem().getLowLevelTypeSystem()
					.ll_getFeatureForCode(featcodes[i]);
	...
	public void setStringValue(int addr, int feat, String s) {
		final int stringCode = ((s == null) ? NULL : this.stringHeap
				.addString(s));
	...
	public String getStringValue(int addr, int feat) {
		return this.stringHeap.getStringForCode(this.heap.heap[addr
				+ this.featureOffset[feat]]);
	}
  ...
  public static final long double2long(double d) {
	 ...
	 * @param typeCode
	 *            The code to check.
	 * @return <code>true</code> iff <code>typeCode</code> is a type for
	 *         which a regular FS can be generated.
	...
	boolean isBuiltinType(Type type) {
		 ...
		 * ts.getTopType()) || isArrayType(typeCode) ||
		 * isAbstractArrayType(typeCode) || isStringType(typeCode) ||
	...
	public boolean isArrayType(Type type) {
		return ((type == this.fsArrayType) || (type == this.intArrayType)
				|| (type == this.floatArrayType)
				|| (type == this.stringArrayType)
				|| (type == this.booleanArrayType)
				|| (type == this.byteArrayType)
				|| (type == this.shortArrayType)
				|| (type == this.doubleArrayType) || (type == this.longArrayType));
  ...
  public boolean isDoubleType(Type type) {
	...
	public boolean isArrayType(int type) {
		return ((type == this.fsArrayType.getCode())
				|| (type == this.intArrayType.getCode())
				|| (type == this.floatArrayType.getCode())
				|| (type == this.stringArrayType.getCode())
				|| (type == this.booleanArrayType.getCode())
				|| (type == this.byteArrayType.getCode())
				|| (type == this.shortArrayType.getCode())
				|| (type == this.longArrayType.getCode()) || (type == this.doubleArrayType
				.getCode()));
	}
	...
	public static void setupTSDefault(TypeSystemImpl ts) {
		...
		TypeImpl floatArray = (TypeImpl) ts.addType(CAS.TYPE_NAME_FLOAT_ARRAY,
				array);
		TypeImpl intArray = (TypeImpl) ts.addType(CAS.TYPE_NAME_INTEGER_ARRAY,
				array);
		TypeImpl stringArray = (TypeImpl) ts.addType(
				CAS.TYPE_NAME_STRING_ARRAY, array);
		// Add lists.
		...
		Type floatNeList = ts.addType(CAS.TYPE_NAME_NON_EMPTY_FLOAT_LIST,
				floatList);
		...
		Type intNeList = ts.addType(CAS.TYPE_NAME_NON_EMPTY_INTEGER_LIST,
				intList);
		...
		Type stringEList = ts.addType(CAS.TYPE_NAME_EMPTY_STRING_LIST,
				stringList);
		Type stringNeList = ts.addType(CAS.TYPE_NAME_NON_EMPTY_STRING_LIST,
				stringList);
		...
		TypeImpl booleanArray = (TypeImpl) ts.addType(
				CAS.TYPE_NAME_BOOLEAN_ARRAY, array);
		TypeImpl byteArray = (TypeImpl) ts.addType(CAS.TYPE_NAME_BYTE_ARRAY,
				array);
		TypeImpl shortArray = (TypeImpl) ts.addType(CAS.TYPE_NAME_SHORT_ARRAY,
				array);
		TypeImpl longArray = (TypeImpl) ts.addType(CAS.TYPE_NAME_LONG_ARRAY,
				array);
		TypeImpl doubleArray = (TypeImpl) ts.addType(
				CAS.TYPE_NAME_DOUBLE_ARRAY, array);
		...
		Type docType = ts
				.addType(TCAS.TYPE_NAME_DOCUMENT_ANNOTATION, annotType);
	...
	public void initCASIndexes() throws CASException {
		...
		comp.addKey(
				this.ts.getFeatureByFullName(CAS.FEATURE_FULL_NAME_SOFANUM),
				FSIndexComparator.STANDARD_COMPARE);
		this.indexRepository.createIndex(comp, CAS.SOFA_INDEX_NAME,
				FSIndex.SET_INDEX);
		...
		comp.addKey(this.indexRepository.getDefaultTypeOrder(),
				FSIndexComparator.STANDARD_COMPARE);
   ...
   */
	...
	public JCas getJCas(SofaFS aSofa) throws CASException {
		...
		JCas aJCas = (JCas) this.baseCAS.sofa2jcasMap.get(new Integer(aSofa
				.getSofaRef()));
		if (null != aJCas) {
			return aJCas;
		}
    ...
    if (null == aTcas) {
      ...
      if (this.baseCAS.sofaCount < aSofa.getSofaRef()) {
        ...
        final Feature idFeat = getTypeSystem().getFeatureByFullName(
            CAS.FEATURE_FULL_NAME_SOFAID);
        String id = getLowLevelCAS().ll_getStringValue(
            aSofa.hashCode(), ((FeatureImpl) idFeat).getCode());
	...
	private final void checkLowLevelParams(int fsRef, int domType, int ranType,
			int feat) {
	...
	private final void checkDomTypeConditions(int domTypeCode, int featCode) {
		if (!this.ts.isApprop(domTypeCode, featCode)) {
			LowLevelException e = new LowLevelException(
					LowLevelException.FEAT_DOM_ERROR);
	 ...
	 *            feature
	 */
	public final void checkTypingConditions(Type domType, Type ranType,
			Feature feat) {
		checkTypingConditions(((TypeImpl) domType).getCode(),
				((TypeImpl) ranType).getCode(), ((FeatureImpl) feat).getCode());
	}

	// Assumes that parameters are valid type system codes, so check that first.
	private final void checkTypingConditions(int domTypeCode, int ranTypeCode,
			int featCode) {
		checkDomTypeConditions(domTypeCode, featCode);
		if (!this.ts.subsumes(this.ts.range(featCode), ranTypeCode)) {
			LowLevelException e = new LowLevelException(
					LowLevelException.FEAT_RAN_ERROR);
			e.addArgument(Integer.toString(featCode));
			e.addArgument(this.ts.ll_getFeatureForCode(featCode).getName());
	...
	private final void checkFsRan(int featCode) throws LowLevelException {
		...
		if (!ll_isRefType(rangeTypeCode)) {
			LowLevelException e = new LowLevelException(
					LowLevelException.FS_RAN_TYPE_ERROR);
			e.addArgument(Integer.toString(featCode));
			e.addArgument(this.ts.ll_getFeatureForCode(featCode).getName());
	...
	private final void checkFeature(int featureCode) {
		if (!this.ts.isFeature(featureCode)) {
			LowLevelException e = new LowLevelException(
					LowLevelException.INVALID_FEATURE_CODE);
	...
	private final void checkTypeAt(int typeCode, int fsRef) {
		if (!this.ts.isType(typeCode)) {
			LowLevelException e = new LowLevelException(
					LowLevelException.VALUE_NOT_A_TYPE);
	...
	final void checkFsRef(int fsRef) {
		if (fsRef <= NULL_FS_REF || fsRef >= this.heap.heap.length) {
			LowLevelException e = new LowLevelException(
					LowLevelException.INVALID_FS_REF);
			e.addArgument(Integer.toString(fsRef));
			throw e;
		}
	}
	...
	public final boolean ll_isRefType(int typeCode) {
		if (typeCode == this.intTypeCode || typeCode == this.floatTypeCode
				|| typeCode == this.stringTypeCode
				|| typeCode == this.byteTypeCode
				|| typeCode == this.booleanTypeCode
				|| typeCode == this.shortTypeCode
				|| typeCode == this.longTypeCode
				|| typeCode == this.doubleTypeCode) {
			return false;
		}
	...
	public int ll_createArray(int typeCode, int arrayLength) {
		final int addr = this.heap.addToTempHeap(arrayContentOffset
				+ arrayLength, typeCode);
		this.heap.heap[(addr + arrayLengthFeatOffset)] = arrayLength;
		return addr;
	}

	public int ll_createAuxArray(int typeCode, int arrayLength) {
		final int addr = this.heap.addToTempHeap(arrayContentOffset + 1,
				typeCode);
		this.heap.heap[(addr + arrayLengthFeatOffset)] = arrayLength;
		return addr;
	}
	...
	public int ll_createByteArray(int arrayLength) {
		...
		this.heap.heap[addr + arrayContentOffset] = this.byteHeap
				.reserve(arrayLength);
		return addr;
	}

	public int ll_createBooleanArray(int arrayLength) {
		final int addr = ll_createAuxArray(this.booleanArrayTypeCode,
				arrayLength);
		this.heap.heap[addr + arrayContentOffset] = this.byteHeap
				.reserve(arrayLength);
		return addr;
	...
	public int ll_createShortArray(int arrayLength) {
		...
		this.heap.heap[addr + arrayContentOffset] = this.shortHeap
				.reserve(arrayLength);
		return addr;
	}
	...
	public int ll_createLongArray(int arrayLength) {
		...
		this.heap.heap[addr + arrayContentOffset] = this.longHeap
				.reserve(arrayLength);
		return addr;
	}

	public int ll_createDoubleArray(int arrayLength) {
		final int addr = ll_createAuxArray(this.doubleArrayTypeCode,
				arrayLength);
		this.heap.heap[addr + arrayContentOffset] = this.longHeap
				.reserve(arrayLength);
		return addr;
	...
	public int ll_createArray(int typeCode, int arrayLength, boolean doChecks) {
		if (doChecks) {
			...
			if (!this.ts.isType(typeCode)) {
				LowLevelException e = new LowLevelException(
						LowLevelException.INVALID_TYPE_ARGUMENT);
				e.addArgument(Integer.toString(typeCode));
				throw e;
			}
			if (!isCreatableArrayType(typeCode)) {
				LowLevelException e = new LowLevelException(
						LowLevelException.CREATE_ARRAY_OF_TYPE_ERROR);
				e.addArgument(Integer.toString(typeCode));
			...
			if (arrayLength < 0) {
				LowLevelException e = new LowLevelException(
						LowLevelException.ILLEGAL_ARRAY_LENGTH);
	...
	public final String ll_getStringValue(int fsRef, int featureCode) {
		return this.stringHeap.getStringForCode(ll_getIntValue(fsRef,
				featureCode));
	}
	...
	public final void ll_setStringValue(int fsRef, int featureCode, String value) {
		final int stringAddr = (value == null) ? NULL : this.stringHeap
				.addString(value);
	...
	public final void ll_setStringValue(int fsRef, int featureCode,
			String value, boolean doTypeChecks) {
		if (doTypeChecks) {
			checkNonArrayConditions(fsRef, this.stringTypeCode, featureCode);
		}
	...
	public final void ll_setCharBufferValue(int fsRef, int featureCode,
			char[] buffer, int start, int length, boolean doTypeChecks) {
		if (doTypeChecks) {
			checkNonArrayConditions(fsRef, this.stringTypeCode, featureCode);
		}
	...
	public final void ll_setCharBufferValue(int fsRef, int featureCode,
		...
		final int stringCode = this.stringHeap.addCharBuffer(buffer, start,
				length);
	 ...
	 *      char, int)
	 */
	public int ll_copyCharBufferValue(int fsRef, int featureCode,
	...
	private void throwAccessTypeError(int fsRef, int typeCode) {
		LowLevelException e = new LowLevelException(
				LowLevelException.ACCESS_TYPE_ERROR);
		e.addArgument(Integer.toString(fsRef));
		e.addArgument(Integer.toString(typeCode));
		e.addArgument(this.ts.ll_getTypeForCode(typeCode).getName());
		e.addArgument(this.ts.ll_getTypeForCode(ll_getFSRefType(fsRef))
				.getName());
		throw e;
	}
	...
	public final void checkArrayBounds(int fsRef, int pos, int length) {
		...
		if (pos < 0 || length < 0 || (pos + length) > arrayLength) {
			LowLevelException e = new LowLevelException(
					LowLevelException.ARRAY_INDEX_LENGTH_OUT_OF_RANGE);
			e.addArgument(Integer.toString(pos));
	...
	private final void checkNonArrayConditions(int fsRef, int typeCode,
			int featureCode) {
		checkFsRef(fsRef);
	...
	private final void checkPrimitiveArrayConditions(int fsRef, int typeCode,
			int position) {
	...
	public final int ll_getIntArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.intArrayTypeCode,
					position);
		}
		return ll_getIntArrayValue(fsRef, position);
	}

	public float ll_getFloatArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.floatArrayTypeCode,
					position);
		}
		return ll_getFloatArrayValue(fsRef, position);
	}

	public String ll_getStringArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.stringArrayTypeCode,
					position);
		}
	...
	public BooleanArrayFS createBooleanArrayFS(int length)
			throws CASRuntimeException {
		checkArrayPreconditions(length);
		return (BooleanArrayFS) createFS(ll_createBooleanArray(length));
	}

	public ShortArrayFS createShortArrayFS(int length)
			throws CASRuntimeException {
		checkArrayPreconditions(length);
	...
	public DoubleArrayFS createDoubleArrayFS(int length)
			throws CASRuntimeException {
		checkArrayPreconditions(length);
  ...
  public long ll_getLongValue(int offset) {
	...
	public long ll_getLongValue(int fsRef, int featureCode) {
		final int offset = this.heap.heap[fsRef
				+ this.featureOffset[featureCode]];
	...
	public void ll_setBooleanValue(int fsRef, int featureCode, boolean value) {
		this.heap.heap[fsRef + this.featureOffset[featureCode]] = value ? CASImpl.TRUE
				: CASImpl.FALSE;
	}
	...
	public void ll_setByteValue(int fsRef, int featureCode, byte value,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkNonArrayConditions(fsRef, this.byteTypeCode, featureCode);
		}
	...
	public void ll_setShortValue(int fsRef, int featureCode, short value,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkNonArrayConditions(fsRef, this.booleanTypeCode, featureCode);
		}
	...
	public byte ll_getByteArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.byteArrayTypeCode,
					position);
		}
	...
	public boolean ll_getBooleanArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.booleanArrayTypeCode,
					position);
		}
	...
	public short ll_getShortArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.shortArrayTypeCode,
					position);
		}
	...
	public long ll_getLongArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.longArrayTypeCode,
					position);
		}
	...
	public double ll_getDoubleArrayValue(int fsRef, int position,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.doubleArrayTypeCode,
					position);
		}
	...
	public void ll_setDoubleArrayValue(int fsRef, int position, double value,
			boolean doTypeChecks) {
		if (doTypeChecks) {
			checkPrimitiveArrayConditions(fsRef, this.doubleArrayTypeCode,
					position);
	...
	public AnnotationFS createAnnotation(Type type, int begin, int end) {
		// Can't create annotation on base CAS
		CASRuntimeException e = new CASRuntimeException(
				CASRuntimeException.INVALID_BASE_CAS_METHOD);
		e.addArgument("createAnnotation");
		throw e;
	}

	public FSIndex getAnnotationIndex() {
		return new AnnotationIndexImpl(getIndexRepository().getIndex(
				TCAS.STD_ANNOTATION_INDEX));
	}

	public FSIndex getAnnotationIndex(Type type) {
		return new AnnotationIndexImpl(getIndexRepository().getIndex(
				TCAS.STD_ANNOTATION_INDEX, type));
	}
  ...
  public String getViewName() {
	...
	public void setSofaDataString(String text, String mime)
			throws CASRuntimeException {
		CASRuntimeException e = new CASRuntimeException(
				CASRuntimeException.INVALID_BASE_CAS_METHOD);
		e.addArgument("setDocumentLanguage");
  ...
  protected ComponentInfo getCurrentComponentInfo()
  {
	...
	public CASImpl ll_getSofaCasView(int addr) {
		...
		if (sofaId > 0) {
			...
			if (!(this instanceof TCASImpl)
					|| (sofaId != ((TCASImpl) this).getSofaRef())) {
