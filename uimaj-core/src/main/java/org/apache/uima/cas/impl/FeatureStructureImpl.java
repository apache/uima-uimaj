...
public abstract class FeatureStructureImpl implements FeatureStructure, Cloneable {
    ...
    public void setFeatureValue(Feature feat, FeatureStructure fs) {
        ...
        final int rangeType = this.getCASImpl().getTypeSystemImpl().range(
                featCode);
        ...
        if (!this.getCASImpl().getTypeSystemImpl().isApprop(thisType, featCode)) {
            ...
            e.addArgument(this.getType().getName());
            throw e;
        }
        if (!this.getCASImpl().getTypeSystemImpl().subsumes(rangeType,
                valueType)) {
            CASRuntimeException e = new CASRuntimeException(
                    CASRuntimeException.INAPPROP_RANGE);
            e.addArgument(feat.getName());
            e.addArgument(feat.getRange().getName());
            e.addArgument(fs.getType().getName());
            throw e;
        }
        this.getCASImpl().setFeatureValue(this.getAddress(), featCode,
                valueAddr);
    }

    private final void setNullValue(int featCode, int rangeType) {
        if (this.getCASImpl().isIntType(rangeType)
                || this.getCASImpl().isFloatType(rangeType)
                || this.getCASImpl().isStringType(rangeType)) {
            CASRuntimeException e = new CASRuntimeException(
                    CASRuntimeException.PRIMITIVE_VAL_FEAT);
            e.addArgument(this.getCASImpl().getTypeSystemImpl().getFeature(
                    featCode).getName());
            throw e;
        }
        this.getCASImpl().setFeatureValue(this.getAddress(), featCode,
    ...
    public void setIntValue(Feature feat, int val) {
        ...
        if (!ts.subsumes(((TypeImpl) feat.getDomain()).getCode(), this
                .getCASImpl().getHeapValue(this.getAddress()))) {
    ...
    public void setStringValue(Feature feat, String val) {
        ...
        final int thisType = this.getCASImpl().getHeapValue(this.getAddress());
        final int stringType = ((TypeImpl) this.getCASImpl().getTypeSystem()
                .getType(CAS.TYPE_NAME_STRING)).getCode();
        ...
        if (!ts.subsumes(stringType, rangeType)) {
            CASRuntimeException e = new CASRuntimeException(
                    CASRuntimeException.INAPPROP_TYPE);
            e.addArgument(feat.getRange().getName());
            e.addArgument(this.getCASImpl().getTypeSystem().getType(
                    CAS.TYPE_NAME_STRING).getName());
            throw e;
        }
        // Check if we're dealing with a subtype of String.
        if (stringType != rangeType) {
            final int stringSetCode = ((StringTypeImpl) ts.getType(rangeType))
                    .getStringSet();
	...
	public void setBooleanValue(Feature feat, boolean b)
	...
	public void setShortValue(Feature feat, short val)
			throws CASRuntimeException {
		final TypeSystemImpl ts = this.getCASImpl().getTypeSystemImpl();
		if (!ts.subsumes(((TypeImpl) feat.getDomain()).getCode(), this
				.getCASImpl().getHeapValue(this.getAddress()))) {
	...
	public void setDoubleValue(Feature feat, double val)
			throws CASRuntimeException {
		final TypeSystemImpl ts = this.getCASImpl().getTypeSystemImpl();
		if (!ts.subsumes(((TypeImpl) feat.getDomain()).getCode(), this
				.getCASImpl().getHeapValue(this.getAddress()))) {
    ...
    public void setFeatureValueFromString(Feature feat, String s)
        ...
        this.getCASImpl().setFeatureValueFromString(this.getAddress(),
        							((FeatureImpl) feat).getCode(),s);
    ...
    public FeatureStructure getFeatureValue(Feature feat)
        ...
        final int featCode = ((FeatureImpl) feat).getCode();
        // assert(featCode > 0);
        if (!ts.isApprop(this.getCASImpl().getHeapValue(this.getAddress()),
                featCode)) {
            CASRuntimeException e = new CASRuntimeException(
                    CASRuntimeException.INAPPROP_FEAT);
            e.addArgument(feat.getName());
        ...
        final int valAddr = this.getCASImpl().getFeatureValue(
    ...
    public String getStringValue(Feature f) throws CASRuntimeException {
        ...
        final TypeSystemImpl ts = (TypeSystemImpl) this.getCASImpl()
                .getTypeSystem();
        if (!ts.subsumes(domType, thisType)) {
            throwUndefinedFeatureExc(f, getType());
        }
        if (!ts.subsumes(stringType, rangeType)) {
            throwIllegalRangeExc(f, this.getCASImpl().getTypeSystem().getType(
        ...
        return this.getCASImpl().getStringValue(this.getAddress(),
                ((FeatureImpl) f).getCode());
    }
    ...
    private static final void throwIllegalRangeExc(Feature f, Type t)
            throws CASRuntimeException {
        CASRuntimeException e = new CASRuntimeException(
                CASRuntimeException.INAPPROP_RANGE);
        e.addArgument(f.getName());
        e.addArgument(t.getName());
        e.addArgument(f.getRange().getName());
        throw e;
    }

    private static final void throwUndefinedFeatureExc(Feature f, Type t)
            throws CASRuntimeException {
        CASRuntimeException e = new CASRuntimeException(
                CASRuntimeException.INAPPROP_FEAT);
        e.addArgument(f.getName());
    ...
    public void prettyPrint(int indent, int incr, StringBuffer buf,
            boolean useShortNames) {
        prettyPrint(indent, incr, buf, useShortNames, null);
    }

    public void prettyPrint(int indent, int incr, StringBuffer buf,
            boolean useShortNames, String s) {
    ...
    public void prettyPrint(int indent, int incr, StringBuffer buf,
            boolean useShortNames, String s, PrintReferences printRefs) {
        indent += incr;
        ...
        final int typeClass = this.getCASImpl().ll_getTypeClass(
                llts.ll_getCodeForType(this.getType()));
        
        if (typeClass == LowLevelCAS.TYPE_CLASS_STRINGARRAY) {
            final int arrayLen = getCASImpl()
                    .ll_getArraySize(this.getAddress());
            ...
            if (arrayLen > 0) {
                ...
                for (int i = 0; i < arrayLen; i++) {
                    ...
                    String element = this.getCASImpl().ll_getStringArrayValue(
                            this.getAddress(), i);
                    buf.append("\"" + element + "\"");
                }
                buf.append("]\n");
            ...
            if (arrayLen > 0) {
                ...
                for (int i = 0; i < arrayLen; i++) {
                    ...
                    int element = this.getCASImpl().ll_getIntArrayValue(
                            this.getAddress(), i);
                    buf.append(element);
                }
                buf.append("]\n");
            ...
            if (arrayLen > 0) {
                ...
                for (int i = 0; i < arrayLen; i++) {
                    ...
                    float element = this.getCASImpl().ll_getFloatArrayValue(
                            this.getAddress(), i);
                    buf.append(element);
                }
                buf.append("]\n");
        ...
        for (int i = 0; i < feats.size(); i++) {
            ...
            if (approp.equals(this.getCASImpl().getTypeSystem().getType(
                    CAS.TYPE_NAME_STRING))
                ...
                if (val == null) {
                  ...
                  if (!approp.getName().equals("uima.cas.Sofa")) {
                    val.prettyPrint(indent, incr, buf, useShortNames, null,
                            printRefs);
                  } else {
            ...
            } else if (approp.equals(this.getCASImpl().getTypeSystem().getType(
                    CAS.TYPE_NAME_FLOAT))) {
    ...
    public Object clone() throws CASRuntimeException {
        ...
        casImpl.copyFeatures(((FeatureStructureImpl) newFS).getAddress(), this
                .getAddress());
        return newFS;
    }
