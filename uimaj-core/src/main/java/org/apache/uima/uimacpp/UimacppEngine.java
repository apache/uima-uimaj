...
//	 Those types can not be created with CAS.createFS().
	...
	/**
	 * this field is the physical pointer to the C++ TafClEngine
	 * object this TafWrapper object is associated with.
	 * Do not use explicitly anywhere. It is set automatically
	 * by the <code>constructorJNI()</code> method and set to 0 by
	 * the <code>destructorJNI()</code> method.
	 */
	...
	private static native void createResourceManagerJNI()
			throws InternalTafException;

	// configuration of the TAF resource manager
	private static native void configureResourceManagerJNI(
		String workDir,
		String dataDir)
		throws InternalTafException;
	...
	private native void initializeJNI(
		String configFile)
			throws InternalTafException;
	private native void typeSystemInitJNI(
		String[] typeNames,
		String[] featureNames,
		int[] typeInheritance,
		int[] featDecls,
	...
	/**
	private native void fillCASJNI(int[] heapArray, int[] fsIndex,
			String[] stringTable);
	*/
	private native void fillCASJNI(int[] heapArray, int[] fsIndex,
			String[] stringTable, byte[] byteArray, short[] shortArray, long[] longArray);

	private native void processJNI(int isTCas, String sofaName,
			int[] resultSpecTypes, int[] resultSpecFeatures)
			throws InternalTafException;
	...
	private native void serializeCASJNI(boolean bSerializeData)
			throws InternalTafException;
	
	private native Object getSerializedDataJNI(int what)
		throws InternalTafException;

	private native void batchProcessCompleteJNI() throws InternalTafException;

	private native void collectionProcessCompleteJNI()
			throws InternalTafException;
	
	//JNI calls to support segmenter
	private native boolean hasNextSegmentJNI()
		throws InternalTafException;
	
	private native void nextSegmentJNI()
		throws InternalTafException;
	
	private native void serializeSegmentJNI(boolean bSerializeData)
		throws InternalTafException;
	
	private native Object getSerializedSegmentDataJNI(int what)
		throws InternalTafException;
	
	private native void releaseSegmentJNI()
		throws InternalTafException;
	
	/** serialized CAS data components - supported args to getSerializedDataJNI
	 *  and getSegmentSerializedDataJNI  */
	...
	// helper functions
	private static native String getErrorMessageJNI(long errorId)
			throws InternalTafException;
	...
	void typeSystemInit(CASMgrSerializer casMgrSerializer) throws UimacppException {
		...
		try {
			typeSystemInitJNI(casMgrSerializer.typeNames,
					casMgrSerializer.featureNames,
					casMgrSerializer.typeInheritance,
					casMgrSerializer.featDecls, casMgrSerializer.topTypeCode,
					casMgrSerializer.featureOffsets,
					casMgrSerializer.typeOrder,
					casMgrSerializer.stringSubtypes,
					casMgrSerializer.stringSubtypeValues,
					casMgrSerializer.stringSubtypeValuePos,
					casMgrSerializer.indexNames,
					casMgrSerializer.nameToIndexMap,
					casMgrSerializer.indexingStrategy,
					casMgrSerializer.comparatorIndex,
	...
	private static void serializeResultSpecification(ResultSpecification rs,
			CASImpl cas, IntVector resultSpecTypes, IntVector resultSpecFeatures) {
		...
		for (int i = 0; i < tofs.length; ++i) {
			if (tofs[i].isType()) {
				...
				FeatureImpl f = (FeatureImpl) tsImpl
						.getFeatureByFullName(tofs[i].getName());
	...
	public void process(ResultSpecification rs, CAS aCas, boolean casIsEmpty)
		...
		//System.out.println("in process" + crTypeSystem.getType("sofa.test.CrossAnnotation").getName());
		Type localSofaType = crTypeSystem.getType(CAS.TYPE_NAME_SOFA);
		Feature sofaID = localSofaType
		...
		try {
			...
			if (!casIsEmpty) {
				...
				/**
				 * fillCASJNI(casSerializerIn.heapArray,
				 * casSerializerIn.fsIndex, casSerializerIn.stringTable);
				 */

				fillCASJNI(casSerializerIn.heapArray, casSerializerIn.fsIndex,
						casSerializerIn.stringTable,
						casSerializerIn.byteHeapArray,
						casSerializerIn.shortHeapArray,
			...
			if (rs != null) {
				...
				serializeResultSpecification(rs, (CASImpl) cas,
						resultSpecTypes, resultSpecFeatures);
			}

			processJNI(isTCas, sofaName, resultSpecTypes.toArray(),
					resultSpecFeatures.toArray());
