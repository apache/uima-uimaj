...
/**
 * Indicates that a CAS should be routed to a single AnalysisEngine.  After it has
 * been processed, the {@link Flow#next()} method will be called again to determine
...
public class SimpleStep extends Step
{
   ...
   * 
   * @param aCasProcessorKey The key of the CAS Processor to which the CAS should be routed.
   *   This must be one of the keys in the FlowController's {@link FlowControllerContext#getAnalysisEngineMetaDataMap()}.
   */
   ...
   * By using this method, a user's Flow implementation can (but is not required to) 
   * reuse the same SimpleStep object multiple times.  
   * 
