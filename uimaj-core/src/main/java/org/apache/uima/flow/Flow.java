...
public interface Flow {
  ...
  /**
   * May be called by the framework to ask the FlowController if processing of the CAS can continue
   * after a failure occurred while executing the last Step that the Flow Controller returned.
   * <p>
   * If this method returns true, then the framework may continue to call the {@link #next()} method
   * to continue routing the CAS. If this method returns false, the framework will not make any more
   * calls to the {@link #next()} method, and will call the {@link #aborted()} method.
   * <p>
   ...
   * <p>
   * Note that it is possible for processing of a CAS to be aborted without this method being
   * called. This method is only called when an attempt is being made to continue processing of the
   * CAS following an error, which may be an application configuration decision.
   * 
   * @param failedAeKey
   *          The key of the analysis engine that failed.
   * @param failure
  ...
  /**
   * Called by the framework if processing has been aborted for the CAS that was being routed by
   * this Flow object. No further processing will take place on the CAS after this method is called,
   * so the framework will not call the {@link #next()} method again.
   * <p>
   * This method provides the Flow object with an opportunity to clean up any resources. Also, it
