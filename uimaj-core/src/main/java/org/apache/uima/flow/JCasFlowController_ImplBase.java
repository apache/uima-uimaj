...
public abstract class JCasFlowController_ImplBase extends FlowController_ImplBase
{
  ...
  /**
   * Overriden to check that <code>aCAS</code> is an instanceof {@link JCas}.
   * If it is, then {@link #computeFlow(JCas)} is called.  If not, an 
   * exception is thrown.
   */
  ...
  /**
   * This method must be overriden by subclasses. It takes a {@link JCas} and 
   * returns a {@link Flow} object that is responsible for routing this particular JCas
   * through the components of this Aggregate.  The <code>Flow</code> object should be 
   * given a handle to the JCas, so that it can use information in the CAS to make 
   * routing decisions.
   * <p>
   * FlowController implementations will typically define their own class that implements 
   * {@link Flow} by extending from the base class {@link JCasFlow_ImplBase}.  This method 
   * would then just instantiate the flow object, call its <code>setCas</code> method to 
   * provide a handle to the JCas, and return the flow object.  
   * 
   * @param aJCas the JCas to be routed
   * 
