...
public final class FloatArray extends TOP implements FloatArrayFS {
	...
	public FloatArray(JCas jcas, int length) {
		this(
		/* addr */
		jcas.getLowLevelCas().ll_createArray(
...
//	}
	...
	/**
	 * return the indexed value from the corresponding Cas FloatArray as a
	 * float,
	 */
	...
	/**
	 * update the Cas, setting the indexed value to the passed in Java float
	 * value.
	 * 
	 * @param i
	 *            index
   ...
   */
    ...
    jcasType.casImpl.checkArrayBounds(addr, srcOffset, length);
		for (int i = 0; i < length; i++) {
			dest[i + destOffset] = Float.toString(jcasType.ll_cas.ll_getFloatArrayValue(addr, i + srcOffset));
		}
	}
  ...
  */
    ...
    jcasType.casImpl.checkArrayBounds(addr, destOffset, length);
    for (int i = 0; i < length; i++) {
      jcasType.ll_cas.ll_setFloatArrayValue(addr, i + destOffset, Float.parseFloat(src[i + srcOffset]));
    }
	}
