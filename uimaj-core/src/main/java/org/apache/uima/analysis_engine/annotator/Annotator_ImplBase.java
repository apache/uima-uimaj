...
/**
 * Base class for annotators in UIMA SDK v1.x.  As of v2.0, annotators
...
public abstract class Annotator_ImplBase implements BaseAnnotator
{
  ...
  /**
   * The only thing this implementation does is store the AnnotatorContext
   * so that it can be accessed later via the {@link #getContext()} method.
   *  
  ...
  /**
   * The only thing this implementation does is store the TypeSystem so
   * that it can be accessed by the {@link #getTypeSystem()} method, and 
   * also so that it can be passed back to the 
   * {@link #typeSystemInit(TypeSystem)} method by the default implementation
   * of {@link #reconfigure()}.
   * 
   ...
   * {@link #initialize(AnnotatorContext)} and 
   * {@link #typeSystemInit(TypeSystem)}.  The <code>typeSystemInit</code>
   * method will be passed the last known TypeSystem.
   * 
 ...
 /**
   * Gets the <code>AnnotatorContext</code> to be used by this Annotator.
   * The <code>AnnotatorContext</code> provides access to external resources 
   * that may be used by this annotator.  This includes configuration parameters,
   * logging and instrumentation services, and access to text analysis
   * resources.
   * 
  ...
  /**
   * Gets the <code>TypeSystem</code> that was passed to the most recent
   * call to {@link #typeSystemInit(TypeSystem)}.
   * 
   * @return the Annotator Context
   */
