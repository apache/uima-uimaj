...
public class IntArrayRBTTest {
  ...
  // FSRBTSetIndex<Annotation> setIndex = new FSRBTSetIndex<Annotation>((CASImpl)cas,
  // cas.getAnnotationType(), FSIndex.SET_INDEX);
  // IntArrayRBT ia = new IntArrayRBT();
  // Integer[] vs = new Integer[] {2, 2, 5, 1, 6, 7, 3, 4};
  // for (Integer i : vs) {
  // ia.insertKey(i);
  ...
  public void testLargeInsertsDeletes() {
    ...
    System.out.print(
            "freemem after intArrayRBT keys deleted to 0 (should be about the same): " + fm1 + " ");
    ...
    for (int j = 0; j < 10; j++) {
      ...
      // System.out.println("debug IntArrayRBT before delete, number of present values is " +
      // presentValues.size());
