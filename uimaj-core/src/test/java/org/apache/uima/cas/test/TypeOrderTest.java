...
public class TypeOrderTest extends TestCase {
   ...
   private static CASMgr initCAS() throws TCASException {
      ...
      try {
         order.add(
            new String[] { TOKEN_TYPE, SENT_TYPE, TCAS.TYPE_NAME_ANNOTATION });
         lo = order.getOrder();
      } catch (CASException e) {
         assertTrue(false);
      }
   ...
   public void testMain() throws Exception {
      ...
      for (int i = 0; i < 10; i++) {
         cas.getIndexRepository().addFS(
            cas.createAnnotation(annotationType, i * 2, (i * 2) + 1));
         cas.getIndexRepository().addFS(
            cas.createAnnotation(sentenceType, i * 2, (i * 2) + 1));
         cas.getIndexRepository().addFS(
            cas.createAnnotation(tokenType, i * 2, (i * 2) + 1));
         cas.getIndexRepository().addFS(
            cas.createAnnotation(tokenType, i * 2, (i * 2) + 1));
         cas.getIndexRepository().addFS(
            cas.createAnnotation(tokenType, i * 2, (i * 2) + 1));
      ...
      FSIterator it =
         cas.getIndexRepository().getIndex(TYPE_ORDER_INDEX).iterator();
