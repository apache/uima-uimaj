...
/**
 * A variation of the XmiWriterCasConsumer that also saves the Type System to an
 * Ecore file and links all of the XMI files to it via their schemaLocation
 * attribute.  This allows integration with the Eclipse Modeling Framework (EMF).
 * This class requires the EMF libraries common.jar, ecore.jar, and ecore.xmi.jar
 ...
 *   <li><code>WriteEcoreTypeSystem</code> - if true, writes the type system
 *       to an Ecore file and links all of the XMI files to it via their
 *       schemaLocation attribute.  Requires EMF libraries in the classpath
 *       if this option is set to true.</li>
 * </ul> 
 */
public class XmiEcoreCasConsumer extends CasConsumer_ImplBase 
{
    ...
    public void initialize() throws ResourceInitializationException
    {
        ...
        writeEcoreTypeSystem = 
          Boolean.TRUE.equals(getUimaContext().getConfigParameterValue(PARAM_WRITE_ECORE_TYPESYSTEM));
    }
    
    
    /** Processes a CAS.  In this case, the CAS is converted to XMI and written into the 
     * output file .
     * 
     * @param aCAS The CAS to write to XMI
     * 
        ...
        try {
            ...
            if (writeEcoreTypeSystem && !isModelGenerated ) {
                ...
                Resource.Factory.Registry.INSTANCE.
                    getExtensionToFactoryMap().put("*", new XMIResourceFactoryImpl());                ResourceSet resourceSet = new ResourceSetImpl();
                URI outputURI = URI.createFileURI(new File(mOutputDir,"typesystem.ecore").getAbsolutePath());
                Resource outputResource = resourceSet.createResource(outputURI);
                schemaLocationMap = new HashMap();
                try
                {
                  UimaTypeSystem2Ecore.uimaTypeSystem2Ecore(tsDesc, outputResource, null, schemaLocationMap);
                }
