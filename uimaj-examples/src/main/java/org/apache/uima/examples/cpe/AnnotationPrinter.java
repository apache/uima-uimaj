...
/**
 * An example of CAS Consumer. <br> AnnotationPrinter prints to an output file
 * all annotations in the CAS.
 * <br>
 ...
 * the descriptor file. 
 * <br>These may also be set by the application by using the 
 * setConfigParameterValue methods.
 * 
 * 
 */
...
public class AnnotationPrinter extends  CasConsumer_ImplBase  implements  CasObjectProcessor {
	...
	public void initialize()
		throws ResourceInitializationException {
			...
			if(oPath ==null) {
				throw new 	ResourceInitializationException(ResourceInitializationException.CONFIG_SETTING_ABSENT, new Object []{"outputFile"} );
			}	
			...
			if(outFile.getParentFile() != null && !outFile.getParentFile().exists())
			{
					if (!outFile.getParentFile().mkdirs()) 
					 throw new ResourceInitializationException(ResourceInitializationException.RESOURCE_DATA_NOT_VALID, new Object []{oPath ,"outputFile"} );	
			}
			try
      {
	 ...
	 * <br> In this case, the CAS index is iterated over selected annotations 
    ...
    boolean titleP = false;
		String docUri = null;
		Iterator it = jcas.getJFSIndexRepository().getAnnotationIndex(SourceDocumentInformation.type).iterator();
		if (it.hasNext())
		{
	...
	/** 
	 * Reconfigures the parameters of this Consumer. <br>This is used in 
	 * conjunction with the setConfigurationParameterValue to set the 
	 * configuration parameter values to values other than the ones specified in
	 *  the descriptor.
	 * 
			...
			try {
				...
				if(oFile.getParentFile() != null && !oFile.getParentFile().exists())
				{
						if (!oFile.getParentFile().mkdirs()) 
						 throw new ResourceConfigurationException(ResourceInitializationException.RESOURCE_DATA_NOT_VALID, new Object []{oPath ,"outputFile"} );	
				}
