...
/**
 * An example application that reads documents from the file system, sends them though a Text
 * Analysis Engine, and produces XML files with inline annotations. This application uses the
 * {@link CollectionProcessingManager} to drive the processing. For a simpler introduction to using
 * TAEs in an application, see {@link ExampleApplication}.
 * <p>
 * <code>Usage: java org.apache.uima.examples.RunTAE [OPTIONS] 
 * &lt;TAE descriptor or JAR file name&gt; &lt;input dir&gt; 
 * [&lt;output dir&gt;]</code>
 * <p>
 * If <code>output dir</code> is not specified, the analysis results will not be output. Tis can
 ...
 * Some TAEs,particularly TAF TAEs, require this. <br>
 ...
 * 
 */
public class RunTAE implements StatusCallbackListener {
  /**
   * Constructor. Sets up and runs a Text Analysis Engine.
   */
  public RunTAE(String[] args) {
    try {
      ...
      // read TAE descriptor from file
      long startTime = System.currentTimeMillis();
      XMLInputSource in = new XMLInputSource(taeSpecifierFile);
      ResourceSpecifier taeSpecifier = UIMAFramework.getXMLParser().parseResourceSpecifier(in);

      // taeSpecifier.toXML(new FileWriter("spec.xml"));

      // instantiate TAE
      TextAnalysisEngine tae = UIMAFramework.produceTAE(taeSpecifier);
      long initTime = System.currentTimeMillis() - startTime;
      System.out.println("TAE initialized in " + initTime + "ms.");
      mCPM.setAnalysisEngine(tae);
  ...
  public void entityProcessComplete(CAS aCas, EntityProcessStatus aStatus) {
    if (aStatus.isException()) {
      ...
      FSIterator it = aCas.getTCAS().getAnnotationIndex(fileLocType).iterator();
  ...
  private void printUsageMessage() {
    System.err.println("\nUsage: java " + this.getClass().getName()
            + " [OPTIONS] <TAE descriptor or JAR file name> <input dir> [<output dir>] ");
  ...
  private boolean processCmdLineArgs(String[] args) {
    ...
    while (index < args.length) {
      ...
      {
        if (taeSpecifierFile == null) {
          taeSpecifierFile = new File(arg);
          if (!taeSpecifierFile.exists() || taeSpecifierFile.isDirectory()) {
    ...
    return (taeSpecifierFile != null) && (inputDir != null);
  }
  ...
  private File taeSpecifierFile = null;
