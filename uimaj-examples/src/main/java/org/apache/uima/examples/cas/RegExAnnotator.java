 ...
 * result in an annotation of type <code>TypeNames[i]</code>.  
 * <p>
...
{    
  public static final String MESSAGE_DIGEST =
     "org.apache.uima.examples.cas.RegExAnnotator_Messages";
          ...
          if (getResultSpecification().containsType(mCASTypes[j].getName()))
          {
            ...
            for (int k = 0; k < mPatterns[j].length; k++)
            {
              ...
              while (pos < subText.length() && matcher.find(pos))
              {
                ...
                FeatureStructure fs = aCAS.createAnnotation(mCASTypes[j],
                  annotStart, annotEnd);
    ...
    throws CASException
  {
    ...
    else
    {
      ...
      FSTypeConstraint constraint =
        aCAS.getConstraintFactory().createTypeConstraint();
      for (int i = 0; i < mContainingAnnotationTypes.length; i++)
      {
