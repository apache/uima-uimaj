 ...
 * meeting as a UimaMeeting if a UIMA acronym occurs in close proximity to 
 * that meeting.  When combined in an aggregate TAE with the 
 * UimaAcronymAnnotator, demonstrates the use of the ResourceManager to share 
 * data between annotators.
 * 
 * 
 */
public class UimaMeetingAnnotator extends JCasAnnotator_ImplBase
{
  ...
  {   
    ...
    FSIndex meetingIndex = 
        aJCas.getJFSIndexRepository().getAnnotationIndex(Meeting.type);    
    ...
    while (iter.isValid())
    {
      ...
      while (tokenizer.hasMoreTokens())
      {
        ...
        if (mMap.get(token) != null)
        {
          //create annotation
          UimaMeeting annot =  
              new UimaMeeting(aJCas, meeting.getBegin(), meeting.getEnd(),
