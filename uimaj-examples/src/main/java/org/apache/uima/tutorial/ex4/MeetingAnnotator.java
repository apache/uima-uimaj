...
public class MeetingAnnotator extends JCasAnnotator_ImplBase
{
    ...
    throws ResourceInitializationException
  {
    ...
    mWindowSize = 
        ((Integer)aContext.getConfigParameterValue("WindowSize")).intValue();    
  }
        ...
        while (time1Iter.hasNext())
        {
          ...
          while (time2Iter.hasNext())
          {
            ...
            if (time1 != time2)
            {
              ...
              int minBegin = Math.min(
                Math.min(time1.getBegin(), time2.getBegin()),
                Math.min(date.getBegin(), room.getBegin()));
              int maxEnd = Math.max(
                Math.max(time1.getEnd(), time2.getEnd()),
              ...
              if (maxEnd - minBegin < mWindowSize)
              {
                ...
                {
                  //annotate
                  Meeting mtg = new Meeting(aJCas, minBegin, maxEnd,
                      room, date, time1, time2);
                  mtg.addToIndexes();
